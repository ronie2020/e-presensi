...
<?php $component->withAttributes([]); ?>
     <?php $__env->slot('header', null, []); ?> 
        <div class="flex items-center justify-between print:hidden">
            ...
            <a href="<?php echo e(route('cbt.analysis.print', $exam->id)); ?>" target="_blank" class="text-sm font-bold text-[#2c3f61] hover:text-[#0d52a1] flex items-center gap-2 transition bg-white px-4 py-2 rounded-xl border border-slate-200 shadow-sm active:scale-95">
            ...
            <div class="bg-white p-6 rounded-[2rem] shadow-xl shadow-[#56bbf1]/5 border border-slate-100 flex flex-col md:flex-row justify-between items-center gap-4 print-area print:mb-6 print:p-0">
                <div>
                    <div class="flex items-center gap-2 mb-1 print:hidden">
                        <a href="<?php echo e(route('cbt.recap', $exam->id)); ?>" class="text-xs font-bold text-slate-400 hover:text-[#0d52a1] transition flex items-center gap-1">
                            <i class="ph-bold ph-arrow-left"></i> Kembali ke Rekap
                        </a>
                    </div>
                    <h3 class="text-2xl font-black text-[#2c3f61]"><?php echo e($exam->title); ?></h3>
                    <p class="text-[#2c3f61]/60 text-sm font-medium">Analisis Kualitas Soal • Mapel: <?php echo e($exam->subject_name); ?> • Sampel: <b><?php echo e($totalStudents ?? 0); ?> Siswa</b></p>
                </div>
                        ...
                        <?php $__currentLoopData = $tagAnalysis; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tag => $data): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            ...
                            <div class="bg-slate-50 p-4 rounded-2xl border border-slate-100 print:border print:border-slate-300 print:bg-transparent">
                                <div class="flex justify-between items-end mb-2">
                                    <h5 class="font-bold text-[#2c3f61] text-sm flex items-center gap-2"><i class="ph-fill ph-tag text-slate-400 print:hidden"></i> <?php echo e($tag); ?></h5>
                        ...
                        <tbody class="divide-y divide-slate-50 print:divide-gray-300">
                            <?php $__empty_1 = true; $__currentLoopData = $analysis; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                <tr x-show="search === '' || '<?php echo e(strtolower(addslashes(strip_tags($item->text . ' ' . ($item->tags ?? ''))))); ?>'.includes(search.toLowerCase())" 
                                    ...
                                    <td class="px-6 py-4">
                                        <p class="font-medium text-[#2c3f61] line-clamp-3 print:line-clamp-none mb-2" title="<?php echo e($item->text); ?>"><?php echo e($item->text); ?></p>
                                        ...
                                        <?php if(!empty($item->tags)): ?>
                                            <div class="flex flex-wrap gap-1 mt-1 print:mt-0">
                                                <?php $__currentLoopData = explode(',', $item->tags); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tag): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                    <span class="text-[9px] font-bold bg-[#56bbf1]/10 text-[#0d52a1] px-2 py-0.5 rounded border border-[#56bbf1]/20 print:border-slate-300 print:text-[#2c3f61] print:bg-transparent"><?php echo e(trim($tag)); ?></span>
                                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                            </div>
                                        <?php endif; ?>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        ...
                                        <?php elseif($item->type == 'essay'): ?>
                                            ...
                                            <button onclick="Swal.fire({title: 'Kunci Jawaban', text: '<?php echo e(addslashes($item->correct_key)); ?>', confirmButtonColor: '#0d52a1', customClass: { popup: 'rounded-[2rem]' }})" 
                                                    class="text-xs font-bold text-[#0d52a1] hover:underline cursor-pointer print:hidden">
                                                Lihat Kunci
                                            </button>
                                            <span class="hidden print:block text-xs text-[#2c3f61]"><?php echo e($item->correct_key ?: '-'); ?></span>
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        ...
                                        <div class="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden flex print:h-2 print:border print:border-black">
                                            <div class="h-full <?php echo e(str_contains($item->difficulty_badge, 'emerald') ? 'bg-emerald-400' : (str_contains($item->difficulty_badge, 'rose') ? 'bg-rose-400' : 'bg-[#56bbf1]')); ?>" 
