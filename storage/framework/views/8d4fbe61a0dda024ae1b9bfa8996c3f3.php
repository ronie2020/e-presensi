<?php
    ...
    $udzurCount = $habits->filter(function($h) {
        return $h->is_udzur_syar_i ?? ($h->notes == 'haid' ?? false);
    })->count();

    // Total hari WAJIB shalat (Total Hari Input - Hari Udzur)
    $obligatedEntries = $totalEntries - $udzurCount;
    // Hindari division by zero
    $divider = $obligatedEntries > 0 ? $obligatedEntries : 1; 

    // A. BADGES LOGIC
    // Definisikan kriteria lencana
    $badges = [
        [
            'id' => 'dhuha_starter',
            'label' => 'Pejuang Dhuha',
    ...
    for ($i = 6; $i >= 0; $i--) {
        ...
        $entry = $habits->first(fn($h) => \Carbon\Carbon::parse($h->report_date)->isSameDay($date));
        
        // Cek Status Udzur per hari
        $isUdzur = $entry ? ($entry->is_udzur_syar_i ?? ($entry->notes == 'haid' ?? false)) : false;

        $score = 0;
        if($entry && !$isUdzur) {
            // Hitung normal jika TIDAK udzur
            ...
            // Jika UDZUR, skor dianggap PENUH (6) secara visual karena taat aturan
            $score = 6; 
        }
...
<div class="space-y-8 animate-enter">
            ...
            <div class="flex items-center justify-between mb-6">
                ...
                <div class="flex gap-2">
                    <div class="px-3 py-1 bg-pink-50 text-pink-600 rounded-full text-xs font-bold border border-pink-100 flex items-center gap-1">
                        <div class="w-2 h-2 rounded-full bg-pink-500"></div> Udzur syar'i
                    </div>
                ...
                <?php $__currentLoopData = $chartData; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $data): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="flex flex-col items-center gap-2 flex-1 group relative">
                        ...
                        <div class="absolute bottom-full mb-1 opacity-0 group-hover:opacity-100 transition-opacity bg-slate-800 text-white text-[10px] py-1 px-2 rounded z-10 whitespace-nowrap">
                            <?php echo e($data['is_udzur'] ? 'Sedang Udzur' : $data['score'] . ' Waktu'); ?>

                        </div>

                        
                        <div class="w-full max-w-[40px] bg-slate-100 rounded-t-xl relative overflow-hidden h-full flex items-end transition-all hover:bg-slate-200">
                            <?php
                                // Logika Warna: Pink jika Udzur, Biru jika Hari Ini, Grading warna jika normal
                                $barColor = '';
                                if ($data['is_udzur']) {
                                    $barColor = 'bg-pink-400'; // Warna khusus Udzur
                            ...
                            <div style="height: <?php echo e($data['height']); ?>%" 
                                 ...
                                 <?php if($data['is_udzur']): ?>
                                    <div class="absolute bottom-2 left-0 right-0 text-center text-white/80 text-xs">
    ...
    <div>
        <div class="flex items-center justify-between mb-4">
            ...
            <?php if($udzurCount > 0): ?>
                <span class="text-xs font-bold text-pink-500 bg-pink-50 px-2 py-1 rounded-lg border border-pink-100">
                    <?php echo e($udzurCount); ?> Hari Udzur (Dikecualikan)
                </span>
            <?php endif; ?>
        </div>
        ...
        <?php if($totalEntries > 0): ?>
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                <?php $__currentLoopData = $prayerConfig; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $prayer): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <?php
                        // Hitung jumlah shalat yang dilakukan
                        $count = $habits->where($prayer['key'], 1)->count();
                        
                        // PERHITUNGAN KHUSUS:
                        // Persentase = (Dilakukan / (Total Hari - Hari Udzur)) * 100
                        // Jadi saat haid, penyebutnya berkurang, persentase tetap bagus.
                    ...
                    <div class="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex flex-col items-center justify-center text-center gap-2 hover:shadow-md transition-all">
        ...
        <?php else: ?>
            <div class="bg-slate-50 rounded-2xl p-6 text-center border border-slate-100 border-dashed">
                <p class="text-slate-500 text-sm">Belum ada data jurnal.</p>
            </div>
        <?php endif; ?>
    </div>
        ...
        <div class="bg-white rounded-[2rem] border border-slate-100 shadow-sm overflow-hidden">
            ...
            <?php else: ?>
                <div class="p-10 text-center flex flex-col items-center justify-center">
                    ...
                    <p class="text-slate-600 font-bold mb-1">Belum Ada Rekaman</p>
                </div>
            <?php endif; ?>
        </div>
    </div>
...
</style><?php /**PATH C:\Users\ronie\Documents\aplikasi\E-Presensi Netila\resources\views/students/portal/partials/tab-keagamaan.blade.php ENDPATH**/ ?>