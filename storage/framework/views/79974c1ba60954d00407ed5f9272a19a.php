    ...
    <div class="lg:col-span-1">
        <div class="bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-100 sticky top-24 group overflow-hidden hover:border-elevate-accent/30 transition-colors">
            ...
            <div class="relative z-10">
                ...
                <p class="text-slate-500 text-xs mb-6 font-medium">Ringkasan kehadiran di kelas (<?php echo e($stats['total']); ?> Sesi).</p>
                ...
                <div class="flex items-end gap-3 h-32 mb-6 px-4 pb-2 border-b border-slate-50">
                    <div class="flex-1 flex flex-col items-center gap-2 group/bar h-full justify-end">
                        <div class="w-full bg-elevate-soft rounded-t-lg relative h-full flex items-end overflow-hidden">
                            <div style="height: <?php echo e($pctPresent); ?>%" class="w-full bg-elevate-primary transition-all duration-1000 group-hover/bar:bg-elevate-dark relative">
                                <span class="absolute -top-6 left-1/2 -translate-x-1/2 text-[10px] font-bold text-elevate-dark opacity-0 group-hover/bar:opacity-100 transition-opacity"><?php echo e($pctPresent); ?>%</span>
                            </div>
                        </div>
                    ...
                    <div class="flex-1 flex flex-col items-center gap-2 group/bar h-full justify-end">
                        <div class="w-full bg-slate-100 rounded-t-lg relative h-full flex items-end overflow-hidden">
                            <div style="height: <?php echo e($pctSickPermit); ?>%" class="w-full bg-slate-400 transition-all duration-1000 group-hover/bar:bg-slate-500 relative">
                                <span class="absolute -top-6 left-1/2 -translate-x-1/2 text-[10px] font-bold text-slate-600 opacity-0 group-hover/bar:opacity-100 transition-opacity"><?php echo e($pctSickPermit); ?>%</span>
                            </div>
                        </div>
                ...
                <div class="space-y-3">
                    <div class="flex items-center justify-between p-3 rounded-2xl bg-elevate-soft/50 border border-elevate-accent/20">
                        ...
                        <span class="text-lg font-black text-elevate-primary"><?php echo e($stats['present']); ?></span>
                    </div>
                            ...
                            <div class="bg-white rounded-3xl p-5 border border-slate-100 shadow-sm hover:shadow-md hover:border-elevate-primary/30 transition-all duration-300 relative overflow-hidden">
                                ...
                                <div class="mb-4 pr-20">
                                    <div class="flex flex-wrap items-center gap-2 mb-1.5">
                                        ...
                                        <span class="text-[10px] font-bold text-slate-400 flex items-center gap-1">
                                            <i class="ph-fill ph-clock"></i> <?php echo e(\Carbon\Carbon::parse($journal->started_at)->format('H:i')); ?>

                                        </span>
                                        <span class="text-[10px] font-bold text-slate-400 flex items-center gap-1">
                                            <i class="ph-fill ph-calendar-blank"></i> <?php echo e(\Carbon\Carbon::parse($journal->date)->translatedFormat('d M Y')); ?>
                                    ...
                                    <p class="text-xs text-slate-500 font-medium mt-1 flex items-center gap-1.5">
                                        <i class="ph-fill ph-chalkboard-teacher text-slate-300"></i> <?php echo e($journal->schedule?->teacher?->name ?? 'Guru Pengajar'); ?>
                                ...
                                <div class="bg-slate-50 rounded-2xl p-4 border border-slate-100 relative mb-4">
                                    ...
                                    <p class="text-xs text-slate-600 leading-relaxed relative z-10 whitespace-pre-line">
                                        <?php echo e($journal->activities ?? 'Tidak ada catatan aktivitas.'); ?>
                                    ...
                                    <div x-data="{ open: false }">
                                        ...
                                        <div x-show="open" x-transition.opacity class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/80 backdrop-blur-sm p-4" style="display: none;">
                                            <div @click.away="open = false" class="relative max-w-3xl w-full">
                                                ...
                                                <img src="<?php echo e(asset('storage/' . $journal->photo_proof)); ?>" class="w-full h-auto rounded-xl shadow-2xl border-2 border-white/10">
                                            </div>
                                        </div>
                                    </div>
                                <?php endif; ?>
                            ...
                            <?php $__currentLoopData = $teaching_journals; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $journal): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                ...
                                <tr class="hover:bg-elevate-soft/30 transition-colors">
                                    <td class="px-6 py-4 font-bold text-elevate-dark">
                                        ...
                                        <span class="block text-[10px] text-slate-400 font-normal"><?php echo e(\Carbon\Carbon::parse($journal->started_at)->format('H:i')); ?> WIB</span>
                                    </td>
                                    <td class="px-6 py-4 font-bold text-elevate-primary">
                                        <?php echo e($journal->schedule?->subject?->name ?? '-'); ?>

                                        <span class="block text-[10px] text-slate-500 font-normal truncate max-w-[200px]">Topik: <?php echo e($journal->topic ?? '-'); ?></span>
                                    </td>
                                    <td class="px-6 py-4 text-slate-500 font-medium"><?php echo e($journal->schedule?->teacher?->name ?? '-'); ?></td>
                                    <td class="px-6 py-4 text-center">
                                        <span class="px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-wider border <?php echo e($badge); ?> shadow-sm">
                                            <?php echo e($label); ?>
...
</div><?php /**PATH C:\Users\ronie\Documents\aplikasi terpadu\sistem_absensi_sekolah\resources\views/students/portal/partials/tab-kbm.blade.php ENDPATH**/ ?>