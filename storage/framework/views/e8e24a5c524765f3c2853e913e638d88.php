...
<?php $component->withAttributes([]); ?>
                                        ...
                                        <?php if(isset($recentScans)): ?>
                                            <?php $__currentLoopData = $recentScans; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $scan): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                <tr class="log-entry group hover:bg-slate-50/80 transition-colors" 
                                                    data-harian="<?php echo e($scan['data_harian'] ? 'true' : 'false'); ?>"
                                                    data-makan="<?php echo e(($scan['data_makan'] ?? false) ? 'true' : 'false'); ?>"
                                                    data-dhuha="<?php echo e($scan['data_dhuha'] ? 'true' : 'false'); ?>"
                                                    data-dhuhur="<?php echo e($scan['data_dhuhur'] ? 'true' : 'false'); ?>"
                                                    ...
                                                    <td class="col-harian px-2 py-3 md:px-4 md:py-4 text-center">
                                                        <?php if($scan['time_in']): ?> <span class="font-mono font-bold text-slate-600 bg-slate-100 px-1.5 py-0.5 md:px-2 md:py-1 rounded text-[10px] md:text-xs"><?php echo e(\Carbon\Carbon::parse($scan['time_in'])->format('H:i')); ?></span>
                                                        <?php else: ?> <span class="text-slate-300">-</span> <?php endif; ?>
                                                    </td>
                                                    <td class="col-harian px-2 py-3 md:px-4 md:py-4 text-center">
                                                        <?php if($scan['time_out']): ?> <span class="font-mono font-bold text-slate-600 bg-slate-100 px-1.5 py-0.5 md:px-2 md:py-1 rounded text-[10px] md:text-xs"><?php echo e(\Carbon\Carbon::parse($scan['time_out'])->format('H:i')); ?></span>
                                                        <?php else: ?> <span class="text-slate-300">-</span> <?php endif; ?>
                                                    </td>
                                                    <td class="col-waktu hidden-col px-2 py-3 md:px-4 md:py-4 text-center">
                                                        ...
                                                        <span class="time-dhuha font-mono font-bold text-slate-600 bg-slate-100 px-1.5 py-0.5 md:px-2 md:py-1 rounded text-[10px] md:text-xs <?php echo e($scan['dhuha_time'] ? '' : 'hidden'); ?>"><?php echo e($timeFormat($scan['dhuha_time'])); ?></span>
                                                        <span class="time-dhuhur font-mono font-bold text-slate-600 bg-slate-100 px-1.5 py-0.5 md:px-2 md:py-1 rounded text-[10px] md:text-xs <?php echo e($scan['dhuhur_time'] ? '' : 'hidden'); ?>"><?php echo e($timeFormat($scan['dhuhur_time'])); ?></span>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', (event) => {
            ...
            const csrfToken = '<?php echo e(csrf_token()); ?>'; const scanProcessUrl = '<?php echo e(route('scan.process')); ?>'; 
