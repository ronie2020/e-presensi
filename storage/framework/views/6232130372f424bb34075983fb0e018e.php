...
<div class="bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-white pt-24 pb-12 relative overflow-hidden transition-colors duration-300">
    ...
    <!-- Aksen garis cyan/blue di atas footer -->
    ...
    <div class="absolute -right-20 top-20 w-96 h-96 bg-cyan-200 dark:bg-cyan-600 rounded-full mix-blend-multiply dark:mix-blend-overlay filter blur-[100px] opacity-50 dark:opacity-20"></div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div class="mb-24">
            ...
            <div class="grid gap-6 md:grid-cols-3">
                <?php $__empty_1 = true; $__currentLoopData = $announcements; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <!-- Hover disesuaikan ke Cyan -->
                    <article class="bg-white dark:bg-slate-900/50 backdrop-blur-md rounded-2xl p-6 border border-slate-200 dark:border-slate-800 hover:border-cyan-400 dark:hover:border-cyan-500/50 shadow-sm hover:shadow-lg transition-all duration-300 hover:-translate-y-1 group h-full flex flex-col cursor-pointer" @click="openAnnouncementByIndex(<?php echo e($index); ?>)" data-aos="fade-up" data-aos-delay="<?php echo e($loop->index * 100); ?>">
                        <div class="flex justify-between items-start mb-4">
                            <span class="px-2 py-1 rounded bg-cyan-50 dark:bg-cyan-500/10 text-cyan-600 dark:text-cyan-400 text-[10px] font-bold uppercase tracking-wide border border-cyan-100 dark:border-cyan-500/20">Info</span>
                        ...
                        <h3 class="text-lg font-bold text-slate-800 dark:text-white mb-3 line-clamp-2 group-hover:text-cyan-600 dark:group-hover:text-cyan-400 transition-colors"><?php echo e($item->title); ?></h3>
                        <p class="text-slate-500 dark:text-slate-400 text-sm line-clamp-3 mb-4 flex-1 leading-relaxed"><?php echo e(Str::limit(strip_tags($item->content), 100)); ?></p>
                        <div class="flex items-center text-sm text-cyan-600 dark:text-cyan-400 font-semibold mt-auto gap-1 group-hover:gap-2 transition-all">
        ...
        <div class="bg-white dark:bg-slate-900/50 rounded-3xl p-8 mb-16 border border-slate-200 dark:border-slate-800 backdrop-blur-md shadow-sm">
            <div class="flex items-center gap-3 mb-8">
                <div class="p-2 bg-cyan-100 dark:bg-cyan-500/10 rounded-lg text-cyan-600 dark:text-cyan-400">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <?php $__empty_1 = true; $__currentLoopData = $agendas; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $agenda): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <?php
                        // Palet warna disesuaikan dengan tone biru cerah/elevate
                        $colors = ['blue', 'cyan', 'indigo', 'sky', 'teal'];
                        $color = $colors[$loop->index % count($colors)];
                    ?>
                    
                    <div class="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border-l-4 border-<?php echo e($color); ?>-500 flex items-start gap-4 hover:bg-slate-100 dark:hover:bg-slate-800 transition cursor-default group h-full">
                        <div class="text-center bg-white dark:bg-slate-900 border border-slate-200 dark:border-none p-2 rounded-lg min-w-[60px] shadow-sm dark:shadow-lg transition-colors shrink-0">
                            <span class="block text-[10px] text-slate-400 font-bold uppercase tracking-wider"><?php echo e($agenda->event_date->format('M')); ?></span>
                            <span class="block text-xl font-bold text-slate-800 dark:text-white"><?php echo e($agenda->event_date->format('d')); ?></span>
                        </div>
                        <div class="flex-1 min-w-0 py-0.5">
                            <h4 class="text-slate-800 dark:text-white font-bold text-sm line-clamp-2 leading-snug mb-1" title="<?php echo e($agenda->title); ?>"><?php echo e($agenda->title); ?></h4>
                            <p class="text-slate-500 dark:text-slate-400 text-xs flex items-center gap-1.5">
                                <i class="ph-fill ph-map-pin shrink-0 text-<?php echo e($color); ?>-500 dark:text-<?php echo e($color); ?>-400"></i> 
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-12 mb-16 border-t border-slate-200 dark:border-slate-800 pt-16">
            ...
            <div>
                ...
                <ul class="space-y-3 text-sm text-slate-500 dark:text-slate-400">
                    <li><a href="#profil" class="hover:text-cyan-600 dark:hover:text-cyan-400 transition-colors flex items-center gap-2"><i class="ph-bold ph-caret-right text-xs"></i> Profil Sekolah</a></li>
                    <li><a href="#guru" class="hover:text-cyan-600 dark:hover:text-cyan-400 transition-colors flex items-center gap-2"><i class="ph-bold ph-caret-right text-xs"></i> Tenaga Pendidik</a></li>
                    <li><a href="#kegiatan" class="hover:text-cyan-600 dark:hover:text-cyan-400 transition-colors flex items-center gap-2"><i class="ph-bold ph-caret-right text-xs"></i> Galeri Kegiatan</a></li>
                    <li><a href="<?php echo e(route('login')); ?>" class="hover:text-cyan-600 dark:hover:text-cyan-400 transition-colors flex items-center gap-2"><i class="ph-bold ph-caret-right text-xs"></i> Login Staff</a></li>
                </ul>
            </div>
            <div>
                ...
                <ul class="space-y-4 text-sm text-slate-500 dark:text-slate-400">
                    <li class="flex items-start gap-3">
                        <i class="ph-fill ph-map-pin mt-1 text-cyan-600 dark:text-cyan-500 shrink-0"></i>
                        <span class="leading-relaxed">Jl. Mekarjaya No.199 Sidaharja Kec. Lakbok, Kab. Ciamis, Jawa Barat 46385</span>
                    </li>
                    <li class="flex items-center gap-3">
                        <i class="ph-fill ph-phone text-cyan-600 dark:text-cyan-500 shrink-0"></i>
                        <span>+62 85135961994</span>
                    </li>
                    <li class="flex items-center gap-3">
                        <i class="ph-fill ph-envelope text-cyan-600 dark:text-cyan-500 shrink-0"></i>
...
</div><?php /**PATH C:\Users\ronie\Documents\aplikasi terpadu\sistem_absensi_sekolah\resources\views/landing/footer.blade.php ENDPATH**/ ?>