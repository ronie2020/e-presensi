<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in duration-500" x-data="{ showAddModal: false, isSubmitting: false, imgPreview: null }">
    ...
    <div x-show="showAddModal" x-cloak class="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6" style="display: none;">
        
        <div class="absolute inset-0 bg-slate-900/60 backdrop-blur-sm transition-opacity" 
             x-show="showAddModal" x-transition.opacity @click="showAddModal = false"></div>
        
        <div class="relative bg-white rounded-[2rem] w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh] transform transition-all" 
            ...
            <!-- Form Input -->
            <!-- GANTI ROUTE DI BAWAH INI SESUAI DENGAN ROUTE CONTROLLER PORTAL SISWA KAMU NANTI -->
            <form action="<?php echo e(route('student.achievements.store') ?? '#'); ?>" method="POST" enctype="multipart/form-data" class="flex-1 overflow-y-auto p-6 space-y-5 custom-scrollbar" @submit="isSubmitting = true">
                <?php echo csrf_field(); ?>
...
</div><?php /**PATH E:\aplikasi terpadu\sistem_absensi_sekolah\resources\views/students/portal/partials/tab-prestasi.blade.php ENDPATH**/ ?>