                            ...
                            <tr class="hover:bg-slate-50/80 transition-colors group">
                                <td class="px-6 py-5 align-top">
                                    ...
                                    <div class="flex items-center gap-3 mb-2">
                                        ...
                                        <div class="w-8 h-8 rounded-full bg-slate-100 text-slate-500 border border-slate-200 flex items-center justify-center font-bold text-xs shrink-0 group-hover:bg-elevate-primary group-hover:text-white transition-colors shadow-sm">
                                            <?php echo e($spt->users->count() > 0 ? substr($spt->users->first()->name, 0, 1) : '?'); ?>
                                        ...
                                        <div>
                                            ...
                                            <div class="font-bold text-elevate-dark text-sm group-hover:text-elevate-primary transition-colors leading-tight">
                                                <?php if($spt->users->count() > 0): ?>
                                                    <?php echo e($spt->users->first()->name); ?>

                                                    <?php if($spt->users->count() > 1): ?>
                                                        <span class="text-[10px] text-slate-400 block font-medium mt-0.5">
                                                            <i class="ph-bold ph-plus"></i> <?php echo e($spt->users->count() - 1); ?> Pegawai lainnya
                                                        </span>
                                                    <?php endif; ?>
                                                <?php else: ?>
                                    ...
                                    <?php if(is_array($spt->pengikut) && count($spt->pengikut) > 0): ?>
                                        <div class="text-[10px] font-bold text-slate-500 flex items-center gap-1 mt-2 pl-11">
                                            <i class="ph-fill ph-users"></i> +<?php echo e(count($spt->pengikut)); ?> Pengikut Luar
                                        </div>
                                    <?php endif; ?>
                                </td>
                                <td class="px-6 py-5 align-top">
                                    ...
                                    <div class="text-xs text-slate-500 flex flex-col gap-1 pl-5 font-medium">
                                        <span class="flex items-center gap-1.5"><i class="ph-bold ph-calendar text-[10px]"></i> <?php echo e(\Carbon\Carbon::parse($spt->tgl_berangkat)->isoFormat('D MMM YYYY')); ?></span>
                                        <?php if($spt->lama_hari > 1): ?>
                                            <span class="text-[10px] text-slate-400 uppercase font-black tracking-widest pl-4">s.d</span>
                                            <span class="flex items-center gap-1.5"><i class="ph-bold ph-calendar-check text-[10px]"></i> <?php echo e(\Carbon\Carbon::parse($spt->tgl_kembali)->isoFormat('D MMM YYYY')); ?></span>
                                        <?php endif; ?>
                                        <span class="inline-block mt-2 px-2.5 py-1 bg-white border border-slate-200 rounded-lg text-[10px] text-elevate-primary font-bold w-fit shadow-sm">
                                            <?php echo e($spt->lama_hari); ?> Hari Tugas
                                        </span>
                                    </div>
                                </td>
                                <td class="px-6 py-5 align-top">
                                ...
                                <td class="px-6 py-5 align-top text-right">
                                    <div class="flex flex-col items-end gap-2">
                                        <a href="<?php echo e(route('letters.spt.print', $spt->id)); ?>" target="_blank" class="inline-flex items-center gap-2 px-4 py-2 bg-elevate-accent/10 border border-elevate-accent/20 text-elevate-primary hover:bg-elevate-primary hover:text-white rounded-xl text-xs font-bold transition-all shadow-sm group/btn">
