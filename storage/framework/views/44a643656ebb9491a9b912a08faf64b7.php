                ...
                <div class="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
                    ...
                    <div class="flex flex-row md:flex-col lg:flex-row gap-4 w-full md:w-auto">
                        <div class="bg-white/60 backdrop-blur-md px-6 py-5 rounded-2xl border border-white/80 flex-1 md:flex-none min-w-[140px] text-center md:text-left hover:bg-white transition-colors shadow-sm">
                            ...
                            <span class="block text-3xl font-black text-elevate-dark tracking-tight"><?php echo e($classes->count()); ?></span>
                        </div>
                    </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                ...
                <div class="lg:col-span-1">
                    <div class="bg-white p-8 rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-slate-100 relative overflow-hidden sticky top-24 group/form hover:shadow-2xl hover:shadow-elevate-accent/10 transition-all duration-300">
                        ...
                        <form action="<?php echo e(route('classes.store')); ?>" method="POST" class="space-y-5">
                            <?php echo csrf_field(); ?>
                            ...
                            <div>
                                ...
                                <div class="relative group">
                                    ...
                                    <input type="text" name="name" value="<?php echo e(old('name')); ?>" required placeholder="Contoh: 7A" 
                                           class="w-full pl-11 pr-4 py-3.5 rounded-2xl border-slate-200 bg-elevate-soft focus:bg-white focus:border-elevate-accent focus:ring-4 focus:ring-elevate-accent/20 font-bold text-elevate-dark transition-all shadow-sm placeholder:font-medium placeholder:text-slate-400 <?php $__errorArgs = ['name'];
...
$message = $__bag->first($__errorArgs[0]); ?>
                                    <p class="mt-1.5 ml-1 text-xs text-rose-500 font-bold flex items-center gap-1"><i class="ph-bold ph-warning"></i> <?php echo e($message); ?></p>
                                <?php unset($message);
if (isset($__messageOriginal)) { $message = $__messageOriginal; }
endif;
unset($__errorArgs, $__bag); ?>
                        ...
                        <div class="p-8 border-b border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-6">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-elevate-dark/60 font-bold uppercase tracking-wider mt-1">Total <?php echo e($classes->count()); ?> Kelas</p>
                                </div>
                            </div>
                                ...
                                <tbody class="divide-y divide-slate-50">
                                    <?php $__empty_1 = true; $__currentLoopData = $classes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $class): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                        <tr class="hover:bg-elevate-soft/30 transition-colors group"
                                            x-show="search === '' || '<?php echo e(strtolower($class->name)); ?>'.includes(search.toLowerCase()) || '<?php echo e(strtolower($class->homeroomTeacher->name ?? '')); ?>'.includes(search.toLowerCase())"
                                            ...
                                            <td class="px-8 py-5 whitespace-nowrap">
                                                <div class="flex items-center gap-4">
                                                    <div class="w-12 h-12 rounded-2xl bg-elevate-soft text-elevate-primary font-black flex items-center justify-center text-sm shadow-sm border border-elevate-accent/30 group-hover:bg-elevate-primary group-hover:text-white transition-all">
                                                        <?php echo e(substr($class->name, 0, 3)); ?>

                                                    </div>
                                                    <div>
                                                        <span class="block font-black text-elevate-dark text-base"><?php echo e($class->name); ?></span>
                                                        <span class="text-[10px] font-bold text-elevate-dark/50 uppercase tracking-wide">ID: <?php echo e($class->id); ?></span>
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td class="px-8 py-5 whitespace-nowrap text-right">
                                                <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity transform translate-x-2 group-hover:translate-x-0 duration-200">
                                                    ...
                                                    <a href="<?php echo e(route('classes.edit', $class->id)); ?>" class="w-10 h-10 flex items-center justify-center rounded-xl bg-white border border-slate-200 text-slate-400 hover:text-elevate-primary hover:border-elevate-accent hover:bg-elevate-soft transition-all shadow-sm" title="Edit Kelas">
                                                    ...
                                                    <form action="<?php echo e(route('classes.destroy', $class->id)); ?>" 
                                                          method="POST" 
                                                          id="delete-form-<?php echo e($class->id); ?>" class="shrink-0 block">
                                                        <?php echo csrf_field(); ?> 
...
<?php endif; ?><?php /**PATH E:\aplikasi terpadu\sistem_absensi_sekolah\resources\views/classes/index.blade.php ENDPATH**/ ?>