            ...
            <main class="flex-1 flex flex-col h-full bg-slate-100 relative z-0">
                ...
                <div class="p-4 bg-white border-t border-slate-200 flex justify-between items-center relative z-10 shrink-0 shadow-[0_-10px_15px_-3px_rgba(0,0,0,0.05)]">
                    ...
                    <button @click="finishExam" 
                            ...
                            :class="timeLeft > (totalDuration * 0.25) ? 'bg-slate-200 text-slate-400 hover:bg-slate-300' : 'bg-blue-950 text-white hover:bg-blue-900 shadow-lg shadow-blue-950/20'">
                        <i class="ph-bold" :class="timeLeft > (totalDuration * 0.25) ? 'ph-lock' : 'ph-check-circle'"></i>
                        <span x-text="timeLeft > (totalDuration * 0.25) ? 'Waktu Tunggu...' : 'Selesai & Keluar'"></span> 
                    </button>
                </div>
            </main>
        <?php else: ?>
                ...
                <div class="bg-white sm:rounded-[2rem] shadow-sm border-0 sm:border border-slate-200 flex-1 flex flex-col overflow-hidden relative min-h-0">
                    ...
                    <div class="p-4 md:p-6 bg-white border-t border-slate-100 flex justify-between items-center relative z-10 shrink-0">
                        ...
                        <button @click="finishExam" x-show="currentQuestion === totalQuestions - 1" 
                                ...
                                :class="timeLeft > (totalDuration * 0.25) ? 'bg-slate-200 text-slate-400 hover:bg-slate-300' : 'bg-blue-950 text-white hover:bg-blue-900 shadow-blue-950/30'">
                            <span>Kumpulkan</span> 
                            <i class="ph-bold" :class="timeLeft > (totalDuration * 0.25) ? 'ph-lock' : 'ph-check-circle'"></i>
                        </button>
                    </div>
        ...
        <aside class="fixed inset-y-0 right-0 w-80 bg-white shadow-2xl z-40 transform transition-transform duration-300 lg:translate-x-0 lg:static lg:w-80 lg:shadow-none lg:border-l border-slate-200 flex flex-col" 
            ...
            <div class="p-6 border-t border-slate-100 bg-slate-50">
                ...
                <button @click="finishExam()" 
                        ...
                        :class="timeLeft > (totalDuration * 0.25) ? 'bg-slate-200 text-slate-400 hover:bg-slate-300' : 'bg-blue-950 text-white hover:shadow-xl hover:bg-blue-900'">
                    <i class="ph-bold" :class="timeLeft > (totalDuration * 0.25) ? 'ph-lock' : 'ph-paper-plane-right'"></i> 
                    <span x-text="timeLeft > (totalDuration * 0.25) ? 'Terkunci (Waktu Minimal)' : 'Kumpulkan Jawaban'"></span>
                </button>
            </div>
        </aside>
