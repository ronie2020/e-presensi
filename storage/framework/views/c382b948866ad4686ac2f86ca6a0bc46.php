                ...
                <div class="relative rounded-[2rem] bg-gradient-to-br from-cyan-500 via-blue-600 to-blue-900 p-8 text-white shadow-xl shadow-cyan-900/30 overflow-hidden border border-white/10">
                    ...
                    <div class="relative z-10">
                        <div class="flex items-center gap-2 mb-2">
                                <a href="<?php echo e(route('bank.index')); ?>" class="text-xs font-bold text-cyan-200 hover:text-white transition flex items-center gap-1 bg-white/10 px-3 py-1 rounded-full border border-white/20 backdrop-blur-sm"><i class="ph-bold ph-arrow-left"></i> Kembali</a>
                                <span class="text-white/30 text-xs">•</span>
                                <span class="px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-wide bg-white/10 text-cyan-100 border border-white/10"><?php echo e($bank->subject_name); ?></span>
                            </div>
                        <h1 class="text-3xl font-black tracking-tight leading-none text-white mb-2"><?php echo e($bank->title); ?></h1>
                        <p class="text-cyan-50 text-sm font-medium">Total: <?php echo e($bank->questions->count()); ?> Soal • Kode: <?php echo e($bank->code); ?></p>
                    </div>
                </div>
                ...
                <div class="flex flex-col lg:flex-row gap-8 items-start">
                        ...
                        <div class="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/50 lg:sticky lg:top-8">
                            ...
                            <form action="<?php echo e(route('bank.questions.store', $bank->id)); ?>" method="POST" enctype="multipart/form-data" class="space-y-5" id="createQuestionForm">
                                ...
                                <div class="bg-slate-50 rounded-2xl p-4 border border-slate-100">
                                        ...
                                        <div class="space-y-4">
                                            ...
                                            <div class="flex gap-3 items-start group">
                                                ...
                                                <div class="flex-1 space-y-2" x-data="{ optPreview: null }">
                                                    <div class="flex gap-2">
                                                        <input type="text" name="option_<?php echo e($opt); ?>" value="<?php echo e(old('option_'.$opt)); ?>" class="flex-1 rounded-xl border-slate-200 bg-white text-sm py-2 px-3 focus:ring-cyan-500" placeholder="Teks Jawaban <?php echo e($opt); ?>">
                                                        <label class="w-10 h-10 shrink-0 bg-white border border-slate-200 rounded-xl flex items-center justify-center text-slate-400 hover:text-cyan-500 hover:bg-cyan-50 cursor-pointer transition" title="Upload Gambar Opsi <?php echo e($opt); ?>">
                                    ...
                                    <template x-if="createType === 'essay'">
                                        <div>
                                            ...
                                            <input type="text" name="correct_answer" value="<?php echo e(old('correct_answer')); ?>" class="w-full rounded-xl border-slate-200 text-sm py-2 px-3" placeholder="Jawaban singkat (Auto-grade)">
                                        </div>
                                    </template>
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="number" name="score_weight" value="<?php echo e(old('score_weight', 2)); ?>" required class="w-full rounded-xl border-slate-200 bg-slate-50 text-sm font-bold text-center h-11 focus:ring-cyan-500">
                                </div>
                    ...
                    <div class="w-full lg:w-3/5 order-1 lg:order-2 space-y-6">
                        ...
                        <div class="flex flex-col sm:flex-row justify-between items-center px-2 gap-4">
                            <div class="flex items-center gap-3 flex-wrap">
                                ...
                                <?php if($bank->questions->count() > 0): ?>
                                    ...
                                    <a href="<?php echo e(route('bank.questions.print', $bank->id)); ?>" target="_blank" class="px-3 py-1.5 bg-white text-slate-600 border border-slate-200 rounded-lg text-xs font-bold hover:text-cyan-600 hover:bg-cyan-50 hover:border-cyan-200 transition shadow-sm flex items-center gap-1.5">
                        ...
                        <?php $__empty_1 = true; $__currentLoopData = $bank->questions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $q): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            ...
                            <div class="absolute top-6 left-6 w-10 h-10 bg-slate-100 rounded-xl flex items-center justify-center font-black text-slate-500 text-sm group-hover:bg-cyan-600 group-hover:text-white transition-colors shadow-inner"><?php echo e($index + 1); ?></div>
                            
                            
                            <div class="absolute top-8 left-[4.5rem] z-10">
                                <input type="checkbox" value="<?php echo e($q->id); ?>" x-model="selectedQuestions" class="question-checkbox w-5 h-5 rounded border-slate-300 text-cyan-600 focus:ring-cyan-500 cursor-pointer shadow-sm">
                            </div>

                            <div class="pl-20 sm:pl-24">
                                
                                    <div class="mb-2">
                                        <?php if($qType == 'choice'): ?> <span class="text-[10px] font-bold bg-cyan-50 text-cyan-600 px-2 py-0.5 rounded border border-cyan-100">PILIHAN GANDA</span>
                                    ...
                                    <div class="space-y-4">
                                        ...
                                        <div class="flex gap-3 items-start">
                                            ...
                                            <div class="flex-1 space-y-2" x-data="{ optPreviewEdit: null, deleteOptImage: false }">
                                                <div class="flex gap-2">
                                                    <input type="text" name="option_<?php echo e($opt); ?>" x-model="editState.option_<?php echo e($opt); ?>" class="flex-1 rounded-xl border-slate-200 text-sm py-2.5 px-4 focus:ring-cyan-500 font-medium" placeholder="Teks opsi <?php echo e($opt); ?>">
                                                    <label class="w-11 h-11 shrink-0 bg-white border border-slate-200 rounded-xl flex items-center justify-center text-slate-400 hover:text-cyan-500 hover:bg-cyan-50 cursor-pointer transition" title="Ganti Gambar Opsi <?php echo e($opt); ?>">
                                                        <i class="ph-bold ph-image text-lg"></i>
                ...
                <div class="relative transform overflow-hidden rounded-[2.5rem] bg-white text-left shadow-2xl transition-all sm:w-full sm:max-w-md border border-slate-100">
                    ...
                    <form action="<?php echo e(route('bank.questions.import', $bank->id)); ?>" method="POST" enctype="multipart/form-data" class="p-8 space-y-6">
                        ...
                        <div class="bg-cyan-50 p-4 rounded-2xl border border-cyan-100 mb-4">
                            ...
                            <a href="<?php echo e(route('bank.questions.template')); ?>" class="inline-flex items-center gap-1 mt-2 text-xs font-black text-cyan-600 hover:underline">
