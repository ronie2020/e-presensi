            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 sm:gap-4 md:gap-6">
                ...
                <?php $__currentLoopData = $menus; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $menu): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <a href="<?php echo e($menu['link']); ?>" class="relative bg-white/80 backdrop-blur-xl p-4 sm:p-6 rounded-[1.5rem] sm:rounded-[2rem] border border-slate-100 hover:border-cyan-300 hover:shadow-xl hover:shadow-cyan-500/10 hover:-translate-y-1.5 transition-all duration-300 group text-center md:text-left flex flex-col items-center md:items-start overflow-hidden" data-aos="fade-up" data-aos-delay="<?php echo e($loop->index * 50); ?>">
                        ...
                        <div class="w-12 h-12 sm:w-14 sm:h-14 rounded-[1rem] sm:rounded-2xl bg-<?php echo e($menu['color']); ?>-50 text-<?php echo e($menu['color']); ?>-600 flex items-center justify-center text-2xl sm:text-3xl mb-3 sm:mb-4 group-hover:scale-110 group-hover:-rotate-3 transition-transform duration-300 shadow-sm border border-<?php echo e($menu['color']); ?>-100/50">
                            <i class="ph-duotone <?php echo e($menu['icon']); ?>"></i>
                        </div>
                        <h3 class="font-bold text-slate-800 text-xs sm:text-sm md:text-base group-hover:text-cyan-600 transition-colors leading-tight"><?php echo e($menu['title']); ?></h3>
                        <!-- PERBAIKAN: Deskripsi sekarang muncul di HP dengan ukuran teks yang proporsional -->
                        <p class="text-[10px] sm:text-xs text-slate-500 mt-1 sm:mt-1.5 leading-relaxed"><?php echo e($menu['desc']); ?></p>
                    </a>
