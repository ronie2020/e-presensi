...
<?php $component->withAttributes([]); ?>
    ...
    <div x-data="examController(<?php echo e($assignment->duration_minutes * 60); ?>, '<?php echo e($assignment->id); ?>')" 
         x-init="startTimer()" 
        ...
        <div class="py-6 md:py-10 px-4 sm:px-6 lg:px-8 max-w-4xl mx-auto min-h-screen pb-40 md:pb-32">
            ...
            <div class="animate-enter bg-elevate-dark bg-gradient-to-br from-elevate-dark via-elevate-primary to-elevate-dark rounded-[2.5rem] shadow-2xl shadow-elevate-primary/20 border border-white/10 p-8 md:p-10 mb-8 relative overflow-hidden text-white group">
                ...
                <div class="relative z-10">
                    ...
                    <p class="text-elevate-soft/80 leading-relaxed max-w-2xl text-sm md:text-base font-medium"><?php echo e($assignment->description); ?></p>
            ...
            <form action="<?php echo e(route('students.learning.assignment.quiz.submit', $assignment->id)); ?>" method="POST" id="quizForm">
                ...
                <div class="space-y-6 md:space-y-8">
                    <?php $__currentLoopData = $assignment->questions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $q): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="animate-enter bg-elevate-surface rounded-[2rem] shadow-sm border border-elevate-soft overflow-hidden relative hover:shadow-xl hover:shadow-elevate-primary/10 transition-all duration-500 group" style="animation-delay: <?php echo e(($index + 1) * 100); ?>ms">
                                            ...
                                            <?php if(isset($q->options[$opt]) && $q->options[$opt]): ?>
                                                <label class="relative flex items-center gap-4 p-4 md:p-5 rounded-2xl border-2 cursor-pointer transition-all duration-200 group/opt select-none bg-elevate-soft/30 hover:bg-elevate-soft border-elevate-soft hover:border-elevate-accent/50 active:scale-[0.99]">
                                                    ...
                                                    <input type="radio" 
                                                       ...
                                                       class="peer sr-only"
                                                       @change="saveAnswer('<?php echo e($q->id); ?>', $event.target.value)"
                                    ...
                                    <?php else: ?>
                                         <div class="relative">
                                            <textarea name="answers[<?php echo e($q->id); ?>]" rows="5" 
                                                ...
                                                @input.debounce.1000ms="saveAnswer('<?php echo e($q->id); ?>', $event.target.value)"><?php echo e(old("answers.{$q->id}")); ?></textarea>
...
<?php endif; ?><?php /**PATH C:\Users\ronie\Documents\aplikasi terpadu\sistem_absensi_sekolah\resources\views/students/lms/quiz.blade.php ENDPATH**/ ?>