...
<body class="min-h-screen py-8 text-gray-900 ui-sans">
    ...
    <div class="max-w-[210mm] mx-auto mb-6 flex flex-col sm:flex-row justify-between items-center gap-4 no-print bg-white p-4 rounded-2xl shadow-sm border border-slate-200 sticky top-4 z-50">
        <div>
            ...
            <p class="text-xs text-slate-500 font-medium ml-7"><?php echo e($student->name); ?></p>
        </div>

        <div class="flex flex-wrap gap-3 items-center">
            
            <a href="<?php echo e(route('students.index', request()->query())); ?>" class="px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-600 hover:bg-slate-100 transition-colors shadow-sm flex items-center gap-2 group">
                <i class="ph-bold ph-arrow-left group-hover:-translate-x-1 transition-transform"></i> Kembali
            </a>
            
            
            <a href="<?php echo e(route('students.edit', array_merge(['student' => $student->id], request()->query()))); ?>" class="px-4 py-2.5 bg-amber-50 border border-amber-200 text-amber-700 rounded-xl shadow-sm text-xs font-bold hover:bg-amber-100 transition-colors flex items-center gap-2">
