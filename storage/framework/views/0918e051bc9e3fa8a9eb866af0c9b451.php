...
<div class="h-screen w-full bg-slate-900 flex relative overflow-hidden font-sans selection:bg-cyan-500 selection:text-white">
    ...
    <div class="flex w-full h-full p-8 gap-10 pt-12 relative z-10">
        ...
        <div class="flex-1 flex flex-col items-center justify-center h-full">
            ...
            <div class="flex bg-slate-800/50 p-1.5 rounded-2xl border border-slate-700 mb-6 backdrop-blur-md">
                <button onclick="setMode('attendance')" id="btn-mode-attendance" class="px-6 py-2.5 rounded-xl font-bold text-sm transition-all flex items-center gap-2 bg-blue-600 text-white shadow-lg shadow-blue-900/50">
                    <i class="ph-bold ph-user-check"></i> Absensi Masuk
                </button>
                <button onclick="setMode('check')" id="btn-mode-check" class="px-6 py-2.5 rounded-xl font-bold text-sm text-slate-400 hover:text-white transition-all flex items-center gap-2">
            ...
            <!-- OVERDUE ALERT BANNER (Logika Lama Dipertahankan) -->
        ...
        <div class="w-[380px] h-full flex flex-col gap-6 shrink-0">
            ...
            <!-- 2. Rekomendasi Buku (Logika Lama Dipertahankan) -->
    ...
    <input type="text" id="scan-input" class="absolute opacity-0 -top-full" autocomplete="off">
</div>
...
<script>
    ...
    function setMode(mode) {
        ...
        if(mode === 'attendance') {
            btnAtt.className = `px-6 py-2.5 rounded-xl font-bold text-sm transition-all flex items-center gap-2 ${activeClass}`;
            btnChk.className = `px-6 py-2.5 rounded-xl font-bold text-sm transition-all flex items-center gap-2 ${inactiveClass}`;
            instruct.innerText = "Untuk Absensi Perpustakaan";
        } else {
            btnAtt.className = `px-6 py-2.5 rounded-xl font-bold text-sm transition-all flex items-center gap-2 ${inactiveClass}`;
            btnChk.className = `px-6 py-2.5 rounded-xl font-bold text-sm transition-all flex items-center gap-2 bg-purple-600 text-white shadow-lg shadow-purple-900/50`;
            ...
            try {
                ...
                if(data.success) {
                    ...
                    if(currentMode === 'check') {
                        ...
                        // Attendance Mode (GAMIFIKASI TAMPIL DISINI)
                        // Pesan motivasi (sub_message) dari Controller ditampilkan disini
                        ...
                        // Perbaikan: data.sub_message ditampilkan jika ada
                        stateResult.innerHTML = `
                            ${specialEffect}
...
<style>
    .custom-scrollbar::-webkit-scrollbar { width: 0px; } /* Hide Scrollbar UI for cleaner Kiosk look */
