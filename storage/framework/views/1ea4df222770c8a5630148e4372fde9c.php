...
<body class="font-sans antialiased bg-[#f8fafc] text-[#2c3f61]">
                ...
                <div class="flex items-center gap-4">
                    <div class="hidden md:block text-right mr-2">
                        <?php if(Auth::guard('student')->check()): ?>
                            <p class="text-sm font-bold text-[#2c3f61]"><?php echo e(Auth::guard('student')->user()->name); ?></p>
                            <p class="text-xs text-slate-400 font-mono font-bold"><?php echo e(Auth::guard('student')->user()->student_id); ?></p>
                        <?php endif; ?>
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            <?php if(session('success')): ?> Toast.fire({ icon: 'success', title: '<?php echo session('success'); ?>' }); <?php endif; ?>
            <?php if(session('error')): ?> Toast.fire({ icon: 'error', title: '<?php echo session('error'); ?>' }); <?php endif; ?>
            <?php if(session('warning')): ?> Toast.fire({ icon: 'warning', title: '<?php echo session('warning'); ?>' }); <?php endif; ?>
            <?php if(session('info')): ?> Toast.fire({ icon: 'info', title: '<?php echo session('info'); ?>' }); <?php endif; ?>
        });
...
</html><?php /**PATH C:\Users\ronie\Documents\aplikasi terpadu\sistem_absensi_sekolah\resources\views/cbt/seb_landing.blade.php ENDPATH**/ ?>