...
<div class="min-h-screen w-full flex flex-col items-center justify-center relative overflow-hidden bg-elevate-surface font-sans py-12 px-4 sm:px-6">
    ...
    <div class="w-full max-w-lg z-10 animate-enter">
        ...
        <a href="<?php echo e(url('/')); ?>" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white/60 border border-white/60 text-elevate-dark font-bold text-sm hover:bg-white transition-all shadow-sm backdrop-blur-md mb-6 active:scale-95">
        ...
        <div class="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/40 border border-slate-100 overflow-hidden">
            ...
            <div class="p-8 md:p-10">
                <?php if(isset($customError) || session('error')): ?>
                    <div class="mb-6 p-4 bg-[#FDE7E9] border border-[#F4C3C9] text-[#D13438] rounded-2xl text-sm font-bold flex items-start gap-3 shadow-sm">
                        ...
                        <p><?php echo e($customError ?? session('error')); ?></p>
                    </div>
                <?php endif; ?>
                ...
                <?php if(!$isOpen && isset($announcementDate)): ?>
                    ...
                    <div class="text-center">
                        ...
                        <p class="text-xs text-slate-400 font-medium italic mt-4">Tanggal: <?php echo e($announcementDate->translatedFormat('d F Y, H:i')); ?> WIB</p>
                    </div>
                <?php else: ?>
                    
                    <form action="<?php echo e(route('ppdb.search')); ?>" method="POST" class="space-y-6">
                        <?php echo csrf_field(); ?>
                        <div>
