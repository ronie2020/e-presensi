<?php $__env->startSection('title', 'Pojok Literasi: Artikel & Opini Guru - ' . config('app.name', 'SMP Negeri 3 Lakbok')); ?>
...
<?php $__env->startSection('content'); ?>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-16 relative z-20 animate-enter" style="animation-delay: 100ms;">
        <div class="bg-white/80 backdrop-blur-xl rounded-[2.5rem] p-4 shadow-xl shadow-elevate-primary/5 border border-white">
            <form action="<?php echo e(route('articles.index')); ?>" method="GET" class="flex flex-col md:flex-row gap-4">
                <!-- Search Input -->
                <div class="flex-1 relative">
                    <div class="absolute inset-y-0 left-0 pl-5 flex items-center pointer-events-none">
                        <i class="ph-bold ph-magnifying-glass text-elevate-dark/30 text-lg"></i>
                    </div>
                    <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Cari judul artikel atau nama penulis..." 
                ...
                <div class="md:w-64 shrink-0 relative">
                    <select name="category" class="w-full px-5 py-4 bg-white border-slate-100 focus:border-elevate-accent focus:ring-4 focus:ring-elevate-accent/10 rounded-2xl text-sm font-bold text-elevate-dark transition-all appearance-none cursor-pointer shadow-sm">
                        ...
                        <option value="pendidikan" <?php echo e(request('category') == 'pendidikan' ? 'selected' : ''); ?>>Pendidikan</option>
                        <option value="opini" <?php echo e(request('category') == 'opini' ? 'selected' : ''); ?>>Opini</option>
                        <option value="teknologi" <?php echo e(request('category') == 'teknologi' ? 'selected' : ''); ?>>Teknologi</option>
        ...
        <?php if(request('search') || request('category')): ?>
            <div class="mb-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <p class="text-elevate-dark/50 font-bold text-sm bg-elevate-soft px-4 py-2 rounded-full w-fit">
                    Hasil pencarian: <span class="text-elevate-primary">"<?php echo e(request('search') ?: request('category')); ?>"</span>
                </p>
                <a href="<?php echo e(route('articles.index')); ?>" class="text-sm font-black text-rose-500 hover:text-rose-600 flex items-center gap-1.5 transition-colors">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 sm:gap-10">
                ...
                <article class="bg-elevate-surface rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-2xl hover:shadow-elevate-accent/10 hover:border-elevate-accent/30 overflow-hidden group hover:-translate-y-2 transition-all duration-500 flex flex-col h-full animate-enter" style="animation-delay: <?php echo e(($index % 6) * 100); ?>ms">
                    ...
                    <div class="p-8 flex flex-col flex-1">
                        ...
                        <div class="flex items-center justify-between mb-6">
                            <a href="<?php echo e(route('teachers.show', $article->user_id)); ?>" class="flex items-center gap-2.5 group/author">
                                <div class="w-9 h-9 rounded-full bg-elevate-soft overflow-hidden border-2 border-white shadow-sm shrink-0">
                                    <img src="<?php echo e(optional($article->user)->photo_path ? asset('storage/' . $article->user->photo_path) : 'https://ui-avatars.com/api/?name='.urlencode(optional($article->user)->name ?? 'Anonim').'&background=e5eff5&color=0d52a1'); ?>" 
                                         alt="Penulis" 
                                         class="w-full h-full object-cover">
                                </div>
                                <span class="text-xs font-black text-elevate-dark group-hover/author:text-elevate-primary transition-colors line-clamp-1"><?php echo e(optional($article->user)->name ?? 'Penulis'); ?></span>
                            </a>
                        ...
                        <!-- Judul & Excerpt -->
                        <a href="<?php echo e($article->url ?? '#'); ?>" target="<?php echo e($article->url ? '_blank' : '_self'); ?>" class="block mb-4">
                            <h3 class="text-xl font-black text-elevate-dark leading-tight line-clamp-2 group-hover:text-elevate-primary transition-colors"><?php echo e($article->title); ?></h3>
                        </a>
                        
                        <p class="text-sm text-elevate-dark/60 line-clamp-3 mb-8 flex-1 font-medium leading-relaxed">
                            <?php echo e(Str::limit(strip_tags($article->excerpt), 150)); ?>

                        </p>

                        <!-- Tombol Baca -->
                        <div class="mt-auto pt-6 border-t border-slate-50">
                            <a href="<?php echo e($article->url ?? route('teachers.show', $article->user_id)); ?>" 
                               target="<?php echo e($article->url ? '_blank' : '_self'); ?>" 
            ...
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                <div class="col-span-full py-24 text-center animate-enter bg-white rounded-[3rem] border-2 border-dashed border-slate-200">
                    ...
                    <?php if(request('search') || request('category')): ?>
                        <a href="<?php echo e(route('articles.index')); ?>" class="inline-flex items-center gap-2 px-6 py-3 bg-elevate-dark text-white font-bold rounded-xl hover:bg-elevate-primary transition-all shadow-lg shadow-elevate-dark/20">
...
<?php echo $__env->make('layouts.public', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\aplikasi terpadu\sistem_absensi_sekolah\resources\views/articles/index.blade.php ENDPATH**/ ?>