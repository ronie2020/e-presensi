...
<div class="min-h-screen w-full bg-slate-900 relative overflow-x-hidden font-sans selection:bg-cyan-500 selection:text-white" x-data="kioskData()">
...
<?php endif; ?>
            ...
            <div id="status-box" class="w-full max-w-3xl aspect-[16/8] md:aspect-[16/7] bg-slate-800/40 backdrop-blur-md rounded-[2rem] md:rounded-[2.5rem] flex flex-col items-center justify-center relative transition-all duration-500 group overflow-visible border border-slate-700 hover:border-cyan-500/50 shadow-2xl">
                ...
                <div id="state-standby" class="flex flex-col items-center z-10 transition-transform duration-300 group-hover:scale-105 p-4 text-center">
                    ...
                    <p class="text-cyan-300/70 mt-2 font-bold text-xs md:text-sm tracking-widest uppercase">Tap Kartu / Scan QR</p>
                </div>
            ...
            <!-- MODE SELECTOR (Tombol Cepat) -->
             <div class="mt-6 w-full max-w-3xl">
                <div class="grid grid-cols-3 md:grid-cols-6 gap-2 md:gap-3">
                    <button onclick="window.setMode('Masuk', true)" class="bg-slate-800/60 hover:bg-cyan-900/40 border border-slate-700 hover:border-cyan-500 text-slate-300 hover:text-cyan-400 py-3 rounded-xl backdrop-blur-sm transition-all flex flex-col items-center gap-1 group">
                    ...
                    <button onclick="window.setMode('Pulang', true)" class="bg-slate-800/60 hover:bg-purple-900/40 border border-slate-700 hover:border-purple-500 text-slate-300 hover:text-purple-400 py-3 rounded-xl backdrop-blur-sm transition-all flex flex-col items-center gap-1 group">
                    ...
                    <button onclick="window.setMode('Makan', true)" class="bg-slate-800/60 hover:bg-orange-900/40 border border-slate-700 hover:border-orange-500 text-slate-300 hover:text-orange-400 py-3 rounded-xl backdrop-blur-sm transition-all flex flex-col items-center gap-1 group">
                    ...
                    <button onclick="window.setMode('Dhuha', true)" class="bg-slate-800/60 hover:bg-emerald-900/40 border border-slate-700 hover:border-emerald-500 text-slate-300 hover:text-emerald-400 py-3 rounded-xl backdrop-blur-sm transition-all flex flex-col items-center gap-1 group">
                    ...
                    <button onclick="window.setMode('Dhuhur', true)" class="bg-slate-800/60 hover:bg-emerald-900/40 border border-slate-700 hover:border-emerald-500 text-slate-300 hover:text-emerald-400 py-3 rounded-xl backdrop-blur-sm transition-all flex flex-col items-center gap-1 group">
                    ...
                    <button @click="openExtraModal" class="bg-slate-800/60 hover:bg-pink-900/40 border border-slate-700 hover:border-pink-500 text-slate-300 hover:text-pink-400 py-3 rounded-xl backdrop-blur-sm transition-all flex flex-col items-center gap-1 group">
    ...
    <!-- Modal Ekskul (Menggunakan AlpineJS) -->
    <div x-show="showExtraModal" x-transition class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-md" x-cloak>
        <div class="bg-slate-800 rounded-3xl border border-slate-700 p-6 w-full max-w-md shadow-2xl" @click.away="showExtraModal = false">
            ...
            <div class="grid grid-cols-2 gap-3 max-h-[60vh] overflow-y-auto custom-scrollbar">
                <?php $__currentLoopData = $extracurriculars; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ex): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <button @click="selectExtra('<?php echo e($ex->id); ?>', '<?php echo e($ex->name); ?>')" class="p-3 bg-slate-700/50 hover:bg-pink-900/30 border border-slate-600 hover:border-pink-500 rounded-xl text-left transition-all group">
                        <span class="font-bold text-slate-300 group-hover:text-pink-300 text-sm block"><?php echo e($ex->name); ?></span>
                    </button>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </div>
            <button @click="showExtraModal = false" class="mt-4 w-full py-3 bg-slate-700 hover:bg-slate-600 text-white font-bold rounded-xl transition-colors">Batal</button>
        </div>
    </div>
...
<script>
    function kioskData() {
        return {
            ...
            selectExtra(id, name) {
                ...
                setTimeout(() => { isProcessing = false; document.getElementById('scan-input').focus(); }, 500);
            }
        }
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function playBeep(type) {
            ...
            else { osc.type = 'sawtooth'; osc.frequency.setValueAtTime(150, audioCtx.currentTime); } // Error sound
        ...
        function addToLog(name, type, message, photoUrl = null) {
            ...
            else if (type === 'warning') cardClass = "bg-slate-800/50 border-slate-700 hover:border-amber-500/50"; // Yellow for double scan
            else if (type === 'inactive') cardClass = "bg-slate-900 border-slate-700 hover:border-slate-500"; // Dark for inactive
            else cardClass = "bg-rose-900/20 border-rose-500/20 hover:border-rose-500/50"; // Red for Not Found

            if (photoUrl) {
                let borderStatus = type === 'success' ? 'border-emerald-500' : (type === 'warning' ? 'border-amber-500' : 'border-rose-500');
                avatarHtml = `<div class="flex-shrink-0 w-10 h-10 md:w-12 md:h-12 rounded-xl border-2 ${borderStatus} overflow-hidden bg-slate-800 shadow-lg"><img src="${photoUrl}" class="w-full h-full object-cover"></div>`;
            } else {
        ...
        document.addEventListener('keydown', (e) => {
            ...
            if(e.key === 'F6') { e.preventDefault(); document.querySelector('[x-data]').__x.$data.openExtraModal(); }
        ...
        async function processScan(data) {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    // SISWA NON-AKTIF (Dark/Black)
                    playBeep('error');
                    // Tampilan khusus siswa non-aktif
        ...
        function showResultUI(bgClass, shadowClass, iconClass, name, message, photoUrl) {
            ...
            let mainVisualHtml = photoUrl 
                ? `<div class="relative mb-4 group"><div class="w-32 h-32 md:w-40 md:h-40 rounded-full border-4 border-white/30 shadow-2xl overflow-hidden relative z-10"><img src="${photoUrl}" class="w-full h-full object-cover"></div><div class="absolute -bottom-2 -right-2 bg-white text-emerald-600 rounded-full p-2 border-4 border-emerald-600 shadow-lg z-20"><i class="ph-bold ${iconClass} text-2xl"></i></div></div>`
