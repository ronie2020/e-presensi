    ...
    <script>
        ...
        window.examApp = function() {
            return {
                ...
                saveStatus: 'idle', // idle, saving, saved, error, pending
                saveQueue: {}, // Untuk Debounce: Object menyimpan timeout ID
                showMobileMap: false,
                zoomedImage: null,
                violationCount: 0,
                maxViolations: 3,
                ...
                initData() {
                    try {
                        ...
                        // Sinkronisasi waktu target agar hitungan mundur akurat meski tab di-minimize
                        this.endTimeTarget = now + (this.timeLeft * 1000);
                        
                        try { this.loadLocalProgress(); } catch (e) { console.error("Local storage error", e); }
                        
                        // Merge jawaban dari server dengan lokal (prioritas lokal yang belum tersinkron)
                        ...
                        // Cek apakah masih ada yang pending dari sesi sebelumnya
                ...
                // FUNGSI UTAMA: Select Answer dengan Debounce
                selectAnswer(questionId, answer) {
                    this.answers[questionId] = answer;
                    this.unsavedQuestions.add(questionId); 
                    this.saveStatus = 'pending';
                    
                    // 1. Simpan ke Local Storage dulu (Instan)
                    try { this.saveToLocal(); } catch(e){}

                    // 2. Clear timeout sebelumnya jika user mengklik cepat (Debounce)
                    if (this.saveQueue[questionId]) {
                        clearTimeout(this.saveQueue[questionId]);
                    }

                    // 3. Set timeout baru (Kirim ke server setelah 1 detik tidak ada aktivitas klik pada soal ini)
                ...
                async pushAnswerToServer(questionId, answer) {
                    ...
                    try {
                        ...
                        try { this.saveToLocal(); } catch(e){} // Update local storage bahwa ini sudah clean
                        ...
                        this.saveStatus = 'error'; // Beri visual feedback merah
                        // Jawaban tetap ada di unsavedQuestions, akan dicoba lagi nanti via syncPendingAnswers
                    }
                },
                ...
                async syncPendingAnswers() {
                    ...
                    // Kirim satu per satu (bisa diubah jadi bulk insert endpoint jika backend mendukung)
                    for (const qId of pendingIds) { 
                        if (this.answers[qId]) {
                            await this.pushAnswerToServer(qId, this.answers[qId]); 
                        } else {
                            // Case aneh: ada di unsaved tapi tidak ada jawaban, hapus dari list
                ...
                triggerViolation() {
                    ...
                    // Kirim log pelanggaran ke server jika perlu (optional)
                    if(this.violationCount >= this.maxViolations) { 
                        alert('DISKUALIFIKASI: Pelanggaran batas aturan.'); 
                ...
                startTimer() {
                    this.timerInterval = setInterval(() => {
                        ...
                        // Update Time Left (ini lebih akurat daripada decrement variabel)
                ...
                finishExam() {
                    ...
                    let htmlContent = remaining > 0 
                        ? `Masih ada <b class='text-rose-600'>${remaining}</b> soal kosong.` 
                        : "Pastikan semua jawaban sudah benar.";

                    // Cek status simpan sebelum konfirmasi
                ...
                async submitExam(forced = false) {
                    ...
                    Swal.fire({ 
                        ...
                        html: 'Mohon tunggu, sedang menyinkronkan jawaban.',
...
<body class="h-screen flex flex-col"
    ...
    <div class="flex-1 flex overflow-hidden relative">
        ...
        <!-- MAIN QUESTION AREA -->
        <main class="flex-1 flex flex-col h-full bg-slate-5 relative z-0 overflow-y-auto custom-scroll">
                    ...
                    <div class="flex-1 p-6 md:p-8 overflow-y-auto custom-scroll">
                        <template x-if="questions.length > 0 && questions[currentQuestion]">
                            <div>
                                ...
                                <!-- Text Question -->
                                <div class="prose prose-slate max-w-none mb-8 select-none">
