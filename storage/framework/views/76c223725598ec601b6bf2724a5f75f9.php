            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-10">
                ...
                <div class="bg-gradient-to-br from-[#25D0FF] via-[#5295FF] to-[#FFC9B9] rounded-xl p-8 text-[#2A3B52] shadow-[0_10px_40px_-10px_rgba(37,208,255,0.4)] relative overflow-hidden group border border-white/40 animate-enter">
                    ...
                    <div class="relative z-10 h-full flex flex-col justify-between">
                        <a href="<?php echo e(route('dashboard')); ?>" class="group/btn bg-white/40 hover:bg-white/60 text-[#2A3B52] px-4 py-2 rounded-xl font-bold text-xs backdrop-blur-sm border border-white/50 transition-all flex items-center gap-2 shadow-sm w-fit mb-4">
            ...
            <?php if($schedules->count() > 0): ?>
                <div class="grid grid-cols-1 gap-5">
                    <?php $__currentLoopData = $schedules; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $schedule): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        ...
                        <div class="bg-white rounded-xl fluent-card p-6 <?php echo e($borderClass); ?> flex flex-col md:flex-row justify-between items-center gap-6 group relative overflow-hidden animate-enter" style="animation-delay: <?php echo e(($index + 3) * 100); ?>ms">
                            
                            <div class="flex items-center gap-5 w-full md:w-auto z-10">
                                <div class="flex flex-col items-center justify-center w-20 h-20 rounded-xl <?php echo e($bgIcon); ?> shrink-0 shadow-sm border">
                                ...
                                <div>
                                    <h4 class="font-black text-[#2A3B52] text-xl group-hover:text-[#5295FF] transition-colors"><?php echo e($schedule->subject->name); ?></h4>
                            ...
                            <div class="w-full md:w-auto z-10">
                                <?php if($status == 'waiting'): ?>
                                    <form action="<?php echo e(route('teaching.start', $schedule->id)); ?>" method="POST">
                                        <?php echo csrf_field(); ?>
                                        <button type="submit" class="w-full md:w-auto px-8 py-3.5 <?php echo e($btnClass); ?> font-bold rounded-xl shadow-md transition transform flex items-center justify-center gap-2 active:scale-95 border border-transparent">
                                ...
                                <?php elseif($status == 'ongoing'): ?>
                                    <div class="flex flex-col md:items-end gap-2">
                                        ...
                                        <a href="<?php echo e(route('teaching.show', $session->id)); ?>" class="w-full md:w-auto px-8 py-3.5 <?php echo e($btnClass); ?> font-bold rounded-xl shadow-md transition transform flex items-center justify-center gap-2 active:scale-95 border border-transparent">
                                ...
                                <?php else: ?>
                                    <div class="flex items-center gap-3">
                                        ...
                                        <a href="<?php echo e(route('teaching.show', $session->id)); ?>" class="p-2.5 bg-white border border-slate-200 rounded-xl text-slate-400 hover:text-[#5295FF] hover:border-[#D0E7F8] hover:bg-[#F3F9FD] transition shadow-sm active:scale-95" title="Lihat Detail">
...
<?php endif; ?><?php /**PATH C:\Users\ronie\Documents\aplikasi terpadu\sistem_absensi_sekolah\resources\views/teaching/index.blade.php ENDPATH**/ ?>