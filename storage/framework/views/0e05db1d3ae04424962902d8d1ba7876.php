                        ...
                        <div class="bg-slate-900 rounded-[2rem] shadow-2xl p-5 text-center text-white relative overflow-hidden group border border-slate-800">
                            <div class="relative z-10">
                                <div class="flex items-center justify-between mb-4">
                                    ...
                                    <div class="text-[10px] font-mono text-slate-500">READY</div>
                                </div>
                                ...
                                <div class="mb-4 relative group/input">
                                    ...
                                    <input type="text" id="rfidInput" x-model="rfidCode" @keydown.enter.prevent="submitScan()"
                                        ...
                                        class="relative w-full bg-slate-800/80 border-2 border-slate-700 focus:border-blue-500 text-white rounded-2xl text-center font-mono text-lg tracking-[0.2em] py-4 transition-all focus:ring-4 focus:ring-blue-500/20 uppercase placeholder:text-slate-600 shadow-inner"
                                ...
                                <?php $__currentLoopData = $allStudents; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $student): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    ...
                                    <div class="relative border rounded-2xl p-3 flex items-center gap-3 transition-all duration-300 bg-white border-slate-200" 
                                        ...
                                        <div class="w-10 h-10 rounded-2xl flex items-center justify-center font-bold text-xs shrink-0 shadow-sm transition-all"
                                             ...
                                             <template x-if="!status"> <span><?php echo e(substr($student->name, 0, 1)); ?></span> </template>
                                        </div>
    ...
    <script>
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('teachingSession', (config) => ({
                ...
                keepFocus(event) {
                    if (!this.showCamera) {
                        // Jika sedang klik input jurnal, jangan paksa pindah fokus ke scanner
                        if (event && event.relatedTarget && event.relatedTarget.classList.contains('journal-input')) return;
                        
                        // FIX SCROLL: preventScroll: true agar layar tidak loncat ke atas di HP
                        setTimeout(() => document.getElementById('rfidInput').focus({ preventScroll: true }), 100);
                    }
                },
                ...
                async setManual(studentId, status) {
                    try {
                        ...
                        if(data.status === 'success') {
                            this.updateLocalStats(data.old_status, status);
                            // Update UI Baris Siswa menggunakan Custom Event
                            window.dispatchEvent(new CustomEvent('update-status-' + studentId, { detail: { status: status } }));
                            ...
                            const statusText = status ? (statusMap[status] || status.toUpperCase()) : 'DIRESET';
                ...
                async submitScan() {
                    ...
                    this.rfidCode = '';
                    document.getElementById('rfidInput').focus({ preventScroll: true });
                },
...
<?php endif; ?><?php /**PATH C:\Users\ronie\Documents\aplikasi\E-Presensi Netila\resources\views/teaching/show.blade.php ENDPATH**/ ?>