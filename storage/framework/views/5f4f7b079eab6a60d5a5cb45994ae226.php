...
<nav x-data="{
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-20 items-center">
            ...
            <a href="<?php echo e(url('/')); ?>" class="flex items-center gap-3 shrink-0 group z-50">
                <div class="relative w-10 h-10 bg-white rounded-xl flex items-center justify-center text-blue-600 shadow-lg shadow-blue-900/20 group-hover:rotate-6 transition-transform overflow-hidden border border-white/20">
                ...
                <div class="flex flex-col leading-tight">
                    ...
                    <span class="font-bold text-cyan-300 uppercase tracking-widest group-hover:text-cyan-100 transition-colors">Berjaya </span>
                    <span class="text-[8px] font-bold text-blue-200 uppercase tracking-widest group-hover:text-white transition-colors">Unggul & Berkarakter </span>
                </div>
            </a>
            ...
            <div class="hidden md:flex items-center gap-8">
                <div class="flex gap-6 text-sm font-medium text-blue-100">
                    ...
                    <a href="#profil" class="hover:text-cyan-300 transition-colors">Profil</a>
                    <a href="#akademik" class="hover:text-cyan-300 transition-colors">Akademik</a>                        
                    <a href="#galeri" class="hover:text-cyan-300 transition-colors">Galeri</a>
                    <a href="#kontak" class="hover:text-cyan-300 transition-colors">Kontak</a>   
                </div>                    
                ...
                <?php if(Auth::guard('student')->check()): ?>
                    <a href="<?php echo e(route('students.learning.index')); ?>" class="px-5 py-2.5 rounded-full bg-cyan-500 text-blue-950 text-xs font-bold shadow-lg shadow-cyan-500/40 hover:bg-cyan-400 transition border border-cyan-400 flex items-center gap-2 group">
                        <span>Dashboard</span>
                        <i class="ph-bold ph-arrow-right group-hover:translate-x-1 transition-transform"></i>
                    </a>
                <?php else: ?>
                    <a href="<?php echo e(route('library.catalogue')); ?>" class="text-sm font-bold text-cyan-200 hover:text-white transition flex items-center gap-2">
                        Katalog Buku
                    </a>
                    <a href="<?php echo e(route('ppdb.create')); ?>" class="text-sm font-bold text-cyan-200 hover:text-white transition flex items-center gap-2">
                        PPDB
                    </a> 
                    <a href="<?php echo e(route('portal.index')); ?>" class="mr-2 text-sm font-bold text-blue-200 hover:text-white transition">
                        Portal Siswa
                    </a>
                    <!-- Tombol Staff -->
                    <a href="<?php echo e(route('login')); ?>" class="px-5 py-2.5 rounded-full bg-white/10 backdrop-blur-md text-white text-xs font-bold hover:bg-white hover:text-blue-700 transition border border-white/20 flex items-center gap-2">
    ...
    <div x-show="mobileMenuOpen" x-cloak
            ...
            class="fixed inset-0 bg-blue-950/98 backdrop-blur-xl z-[60] md:hidden flex flex-col pt-24 px-6 overflow-y-auto">
            
        <nav class="flex flex-col items-center space-y-6 text-center w-full px-8">
            <!-- Mobile PPDB Link -->
            <a href="<?php echo e(route('ppdb.create')); ?>" class="w-full py-3 bg-cyan-500 rounded-xl text-blue-950 font-bold text-lg shadow-lg shadow-cyan-500/30">
                <i class="ph-bold ph-student mr-2"></i> Info PPDB 2025
            </a>
            
            <a href="#profil" @click="mobileMenuOpen = false" class="text-2xl font-bold text-blue-100 hover:text-cyan-300 transition">Profil Sekolah</a>
            <a href="#guru" @click="mobileMenuOpen = false" class="text-2xl font-bold text-blue-100 hover:text-cyan-300 transition">Guru & Staff</a>
            <a href="#kegiatan" @click="mobileMenuOpen = false" class="text-2xl font-bold text-blue-100 hover:text-cyan-300 transition">Kegiatan</a>
            <a href="#prestasi" @click="mobileMenuOpen = false" class="text-2xl font-bold text-blue-100 hover:text-cyan-300 transition">Prestasi</a>
            <a href="#ekskul" @click="mobileMenuOpen = false" class="text-2xl font-bold text-blue-100 hover:text-cyan-300 transition">Ekskul</a>
            
            <hr class="w-16 border-blue-800/50">

            <div class="flex flex-col gap-4 w-full pb-10">
                <a href="<?php echo e(route('portal.index')); ?>" class="text-lg font-bold text-cyan-400">Portal Siswa</a>
                <?php if(Auth::guard('student')->check()): ?>
                    <a href="<?php echo e(route('students.learning.index')); ?>" class="block w-full py-3 rounded-xl bg-cyan-600 text-white font-bold shadow-lg shadow-cyan-900/30">Dashboard Siswa</a>
                <?php else: ?>
                    <a href="<?php echo e(route('login')); ?>" class="block w-full py-3 rounded-xl bg-white/10 border border-white/20 text-white font-bold">Login Staff</a>
                <?php endif; ?>
            </div>
        </nav>
    </div>
        ...
        <div x-show="searchOpen"
            ...
            <div class="p-5 bg-slate-50 dark:bg-slate-800/30">
                ...
                <div class="flex flex-wrap gap-2">
                    <a href="<?php echo e(route('ppdb.create')); ?>" class="px-3 py-1.5 bg-white dark:bg-slate-800 rounded-lg text-sm font-medium text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 hover:border-cyan-500 dark:hover:border-cyan-500 hover:text-cyan-600 dark:hover:text-cyan-400 transition shadow-sm">🎓 Pendaftaran PPDB</a>
                    <a href="<?php echo e(route('library.catalogue')); ?>" class="px-3 py-1.5 bg-white dark:bg-slate-800 rounded-lg text-sm font-medium text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 hover:border-cyan-500 dark:hover:border-cyan-500 hover:text-cyan-600 dark:hover:text-cyan-400 transition shadow-sm">📚 Katalog E-Book</a>
...
</nav><?php /**PATH C:\Users\ronie\Documents\aplikasi terpadu\sistem_absensi_sekolah\resources\views/landing/navbar.blade.php ENDPATH**/ ?>