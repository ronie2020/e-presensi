...
<nav x-data="{
        ...
        <div class="flex justify-between items-center transition-all duration-500"
            ...
            <a href="<?php echo e(url('/')); ?>" class="flex items-center gap-3 shrink-0 group z-50">
                <div class="relative w-10 h-10 md:w-12 md:h-12 bg-white dark:bg-slate-800 rounded-xl md:rounded-2xl flex items-center justify-center text-elevate-primary shadow-sm border border-slate-100 dark:border-slate-700 group-hover:scale-105 group-hover:rotate-3 transition-transform overflow-hidden">
                    <img src="<?php echo e(asset('images/logo.png')); ?>" alt="Logo" class="w-7 h-7 md:w-8 md:h-8 object-contain z-10" onerror="this.style.display='none'; this.nextElementSibling.style.display='block'">
            ...
            <div class="hidden md:flex items-center gap-4">
                ...
                <?php if(Auth::guard('student')->check()): ?>
                    <a href="<?php echo e(route('students.learning.index')); ?>" class="px-6 py-2.5 rounded-full bg-elevate-accent hover:bg-elevate-accent/80 text-elevate-dark text-xs font-bold shadow-lg shadow-elevate-accent/20 transition-all flex items-center gap-2 group">
                        <span>Dashboard Siswa</span>
                        <i class="ph-bold ph-arrow-right group-hover:translate-x-1 transition-transform"></i>
                    </a>
                <?php else: ?>
                    <div class="flex items-center gap-3 mr-2">
                        <a href="<?php echo e(route('library.catalogue')); ?>" class="text-xs font-bold text-slate-700 dark:text-slate-300 hover:text-elevate-primary dark:hover:text-elevate-accent transition flex items-center gap-1.5">
                            <i class="ph-bold ph-books text-sm"></i> Katalog
                        </a>
                        <a href="<?php echo e(route('ppdb.create')); ?>" class="text-xs font-bold text-slate-700 dark:text-slate-300 hover:text-elevate-primary dark:hover:text-elevate-accent transition flex items-center gap-1.5">
                    ...
                    <div class="flex items-center gap-2">
                        <a href="<?php echo e(route('login')); ?>" class="px-4 py-2.5 rounded-full bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-xs font-bold border border-slate-200 dark:border-slate-700 hover:text-elevate-primary dark:hover:text-elevate-accent hover:bg-slate-50 transition-all shadow-sm flex items-center gap-1.5">
                            <i class="ph-bold ph-lock-key"></i> Akses Guru
                        </a>
                        <a href="<?php echo e(route('portal.index')); ?>" class="px-5 py-2.5 rounded-full bg-elevate-dark dark:bg-elevate-primary text-white text-xs font-black hover:bg-elevate-primary dark:hover:bg-elevate-accent dark:hover:text-elevate-dark transition-all shadow-lg shadow-elevate-dark/20 flex items-center gap-1.5">
    ...
    <div x-show="mobileMenuOpen" x-cloak
        ...
        <nav class="flex flex-col items-center space-y-6 text-center w-full px-4">
            <!-- Mobile PPDB Link -->
            <a href="<?php echo e(route('ppdb.create')); ?>" class="w-full py-4 bg-elevate-primary hover:bg-elevate-dark rounded-2xl text-white font-black text-lg shadow-xl shadow-elevate-primary/30 flex justify-center items-center transition-colors">
            ...
            <div class="flex flex-col gap-4 w-full mt-2">
                <a href="<?php echo e(route('portal.index')); ?>" class="text-lg font-black text-elevate-accent text-center mb-2">Pusat Portal Siswa</a>
                
                <?php if(Auth::guard('student')->check()): ?>
                    <a href="<?php echo e(route('students.learning.index')); ?>" class="block w-full py-3.5 rounded-xl bg-elevate-dark text-white font-black shadow-lg shadow-elevate-dark/30">Dashboard Siswa</a>
                <?php else: ?>
                    <a href="<?php echo e(route('portal.index')); ?>" class="block w-full py-3.5 rounded-xl bg-elevate-dark text-white font-black shadow-lg shadow-elevate-dark/20 flex items-center justify-center gap-2">
                        <i class="ph-bold ph-sign-in text-elevate-accent"></i> Portal Siswa
                    </a>
                    <a href="<?php echo e(route('login')); ?>" class="block w-full py-3.5 rounded-xl bg-white dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 font-bold flex items-center justify-center gap-2 mt-2">
                        <i class="ph-bold ph-lock-key text-elevate-primary"></i> Login Staff / Guru
                    </a>
                    <a href="<?php echo e(route('library.catalogue')); ?>" class="block w-full py-3.5 rounded-xl bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-300 font-bold flex items-center justify-center gap-2 mt-2">
        ...
        <div x-show="searchOpen"
            ...
            <div class="p-6 bg-slate-50 dark:bg-slate-800/30">
                ...
                <div class="flex flex-wrap gap-2.5">
                    <a href="<?php echo e(route('ppdb.create')); ?>" class="px-4 py-2 bg-white dark:bg-slate-800 rounded-xl text-sm font-bold text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 hover:border-elevate-accent dark:hover:border-elevate-accent hover:text-elevate-primary dark:hover:text-elevate-accent transition shadow-sm">🎓 Pendaftaran PPDB</a>
                    <a href="<?php echo e(route('library.catalogue')); ?>" class="px-4 py-2 bg-white dark:bg-slate-800 rounded-xl text-sm font-bold text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 hover:border-elevate-accent dark:hover:border-elevate-accent hover:text-elevate-primary dark:hover:text-elevate-accent transition shadow-sm">📚 Katalog E-Book</a>
