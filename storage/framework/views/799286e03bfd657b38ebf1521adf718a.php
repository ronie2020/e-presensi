    ...
    <div x-data="{ 
        ...
        removeCurrentImage() {
         ...
         <?php $__env->slot('header', null, []); ?> 
            <h2 class="font-semibold text-xl text-[#2c3f61] leading-tight"><?php echo e(__('Kelola Soal Ujian')); ?></h2>
         <?php $__env->endSlot(); ?>
        ...
        <div class="py-8 sm:py-10 font-sans text-[#2c3f61]">
                    ...
                    <div class="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                        <div>
                            <div class="flex items-center gap-2 mb-2">
                                <a href="<?php echo e(route('cbt.index')); ?>" class="text-xs font-bold text-[#0d52a1] hover:text-[#2c3f61] transition flex items-center gap-1 bg-white/60 px-3 py-1 rounded-full border border-white backdrop-blur-sm shadow-sm"><i class="ph-bold ph-arrow-left"></i> Dashboard</a>
                                <span class="text-[#2c3f61]/30 text-xs">•</span>
                                <span class="px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-wide bg-white/60 text-[#0d52a1] border border-white"><?php echo e($exam->subject_name); ?></span>
                            </div>
                        ...
                        <div class="flex flex-wrap items-center gap-3 w-full md:w-auto">
                            <?php if($exam->questions->count() > 0): ?>
                                ...
                                <a href="<?php echo e(route('cbt.questions.export_excel', $exam->id)); ?>" class="group px-4 py-2.5 bg-white/80 text-[#0d52a1] font-bold rounded-xl hover:bg-white transition flex items-center justify-center gap-2 border border-white text-xs shadow-sm">
                                    <i class="ph-bold ph-file-xls text-lg"></i> <span>Download Excel</span>
                                </a>

                                
                                <a href="<?php echo e(route('cbt.export_word', $exam->id)); ?>" class="group px-4 py-2.5 bg-white/80 text-[#0d52a1] font-bold rounded-xl hover:bg-white transition flex items-center justify-center gap-2 border border-white text-xs shadow-sm">
                                    <i class="ph-bold ph-file-doc text-lg"></i> <span>Export Word</span>
                                </a>

                                
                                <a href="<?php echo e(route('cbt.preview', $exam->id)); ?>" target="_blank" class="group px-4 py-2.5 bg-[#2c3f61] text-white font-bold rounded-xl hover:bg-[#1c2940] transition flex items-center justify-center gap-2 border border-transparent text-xs shadow-lg shadow-[#2c3f61]/20">
                ...
                <?php else: ?>
                    ...
                    <div class="flex flex-col lg:flex-row gap-8 items-start">
                            ...
                            <div class="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/50 lg:sticky lg:top-8">
                                <div class="flex items-center gap-3 mb-6 pb-4 border-b border-slate-50">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-[10px] text-slate-400 font-bold mt-1 uppercase tracking-wider">Mendukung Rumus $...$</p>
                                    </div>
                                </div>
                                ...
                                <form action="<?php echo e(route('cbt.questions.store', $exam->id)); ?>" method="POST" enctype="multipart/form-data" class="space-y-5" id="createQuestionForm">
                                    ...
                                    <div>
                                        ...
                                        <trix-editor input="q_input_create" @trix-change="createQuestionText = $event.target.value" placeholder="Tulis soal di sini... Contoh Rumus: $x^2$" class="prose prose-sm max-w-none border-slate-200 focus:border-[#56bbf1] transition-colors rounded-2xl"></trix-editor>
                                    </div>
                                                ...
                                                <div class="flex gap-3 items-start group">
                                                    ...
                                                    <div class="flex-1 space-y-2 min-w-0" x-data="{ optPreview: null }">
                                                        <div class="flex gap-2 min-w-0">
                                                            <input type="text" name="option_<?php echo e($opt); ?>" class="flex-1 min-w-0 rounded-xl border-slate-200 bg-white text-[#2c3f61] text-sm py-2 px-3 focus:ring-[#56bbf1] focus:border-[#56bbf1] transition-colors" placeholder="Teks Jawaban <?php echo e($opt); ?>">
                                                            <label class="w-10 h-10 shrink-0 bg-white border border-slate-200 rounded-xl flex items-center justify-center text-slate-400 hover:text-[#0d52a1] hover:bg-[#e5eff5] hover:border-[#56bbf1] cursor-pointer transition" title="Upload Gambar Opsi <?php echo e($opt); ?>">
                        ...
                        <div class="w-full lg:w-3/5 order-1 lg:order-2 space-y-6">
                            ...
                            <form id="bulkDeleteForm" action="<?php echo e(route('cbt.questions.bulk_delete', $exam->id)); ?>" method="POST" class="hidden">
                                <?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
                                <input type="hidden" name="question_ids" :value="selectedQuestions.join(',')">
                            </form>
                            <form id="bulkWeightForm" action="<?php echo e(route('cbt.questions.bulk_weight', $exam->id)); ?>" method="POST" class="hidden">
                            ...
                            <div class="flex flex-col sm:flex-row justify-between items-center px-2 gap-4">
                                ...
                                <?php if($exam->questions->count() > 0): ?>
                                    <a href="<?php echo e(route('cbt.questions.print', $exam->id)); ?>" target="_blank" class="ml-2 px-3 py-1.5 bg-white text-slate-600 border border-slate-200 rounded-lg text-xs font-bold hover:text-[#0d52a1] hover:bg-[#e5eff5] hover:border-[#56bbf1]/50 transition shadow-sm flex items-center gap-1.5">
                            ...
                            <?php $__empty_1 = true; $__currentLoopData = $exam->questions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $q): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                ...
                                <div x-show="questionSearch === '' || '<?php echo e(strtolower(addslashes(strip_tags($q->question_text . ' ' . ($q->tags ?? ''))))); ?>'.includes(questionSearch.toLowerCase())"
                                    ...
                                    <div class="absolute top-6 left-6 w-10 h-10 bg-[#e5eff5] border border-[#56bbf1]/30 rounded-xl flex items-center justify-center font-black text-[#0d52a1] text-sm group-hover:bg-[#0d52a1] group-hover:text-white transition-colors shadow-inner"><?php echo e($index + 1); ?></div>
                                    
                                    
                                    <div class="absolute top-8 left-[4.5rem] z-10">
                                        <input type="checkbox" value="<?php echo e($q->id); ?>" x-model="selectedQuestions" class="question-checkbox w-5 h-5 rounded border-slate-300 text-[#0d52a1] focus:ring-[#56bbf1] cursor-pointer shadow-sm">
                                    </div>

                                    <div class="pl-20 sm:pl-24">
                                        
                                        <div class="mb-2">
                                            <?php if($qType == 'choice'): ?> <span class="text-[10px] font-bold bg-[#56bbf1]/10 text-[#0d52a1] px-2 py-0.5 rounded border border-[#56bbf1]/20">PILIHAN GANDA</span>
                                        ...
                                        <?php endif; ?>
                                        <div class="text-[#2c3f61] font-medium text-lg mb-5 leading-relaxed trix-content prose prose-sm max-w-none"><?php echo $q->question_text; ?></div>
                                        ...
                                        <?php if($qType == 'choice'): ?>
                                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                                                <?php $__currentLoopData = ['A','B','C','D']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $opt): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                    ...
                                                    ?>
                                                    <div class="flex items-start gap-3 p-2.5 rounded-[1.5rem] border transition-colors <?php echo e($opt == $q->correct_answer ? 'bg-emerald-50 border-emerald-200 shadow-sm' : 'bg-slate-50/50 border-slate-100'); ?>">
                                                        <span class="w-8 h-8 flex items-center justify-center rounded-xl border <?php echo e($opt == $q->correct_answer ? 'border-emerald-500 bg-emerald-500 text-white shadow-md' : 'border-slate-300 text-slate-400 bg-white'); ?> text-[10px] font-black shrink-0"><?php echo e($opt); ?></span>
                                                        <div class="flex-1 overflow-hidden mt-1">
                                                            <?php if($val && $val !== '-'): ?>
                                                                <span class="leading-relaxed block <?php echo e($opt == $q->correct_answer ? 'text-emerald-800 font-bold' : 'text-[#2c3f61]/80 font-medium'); ?>"><?php echo e($val); ?></span>
                                                            <?php endif; ?>
                                                            <?php if($imgVal): ?>
                                                        ...
                                                        <div class="flex items-center gap-3 bg-white p-2 rounded-lg border border-slate-100 shadow-sm">
                                                            <div class="flex-1 flex flex-col sm:flex-row items-center sm:items-start gap-2 text-center sm:text-left">
                                                                ...
                                                                <?php if(isset($p['left']) && $p['left'] !== ''): ?>
                                                                    <span class="font-medium text-[#2c3f61] mt-1 sm:mt-0"><?php echo e($p['left']); ?></span>
                                                                <?php endif; ?>
                                                            </div>
                                                            ...
                                                            <div class="flex-1 flex flex-col sm:flex-row items-center sm:items-start gap-2 text-center sm:text-left">
                                                                ...
                                                                <?php if(isset($p['right']) && $p['right'] !== ''): ?>
                                                                    <span class="font-medium text-[#2c3f61] mt-1 sm:mt-0"><?php echo e($p['right']); ?></span>
                                                                <?php endif; ?>
                                                            </div>
                                                        </div>
                                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                    ...
                                    <template x-if="editState.question_type === 'choice'">
                                        <div class="space-y-4">
                                            ...
                                            <div class="flex gap-3 items-start">
                                                <span class="w-10 h-10 rounded-xl bg-slate-50 border border-slate-200 text-slate-500 flex items-center justify-center font-black text-sm shrink-0 mt-1"><?php echo e($opt); ?></span>
                                                <div class="flex-1 space-y-2 min-w-0" x-data="{ optPreviewEdit: null, deleteOptImage: false }">
                                                    <div class="flex gap-2 min-w-0">
                                                        <input type="text" name="option_<?php echo e($opt); ?>" x-model="editState.option_<?php echo e($opt); ?>" class="flex-1 min-w-0 rounded-xl border-slate-200 text-sm py-2.5 px-4 focus:ring-[#56bbf1] focus:border-[#56bbf1] font-medium text-[#2c3f61]" placeholder="Teks opsi <?php echo e($opt); ?>">
                                                        <label class="w-11 h-11 shrink-0 bg-white border border-slate-200 rounded-xl flex items-center justify-center text-slate-400 hover:text-[#0d52a1] hover:bg-[#e5eff5] hover:border-[#56bbf1] cursor-pointer transition" title="Ganti Gambar Opsi <?php echo e($opt); ?>">
                                                            <i class="ph-bold ph-image text-lg"></i>
            ...
            <div x-show="showImportModal" style="display: none;" class="fixed inset-0 z-50 overflow-y-auto">
                ...
                <div class="flex min-h-screen items-center justify-center p-4">
                    <div class="relative bg-white rounded-[2.5rem] shadow-2xl max-w-md w-full p-8 border border-slate-100">
                        ...
                        <form action="<?php echo e(route('cbt.questions.import', $exam->id)); ?>" method="POST" enctype="multipart/form-data" id="importQuestionForm" onsubmit="event.preventDefault(); confirmSubmit(this, 'Import File Excel?', 'Soal dari excel akan ditambahkan ke ujian ini.', '#0d52a1', 'Memproses Data...');">
                            <?php echo csrf_field(); ?>
                        ...
                        <div class="mt-4 text-center">
                            <a href="<?php echo e(route('cbt.questions.template')); ?>" class="text-xs font-bold text-[#0d52a1] hover:underline">Download Template Excel</a>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="flex min-h-screen items-center justify-center p-4">
                    ...
                    <div class="relative bg-white rounded-[2.5rem] shadow-2xl max-w-2xl w-full flex flex-col border border-slate-100 max-h-[90vh]" 
                        x-data="{
                            ...
                            banksData: <?php echo e(Js::from(\App\Models\CbtQuestionBank::with('questions')->latest()->get())); ?>,
                        ...
                        <div class="p-8 pt-4 overflow-y-auto custom-scrollbar">
                            <form action="<?php echo e(route('cbt.import_from_bank', $exam->id)); ?>" method="POST" id="formAmbilBank"
                                @submit.prevent="
            ...
            <script>
                // Fungsi untuk merender ulang MathJax secara real-time
                ...
                // Pantau perubahan pada Trix Editor (saat mengetik)
                document.addEventListener("trix-change", function(event) {
                    clearTimeout(window.mathTimeout);
                    window.mathTimeout = setTimeout(() => {
                        reRenderMath();
                    }, 800); // Render ulang setelah jeda mengetik
                });
                ...
                document.addEventListener('DOMContentLoaded', function() {
                    // Panggil MathJax di awal muat halaman untuk merender daftar soal
                    setTimeout(reRenderMath, 1000);

                    // LOGIKA ASLI: Konfirmasi Hapus Data
                    ...
                    // LOGIKA ASLI: Loading saat submit form soal
                    ...
                    // LOGIKA ASLI: Pesan Sukses/Error Alert
                    ...
                    // LOGIKA ASLI: Validasi Error dari Backend Laravel
                    <?php if($errors->any()): ?>
                        Swal.fire({
                            icon: 'error',
                ...
                // LOGIKA ASLI: Preview Gambar Pop Up
...
<?php endif; ?><?php /**PATH C:\Users\ronie\Documents\aplikasi terpadu\sistem_absensi_sekolah\resources\views/cbt/manage_questions.blade.php ENDPATH**/ ?>