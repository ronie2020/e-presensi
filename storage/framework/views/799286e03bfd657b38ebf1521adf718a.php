                    ...
                    <div class="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                        ...
                        <div class="flex flex-wrap items-center gap-3 w-full md:w-auto">
                            <?php if($exam->questions->count() > 0): ?>
                                ...
                                <a href="<?php echo e(route('cbt.questions.export_excel', $exam->id)); ?>" class="group px-4 py-2.5 bg-emerald-100 text-emerald-700 font-bold rounded-xl hover:bg-emerald-200 transition flex items-center justify-center gap-2 border border-emerald-200 text-xs">
                                    <i class="ph-bold ph-file-xls text-lg"></i> <span>Download Excel</span>
                                </a>

                                
                                <a href="<?php echo e(route('cbt.export_word', $exam->id)); ?>" class="group px-4 py-2.5 bg-blue-100 text-blue-700 font-bold rounded-xl hover:bg-blue-200 transition flex items-center justify-center gap-2 border border-blue-200 text-xs">
                    ...
                    <div class="relative bg-white rounded-[2.5rem] shadow-2xl max-w-2xl w-full flex flex-col border border-slate-100 max-h-[90vh]" 
                        ...
                        <div class="p-8 pb-4 shrink-0 border-b border-slate-100">
                            <div class="flex justify-between items-center mb-6">
                                <h3 class="text-xl font-black text-slate-800 flex items-center gap-2"><i class="ph-fill ph-download-simple text-indigo-600"></i> Ambil dari Bank Soal</h3>
                                <button type="button" @click="showBankModal = false" class="text-slate-400 hover:text-rose-500 transition"><i class="ph-bold ph-x text-xl"></i></button>
                            </div>
                        </div>
                            ...
                            <form action="<?php echo e(route('cbt.import_from_bank', $exam->id)); ?>" method="POST" id="formAmbilBank"
                                @submit.prevent="
                                    if(importMode === 'partial' && selectedBankQuestions.length === 0) {
                                        Swal.fire({icon: 'warning', title: 'Perhatian', text: 'Pilih minimal satu soal!', customClass: {popup: 'rounded-2xl'}});
                                        return;
                                    }
                                    confirmSubmit($el, 'Salin Soal?', (importMode === 'all' ? 'Seluruh soal dari bank akan disalin.' : selectedBankQuestions.length + ' soal akan disalin ke ujian ini.'), '#4f46e5', 'Menyalin Soal...');
                                ">
                                <?php echo csrf_field(); ?>
                                ...
                                <div class="space-y-6">
                                    ...
                                    <div>
                                        ...
                                        <select name="bank_id" x-model="selectedBankId" @change="updateQuestionList()" required class="w-full rounded-xl border-slate-200 font-bold text-slate-700 py-3 px-4 focus:ring-indigo-500">
                                    ...
                                    <div x-show="selectedBankId" x-transition>
                                        ...
                                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                            <label class="cursor-pointer">
                                                ...
                                                <div class="p-4 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 peer-checked:border-indigo-500 peer-checked:bg-indigo-50 peer-checked:text-indigo-700 transition">
                                                    <div class="font-bold text-sm mb-1 flex items-center justify-between">Semua Soal <i class="ph-fill ph-check-circle text-lg opacity-0 peer-checked:opacity-100"></i></div>
                                            ...
                                            <label class="cursor-pointer">
                                                ...
                                                <div class="p-4 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 peer-checked:border-indigo-500 peer-checked:bg-indigo-50 peer-checked:text-indigo-700 transition">
                                                    <div class="font-bold text-sm mb-1 flex items-center justify-between">Pilih Manual <i class="ph-fill ph-check-circle text-lg opacity-0 peer-checked:opacity-100"></i></div>
                                    ...
                                    <div x-show="importMode === 'partial' && selectedBankId" x-transition class="border border-slate-200 rounded-xl overflow-hidden bg-slate-50">
                                        <div class="bg-slate-100 px-4 py-3 border-b border-slate-200 flex justify-between items-center">
                                            <span class="text-xs font-bold text-slate-600">Pilih Soal (<span x-text="selectedBankQuestions.length"></span> dari <span x-text="bankQuestions.length"></span>)</span>
                                            <button type="button" @click="selectAll()" class="text-xs font-bold text-indigo-600 hover:text-indigo-700" x-text="selectedBankQuestions.length === bankQuestions.length ? 'Batal Pilih Semua' : 'Pilih Semua'"></button>
                                        </div>
                                        <div class="max-h-60 overflow-y-auto divide-y divide-slate-100 custom-scrollbar p-2 space-y-2">
                                            <template x-for="(q, index) in bankQuestions" :key="q.id">
                                                <label class="flex items-start gap-3 p-3 rounded-lg bg-white border border-slate-100 hover:border-indigo-200 hover:shadow-sm cursor-pointer transition">
                                                    <div class="pt-0.5">
                                                        <input type="checkbox" :value="q.id" x-model="selectedBankQuestions" class="w-4 h-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer">
                                                    </div>
                                                    <div class="flex-1 min-w-0">
                                                        <div class="flex items-center gap-2 mb-1">
                                                            <span class="text-[10px] font-bold bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded" x-text="'No. ' + (index+1)"></span>
                                                            <span class="text-[10px] font-bold bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded" x-show="q.question_type == 'choice'">PILGANDA</span>
                                                        ...
                                                        <p class="text-xs text-slate-700 font-medium line-clamp-2" x-text="stripHtml(q.question_text)"></p>
                                                    </div>
                                                </label>
                                            </template>
                                    ...
                                    <button type="submit" class="w-full py-3.5 rounded-xl bg-indigo-600 text-white font-bold hover:bg-indigo-700 shadow-lg transition active:scale-95">Salin Soal Terpilih</button>
                                </div>
                            </form>
                        </div>
                    </div>
