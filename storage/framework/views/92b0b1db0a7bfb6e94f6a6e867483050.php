...
<?php $component->withAttributes([]); ?>
    ...
    <div class="py-8 sm:py-10 font-sans text-slate-800" x-data="monitoringData">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
                    ...
                    <div class="relative z-10">
                        <div class="flex flex-wrap items-center gap-3 mb-2">
                            ...
                            <?php if(isset($exam->exam_type) && $exam->exam_type == 'google_form'): ?>
                                <span class="bg-emerald-500/20 border border-emerald-500/50 text-emerald-300 px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider flex items-center gap-1.5 shadow-sm" title="Ujian Terintegrasi Google Form">
            ...
            <div class="flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                <div class="text-[10px] font-bold px-3 py-1.5 rounded-full border shadow-sm flex items-center gap-2 transition-colors duration-300"
                     :class="isUpdating ? 'bg-blue-50 text-blue-600 border-blue-200' : 'bg-white text-slate-400 border-slate-200'">
                    <i class="ph-bold ph-arrows-clockwise text-blue-500" :class="isUpdating ? 'animate-spin' : ''"></i>
            ...
            <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden min-h-[500px]">
                <div class="p-6 border-b border-slate-100 bg-slate-50/50 flex flex-col md:flex-row justify-between items-center gap-4">
                    <h4 class="font-bold text-slate-700 flex items-center gap-2 text-lg">
                        <i class="ph-fill ph-users-three text-blue-500"></i> Peserta Ujian 
                        <span class="bg-slate-200 text-slate-600 text-xs px-2 py-0.5 rounded-full" x-text="students.length"></span>
                    </h4>
                    <div class="relative w-full md:w-72">
                        <i class="ph-bold ph-magnifying-glass absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"></i>
                        <input type="text" x-model="search" placeholder="Cari siswa..." class="w-full pl-10 pr-4 py-2.5 text-sm font-bold border-slate-200 rounded-xl focus:ring-blue-500 focus:border-blue-500 bg-white shadow-sm transition">
                    </div>
                </div>
                    ...
                    <table class="w-full text-left text-sm text-slate-600">
                        ...
                        <tbody class="divide-y divide-slate-50">
                            <template x-for="(student, index) in filteredStudents" :key="student.id">
                                <tr class="hover:bg-blue-50/20 transition group" :class="student.is_active ? 'bg-blue-50/10' : ''">
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <template x-if="student.status !== 'Belum Mengerjakan'">
                                            <div class="flex items-center justify-center gap-2">
                                                ...
                                                <template x-if="!student.is_seb">
                                                    <span title="Browser Biasa (Mode Darurat)" class="flex items-center justify-center w-8 h-8 rounded-lg bg-amber-100 text-amber-600 animate-pulse">
                                                ...
                                                <template x-if="student.device === 'Desktop'">
                                                    <span title="Menggunakan Laptop/PC" class="flex items-center justify-center w-8 h-8 rounded-lg bg-slate-100 text-slate-500">
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <template x-if="examType === 'google_form'">
                                            <span class="text-[10px] font-bold text-slate-500 bg-slate-100 border border-slate-200 px-2 py-1 rounded-lg" title="Nilai ada di platform Google">Cek G-Form</span>
                                        </template>
                                        <template x-if="examType !== 'google_form'">
                                            <span class="font-black text-lg" 
                                                :class="student.status === 'Selesai' ? (student.score > 0 ? 'text-blue-600' : 'text-slate-800') : 'text-slate-300'" 
                                    ...
                                    <td class="px-6 py-4 text-right flex items-center justify-end gap-2">
                                        <template x-if="student.is_active || student.status === 'Selesai'">
                                            <button @click="openPhotoModal(student.id, student.name)" class="w-8 h-8 rounded-lg bg-indigo-50 text-indigo-600 hover:bg-indigo-600 hover:text-white flex items-center justify-center transition" title="Lihat Foto Pengawasan">
                                                <i class="ph-bold ph-camera"></i>
                                            </button>
                                        </template>
                                        <template x-if="student.is_active">
                                            <button @click="confirmResetLogin(getResetUrl(student.id), student.name)" class="w-8 h-8 rounded-lg bg-rose-50 text-rose-500 hover:bg-rose-500 hover:text-white flex items-center justify-center transition" title="Keluarkan Siswa (Reset)">
                    ...
                    <template x-for="student in filteredStudents" :key="'m-' + student.id">
                        <div class="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm relative overflow-hidden">
                            <div class="absolute left-0 top-0 bottom-0 w-1.5" 
                                 :class="student.status == 'Sedang Mengerjakan' ? 'bg-blue-500' : (student.status == 'Selesai' ? 'bg-emerald-500' : 'bg-slate-200')"></div>
                            
                            <div class="pl-3 flex justify-between items-start">
                                <div>
                                    <h5 class="font-bold text-slate-800 text-sm mb-1" x-text="student.name"></h5>
                                    <div class="flex items-center gap-2 mb-2">
                                        <span class="text-[10px] font-bold px-2 py-0.5 rounded border"
                                              :class="student.status == 'Sedang Mengerjakan' ? 'bg-blue-50 text-blue-700 border-blue-100' : (student.status == 'Selesai' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'text-slate-400 border-transparent')"
                                    ...
                                    <template x-if="student.status !== 'Belum Mengerjakan'">
                                        <div class="flex gap-2">
                                            ...
                                            <span x-show="!student.is_seb" class="text-[10px] font-bold text-amber-600 bg-amber-50 px-2 py-0.5 rounded flex items-center gap-1"><i class="ph-fill ph-warning-circle"></i> Browser</span>
                                        </div>
                                    </template>
                                </div>
                                <div class="text-right">
                                    ...
                                    <template x-if="examType === 'google_form'">
                                        <span class="block text-[10px] font-bold text-slate-500 bg-slate-100 border border-slate-200 px-2 py-1 rounded mt-1">Cek G-Form</span>
                                    </template>
                                    <template x-if="examType !== 'google_form'">
                                        <span class="block text-xl font-black text-slate-800 leading-none" x-text="student.score"></span>
                                    </template>
                                </div>
                            </div>
                            ...
                            <template x-if="student.is_active">
                                <div class="mt-3 pt-3 border-t border-slate-50 flex justify-end gap-2">
                                    <button @click="openPhotoModal(student.id, student.name)" class="text-xs font-bold text-indigo-600 bg-indigo-50 px-3 py-1.5 rounded-lg">
                                        <i class="ph-bold ph-camera"></i> Foto
                                    </button>
                                    <button @click="confirmResetLogin(getResetUrl(student.id), student.name)" class="text-xs font-bold text-rose-600 bg-rose-50 px-3 py-1.5 rounded-lg">
        ...
        <template x-teleport="body">
            <div x-show="showPhotoModal" style="display: none;" class="fixed inset-0 z-[999] overflow-y-auto" @keydown.escape.window="showPhotoModal = false">
                ...
                <div class="flex min-h-screen items-center justify-center p-4">
                    <div class="relative bg-white rounded-[2rem] shadow-2xl max-w-3xl w-full p-8 overflow-hidden transform transition-all">
                        <div class="flex justify-between items-center mb-6">
                            <div>
                                <h3 class="text-xl font-black text-slate-800 flex items-center gap-2">
                                    <i class="ph-fill ph-camera text-indigo-600"></i> Log Foto Pengawasan
                                </h3>
                                <p class="text-sm text-slate-500 font-bold" x-text="activeStudentName"></p>
                            </div>
                            <button @click="showPhotoModal = false" class="w-10 h-10 rounded-xl bg-slate-100 text-slate-500 hover:bg-slate-200 flex items-center justify-center transition">
                                ...
                                <template x-for="photo in studentPhotos">
                                    <div class="bg-white p-2 rounded-xl shadow-sm border border-slate-200 group">
                                        ...
                                        <div class="mt-2 flex justify-between items-center px-1">
                                            <span class="text-[10px] font-black text-slate-700 bg-slate-100 px-2 py-0.5 rounded" x-text="photo.time"></span>
                                            <span class="text-[10px] text-slate-400" x-text="photo.ago"></span>
                                        </div>
                                    </div>
                                </template>
                            </div>
...
<?php endif; ?><?php /**PATH E:\aplikasi terpadu\sistem_absensi_sekolah\resources\views\cbt\monitoring.blade.php ENDPATH**/ ?>