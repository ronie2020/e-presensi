<div class="space-y-6 animate-in fade-in duration-500">
    ...
    <form action="<?php echo e(route('student.ramadan.save')); ?>" method="POST" class="grid grid-cols-1 md:grid-cols-12 gap-6">
        <?php echo csrf_field(); ?>
        ...
        <div class="md:col-span-8 space-y-6">
            ...
            <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex items-center justify-between">
                ...
                <input type="checkbox" name="is_fasting" class="w-6 h-6 rounded-lg text-emerald-600 focus:ring-emerald-500 border-slate-300" <?php echo e(($todayRamadanLog->is_fasting ?? true) ? 'checked' : ''); ?>>
            </div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-5 gap-3">
                    ...
                    <label class="cursor-pointer group">
                        ...
                        <div class="p-3 rounded-2xl border-2 border-slate-50 bg-slate-50 text-slate-400 transition-all peer-checked:bg-emerald-50 peer-checked:border-emerald-200 peer-checked:text-emerald-700 flex flex-col items-center gap-2">
                            <span class="text-[10px] font-bold uppercase tracking-widest"><?php echo e($p); ?></span>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="space-y-2">
                        ...
                        <div class="flex gap-2">
                            <input type="text" name="tadarus_surah" value="<?php echo e($todayRamadanLog->tadarus_surah ?? ''); ?>" class="flex-1 bg-slate-50 border-none rounded-xl text-sm font-bold focus:ring-emerald-500" placeholder="Nama Surah">
                            <input type="number" name="tadarus_ayah" value="<?php echo e($todayRamadanLog->tadarus_ayah ?? ''); ?>" class="w-20 bg-slate-50 border-none rounded-xl text-sm font-bold text-center focus:ring-emerald-500" placeholder="Ayat">
                        </div>
                    </div>
                    <div class="space-y-2">
                        <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Target Murojaah</label>
                        <input type="text" name="murojaah_surah" value="<?php echo e($todayRamadanLog->murojaah_surah ?? ''); ?>" class="w-full bg-slate-50 border-none rounded-xl text-sm font-bold focus:ring-emerald-500" placeholder="Contoh: An-Naba">
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm h-full flex flex-col">
                ...
                <div class="space-y-3 flex-1">
                    ...
                    <label class="flex items-center justify-between p-3 rounded-xl bg-slate-50 border border-slate-50 cursor-pointer hover:border-emerald-200 transition-all group">
                        <span class="text-xs font-bold text-slate-600 capitalize group-hover:text-emerald-700"><?php echo e($s); ?></span>
                        <input type="checkbox" name="sunnah_<?php echo e($s); ?>" class="w-5 h-5 rounded text-emerald-600 focus:ring-emerald-500 border-slate-300" <?php echo e($checked ? 'checked' : ''); ?>>
                    </label>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </div>
