                                    ...
                                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                        ...
                                        <?php $__currentLoopData = $availableRoles; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $option): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <label class="relative flex items-start gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all hover:bg-blue-50
                                                <?php echo e(in_array($option, $currentRoles ?? []) ? 'border-blue-500 bg-blue-50/50' : 'border-slate-100 bg-slate-50'); ?>">
                                                <input type="checkbox" name="role[]" value="<?php echo e($option); ?>" 
                                                       ...
                                                       <?php echo e(in_array($option, $currentRoles ?? []) ? 'checked' : ''); ?>>
                                                <div>
                                                    <span class="block text-sm font-bold <?php echo e(in_array($option, $currentRoles ?? []) ? 'text-blue-700' : 'text-slate-600'); ?>">
                                                        <?php echo e($option); ?>
...
unset($__errorArgs, $__bag); ?>
                        ...
                        <?php if(in_array('Guru', $currentRoles ?? []) || in_array('Wali Kelas', $currentRoles ?? []) || in_array('Kepala Sekolah', $currentRoles ?? []) || in_array('Guru Mata Pelajaran', $currentRoles ?? [])): ?>
                        <div class="mt-8 bg-gradient-to-br from-blue-50 to-indigo-50 p-6 rounded-[2rem] border border-blue-100 flex flex-col md:flex-row items-center justify-between gap-6 shadow-sm">
                            <div class="flex items-start gap-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-slate-500 mt-1 leading-relaxed">Kelola riwayat pelatihan, materi ajar, galeri prestasi, dan artikel milik <b class="text-blue-700"><?php echo e($user->name); ?></b>.</p>
                                </div>
                            </div>
                            <a href="<?php echo e(route('portfolio.index', ['user_id' => $user->id])); ?>" class="shrink-0 w-full md:w-auto px-6 py-3.5 bg-blue-600 text-white font-bold rounded-2xl hover:bg-blue-700 shadow-lg shadow-blue-600/20 transition-all flex items-center justify-center gap-2 group">
