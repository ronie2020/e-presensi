        ...
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="animate-enter relative rounded-xl bg-gradient-to-br from-[#25D0FF] via-[#5295FF] to-[#FFC9B9] p-8 md:p-10 mb-10 text-[#2A3B52] shadow-[0_10px_40px_-10px_rgba(37,208,255,0.4)] overflow-hidden border border-white/40 group">
                ...
                <div class="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    <div>
                        <a href="<?php echo e(route('dashboard')); ?>" class="group/btn bg-white/40 hover:bg-white/60 text-[#2A3B52] px-4 py-2 rounded-xl font-bold text-xs backdrop-blur-sm border border-white/50 transition-all flex items-center gap-2 shadow-sm w-fit mb-4 active:scale-95">
                    ...
                    <div class="bg-white/40 backdrop-blur-md p-5 rounded-xl border border-white/50 shadow-sm w-full md:w-auto shrink-0 mt-4 md:mt-0">
                        <form action="<?php echo e(route('teaching.history')); ?>" method="GET" class="flex flex-col gap-2">
            ...
            <div class="space-y-6">
                ...
                <?php $__empty_1 = true; $__currentLoopData = $histories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $history): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    ...
                    <div class="animate-enter bg-white rounded-xl fluent-card overflow-hidden flex flex-col md:flex-row group hover:border-[#5295FF]" style="animation-delay: <?php echo e(($index + 1) * 100); ?>ms">
                        ...
                        <div class="bg-slate-50/50 border-r border-slate-100 p-6 md:w-48 flex flex-row md:flex-col items-center justify-between md:justify-center gap-4 shrink-0 transition-colors group-hover:bg-[#F3F9FD]/30">
                            <div class="text-center">
                                ...
                                <div class="text-4xl font-black text-[#2A3B52] leading-none"><?php echo e($startDate->format('d')); ?></div>
                                <div class="text-xs font-bold text-[#5295FF] mt-1"><?php echo e($startDate->translatedFormat('M Y')); ?></div>
                            </div>
                            ...
                            <div class="text-center flex flex-col items-center">
                                <div class="bg-white border border-slate-200 rounded-lg px-3 py-1.5 shadow-sm">
                                    <div class="text-xs font-bold text-slate-600 font-mono"><?php echo e($startTime->format('H:i')); ?></div>
                                    <div class="text-[9px] text-slate-400 font-bold uppercase tracking-wider text-center mt-0.5">S/D</div>
                                    <div class="text-xs font-bold text-slate-600 font-mono"><?php echo e($history->ended_at ? $endTime->format('H:i') : '...'); ?></div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mt-auto pt-4 border-t border-slate-100">
                                ...
                                <div class="flex items-center gap-2 flex-wrap">
                                    <div class="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-[#DFF6DD] border border-[#B7DFB9] shadow-sm" title="Siswa Hadir">
                                        ...
                                        <span class="text-xs font-bold text-[#107C10]"><?php echo e($history->present_count ?? 0); ?></span>
                                    </div>
                                    <?php if(($history->late_count ?? 0) > 0): ?>
                                        <div class="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-[#FFEFD6] border border-[#FFD8A8] shadow-sm" title="Siswa Terlambat">
                                            ...
                                            <span class="text-xs font-bold text-[#D83B01]"><?php echo e($history->late_count); ?></span>
                                        </div>
                                    <?php endif; ?>
                                    <?php if(($history->alpha_count ?? 0) > 0): ?>
                                        <div class="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-[#FDE7E9] border border-[#F4C3C9] shadow-sm" title="Siswa Alpha">
                                            ...
                                            <span class="text-xs font-bold text-[#D13438]"><?php echo e($history->alpha_count); ?></span>
                                        </div>
                                    <?php endif; ?>
                                </div>

                                
                                <a href="<?php echo e(route('teaching.show', $history->id)); ?>" class="group/link w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-[#2A3B52] hover:bg-[#182436] text-white px-5 py-2.5 rounded-xl text-sm font-bold shadow-md transition-all active:scale-95 border border-transparent">
                ...
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                    <div class="animate-enter relative flex flex-col items-center justify-center py-20 text-center z-10 bg-white rounded-xl fluent-card border-2 border-dashed border-slate-200">
                        ...
                        <a href="<?php echo e(route('teaching.index')); ?>" class="mt-6 px-6 py-3 bg-[#2A3B52] hover:bg-[#182436] text-white font-bold rounded-xl shadow-md transition-all flex items-center gap-2 active:scale-95 border border-transparent">
