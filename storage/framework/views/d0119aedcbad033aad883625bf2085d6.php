        ...
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            <div class="animate-enter relative rounded-[2rem] bg-gradient-to-r from-elevate-accent via-elevate-peach-light to-elevate-peach p-8 md:p-10 mb-10 text-elevate-dark shadow-xl shadow-elevate-accent/20 overflow-hidden border border-white/60 group">
                ...
                <div class="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    ...
                    <div class="bg-white/60 backdrop-blur-md p-5 rounded-[1.5rem] border border-white/60 shadow-lg w-full lg:w-auto shrink-0 mt-4 md:mt-0" x-data="{ filterType: '<?php echo e($filterType ?? 'monthly'); ?>' }">
                        <form action="<?php echo e(route('teaching.history')); ?>" method="GET" class="flex flex-col gap-3">
                            ...
                            <div class="flex flex-col sm:flex-row items-center gap-2">
                                ...
                                <div class="relative w-full sm:w-[190px]">
                                    <input style="display: none;" x-show="filterType === 'daily'" type="date" name="filter_value_daily" value="<?php echo e($filterType == 'daily' ? $filterValue : \Carbon\Carbon::now()->format('Y-m-d')); ?>" :disabled="filterType !== 'daily'" class="w-full bg-white hover:bg-elevate-soft focus:bg-white border border-slate-200 focus:border-elevate-accent focus:ring-elevate-accent/30 text-elevate-dark text-sm font-bold rounded-xl px-4 py-3.5 transition-all shadow-sm cursor-pointer">
                                    
                                    <input style="display: none;" x-show="filterType === 'weekly'" type="week" name="filter_value_weekly" value="<?php echo e($filterType == 'weekly' ? $filterValue : \Carbon\Carbon::now()->format('Y-\WW')); ?>" :disabled="filterType !== 'weekly'" class="w-full bg-white hover:bg-elevate-soft focus:bg-white border border-slate-200 focus:border-elevate-accent focus:ring-elevate-accent/30 text-elevate-dark text-sm font-bold rounded-xl px-4 py-3.5 transition-all shadow-sm cursor-pointer">
                                    
                                    <input style="display: none;" x-show="filterType === 'monthly'" type="month" name="filter_value_monthly" value="<?php echo e($filterType == 'monthly' ? $filterValue : \Carbon\Carbon::now()->format('Y-m')); ?>" :disabled="filterType !== 'monthly'" class="w-full bg-white hover:bg-elevate-soft focus:bg-white border border-slate-200 focus:border-elevate-accent focus:ring-elevate-accent/30 text-elevate-dark text-sm font-bold rounded-xl px-4 py-3.5 transition-all shadow-sm cursor-pointer">
                                </div>
                    ...
                    <div class="animate-enter bg-white rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/40 overflow-hidden flex flex-col md:flex-row group hover:border-elevate-accent/50 transition-colors" style="animation-delay: <?php echo e(($index + 1) * 100); ?>ms">
                        ...
                        <div class="bg-elevate-soft/50 border-r border-slate-100 p-6 md:w-56 flex flex-row md:flex-col items-center justify-between md:justify-center gap-4 shrink-0 transition-colors group-hover:bg-elevate-peach-light/30">
                            <div class="text-center">
                                <div class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1"><?php echo e($history->date->translatedFormat('l')); ?></div>
                                <div class="text-5xl font-black text-elevate-dark leading-none"><?php echo e($history->date->format('d')); ?></div>
                                <div class="text-sm font-bold text-elevate-primary mt-2"><?php echo e($history->date->translatedFormat('M Y')); ?></div>
                            </div>
                            ...
                            <div class="text-center flex flex-col items-center">
                                <div class="bg-white border border-slate-200 rounded-xl px-4 py-2 shadow-sm">
                                    <div class="text-sm font-black text-elevate-dark"><?php echo e($history->started_at->format('H:i')); ?></div>
                                    <div class="text-[9px] text-slate-400 font-bold uppercase tracking-widest text-center mt-0.5">S/D</div>
                                    <div class="text-sm font-black text-elevate-dark"><?php echo e($history->ended_at ? $history->ended_at->format('H:i') : '...'); ?></div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5 mt-auto pt-5 border-t border-slate-100">
                                ...
                                <div class="flex items-center gap-3 flex-wrap">
                                    <div class="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-[#DFF6DD] border border-[#B7DFB9] shadow-sm" title="Siswa Hadir">
                                        ...
                                        <span class="text-sm font-black text-[#107C10]"><?php echo e($history->total_hadir); ?></span>
                                    </div>
                                    <?php if($history->jml_telat > 0): ?>
                                        <div class="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-[#FFEFD6] border border-[#FFD8A8] shadow-sm" title="Siswa Terlambat">
                                            ...
                                            <span class="text-sm font-black text-[#D83B01]"><?php echo e($history->jml_telat); ?></span>
                                        </div>
                                    <?php endif; ?>
                                    <?php if($history->jml_alpha > 0): ?>
                                        <div class="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-[#FDE7E9] border border-[#F4C3C9] shadow-sm" title="Siswa Alpha">
                                            ...
                                            <span class="text-sm font-black text-[#D13438]"><?php echo e($history->jml_alpha); ?></span>
                                        </div>
                                    <?php endif; ?>
                                </div>
                ...
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                    <div class="animate-enter relative flex flex-col items-center justify-center py-24 text-center z-10 bg-white rounded-[2.5rem] border-2 border-dashed border-slate-200 shadow-sm">
                        ...
                        <p class="text-slate-500 text-sm mt-3 max-w-sm mx-auto font-medium leading-relaxed">
                            Aktivitas mengajar Anda pada <span class="font-bold text-slate-700"><?php echo e($filterLabel); ?></span> belum terekam.
                        </p>
