...
<?php $component->withAttributes([]); ?>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-8">
            <div class="animate-enter relative rounded-[2rem] bg-elevate-gradient-main p-8 sm:p-10 text-elevate-dark shadow-xl shadow-elevate-accent/10 overflow-hidden border border-white/60 group">
                ...
                <div class="relative z-10 flex flex-col xl:flex-row justify-between items-start xl:items-center gap-8">
                    <div class="space-y-3">
                        <a href="<?php echo e(route('dashboard')); ?>" class="group bg-white hover:bg-slate-50 text-[#2A3B52] px-5 py-2.5 rounded-xl font-bold text-sm border border-slate-100 transition-all flex items-center gap-2 shadow-sm hover:shadow-md w-fit mb-4 mx-auto xl:mx-0">
                ...
                <div class="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
                    ...
                    <div class="px-6 py-5 border-b border-slate-100 bg-white flex flex-col sm:flex-row justify-between items-center gap-4">
                        <div class="flex items-center gap-2">
                            <span class="bg-elevate-accent/10 border border-elevate-accent/20 text-elevate-primary px-4 py-1.5 rounded-lg text-xs font-bold shadow-sm">Total: <?php echo e($students->total()); ?> Siswa</span>
                        </div>
                                ...
                                <tr class="hover:bg-slate-50/50 transition-colors group">
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <div class="font-bold text-elevate-dark group-hover:text-elevate-primary transition-colors"><?php echo e($student->name); ?></div>
                                    ...
                                    <td class="px-6 py-4">
                                        <select name="students[<?php echo e($student->id); ?>][status]" class="w-36 py-2 px-3 rounded-lg text-xs font-bold border-slate-200 focus:ring-elevate-primary focus:border-elevate-primary cursor-pointer shadow-sm text-elevate-dark
                                            <?php echo e($student->graduation?->status == 'LULUS' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : ''); ?>

                                            <?php echo e($student->graduation?->status == 'TIDAK LULUS' ? 'bg-rose-50 text-rose-600 border-rose-100' : ''); ?>">
                                    ...
                                    <td class="px-6 py-4 text-center bg-white border-l border-slate-100">
                                        <div class="flex items-center justify-center gap-2">
                                            <button type="button" onclick="saveSingle('<?php echo e($student->id); ?>')" class="w-8 h-8 flex items-center justify-center rounded-lg text-slate-400 hover:bg-elevate-accent/10 hover:text-elevate-primary border border-transparent hover:border-elevate-accent/20 transition-all" title="Simpan Baris Ini">
                                                <i class="ph-bold ph-check text-lg"></i>
                                            </button>
                                            
                                            <?php if($student->graduation?->status == 'LULUS'): ?>
                                                <a href="<?php echo e(route('graduation.print', $student->id)); ?>" target="_blank" class="w-8 h-8 flex items-center justify-center rounded-lg text-emerald-600 hover:bg-emerald-50 border border-transparent hover:border-emerald-200 transition-all" title="Cetak SKL">
                                                    <i class="ph-bold ph-printer text-lg"></i>
                                                </a>
                                            <?php endif; ?>
                                        </div>
                                        <span id="msg_<?php echo e($student->id); ?>" class="text-[10px] text-emerald-600 font-bold hidden animate-pulse mt-1">Tersimpan!</span>
                                    </td>
                                </tr>
    ...
    <dialog id="modalSkl" class="rounded-[2.5rem] p-0 w-full max-w-md bg-transparent shadow-2xl border border-white/20 backdrop-blur-md">
        <form action="<?php echo e(route('admin.graduation.bulk_skl')); ?>" method="POST" class="bg-white p-8 rounded-[2.5rem]">
            <?php echo csrf_field(); ?>
    ...
    <dialog id="modalGlobalDate" class="rounded-[2.5rem] p-0 w-full max-w-md bg-transparent shadow-2xl border border-white/20 backdrop-blur-md">
        <form action="<?php echo e(route('admin.graduation.set_date')); ?>" method="POST" class="bg-white p-8 rounded-[2.5rem]">
            <?php echo csrf_field(); ?>
            <div class="flex items-center gap-3 mb-6 border-b border-slate-100 pb-4">
    ...
    <dialog id="modalImport" class="rounded-[2.5rem] p-0 w-full max-w-md bg-transparent shadow-2xl border border-white/20 backdrop-blur-md">
        <form action="<?php echo e(route('admin.graduation.import')); ?>" method="POST" enctype="multipart/form-data" class="bg-white p-8 rounded-[2.5rem]">
            <?php echo csrf_field(); ?>
    ...
    <dialog id="modalSettings" class="rounded-[2.5rem] p-0 w-full max-w-md bg-transparent shadow-2xl border border-white/20 backdrop-blur-md">
        <form action="<?php echo e(route('admin.graduation.save_settings')); ?>" method="POST" class="bg-white p-8 rounded-[2.5rem]">
            <?php echo csrf_field(); ?>
            <div class="flex items-center gap-3 mb-6 border-b border-slate-100 pb-4">
            ...
            <div class="space-y-5 mb-8">
                <div>
                    ...
                    <input type="text" name="letter_number" value="<?php echo e($settings['letter_number'] ?? ''); ?>" required class="w-full py-3 px-4 rounded-xl border-slate-200 text-sm font-bold text-elevate-dark focus:border-elevate-primary focus:ring-elevate-primary shadow-sm transition-all" placeholder="Contoh: 421.3/ ... /SMP.03/2026">
                ...
                <div>
                    ...
                    <input type="text" name="principal_name" value="<?php echo e($settings['principal_name'] ?? ''); ?>" required class="w-full py-3 px-4 rounded-xl border-slate-200 text-sm font-bold text-elevate-dark focus:border-elevate-primary focus:ring-elevate-primary shadow-sm transition-all" placeholder="Nama beserta gelar">
                </div>
                <div>
                    <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 ml-1">NIP Kepala Sekolah</label>
                    <input type="text" name="principal_nip" value="<?php echo e($settings['principal_nip'] ?? ''); ?>" required class="w-full py-3 px-4 rounded-xl border-slate-200 text-sm font-bold text-elevate-dark focus:border-elevate-primary focus:ring-elevate-primary shadow-sm transition-all" placeholder="NIP Kepala Sekolah">
                </div>
            </div>
...
<?php endif; ?><?php /**PATH C:\Users\ronie\Documents\aplikasi terpadu\sistem_absensi_sekolah\resources\views/admin/graduation/index.blade.php ENDPATH**/ ?>