...
<div class="space-y-8 animate-in fade-in duration-500 font-sans">
    ...
    <?php if(!$isAlumni && isset($student) && !$student->is_validated): ?>
        <div class="flex flex-col sm:flex-row items-center gap-4 p-5 md:p-6 rounded-[2rem] bg-rose-50 dark:bg-rose-500/10 border border-rose-200 dark:border-rose-500/20 shadow-sm relative overflow-hidden group">
            ...
            <div class="shrink-0 w-full sm:w-auto z-10 mt-2 sm:mt-0">
                <a href="<?php echo e(route('students.verify')); ?>" class="w-full sm:w-auto px-6 py-3.5 bg-rose-600 hover:bg-rose-700 text-white text-sm font-black rounded-xl transition-all shadow-lg shadow-rose-600/30 flex items-center justify-center gap-2 transform active:scale-95">
                ...
                <div class="relative overflow-hidden rounded-[2.5rem] bg-elevate-dark p-8 text-white shadow-xl shadow-elevate-dark/20 lg:p-10 border border-elevate-primary/30">
                    ...
                    <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                        <div>
                            ...
                            <div class="flex flex-wrap items-center gap-3 md:gap-4 text-sm font-medium text-slate-300 mt-4">
                                ...
                                <span class="flex items-center gap-1.5 bg-white/10 px-3 py-1.5 rounded-lg border border-white/5">
                                    <i class="ph-fill ph-book-open text-elevate-accent"></i> <?php echo e($priorityExam->subject_name); ?>
                                ...
                                <span class="flex items-center gap-1.5 bg-white/10 px-3 py-1.5 rounded-lg border border-white/5">
                                    <i class="ph-fill ph-timer text-elevate-peach"></i> <?php echo e($priorityExam->duration_minutes); ?> Menit
                                </span>
                        ...
                        <div class="shrink-0">
                            ...
                            <?php else: ?>
                                <a href="<?php echo e(route('student.exam.show', $priorityExam->id)); ?>" class="group relative inline-flex items-center justify-center gap-2 overflow-hidden rounded-2xl bg-white px-8 py-4 text-sm font-bold text-elevate-dark transition-all hover:bg-elevate-soft hover:scale-105 shadow-lg">
                   ...
                   <div class="w-full bg-white dark:bg-slate-800 rounded-2xl shadow-xl shadow-elevate-dark/5 border border-slate-100 dark:border-slate-700/50 overflow-hidden transform group-hover:rotate-1 transition-transform duration-500 mb-3">
                        <div class="bg-elevate-primary h-7 flex items-center justify-center relative">
                         ...
                         <div class="bg-slate-50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-700/50 py-1.5 text-center px-1">
                            ...
                            <span class="text-[8px] font-bold text-slate-400 dark:text-slate-500 block border-t border-slate-200 dark:border-slate-700/50 pt-0.5 mt-0.5">
                                <i class="ph-bold ph-moon-stars text-elevate-accent mr-0.5"></i> <?php echo e($hijriDateFull); ?>
                    ...
                    <div class="w-full space-y-2">
                        ...
                        <button @click="updateTab('kebiasaan')" 
                            ...
                            <i class="ph-fill ph-sun-horizon text-xs <?php echo e(isset($todayEntry) && $todayEntry ? 'text-elevate-primary' : 'text-slate-400'); ?>"></i>
                ...
                <div class="flex-1 min-w-0 flex flex-col">
                    ...
                    <div class="flex-1 space-y-3 relative z-10 overflow-y-auto custom-scrollbar max-h-[350px] pr-1">
                        <?php $__empty_1 = true; $__currentLoopData = $todaysSchedule ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $schedule): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            ...
                            ?>
                            <div class="flex items-center gap-3 p-3 rounded-2xl transition-all border <?php echo e($isActive ? 'bg-elevate-soft dark:bg-elevate-dark/30 border-elevate-accent/50 dark:border-elevate-accent shadow-sm' : ($isPast ? 'bg-slate-50 dark:bg-slate-800/50 border-transparent opacity-60' : 'bg-white dark:bg-slate-800 border-slate-100 dark:border-slate-700/50 hover:border-elevate-accent/50 dark:hover:border-elevate-accent')); ?>">
                                
                                <div class="w-12 text-center shrink-0">
                                    <p class="text-[10px] font-black <?php echo e($isActive ? 'text-elevate-primary dark:text-elevate-accent' : 'text-slate-700 dark:text-slate-300'); ?>">
                                ...
                                <div class="flex-1 min-w-0">
                                    <h4 class="font-bold text-xs truncate <?php echo e($isActive ? 'text-elevate-dark dark:text-elevate-accent' : 'text-slate-800 dark:text-slate-200'); ?>">
                                    ...
                                    <div class="flex items-center gap-1 text-[10px] text-slate-500 dark:text-slate-400 mt-0.5 truncate">
                                        <i class="ph-fill ph-user <?php echo e($isActive ? 'text-elevate-primary' : 'text-slate-400'); ?>"></i>
                ...
                <?php $__empty_1 = true; $__currentLoopData = $pendingTasks ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $task): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    ...
                    <div class="p-4 rounded-2xl border <?php echo e($isUrgent ? 'bg-rose-50 dark:bg-rose-500/10 border-rose-100 dark:border-rose-500/20' : 'bg-white dark:bg-slate-800 border-slate-100 dark:border-slate-700/50'); ?> hover:shadow-md transition-all group/task">
                        <div class="flex justify-between items-start mb-2">
                            <span class="text-[10px] font-black uppercase tracking-wider <?php echo e($isUrgent ? 'text-rose-600 dark:text-rose-400' : 'text-elevate-primary dark:text-elevate-accent'); ?>">
            ...
            <?php $__currentLoopData = $upcomingAgendas; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $agenda): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                ...
                <div class="flex items-start gap-4 p-4 rounded-2xl border border-slate-100 dark:border-slate-700/50 bg-slate-50/50 dark:bg-slate-800/50 hover:bg-white dark:hover:bg-slate-800 hover:shadow-sm <?php echo e($borderHover); ?> transition-all relative overflow-hidden">
                    ...
                    <div>
                        ...
                        <h4 class="font-bold text-elevate-dark dark:text-slate-200 text-sm mb-1 line-clamp-1" title="<?php echo e($agenda->title); ?>"><?php echo e($agenda->title); ?></h4>
                        ...
                        <div class="flex flex-wrap justify-center gap-3">
                            <?php if($student->alumniProfile): ?>
                                <div class="flex flex-col items-end">
                                    ...
                                    <span class="font-black text-elevate-peach-dark dark:text-amber-400 text-lg"><?php echo e($student->alumniProfile->activity_status); ?></span>
                                </div>
                            <?php else: ?>
                                <a href="<?php echo e(Route::has('alumni.tracer') ? route('alumni.tracer') : '#'); ?>" class="group inline-flex items-center gap-2 px-6 py-3 bg-elevate-peach-dark hover:bg-elevate-peach text-white rounded-xl font-bold shadow-xl shadow-elevate-peach/30 transition-all">
            ...
            <div class="bg-white dark:bg-slate-800/80 p-6 rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-slate-700/50 relative overflow-hidden group hover:shadow-lg transition-all duration-300 flex flex-col justify-between h-full hover:border-elevate-accent/30">
                ...
                <div class="relative z-10">
                    ...
                    <div class="flex items-baseline gap-2 mb-4">
                        ...
                        ?>
                        <span class="text-5xl font-black text-elevate-dark dark:text-slate-100 tracking-tight"><?php echo e($persen); ?><span class="text-2xl text-slate-400 dark:text-slate-500">%</span></span>
                    </div>
                    
                    <div class="w-full bg-slate-100 dark:bg-slate-700 rounded-full h-3 mb-4 overflow-hidden flex">
                        <div class="h-full bg-elevate-accent" style="width: <?php echo e($persen); ?>%"></div>
                        <?php
                            $persenSakitIzin = $total_hari > 0 ? round((($sakit+$izin)/$total_hari)*100) : 0;
                        ?>
                        <div class="h-full bg-elevate-primary/40" style="width: <?php echo e($persenSakitIzin); ?>%"></div>
                    </div>
            ...
            <div class="relative z-10 w-full flex flex-col h-full">
                <div class="flex justify-between items-start mb-4">
                    <div class="flex flex-col">
                        ...
                        <span class="text-[10px] font-black text-elevate-peach-dark flex items-center gap-1">
                            <i class="ph-fill ph-fire text-elevate-peach animate-pulse"></i> <?php echo e($streak); ?> Hari Streak!
                        </span>
                    </div>
                ...
                <div class="grid grid-cols-2 gap-3 mt-auto">
                    <div class="bg-elevate-soft/50 dark:bg-elevate-dark/20 p-3 rounded-2xl border border-elevate-accent/20 dark:border-elevate-accent/50 text-center flex flex-col justify-center hover:bg-elevate-soft dark:hover:bg-elevate-dark/40 transition-colors cursor-default">
                        ...
                        <p class="text-xl font-black text-elevate-dark dark:text-white"><?php echo e($total_merit_points ?? 0); ?></p>
                    </div>
        ...
        <div class="bg-white dark:bg-slate-800/80 p-6 rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-slate-700/50 relative overflow-hidden group hover:shadow-lg transition-all duration-300 flex flex-col justify-between h-full hover:border-elevate-primary/30">
            ...
            <div class="relative z-10">
                <div class="flex justify-between items-start mb-2">
                    ...
                    <a href="<?php echo e(Route::has('student.library.index') ? route('student.library.index') : '#'); ?>" class="text-[10px] font-bold text-elevate-primary dark:text-elevate-accent hover:underline">
                        Lihat Katalog
                    </a>
                </div>
                ...
                <div class="flex items-baseline gap-2 mb-4">
                    <span class="text-5xl font-black text-elevate-dark dark:text-slate-100 tracking-tight"><?php echo e($totalLiterasi); ?></span>
                ...
                <div class="space-y-2">
                    <div class="flex justify-between items-center p-3 rounded-xl bg-elevate-soft/50 dark:bg-elevate-dark/30 border border-elevate-accent/20 dark:border-elevate-primary/50 hover:bg-elevate-soft dark:hover:bg-elevate-dark/50 transition-colors">
                        ...
                        <span class="font-black text-elevate-dark dark:text-white"><?php echo e($library_visits ?? 0); ?></span>
                    </div>
                    <div class="flex justify-between items-center p-3 rounded-xl bg-white dark:bg-slate-800/30 border border-slate-200 dark:border-slate-700/50 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                        ...
                        <span class="font-black text-slate-800 dark:text-slate-300"><?php echo e($ebookCount); ?></span>
                    </div>
                </div>
            </div>
        </div>
