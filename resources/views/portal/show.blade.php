    ...
    <div class="mb-8 sticky top-24 z-40">
        <div class="bg-white/80 backdrop-blur-md p-1.5 rounded-2xl shadow-lg border border-gray-100 overflow-x-auto custom-scrollbar">
            <div class="flex space-x-1 min-w-max">
                <button @click="activeTab = 'ringkasan'" :class="activeTab === 'ringkasan' ? 'bg-blue-600 text-white shadow-md' : 'text-gray-500 hover:bg-gray-50'" class="px-5 py-2.5 rounded-xl text-sm font-bold transition-all duration-200 flex items-center gap-2"><i class="ph-bold ph-squares-four text-lg"></i> Ringkasan</button>
                <button @click="activeTab = 'akademik'" :class="activeTab === 'akademik' ? 'bg-blue-600 text-white shadow-md' : 'text-gray-500 hover:bg-gray-50'" class="px-5 py-2.5 rounded-xl text-sm font-bold transition-all duration-200 flex items-center gap-2"><i class="ph-bold ph-exam text-lg"></i> Akademik</button>
                <button @click="activeTab = 'kehadiran'" :class="activeTab === 'kehadiran' ? 'bg-blue-600 text-white shadow-md' : 'text-gray-500 hover:bg-gray-50'" class="px-5 py-2.5 rounded-xl text-sm font-bold transition-all duration-200 flex items-center gap-2"><i class="ph-bold ph-calendar-check text-lg"></i> Kehadiran</button>
                <button @click="activeTab = 'keagamaan'" :class="activeTab === 'keagamaan' ? 'bg-blue-600 text-white shadow-md' : 'text-gray-500 hover:bg-gray-50'" class="px-5 py-2.5 rounded-xl text-sm font-bold transition-all duration-200 flex items-center gap-2"><i class="ph-bold ph-book-open-text text-lg"></i> Keagamaan</button>
                <button @click="activeTab = 'disiplin'" :class="activeTab === 'disiplin' ? 'bg-blue-600 text-white shadow-md' : 'text-gray-500 hover:bg-gray-50'" class="px-5 py-2.5 rounded-xl text-sm font-bold transition-all duration-200 flex items-center gap-2"><i class="ph-bold ph-warning-circle text-lg"></i> Disiplin</button>
                <button @click="activeTab = 'prestasi'" :class="activeTab === 'prestasi' ? 'bg-blue-600 text-white shadow-md' : 'text-gray-500 hover:bg-gray-50'" class="px-5 py-2.5 rounded-xl text-sm font-bold transition-all duration-200 flex items-center gap-2"><i class="ph-bold ph-trophy text-lg"></i> Prestasi</button>
                <button @click="activeTab = 'perpustakaan'" :class="activeTab === 'perpustakaan' ? 'bg-blue-600 text-white shadow-md' : 'text-gray-500 hover:bg-gray-50'" class="px-5 py-2.5 rounded-xl text-sm font-bold transition-all duration-200 flex items-center gap-2"><i class="ph-bold ph-books text-lg"></i> Pustaka</button>
            </div>
        </div>
    </div>
    ...
    <div class="min-h-[400px]">
        ...
        <div x-show="activeTab === 'ringkasan'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 relative overflow-hidden group hover:shadow-lg transition">
                    <div class="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition"><i class="ph-fill ph-chart-pie-slice text-9xl text-blue-500"></i></div>
                ...
                <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 relative overflow-hidden group hover:shadow-lg transition">
                    <div class="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition"><i class="ph-fill ph-star text-9xl text-yellow-500"></i></div>
                    <h3 class="text-gray-500 text-sm font-bold uppercase tracking-wider mb-2">Poin Karakter</h3>
                    <div class="grid grid-cols-2 gap-4 mt-2">
                        <div><p class="text-xs text-green-600 font-bold mb-1">Kebaikan</p><p class="text-3xl font-black text-green-600">+{{ $poin_kebaikan ?? 0 }}</p></div>
                        <div><p class="text-xs text-red-600 font-bold mb-1">Pelanggaran</p><p class="text-3xl font-black text-red-600">{{ $poin_pelanggaran ?? 0 }}</p></div>
                    </div>
                </div>
                <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 relative overflow-hidden group hover:shadow-lg transition">
                    <div class="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition"><i class="ph-fill ph-books text-9xl text-orange-500"></i></div>
                    <h3 class="text-gray-500 text-sm font-bold uppercase tracking-wider mb-2">Literasi</h3>
                    <div class="flex items-baseline gap-2"><span class="text-5xl font-black text-gray-800">{{ $library_visits }}</span><span class="text-sm text-gray-400 font-medium">Kunjungan</span></div>
        ...
        <div x-show="activeTab === 'akademik'" x-cloak x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0">
            @if($academic_record)
                <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-6 mb-6 relative overflow-hidden">
                    <div class="flex justify-between items-center mb-6">
                        <h3 class="font-bold text-gray-800 text-lg flex items-center gap-2">
                            <i class="ph-fill ph-chart-bar text-violet-500"></i> Grafik Capaian Kompetensi
                        </h3>
                    </div>
                ...
                <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                    <div class="p-6 border-b border-gray-100 bg-violet-50/50 flex justify-between items-center">
                        <div>
                            <h3 class="font-bold text-gray-800 text-lg">Rincian Nilai</h3>
                            <p class="text-sm text-gray-500">
                                Tahun Ajaran: <span class="font-semibold text-violet-600">{{ $academic_record->academic_year }}</span> • 
                                Semester: <span class="font-semibold text-violet-600">{{ $academic_record->semester }}</span>
                            </p>
                        </div>
                    </div>
                        ...
                        <table class="w-full text-left border-collapse">
                            <thead class="bg-gray-50 text-xs font-bold text-gray-400 uppercase tracking-wider">
                                <tr>
                                    ...
                                    <th class="px-6 py-4 hidden md:table-cell">Capaian Kompetensi</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-50 text-sm">
                                @foreach($academic_record->items as $item)
                                    <tr class="hover:bg-gray-50 transition">
                                        <td class="px-6 py-4 font-semibold text-gray-700">{{ $item->subject->name ?? 'Mapel Dihapus' }}</td>
                                        <td class="px-6 py-4 text-center"><span class="inline-block font-bold text-slate-700 text-base">{{ $item->score }}</span></td>
                                        <td class="px-6 py-4 text-center">
                                            @php $gradeColor = match($item->predicate) { 'A' => 'bg-emerald-100 text-emerald-700', 'B' => 'bg-blue-100 text-blue-700', 'C' => 'bg-yellow-100 text-yellow-700', default => 'bg-rose-100 text-rose-700' }; @endphp
                                            <span class="px-2.5 py-1 rounded-lg text-xs font-bold {{ $gradeColor }}">{{ $item->predicate }}</span>
                                        </td>
                                        <td class="px-6 py-4 text-gray-500 italic hidden md:table-cell max-w-xs truncate">{{ $item->description ?? '-' }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
            ...
            @else
                <div class="bg-white rounded-3xl border-2 border-dashed border-gray-200 p-12 text-center">
                    <div class="w-16 h-16 bg-gray-50 rounded-full flex items-center justify-center mx-auto mb-4 text-gray-300"><i class="ph-duotone ph-exam text-3xl"></i></div>
                    <h3 class="font-bold text-gray-800">Belum Ada Data Nilai</h3>
                    <p class="text-gray-500 text-sm mt-1">Data nilai akademik untuk semester ini belum tersedia.</p>
                </div>
            @endif
        </div>

        <!-- [UPDATE] TAB KEHADIRAN: GRAFIK DONUT -->
        <div x-show="activeTab === 'kehadiran'" x-cloak x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0">
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                ...
                <!-- GRAFIK DONUT -->
                <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 lg:col-span-1 flex flex-col justify-center items-center">
                    <h3 class="font-bold text-gray-800 mb-6 flex items-center gap-2 self-start w-full border-b border-gray-50 pb-4">
                    ...
                    <div class="h-48 w-full relative">
                        ...
                        <div class="absolute inset-0 flex items-center justify-center pointer-events-none">
                            <div class="text-center">
                                ...
                                <br>
                                <span class="text-3xl font-black text-slate-800">{{ $attendanceChart['hadir'] + $attendanceChart['sakit'] + $attendanceChart['izin'] + $attendanceChart['alpa'] }}</span>
                            </div>
                        </div>
                    </div>
                </div>

                <!-- KARTU STATISTIK (Geser ke Kanan) -->
                <div class="lg:col-span-2 grid grid-cols-2 gap-4">
                    <div class="bg-emerald-50 p-5 rounded-2xl border border-emerald-100 flex flex-col justify-center text-center h-full">
                    ...
                    <div class="bg-blue-50 p-5 rounded-2xl border border-blue-100 flex flex-col justify-center text-center h-full">
                    ...
                    <div class="bg-amber-50 p-5 rounded-2xl border border-amber-100 flex flex-col justify-center text-center h-full">
                    ...
                    <div class="bg-rose-50 p-5 rounded-2xl border border-rose-100 flex flex-col justify-center text-center h-full">
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-100 bg-gray-50"><h3 class="font-bold text-gray-800">Riwayat Kehadiran Terakhir</h3></div>
                <div class="divide-y divide-gray-100">
                    ...
                    <div class="px-6 py-4 flex items-center justify-between hover:bg-gray-50 transition">
                        <div>
                            ...
                            <p class="text-xs text-gray-500">Masuk: {{ $log->time_in ? \Carbon\Carbon::parse($log->time_in)->format('H:i') : '-' }} | Pulang: {{ $log->time_out ? \Carbon\Carbon::parse($log->time_out)->format('H:i') : '-' }}</p>
                        </div>
                        <span class="px-3 py-1 rounded-full text-xs font-bold uppercase {{ ($log->status == 'Hadir' || $log->status == 'Masuk' || $log->status == 'Terlambat') ? 'bg-green-100 text-green-700' : ($log->status == 'Sakit' ? 'bg-blue-100 text-blue-700' : ($log->status == 'Izin' ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700')) }}">{{ $log->status }}</span>
                    </div>
                    @empty
        ...
        <div x-show="activeTab === 'keagamaan'" x-cloak x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0">
             <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <div class="bg-white p-6 rounded-3xl border border-teal-100 shadow-sm flex items-center gap-6">
                    <div class="w-16 h-16 rounded-2xl bg-teal-100 flex items-center justify-center text-teal-600"><i class="ph-duotone ph-sun-horizon text-3xl"></i></div>
                    <div><h4 class="text-sm font-bold text-gray-500 uppercase tracking-widest">Sholat Dhuha</h4><p class="text-4xl font-black text-gray-800">{{ $sholat_dhuha }} <span class="text-base font-medium text-gray-400">Kali</span></p></div>
                </div>
                <div class="bg-white p-6 rounded-3xl border border-orange-100 shadow-sm flex items-center gap-6">
                    <div class="w-16 h-16 rounded-2xl bg-orange-100 flex items-center justify-center text-orange-600"><i class="ph-duotone ph-clock-afternoon text-3xl"></i></div>
                    <div><h4 class="text-sm font-bold text-gray-500 uppercase tracking-widest">Sholat Dhuhur</h4><p class="text-4xl font-black text-gray-800">{{ $sholat_dhuhur }} <span class="text-base font-medium text-gray-400">Kali</span></p></div>
                </div>
             </div>
             <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-6">
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex gap-4">
                        <div class="flex flex-col items-center"><div class="w-2 h-2 bg-gray-200 rounded-full"></div><div class="w-0.5 flex-1 bg-gray-100 my-1"></div></div>
                        <div class="pb-4"><p class="text-sm font-bold text-gray-800">Melaksanakan Sholat {{ $rel->activity }}</p><p class="text-xs text-gray-500">{{ \Carbon\Carbon::parse($rel->created_at)->translatedFormat('l, d F Y H:i') }} WIB</p></div>
                    </div>
                    @empty
        ...
        <div x-show="activeTab === 'disiplin'" x-cloak x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0">
             ...
             <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="px-6 py-4 bg-gray-50 border-b border-gray-100"><h3 class="font-bold text-gray-800">Catatan Disiplin Terbaru</h3></div>
                <div class="divide-y divide-gray-100">
                    @forelse ($discipline_history as $record)
                        <div class="flex items-start gap-4 p-5 hover:bg-gray-50 transition">
                            ...
                            <div class="flex-1">
                                ...
                                <p class="text-xs text-gray-400 mt-2 flex items-center gap-1"><i class="ph-regular ph-calendar"></i> {{ \Carbon\Carbon::parse($record->date)->translatedFormat('d M Y') }}</p>
                            </div>
                        </div>
                    @empty
        ...
        <div x-show="activeTab === 'prestasi'" x-cloak x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0">
             @if(count($achievements) > 0)
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-6 flex items-start gap-4 relative overflow-hidden group hover:shadow-lg transition">
                        ...
                        <div class="w-16 h-16 rounded-2xl bg-yellow-50 border border-yellow-100 flex items-center justify-center flex-shrink-0"><i class="ph-duotone ph-trophy text-3xl text-yellow-600"></i></div>
                        <div>
                            <span class="inline-block px-2 py-1 bg-yellow-100 text-yellow-700 text-[10px] font-bold uppercase rounded-md mb-2">{{ $ach->level ?? 'Sekolah' }}</span>
             ...
             @else
                <div class="bg-white rounded-3xl border border-dashed border-gray-300 p-12 text-center">
                    <div class="w-20 h-20 bg-gray-50 rounded-full flex items-center justify-center mx-auto mb-4"><i class="ph-duotone ph-trophy text-4xl text-gray-300"></i></div>
        ...
        <div x-show="activeTab === 'perpustakaan'" x-cloak x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0">
             <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 relative overflow-hidden h-full">
                <div class="flex items-center justify-between mb-6">
                    <div><p class="text-3xl font-black text-gray-800">{{ $library_visits }}</p><p class="text-xs font-bold text-gray-400 uppercase tracking-widest">Total Kunjungan</p></div>
                    <div class="p-3 bg-orange-50 rounded-2xl text-orange-500"><i class="ph-duotone ph-read-cv-logo text-4xl"></i></div>
                </div>
                    ...
                    @forelse($borrowing_history as $loan)
                        <div class="flex items-center justify-between p-3 rounded-xl bg-gray-50 border border-gray-100 hover:bg-white hover:shadow-md transition">
                            <div class="flex items-center gap-3 overflow-hidden">
                                ...
                                <div class="min-w-0"><p class="text-sm font-bold text-gray-800 truncate">{{ $loan->book->title }}</p><p class="text-[10px] text-gray-500">Pinjam: {{ \Carbon\Carbon::parse($loan->borrow_date)->format('d M Y') }}</p></div>
                            </div>
                            <span class="px-2 py-1 rounded text-[10px] font-bold uppercase {{ $loan->status == 'returned' ? 'bg-blue-100 text-blue-600' : 'bg-orange-100 text-orange-600' }}">{{ $loan->status == 'returned' ? 'Kembali' : 'Dipinjam' }}</span>
                        </div>
                    @empty
                        <div class="text-center py-12 text-gray-400"><i class="ph-duotone ph-book-open text-3xl mb-2"></i><p class="text-sm">Belum pernah meminjam buku.</p></div>
                    @endforelse
                </div>
            </div>
        </div>
