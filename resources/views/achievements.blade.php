    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-20 relative z-20 pb-20">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
            @forelse($achievements ?? [] as $index => $prestasi)
                <div class="bg-white rounded-[2rem] overflow-hidden shadow-sm hover:shadow-2xl hover:shadow-yellow-900/10 hover:-translate-y-2 transition-all duration-500 border border-slate-100 flex flex-col h-full relative animate-enter" style="animation-delay: {{ ($index % 6) * 100 }}ms">
                    ...
                    <div class="p-6 flex-1 flex flex-col relative bg-white">
                        ...
                        <div class="mt-auto pt-4 border-t border-slate-100 flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-full bg-slate-50 border border-slate-100 flex items-center justify-center shrink-0
                                    {{ $prestasi->type === 'Siswa' ? 'text-blue-500' : ($prestasi->type === 'Guru' ? 'text-emerald-500' : 'text-orange-500') }}">
                        ...
                        @if(!empty($prestasi->photo_path) || !empty($prestasi->certificate_path) || !empty($prestasi->video_link))
                            <div class="mt-5 flex flex-wrap gap-2">
                                ...
                                @if(!empty($prestasi->photo_path))
                                    <a href="{{ asset('storage/' . $prestasi->photo_path) }}" target="_blank" class="flex-1 min-w-[30%] flex items-center justify-center gap-1.5 px-3 py-2.5 bg-amber-50 text-amber-600 hover:bg-amber-600 hover:text-white rounded-xl text-xs font-bold transition-colors">
