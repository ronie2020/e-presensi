                ...
                <div class="overflow-x-auto custom-scrollbar max-h-[500px]">
                    <table class="w-full text-left border-collapse min-w-[800px]">
                        ...
                        <tbody class="divide-y divide-slate-50 text-sm">
                            @foreach($mappedStudents as $index => $ms)
                                @php
                                    ...
                                    <td class="p-4">
                                        <div class="flex items-center gap-3">
                                            <div class="w-8 h-8 rounded-full bg-slate-100 overflow-hidden shrink-0 border border-slate-200">
                                                @if($ms['photo'])
                                                    <img src="{{ $ms['photo'] }}" class="w-full h-full object-cover">
                                                @else
                                                    <div class="w-full h-full flex items-center justify-center font-black text-slate-400 text-xs">{{ substr($ms['name'], 0, 1) }}</div>
                                                @endif
                                            </div>
                                            <div>
                                                <h4 class="font-bold text-slate-700 truncate max-w-[200px]">{{ $ms['name'] }}</h4>
                                                <p class="text-[10px] text-slate-400 font-bold uppercase tracking-wider">{{ $ms['nisn'] }}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="p-4 text-center font-bold {{ $ms['sakit_count'] > 0 ? 'text-blue-600 bg-blue-50/30' : 'text-slate-300' }}">{{ $ms['sakit_count'] ?: '-' }}</td>
                                    <td class="p-4 text-center font-bold {{ $ms['izin_count'] > 0 ? 'text-indigo-600 bg-indigo-50/30' : 'text-slate-300' }}">{{ $ms['izin_count'] ?: '-' }}</td>
                                    <td class="p-4 text-center font-bold {{ $ms['alfa_count'] > 0 ? 'text-rose-600 bg-rose-50/30' : 'text-slate-300' }}">{{ $ms['alfa_count'] ?: '-' }}</td>
                                    <td class="p-4 text-center font-bold {{ $ms['late_count'] > 0 ? 'text-cyan-600 bg-cyan-50/30' : 'text-slate-300' }}">{{ $ms['late_count'] ?: '-' }}</td>
                                    <td class="p-4 text-center font-black {{ $totalAbsen > 0 ? 'text-slate-700' : 'text-slate-300' }}">
                                        {{ $totalAbsen ?: '-' }}
                                    </td>
