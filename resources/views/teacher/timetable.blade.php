            ...
            <table class="w-full text-left text-sm min-w-[900px] mt-4">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        @foreach($days as$day)
                            @php
                                // Fix 1: Mencegah error explode() jika day_of_week null
                                $dayOfWeekRaw =$slot->day_of_week ?? '';
                                $allowedDays = array_map('trim', explode(',', $dayOfWeekRaw));$isValidDay = in_array($day,$allowedDays) 
                                    || $dayOfWeekRaw === 'Semua Hari' 
                                    || ($dayOfWeekRaw === 'Selain Senin' &&$day !== 'Senin') 
                                    || ($dayOfWeekRaw === 'Selain Jumat' && $day !== 'Jumat');$key = $day . '-' .$slot->id;
                                
                                // Fix 2: Sinkronisasi pencarian ID dengan format admin
                                $currentLoadId = '';
                                if(isset($myTimetables[$key])) {
                                    // Cek apakah pakai teaching_load_id langsung atau class_id & subject_id
                                    if (isset($myTimetables[$key]->teaching_load_id) &&$myTimetables[$key]->teaching_load_id != null) {$currentLoadId = $myTimetables[$key]->teaching_load_id;
                                    } else {
                            ...
                            @else
                                <td class="p-2 border border-slate-200">
                                    {{-- Tambahan onfocus untuk menyimpan nilai sebelum diubah --}}
                                    <select 
...
<script>
    // Variabel untuk melacak request yang sedang berjalan (mengatasi Race Condition)
    ...
    function autoSaveJadwal(selectElement, day, timeslotId) {
        ...
        .then(data => {
            ...
            if(!data.success) {
                // Kembalikan UI ke pilihan sebelumnya jika server menolak
                ...
                // Update dataset.previousValue ke nilai baru yang berhasil disimpan
        ...
        .catch(error => {
            ...
            selectElement.disabled = false;
            // Kembalikan UI ke pilihan sebelumnya jika terjadi error jaringan
            selectElement.value = previousValue; 
