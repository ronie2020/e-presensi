<div x-data="{
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div class="lg:col-span-2 bg-white p-6 sm:p-8 rounded-[2.5rem] shadow-sm border border-slate-100 flex flex-col justify-center relative overflow-hidden group hover:border-[#5295FF]/30 transition-colors">
            ...
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4 relative z-10">
                <div>
                    ...
                    <p class="text-sm font-bold text-slate-500 flex items-center gap-2 mt-1">
                        <i class="ph-bold ph-calendar-blank text-[#5295FF]"></i> {{ \Carbon\Carbon::now()->translatedFormat('l, d F Y') }}
                    </p>
                </div>
            ...
            <div class="relative z-10 mt-2">
                <div class="flex justify-between items-end mb-2">
                    ...
                    <span class="text-lg font-black text-[#2A3B52]">{{ $completedHabits }} <span class="text-sm text-slate-400">/ 7 Selesai</span></span>
                </div>
        ...
        {{-- Konsistensi Bulan Ini (Dipindah dari Wrapper lama) --}}
        <div class="bg-white p-6 sm:p-8 rounded-[2.5rem] border border-slate-100 shadow-sm relative overflow-hidden group hover:border-[#5295FF]/30 transition-colors flex flex-col justify-center">
            ...
            <div class="relative z-10">
                ...
                <div class="flex items-baseline gap-2 mb-4">
                    <h3 class="text-4xl font-black text-[#2A3B52]">{{ $monthlyCount ?? 0 }}</h3>
                ...
                <div class="w-full bg-[#F3F9FD] h-2 rounded-full overflow-hidden border border-[#D0E7F8]">
                    <div class="bg-[#5295FF] h-full rounded-full transition-all duration-1000" style="width: {{ min((($monthlyCount ?? 0)/30)*100, 100) }}%"></div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-6 sm:p-8 hover:shadow-md transition-shadow mb-8">
        <div class="flex justify-between items-center mb-6">
            ...
            <a href="{{ route('student.habits.index') }}" class="px-5 py-2.5 bg-[#2A3B52] text-white rounded-xl text-xs font-bold hover:bg-[#5295FF] transition shadow-lg shadow-slate-200 flex items-center gap-2">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            @foreach($habitsGrid as $h)
                ...
                <div class="relative p-5 rounded-2xl border transition-all duration-300 overflow-hidden {{ $bgColor }} {{ $borderColor }}">
                    <div class="relative z-10 flex flex-col items-center text-center h-full justify-center">
                        ...
                        <div class="mb-3 p-3 rounded-full {{ $isDone ? 'bg-white/20' : 'bg-white shadow-sm border border-white' }}">
                            <i class="ph-duotone ph-{{ $h['icon'] }} text-2xl {{ $iconColor }}"></i>
                        </div>
                        
                        <h4 class="font-bold text-xs md:text-sm {{ $textColor }}">{{ $h['label'] }}</h4>
                        @if(!$isDone)
                            <p class="text-[9px] {{ $textColor }} opacity-70 mt-1 font-medium">Belum tuntas</p>
                        @endif
                    </div>
                </div>
            @endforeach
        ...
        <div class="mt-4 p-4 rounded-xl border flex items-center gap-4 relative overflow-hidden transition-all {{ $isTidurDone ? 'bg-[#5295FF] border-[#5295FF] text-white shadow-sm' : 'bg-slate-50 border-slate-200 text-[#2A3B52]' }}">
            <div class="w-12 h-12 rounded-lg flex items-center justify-center text-2xl shrink-0 {{ $isTidurDone ? 'bg-white/20 text-white' : 'bg-white text-slate-500 border border-slate-200 shadow-sm' }}">
                <i class="ph-duotone ph-moon-stars"></i>
            </div>
            <div class="relative z-10">
                <h4 class="font-bold text-sm">7. Tidur Cepat</h4>
                <p class="text-xs {{ $isTidurDone ? 'text-white/80' : 'text-slate-500' }} mt-0.5 font-medium">
                    {{ $isTidurDone ? 'Tercatat tidur pukul ' . $todayEntry->habit_7_time : 'Maksimal istirahat jam 22:00 malam.' }}
                </p>
            </div>
            @if($isTidurDone)
    ...
    {{-- 3. RIWAYAT AKTIVITAS & FEEDBACK GURU --}}
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors group">
                            ...
                            <td class="py-4 px-6">
                                <div class="flex -space-x-2">
                                    @if($activity->habit_1) <div title="Bangun Pagi" class="w-8 h-8 rounded-full bg-[#F3F9FD] border-2 border-white flex items-center justify-center text-[#5295FF] shadow-sm"><i class="ph-fill ph-sun-horizon text-xs"></i></div> @endif
                                    @if($activity->habit_3) <div title="Olahraga" class="w-8 h-8 rounded-full bg-[#FFEFD6] border-2 border-white flex items-center justify-center text-[#D83B01] shadow-sm"><i class="ph-fill ph-sneaker-move text-xs"></i></div> @endif
                                    @if($activity->habit_5) <div title="Makan Sehat" class="w-8 h-8 rounded-full bg-[#FDE7E9] border-2 border-white flex items-center justify-center text-[#D13438] shadow-sm"><i class="ph-fill ph-carrot text-xs"></i></div> @endif
                                ...
                                @if($activity->teacher_feedback)
                                    <div class="flex items-start gap-2 bg-[#F3F9FD] p-2.5 rounded-lg border border-[#D0E7F8] shadow-sm">
                                        ...
                                        <p class="text-[11px] text-[#2A3B52] font-medium leading-relaxed italic break-words">
                                            "{{ Str::limit($activity->teacher_feedback, 80) }}"
                                        </p>
                                    </div>
                                @else
