<div x-data="{ activeTab: 'assignments' }" class="space-y-8 animate-in fade-in duration-500 font-sans">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div class="bg-white rounded-[2rem] p-6 border border-slate-100 shadow-sm relative overflow-hidden group hover:border-emerald-200 transition-colors">
            ...
            <p class="text-xs text-slate-400 mt-3 font-medium">Dari {{ $submittedCount }} tugas yang dinilai.</p>
        </div>
        ...
        <div class="bg-white rounded-[2rem] p-6 border border-slate-100 shadow-sm relative overflow-hidden group hover:border-purple-200 transition-colors">
            ...
            @php $totalMateri = 0; foreach($lms_materials_grouped as $g) { $totalMateri += $g->count(); } @endphp
            <div class="flex items-baseline gap-2">
    ...
    <div class="flex justify-center">
        <div class="bg-slate-100 p-1.5 rounded-2xl inline-flex relative">
            {{-- Sliding Background (Visual trick handled by dynamic classes) --}}
    ...
    <div x-show="activeTab === 'assignments'" 
        ...
        @if(count($lms_assignments_grouped) > 0)
            <div class="space-y-8">
                @foreach($lms_assignments_grouped as $subjectName => $assignments)
                    <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
                        ...
                        <div class="bg-slate-50/50 px-8 py-4 border-b border-slate-100 flex items-center gap-3">
                            ...
                            <h3 class="text-lg font-black text-slate-800">{{ $subjectName }}</h3>
                        </div>
                                        ...
                                        <div class="flex-1">
                                            <div class="flex items-center gap-2 mb-2">
                                                ...
                                                <span class="text-xs font-bold text-slate-400 flex items-center gap-1">
                                                    <i class="ph-bold ph-calendar-blank"></i> {{ \Carbon\Carbon::parse($assignment->due_date)->format('d M Y') }}
                                                </span>
                                            </div>
                                        ...
                                        <div class="flex items-center gap-4 shrink-0">
                                            @if($isGraded)
                                                <div class="text-center bg-white p-3 rounded-2xl border border-slate-200 shadow-sm min-w-[80px]">
                                                    ...
                                                    <span class="text-3xl font-black text-emerald-600 tracking-tight">{{ $score }}</span>
                                                </div>
                                            @else
    ...
    <div x-show="activeTab === 'materials'" 
        ...
        @if(count($lms_materials_grouped) > 0)
                        ...
                        <div class="p-4 space-y-3">
                            @foreach($materials as $material)
                                ...
                                <div class="flex items-start gap-3 p-3 rounded-2xl border border-slate-100 hover:border-purple-200 hover:bg-purple-50/30 transition-all group cursor-pointer">
                                    <div class="w-10 h-10 rounded-xl bg-{{ $color }}-50 text-{{ $color }}-500 flex items-center justify-center shrink-0 border border-{{ $color }}-100 group-hover:scale-110 transition-transform">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <p class="text-[10px] text-slate-400 mt-0.5">
                                            Diposting: {{ \Carbon\Carbon::parse($material->created_at)->format('d M') }}
                                        </p>
                                    </div>
