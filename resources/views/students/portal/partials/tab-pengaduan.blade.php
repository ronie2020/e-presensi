<div class="space-y-8">
    ...
    {{-- 1. HEADER SECTION (Zona Aman Bercerita) --}}
    <div class="relative rounded-[2.5rem] bg-gradient-to-br from-rose-900 via-slate-900 to-black p-8 md:p-10 mb-8 text-white shadow-2xl shadow-rose-900/30 overflow-hidden border border-white/10 group">
        {{-- Background Effects --}}
        <div class="absolute top-0 right-0 -mt-20 -mr-20 w-[400px] h-[400px] bg-rose-500/20 rounded-full blur-[100px] opacity-40 group-hover:opacity-50 transition-opacity duration-1000"></div>
        ...
        <div class="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
            <div class="max-w-2xl">
                ...
                <h2 class="text-3xl md:text-4xl font-black tracking-tighter mb-4 leading-none">Layanan <span class="text-transparent bg-clip-text bg-gradient-to-r from-rose-300 to-pink-200">Pengaduan</span></h2>
                <p class="text-rose-100/60 text-sm leading-relaxed font-medium max-w-lg">
                    Suaramu sangat berharga. Kami siap mendengarkan dan membantu menyelesaikan masalahmu di sekolah dengan aman dan rahasia.
                </p>
            </div>
            
            <a href="{{ route('student.complaints.create') }}" class="group bg-white text-rose-600 px-8 py-4 rounded-[1.5rem] font-black shadow-xl shadow-rose-900/20 hover:bg-rose-50 transition-all flex items-center gap-3 shrink-0 active:scale-95 text-xs uppercase tracking-widest border-2 border-transparent hover:border-rose-100">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div class="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex items-center gap-5 group hover:border-blue-100 transition-all">
            ...
            <div>
                ...
                <p class="text-3xl font-black text-slate-800 tracking-tight">{{ $complaints->count() }}</p>
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex items-center gap-5 group hover:border-amber-100 transition-all">
            ...
            <div>
                ...
                <p class="text-3xl font-black text-slate-800 tracking-tight">{{ $complaints->where('status', 'pending')->count() + $complaints->where('status', 'investigating')->count() }}</p>
            </div>
        </div>
        ...
        <div class="bg-emerald-600 p-6 rounded-[2rem] shadow-xl shadow-emerald-900/10 flex items-center gap-5 group hover:bg-emerald-700 transition-all border border-emerald-500/20 text-white">
            ...
            <div>
                ...
                <p class="text-3xl font-black tracking-tight">{{ $complaints->where('status', 'resolved')->count() }}</p>
            </div>
        </div>
    </div>
                ...
                <div class="p-8 hover:bg-slate-50/50 transition-all group relative">
                    <div class="flex flex-col md:flex-row justify-between gap-6">
                        <div class="flex gap-6">
                            ...
                            @endphp
                            <div class="shrink-0 w-16 h-16 rounded-[1.5rem] flex items-center justify-center text-3xl border {{ $catColor }} shadow-inner transition-transform group-hover:rotate-6">
                                <i class="ph-fill {{ $catIcon }}"></i>
                            </div>
                            
                            <div class="flex-1">
                                <div class="flex flex-wrap items-center gap-3 mb-2">
                                    <span class="text-sm font-black text-slate-800 uppercase tracking-tight">{{ $item->category }}</span>
                                ...
                                <p class="text-sm text-slate-500 line-clamp-2 mb-4 font-medium leading-relaxed italic">"{{ $item->description }}"</p>
