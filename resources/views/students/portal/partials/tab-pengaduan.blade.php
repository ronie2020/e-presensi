    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div class="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex items-center gap-5 group hover:border-elevate-accent/40 transition-all">
            ...
            <div>
                ...
                <p class="text-3xl font-black text-elevate-dark tracking-tight">{{ $complaints->count() }}</p>
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex items-center gap-5 group hover:border-elevate-peach/40 transition-all">
            ...
            <div>
                ...
                <p class="text-3xl font-black text-elevate-dark tracking-tight">{{ $complaints->where('status', 'pending')->count() + $complaints->where('status', 'investigating')->count() }}</p>
            </div>
        </div>
                ...
                <div class="p-8 hover:bg-elevate-soft/30 transition-all group relative">
                    <div class="flex flex-col md:flex-row justify-between gap-6">
                        <div class="flex gap-6">
                            {{-- Icon Category (Tetap semantic namun diadaptasi ke Elevate Tone) --}}
                            @php
                            ...
                            @endphp
                            <div class="shrink-0 w-16 h-16 rounded-[1.5rem] flex items-center justify-center text-3xl border shadow-sm transition-transform group-hover:rotate-6 {{ $catConfig['bg'] }} {{ $catConfig['text'] }} {{ $catConfig['border'] }}">
                                <i class="ph-fill {{ $catConfig['icon'] }}"></i>
                            </div>
                            
                            <div class="flex-1">
                                <div class="flex flex-wrap items-center gap-3 mb-2">
                                    <span class="text-sm font-black text-elevate-dark uppercase tracking-tight group-hover:text-elevate-primary transition-colors">{{ $item->category }}</span>
                                ...
                                @endphp
                                <span class="inline-flex items-center gap-2 px-4 py-1.5 rounded-xl text-[9px] font-black border uppercase tracking-widest {{ $statusStyle }} shadow-sm">
