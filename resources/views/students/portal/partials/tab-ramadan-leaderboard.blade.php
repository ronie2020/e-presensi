<div class="space-y-8 animate-in fade-in duration-500 font-sans">
    ...
    {{-- 1. HEADER ROYAL (Konsep Kejuaraan) --}}
            ...
            <div class="flex flex-col items-center {{ !isset($topRamadanStudents[1]) ? 'invisible' : '' }}">
                ...
                <div class="relative group">
                    <div class="w-16 h-16 sm:w-20 sm:h-20 rounded-full border-4 border-slate-300 p-1 bg-white shadow-xl relative z-10 group-hover:scale-105 transition-transform duration-300">
                        <img src="https://ui-avatars.com/api/?name={{ urlencode($topRamadanStudents[1]->name) }}&background=cbd5e1&color=64748b&bold=true" class="w-full h-full object-cover rounded-full">
                ...
                <div class="bg-white/80 backdrop-blur-sm p-3 rounded-2xl shadow-sm border border-slate-200 mt-4 w-full text-center relative">
                    ...
                    <p class="text-[10px] font-black text-slate-800 line-clamp-1 capitalize">{{ strtolower(strtok($topRamadanStudents[1]->name, ' ')) }}</p>
                    <p class="text-[9px] font-bold text-slate-400">{{ number_format($topRamadanStudents[1]->ramadan_points) }} Pts</p>
                </div>
                @endif
            </div>
            ...
            <div class="flex flex-col items-center -mt-8 relative z-20">
                ...
                <div class="relative group">
                    ...
                    <div class="w-20 h-20 sm:w-24 sm:h-24 rounded-full border-4 border-amber-400 p-1 bg-gradient-to-b from-amber-100 to-white shadow-2xl relative z-10 ring-4 ring-amber-400/20 group-hover:scale-105 transition-transform duration-300">
                        <img src="https://ui-avatars.com/api/?name={{ urlencode($topRamadanStudents[0]->name) }}&background=fbbf24&color=78350f&bold=true" class="w-full h-full object-cover rounded-full">
                ...
                <div class="bg-gradient-to-b from-white to-amber-50 p-4 rounded-3xl shadow-lg border border-amber-200 mt-5 w-full text-center relative min-w-[120px]">
                    <p class="text-xs font-black text-slate-900 line-clamp-1 capitalize mb-0.5">{{ strtolower(strtok($topRamadanStudents[0]->name, ' ')) }}</p>
            ...
            <div class="flex flex-col items-center {{ !isset($topRamadanStudents[2]) ? 'invisible' : '' }}">
                ...
                <div class="relative group">
                    <div class="w-16 h-16 sm:w-20 sm:h-20 rounded-full border-4 border-orange-700/40 p-1 bg-white shadow-xl relative z-10 group-hover:scale-105 transition-transform duration-300">
                        <img src="https://ui-avatars.com/api/?name={{ urlencode($topRamadanStudents[2]->name) }}&background=d97706&color=ffffff&bold=true" class="w-full h-full object-cover rounded-full">
                ...
                <div class="bg-white/80 backdrop-blur-sm p-3 rounded-2xl shadow-sm border border-slate-200 mt-4 w-full text-center relative">
                    ...
                    <p class="text-[10px] font-black text-slate-800 line-clamp-1 capitalize">{{ strtolower(strtok($topRamadanStudents[2]->name, ' ')) }}</p>
                    <p class="text-[9px] font-bold text-slate-400">{{ number_format($topRamadanStudents[2]->ramadan_points) }} Pts</p>
                </div>
                @endif
            </div>                   
        </div>
    ...
    <div class="px-2">
        <div class="flex items-center gap-3 mb-4 px-2">
            ...
            <a href="{{ route('ramadan.leaderboard') }}" class="px-4 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-white text-[10px] font-black uppercase tracking-wider transition-colors shadow-md flex items-center gap-1">
        ...
        <div class="space-y-3">
            ...
            <div class="flex items-center gap-4 p-4 rounded-2xl border transition-all duration-300
                ...
                <div class="flex-shrink-0 w-8 text-center">
                    <span class="text-sm font-black {{ $isMe ? 'text-emerald-400' : 'text-slate-300' }}">#{{ $index + 4 }}</span>
                </div>

                {{-- Avatar --}}
                <div class="relative">
                    <div class="w-12 h-12 rounded-full overflow-hidden border-2 {{ $isMe ? 'border-emerald-500' : 'border-slate-100' }}">
                        <img src="https://ui-avatars.com/api/?name={{ urlencode($s->name) }}&size=100&background={{ $isMe ? '064e3b' : 'random' }}&color={{ $isMe ? 'fff' : 'fff' }}" class="w-full h-full object-cover">
                    </div>
                </div>
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-[10px] font-bold uppercase tracking-wide {{ $isMe ? 'text-emerald-300' : 'text-slate-400' }}">
                ...
                <div class="text-right">
                    <div class="text-base font-black {{ $isMe ? 'text-emerald-400' : 'text-emerald-600' }}">
                        {{ number_format($s->ramadan_points, 0, ',', '.') }}
                    </div>
                    <div class="text-[9px] font-bold uppercase {{ $isMe ? 'text-emerald-600' : 'text-slate-400' }}">Poin</div>
                </div>
            </div>
            @endforeach
        </div>
