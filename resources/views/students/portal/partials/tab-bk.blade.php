<div class="space-y-8 font-sans text-elevate-dark animate-in fade-in duration-500" 
    ...
    <div class="bg-elevate-dark rounded-[2.5rem] p-8 md:p-10 text-white shadow-xl shadow-elevate-dark/10 relative overflow-hidden flex flex-col md:flex-row items-start md:items-center justify-between gap-8 border border-elevate-primary/30">
        ...
        <div class="relative z-10 flex flex-col sm:flex-row gap-3 shrink-0 w-full md:w-auto">
            <a href="{{ route('student.bk.create') }}" class="group bg-white text-elevate-dark px-8 py-4 rounded-[1.5rem] font-black shadow-xl shadow-white/10 hover:bg-elevate-soft transition-all flex items-center justify-center gap-3 active:scale-95 text-xs uppercase tracking-widest border border-transparent">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div class="bg-white p-5 rounded-[2rem] border border-slate-100 shadow-sm hover:border-elevate-accent/30 hover:shadow-md transition-all group">
            ...
            <p class="text-3xl font-black text-elevate-dark">{{ $bkSessions->count() }}</p>
        ...
        <div class="bg-white p-5 rounded-[2rem] border border-slate-100 shadow-sm hover:border-elevate-primary/30 hover:shadow-md transition-all group relative overflow-hidden">
            ...
            <p class="text-3xl font-black text-elevate-dark relative z-10">{{ $bkSessions->where('status', 'approved')->count() }}</p>
        ...
        <div class="bg-white p-5 rounded-[2rem] border border-slate-100 shadow-sm hover:border-elevate-peach/30 hover:shadow-md transition-all group relative overflow-hidden">
            ...
            <p class="text-3xl font-black text-elevate-dark relative z-10">{{ $bkSessions->where('status', 'pending')->count() }}</p>
        ...
        <div class="bg-white p-5 rounded-[2rem] border border-slate-100 shadow-sm hover:border-emerald-200 hover:shadow-md transition-all group relative overflow-hidden">
            ...
            <p class="text-3xl font-black text-elevate-dark relative z-10">{{ $bkSessions->where('status', 'finished')->count() }}</p>
    ...
    <div class="bg-white rounded-[2.5rem] shadow-xl shadow-elevate-dark/5 border border-slate-100 overflow-hidden">
                    ...
                    <tbody class="divide-y divide-slate-50 text-sm">
                        ...
                        <tr class="hover:bg-elevate-soft/30 transition-colors group">
                                ...
                                <div class="flex items-start gap-4">
                                    ...
                                    <div>
                                        <div class="flex items-center gap-2 mb-1">
                                            <span class="font-bold text-elevate-dark text-sm group-hover:text-elevate-primary transition-colors">{{ $session->category->name }}</span>
                                        </div>
                            ...
                            <td class="px-6 py-5 text-center">
                                <a href="{{ route('student.bk.show', $session->id) }}" class="inline-flex items-center justify-center w-10 h-10 rounded-2xl bg-white border-2 border-slate-100 text-slate-400 hover:bg-elevate-primary hover:text-white hover:border-elevate-primary transition-all">
        ...
        @else
            <div class="text-center py-20 px-4 bg-slate-50/30">
                ...
                <a href="{{ route('student.bk.create') }}" class="inline-flex items-center justify-center gap-2 px-8 py-3 bg-white border-2 border-slate-200 text-slate-600 font-bold rounded-2xl text-sm hover:border-elevate-primary hover:text-elevate-primary hover:shadow-lg transition-all">
