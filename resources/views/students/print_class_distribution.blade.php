...
<head>
    ...
    <title>Cetak Daftar Siswa - {{ $year }}</title>
    ...
    <style>
       ...
       @media print {
        ...
        /* Ubah border menjadi 1.5px agar saat diprint lebih tegas */
        table.data-kelas th, table.data-kelas td { border: 1.5px solid black; padding: 6px 8px; vertical-align: middle; }
    ...
    <div class="w-[21.5cm] mx-auto mt-6 mb-6 flex flex-col sm:flex-row justify-between items-center gap-4 no-print bg-white/80 backdrop-blur-md p-4 rounded-2xl shadow-lg shadow-elevate-dark/5 border border-white/60 sticky top-4 z-50">
        <div>
            <h2 class="font-black text-elevate-dark font-sans flex items-center gap-2">
                <i class="ph-bold ph-printer text-elevate-primary text-xl"></i> Pratinjau Daftar Siswa
            </h2>
        ...
        <div class="flex flex-wrap gap-3 items-center font-sans">
            <!-- PENTING: Tombol kembali mengarah ke Buku Induk Siswa -->
            <a href="{{ route('students.index') }}" class="px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-xs font-bold text-slate-600 hover:bg-slate-50 hover:text-elevate-primary transition-colors shadow-sm flex items-center gap-2 group">
    ...
    @foreach($classes as $class)
        ...
        <div class="sheet">
            ...
            <div class="judul-surat">
                <h2>DAFTAR PESERTA DIDIK KELAS</h2>
            ...
            <table class="data-kelas">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="text-align: center; font-weight: bold;">{{ $student->nisn ?? $student->student_id ?? '-' }}</td>
                        <td>{{ $student->name }}</td>
                        <td style="text-align: center;">{{ $student->gender }}</td>
                        <td></td> <!-- Kolom Keterangan (Dibiarkan kosong untuk diisi tulisan tangan jika diperlukan) -->
                    </tr>
                    @endforeach
                </tbody>
            </table>
            
            <!-- REKAPITULASI (Format Grid/Tabel tanpa garis agar presisi) -->
            <table style="font-size: 11pt; margin-top: 5px; border: none; width: auto; font-weight: bold;">
                <tr>
                    <td style="padding: 2px 5px 2px 0; border: none;">Jumlah Siswa Laki-laki</td>
                    <td style="padding: 2px 5px; border: none;">:</td>
                    <td style="padding: 2px 0; border: none;">{{ $class->students->where('gender', 'L')->count() }} Orang</td>
                </tr>
                <tr>
                    <td style="padding: 2px 5px 2px 0; border: none;">Jumlah Siswa Perempuan</td>
                    <td style="padding: 2px 5px; border: none;">:</td>
                    <td style="padding: 2px 0; border: none;">{{ $class->students->where('gender', 'P')->count() }} Orang</td>
                </tr>
                <tr>
                    <td style="padding: 2px 5px 2px 0; border: none;">Total Keseluruhan</td>
                    <td style="padding: 2px 5px; border: none;">:</td>
                    <td style="padding: 2px 0; border: none;">{{ $class->students->count() }} Orang</td>
                </tr>
            </table>
