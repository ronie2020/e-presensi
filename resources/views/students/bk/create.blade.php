...
<div class="w-full max-w-4xl mx-auto pb-20 px-4 sm:px-6 pt-6 md:pt-10">
    ...
    <!-- Tombol Kembali -->
    <a href="{{ route('student.bk.index') }}" class="inline-flex items-center text-sm text-slate-500 hover:text-blue-600 font-bold mb-6 transition-colors">
        ...
        <div class="p-6 md:p-10">
            ...
            <form action="{{ route('student.bk.store') }}" method="POST" class="space-y-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <label class="relative cursor-pointer group">
                            ...
                            <div class="rounded-xl border-2 border-slate-200 p-4 hover:border-pink-300 hover:bg-pink-50/50 transition-all peer-checked:border-pink-500 peer-checked:bg-pink-50 peer-checked:shadow-md flex items-center gap-3">
                                ...
                                <span class="text-sm font-bold text-slate-600 peer-checked:text-pink-700">{{ $cat->name }}</span>
                            </div>
                        </label>
                        @endforeach
                    </div>
                    @error('bk_category_id') <p class="text-red-500 text-xs mt-2 font-bold">{{ $message }}</p> @enderror
                </div>
                ...
                <div>
                    ...
                    @error('initial_message') <p class="text-red-500 text-xs mt-2 font-bold">{{ $message }}</p> @enderror
                </div>
