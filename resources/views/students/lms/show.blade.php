                                        ...
                                        @elseif($task->assignment_type == 'link')
                                            <div class="flex flex-col sm:flex-row gap-4">
                                                ...
                                                <form action="{{ route('students.learning.assignment.submit', $task->id) }}" method="POST" class="flex-1" id="form-complete-{{ $task->id }}">
                                                    @csrf
                                                    <button type="button" onclick="confirmTaskSubmit('{{ $task->id }}')" class="w-full py-3.5 bg-emerald-600 text-white font-bold rounded-2xl hover:bg-emerald-700 transition shadow-lg shadow-emerald-200 flex items-center justify-center gap-2 active:scale-[0.98]">
