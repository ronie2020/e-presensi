<x-student-learning-layout>
    ...
    <div x-data="{ activeTab: 'materi' }" class="min-h-screen bg-slate-50/50 pb-20">
        ...
        {{-- HEADER MATA PELAJARAN (NEW CBT STYLE - BLUE THEME) --}}
        <div class="animate-enter relative bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 pb-24 pt-12 px-4 sm:px-6 lg:px-8 overflow-hidden rounded-b-[3rem] shadow-2xl shadow-blue-900/20">
            ...
            <div class="relative max-w-6xl mx-auto z-10">
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
                    <div class="flex-1">
                        {{-- Tombol Kembali & Badge --}}
                        <div class="flex items-center gap-4 mb-4">
                            <a href="{{ route('students.learning.index') }}" class="group flex items-center justify-center w-10 h-10 rounded-xl bg-white/10 backdrop-blur-md border border-white/10 text-slate-300 hover:bg-white/20 hover:text-white hover:border-white/30 transition-all active:scale-95 shrink-0 shadow-sm" title="Kembali">
                    ...
                    {{-- Statistik Ringkas (Style Ala CBT) --}}
                    <div class="flex gap-3 w-full md:w-auto overflow-x-auto no-scrollbar pb-2 md:pb-0">
                        <div class="bg-white/5 backdrop-blur-md border border-white/10 p-4 rounded-2xl text-center min-w-[100px] shadow-lg flex-1 md:flex-none">
                            ...
                            <p class="text-2xl font-black text-blue-300">{{ $materials->count() }}</p>
                        </div>
                        <div class="bg-white/5 backdrop-blur-md border border-white/10 p-4 rounded-2xl text-center min-w-[100px] shadow-lg flex-1 md:flex-none">
                            <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-1">Tugas</p>
                            <p class="text-2xl font-black text-yellow-400">{{ $assignments->count() }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div x-show="activeTab === 'materi'" class="space-y-6" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0">
                ...
                @else
                    @foreach($materials as $index => $item)
                        <div class="animate-enter bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden hover:shadow-xl hover:shadow-blue-900/5 transition-all duration-300 group" style="animation-delay: {{ ($index + 1) * 100 }}ms">
                            ...
                            <div class="p-6 md:p-8 border-b border-slate-50 bg-gradient-to-r from-white to-slate-50/50">
                                <div class="flex flex-col md:flex-row md:items-start justify-between gap-4">
                                    <div class="flex items-start gap-4">
                                        ...
                                        <div>
                                            <h3 class="font-bold text-xl text-slate-900 leading-tight group-hover:text-blue-800 transition-colors">{{ $item->title }}</h3>
                            ...
                            <div class="p-6 md:p-8">
                                ...
                                @elseif($item->description)
                                    <p class="text-slate-600 mb-8 italic pl-4 border-l-4 border-slate-200 py-2">{{ $item->description }}</p>
                                @endif
                                        ...
                                        @foreach($item->attachments as $att)
                                            <div class="flex items-center gap-4 p-4 rounded-2xl border border-slate-200 bg-white hover:border-blue-400 hover:shadow-lg hover:shadow-blue-900/5 transition-all group/file cursor-pointer relative overflow-hidden"
                                                ...
                                                <div class="w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 shadow-sm transition-all duration-300
                                                    {{ $att->file_type == 'file' ? 'bg-orange-50 text-orange-600 group-hover/file:bg-orange-600 group-hover/file:text-white' : ($att->file_type == 'video' ? 'bg-rose-50 text-rose-600 group-hover/file:bg-rose-600 group-hover/file:text-white' : 'bg-blue-50 text-blue-600 group-hover/file:bg-blue-600 group-hover/file:text-white') }}">
                    ...
                    @foreach($assignments as $index => $task)
                        ...
                        <div class="animate-enter bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden hover:shadow-xl hover:shadow-slate-200/50 transition-all duration-300 group hover:-translate-y-1" style="animation-delay: {{ ($index + 1) * 100 }}ms" x-data="{ openUpload: false }">
                            ...
                            <div class="p-6 md:p-8 flex flex-col md:flex-row md:items-start justify-between gap-6 relative">
                                {{-- Status Stripe --}}
                                <div class="absolute left-0 top-8 bottom-8 w-1.5 rounded-r-full {{ $mySubmission ? 'bg-emerald-500' : ($isLate ? 'bg-rose-500' : 'bg-slate-300') }}"></div>
                                ...
                                <div class="flex items-start gap-5 pl-4">
                                    ...
                                    <div>
                                        <div class="flex flex-wrap items-center gap-2 mb-2">
                                            <span class="text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded-md bg-slate-100 text-slate-500 border border-slate-200">{{ $typeLabel }}</span>
                                            @if($isLate)
                                        ...
                                        <h3 class="font-bold text-slate-900 text-xl group-hover:text-blue-700 transition-colors leading-tight mb-2">{{ $task->title }}</h3>
                                        
                                        <div class="flex items-center gap-4 text-xs font-bold text-slate-500">
                                            <span class="flex items-center gap-1.5 {{ $isLate ? 'text-rose-600' : '' }}">
                            ...
                            <div class="px-6 md:px-8 pb-8">
                                <div class="prose prose-sm max-w-none text-slate-600 bg-slate-50 p-6 rounded-2xl border border-slate-100 mb-6 leading-relaxed">
                                    ...
                                    @if($task->assignment_type == 'quiz')
                                        <div class="mt-4 pt-4 border-t border-slate-200 text-xs font-bold text-slate-500 flex gap-6">
                                            <span class="flex items-center gap-1.5"><i class="ph-bold ph-timer text-purple-600 text-lg"></i> {{ $task->duration_minutes ?? 60 }} Menit</span>
                                            <span class="flex items-center gap-1.5"><i class="ph-bold ph-list-numbers text-purple-600 text-lg"></i> {{ $task->questions->count() }} Soal</span>
                                        </div>
                                    @endif
                                </div>
                                ...
                                @if(!$mySubmission)
                                    ...
                                    @else
                                        ...
                                        {{-- KUIS ONLINE --}}
                                        @if($task->assignment_type == 'quiz')
                                            <a href="{{ route('students.learning.assignment.quiz', $task->id) }}" class="w-full py-4 bg-purple-600 text-white font-bold rounded-2xl hover:bg-purple-700 transition shadow-lg shadow-purple-200 flex items-center justify-center gap-2 group/btn relative overflow-hidden active:scale-[0.98]">
                                        ...
                                        @elseif($task->assignment_type == 'link')
                                            <div class="flex flex-col sm:flex-row gap-4">
                                                ...
                                                <form action="{{ route('students.learning.assignment.submit', $task->id) }}" method="POST" class="flex-1">
                                                    @csrf
                                                    <button type="submit" class="w-full py-3.5 bg-emerald-600 text-white font-bold rounded-2xl hover:bg-emerald-700 transition shadow-lg shadow-emerald-200 flex items-center justify-center gap-2 active:scale-[0.98]" onclick="return confirm('Pastikan Anda sudah mengerjakan soal di link tersebut. Tandai selesai?')">
