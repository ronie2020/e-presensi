            ...
            <div class="absolute top-0 left-0 w-full flex justify-between items-start pt-2">
                <div class="flex items-center gap-4">
                    <div class="p-3 bg-slate-800/50 rounded-2xl border border-slate-700/50 shadow-2xl backdrop-blur-sm">
                        <x-application-logo class="w-12 h-12 text-white fill-current drop-shadow-lg" />
                    </div>
                    <div>
            ...
            <!-- MODE SELECTOR (Tombol Cepat - F1 s/d F6) -->
             <div class="mt-8 w-full max-w-3xl">
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
                    ...
                    <button type="button" onclick="window.setMode('{{ $btn['type'] }}', true)" class="bg-slate-800/60 hover:bg-{{ $btn['color'] }}-900/40 border border-slate-700 hover:border-{{ $btn['color'] }}-500 text-slate-300 hover:text-{{ $btn['color'] }}-400 py-4 rounded-2xl backdrop-blur-sm transition-all flex flex-col items-center gap-2 group shadow-sm hover:shadow-{{ $btn['color'] }}-500/20 active:scale-95">
                        <i class="ph-bold ph-{{ $btn['icon'] }} text-2xl group-hover:scale-110 transition-transform"></i>
                        <span class="text-[10px] font-bold uppercase tracking-wider">{{ $btn['label'] }}</span>
                    </button>
                    @endforeach
        ...
        <div class="bg-slate-800 rounded-3xl border border-slate-700 p-8 w-full max-w-lg shadow-2xl" @click.away="closeModal()">
            ...
            <div class="grid grid-cols-2 gap-4 max-h-[50vh] overflow-y-auto custom-scrollbar pr-2">
                @forelse($extracurriculars as $ex)
                    <button type="button" @click="selectExtra('{{ $ex->id }}', '{{ $ex->name }}')" class="p-4 bg-slate-700/50 hover:bg-pink-900/30 border border-slate-600 hover:border-pink-500 rounded-xl text-left transition-all group active:scale-95">
                        <span class="font-bold text-slate-300 group-hover:text-pink-300 text-base block">{{ $ex->name }}</span>
                    </button>
                @empty
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        let csrfToken = '{{ csrf_token() }}'; // DIUBAH JADI LET AGAR BISA DI-UPDATE OTOMATIS
       ...
       // 1. Tambahkan 2 variabel ini di luar listener (di atasnya)
        ...
        scanInput.addEventListener('change', async function(e) {
            ...
            // Catat barcode ini sebagai scan terakhir
