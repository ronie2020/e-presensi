            ...
            <div class="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
                <div class="text-center md:text-left flex-1">
                    ...
                    <h1 class="text-3xl md:text-5xl font-black mb-4 leading-tight tracking-tight">
                        Halo, <span class="text-transparent bg-clip-text bg-gradient-to-r from-blue-300 to-cyan-200">{{ $user->name ?? 'Sahabat' }}!</span> 👋
                    </h1>
                ...
                <div class="shrink-0 relative group">
                    ...
                    <div class="relative w-40 h-40 md:w-52 md:h-52 bg-slate-900/60 rounded-full flex flex-col items-center justify-center backdrop-blur-md border border-white/10 shadow-inner">
                        ...
                        <h2 class="text-5xl md:text-6xl font-black text-white tracking-tighter">{{ number_format($totalPoints) }}</h2>
          ...
          {{-- LEFT COLUMN: STATS --}}
            <div class="space-y-6">
                <div class="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm relative overflow-hidden group hover:shadow-md transition-shadow">
                    ...
                    <div class="relative z-10">
                        ...
                        <div class="flex items-baseline gap-2 mb-4">
                            <h3 class="text-4xl font-black text-slate-800">{{ $monthlyCount }}</h3>
                            <span class="text-sm text-slate-500 font-bold">Hari Lapor</span>
                        </div>
                        <div class="w-full bg-slate-100 h-3 rounded-full overflow-hidden">
                            <div class="bg-gradient-to-r from-blue-500 to-cyan-400 h-full rounded-full transition-all duration-1000" style="width: {{ min(($monthlyCount/30)*100, 100) }}%"></div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-gradient-to-br from-indigo-600 to-blue-700 p-6 rounded-[2rem] text-white shadow-xl shadow-indigo-500/20 relative overflow-hidden group">
                    ...
                    @if($lastFeedback)
                        <p class="font-medium text-sm leading-relaxed mb-4 italic text-white/90 relative z-10">"{{ $lastFeedback->teacher_feedback }}"</p>
                        <p class="text-[10px] font-bold uppercase opacity-80">{{ \Carbon\Carbon::parse($lastFeedback->report_date)->translatedFormat('d M Y') }}</p>
                    @else
            ...
            <div class="lg:col-span-2 bg-white rounded-[2.5rem] p-6 md:p-8 border border-slate-100 shadow-sm relative">
                <div class="flex items-center justify-between mb-8">
                    ...
                    <div class="px-3 py-1 bg-slate-50 rounded-lg border border-slate-100 text-[10px] font-bold text-slate-500">{{ \Carbon\Carbon::now()->translatedFormat('l, d F Y') }}</div>
                </div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                    ...
                    @foreach($habits as $h)
                        ...
                        <div class="relative p-5 rounded-2xl border transition-all duration-300 group overflow-hidden {{ $bgColor }} {{ $borderColor }} {{ $shadow }}">
                            <div class="relative z-10 flex flex-col items-center text-center h-full justify-center">
                                ...
                                <div class="mb-3 p-3 rounded-full {{ $isDone ? 'bg-white/20' : 'bg-white shadow-sm' }}">
                                    <i class="ph-duotone ph-{{ $h['icon'] }} text-2xl md:text-3xl {{ $iconColor }}"></i>
                                </div>
                                
                                <h4 class="font-bold text-xs md:text-sm {{ $textColor }}">{{ $h['label'] }}</h4>
                                
                                {{-- Instruksi Khusus Makan --}}
                                @if($h['id'] == 'habit_5' && !$isDone)
                                ...
                                @elseif(!$isDone)
                                    <p class="text-[9px] {{ $textColor }} opacity-60 mt-1">Belum tuntas</p>
                                @endif
                            </div>
                        </div>
                    @endforeach
                ...
                {{-- 7. Tidur Cepat (Card Full Width) --}}
                @php
                    $isTidurDone = $todayHabit && $todayHabit->habit_7;
                @endphp
                <div class="mt-4 p-5 rounded-2xl {{ $isTidurDone ? 'bg-indigo-600 border-indigo-500' : 'bg-slate-900 border-slate-800' }} border flex items-center gap-5 text-white relative overflow-hidden group transition-all">
                    ...
                    <div class="w-14 h-14 bg-white/10 rounded-2xl flex items-center justify-center text-3xl text-indigo-400 shrink-0 border border-white/5 shadow-inner">
                        <i class="ph-duotone ph-moon-stars {{ $isTidurDone ? 'text-white' : 'text-indigo-300' }}"></i>
                    </div>
                    <div class="relative z-10">
                        ...
                        <p class="text-xs {{ $isTidurDone ? 'text-indigo-100' : 'text-slate-400' }} mt-0.5">
                            {{ $isTidurDone ? 'Tercatat tidur pukul ' . $todayHabit->habit_7_time : 'Jangan begadang ya! Istirahat maksimal jam 22:00.' }}
                        </p>
                    </div>
                    @if($isTidurDone)
                                ...
                                <tr class="hover:bg-blue-50/20 transition-colors group">
                                    <td class="py-5 px-6">
                                        <div class="flex items-center gap-3">
                                            <div class="w-10 h-10 rounded-xl bg-slate-100 flex items-center justify-center text-slate-500 font-bold text-xs group-hover:bg-blue-100 group-hover:text-blue-600 transition-colors">
                                                {{ \Carbon\Carbon::parse($activity->report_date)->format('d') }}
                                            </div>
                                            <div>
                                                <span class="font-bold text-slate-700 block text-xs md:text-sm">
                                                    {{ \Carbon\Carbon::parse($activity->report_date)->translatedFormat('F Y') }}
                                                </span>
                                                <span class="text-[9px] text-slate-400 font-bold uppercase tracking-wide">
                                                    {{ \Carbon\Carbon::parse($activity->report_date)->translatedFormat('l') }}
                                                </span>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="py-5 px-6">
                                        <div class="flex -space-x-2">
                                            {{-- Icon Logic Mini --}}
                                            @if($activity->habit_1) <div title="Bangun Pagi" class="w-8 h-8 rounded-full bg-blue-100 border-2 border-white flex items-center justify-center text-blue-600"><i class="ph-fill ph-sun-horizon text-xs"></i></div> @endif
                                            @if($activity->habit_3) <div title="Olahraga" class="w-8 h-8 rounded-full bg-indigo-100 border-2 border-white flex items-center justify-center text-indigo-600"><i class="ph-fill ph-sneaker-move text-xs"></i></div> @endif
                                            @if($activity->habit_5) <div title="Makan Sehat" class="w-8 h-8 rounded-full bg-orange-100 border-2 border-white flex items-center justify-center text-orange-600"><i class="ph-fill ph-carrot text-xs"></i></div> @endif
                                        ...
                                        @if($activity->teacher_feedback)
                                            <div class="flex items-start gap-3 bg-indigo-50 p-3 rounded-xl border border-indigo-100 max-w-xs">
                                                ...
                                                <p class="text-xs text-indigo-800 italic leading-snug">
                                                    "{{ Str::limit($activity->teacher_feedback, 60) }}"
                                                </p>
                                            </div>
                                        @else
