    ...
    <div class="min-h-screen bg-slate-50 font-jakarta p-4 md:p-8 pb-20">
        ...
        <div class="relative rounded-[3rem] bg-gradient-to-b from-amber-500 to-orange-600 p-8 md:p-12 text-white shadow-2xl shadow-orange-500/20 overflow-hidden text-center mb-16">
            ...
            <div class="relative z-10">
                <a href="{{ route('teacher.habits.index') }}" class="absolute top-0 left-0 bg-white/20 hover:bg-white/30 px-5 py-2.5 rounded-2xl text-xs font-bold backdrop-blur transition-colors flex items-center gap-2">
                ...
                <p class="text-orange-100 font-medium text-sm md:text-base max-w-xl mx-auto">
                    Daftar apresiasi siswa paling konsisten dalam membangun kebiasaan baik pada bulan <span class="font-bold border-b border-white/50">{{ \Carbon\Carbon::now()->translatedFormat('F Y') }}</span>.
                </p>
            </div>
        </div>
                    ...
                    @foreach($leaderboard->take(3) as $index => $top)
                        ...
                        <div class="w-full md:w-1/3 flex flex-col items-center {{ $orderClass }} transform transition-transform hover:-translate-y-2">
                            <div class="relative mb-4 group cursor-pointer">
                                ...
                                <div class="w-20 h-20 md:w-24 md:h-24 rounded-full bg-white shadow-xl flex items-center justify-center text-2xl font-black text-slate-700 border-4 {{ $rank==1 ? 'border-yellow-400' : ($rank==2 ? 'border-slate-300' : 'border-amber-600') }} relative z-10 overflow-hidden">
                                    @if($top->student->photo_path)
                                        <img src="{{ asset('storage/' . $top->student->photo_path) }}" class="w-full h-full object-cover">
                                    @else
                                        {{ substr($top->student->name, 0, 1) }}
                                    @endif
                                </div>
                            ...
                            <div class="w-full rounded-t-3xl border shadow-lg {{ $bgClass }} {{ $heightClass }} p-4 flex flex-col items-center justify-start pt-8 relative overflow-hidden text-center">
                                <div class="absolute inset-0 bg-white opacity-20 mix-blend-overlay"></div>
                                <h3 class="font-black text-sm md:text-base {{ $textColor }} relative z-10 leading-tight mb-1">{{ $top->student->name }}</h3>
                                <p class="{{ $textColor }} opacity-80 text-[10px] font-bold uppercase tracking-widest relative z-10">{{ $top->student->schoolClass->name ?? '-' }}</p>
                                
                                <div class="mt-auto bg-black/10 backdrop-blur rounded-2xl px-4 py-3 relative z-10 w-full">
                                    <p class="text-xs font-black {{ $textColor }} uppercase tracking-widest mb-0.5">Konsistensi</p>
                                    <p class="text-2xl font-black {{ $textColor }} leading-none">{{ $top->total_days }} <span class="text-xs font-bold">Hari</span></p>
                                </div>
                            </div>
                        </div>
                    @endforeach
                            ...
                            @foreach($leaderboard->skip(3) as $index => $row)
                                <div class="flex items-center justify-between p-4 rounded-2xl hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-100 group">
                                    <div class="flex items-center gap-5">
                                        ...
                                        <div>
                                            <h4 class="font-black text-slate-800 uppercase tracking-tight text-sm">{{ $row->student->name }}</h4>
                                    ...
                                    <div class="text-right">
                                        <div class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-blue-50 border border-blue-100">
                                            ...
                                            <span class="font-black text-blue-700 text-sm">{{ $row->total_days }} <span class="text-[10px] text-blue-400">Hari</span></span>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
