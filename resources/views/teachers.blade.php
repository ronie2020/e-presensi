    ...
    <nav class="bg-white/80 backdrop-blur-md border-b border-slate-200 sticky top-0 z-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between h-20 items-center">
                ...
                <a href="{{ route('landing') }}" class="text-sm font-bold text-slate-600 hover:text-blue-600 transition flex items-center gap-2 bg-slate-50 px-4 py-2 rounded-full hover:bg-blue-50">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center animate-enter">
            ...
            <form action="{{ route('teachers.index') }}" method="GET" class="max-w-lg mx-auto relative group">
                ...
                <div class="relative">
                    ...
                    @if(request('q'))
                        <a href="{{ route('teachers.index') }}" class="absolute right-4 top-1/2 -translate-y-1/2 w-9 h-9 flex items-center justify-center rounded-full bg-slate-100 text-slate-500 hover:bg-red-100 hover:text-red-600 transition-colors" title="Hapus Pencarian">
                    ...
                    <div class="aspect-[4/5] sm:aspect-square bg-slate-200 relative overflow-hidden">
                        ...
                        <div class="absolute inset-0 bg-gradient-to-t from-slate-900/90 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-end justify-center pb-6 gap-3 translate-y-4 group-hover:translate-y-0">
                            @if($teacher->phone)
                                ...
                                @endphp
                                <a href="https://wa.me/{{ $waLink }}" target="_blank" class="w-10 h-10 rounded-full bg-white/20 backdrop-blur-md border border-white/30 text-white flex items-center justify-center hover:bg-green-500 hover:border-green-500 hover:scale-110 transition shadow-lg">
                                    <i class="ph-fill ph-whatsapp-logo text-xl"></i>
                                </a>
                            @endif
                            @if($teacher->email)
                                <a href="mailto:{{ $teacher->email }}" class="w-10 h-10 rounded-full bg-white/20 backdrop-blur-md border border-white/30 text-white flex items-center justify-center hover:bg-blue-500 hover:border-blue-500 hover:scale-110 transition shadow-lg">
