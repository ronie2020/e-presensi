<x-app-layout>
    {{-- 
        Halaman Index Admin PPDB
        Style disesuaikan dengan subjects.blade.php (Hero Dark Blue + Glassmorphism)
    --}}
    ...
    <div class="py-8 sm:py-10 font-sans text-slate-800">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {{-- HERO SECTION: GABUNGAN JUDUL, STATISTIK, DAN JADWAL --}}
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-3 gap-4">
                            <div class="bg-white/10 backdrop-blur-md px-5 py-4 rounded-2xl border border-white/10 hover:bg-white/15 transition-colors">
                                ...
                                <span class="block text-2xl font-black text-white tracking-tight">{{ $stats['total'] }}</span>
                            </div>
                            <div class="bg-white/10 backdrop-blur-md px-5 py-4 rounded-2xl border border-white/10 hover:bg-white/15 transition-colors">
                                <span class="block text-[10px] font-bold uppercase tracking-wider text-emerald-300 mb-1">Diterima</span>
                                <span class="block text-2xl font-black text-white tracking-tight">{{ $stats['accepted'] }}</span>
                            </div>
                            <div class="bg-white/10 backdrop-blur-md px-5 py-4 rounded-2xl border border-white/10 hover:bg-white/15 transition-colors">
                                <span class="block text-[10px] font-bold uppercase tracking-wider text-yellow-300 mb-1">Pending</span>
                                <span class="block text-2xl font-black text-white tracking-tight">{{ $stats['pending'] }}</span>
                            </div>
                        </div>
                    </div>

                    {{-- KANAN: PANEL JADWAL (GLASSMORPHISM) --}}
                            ...
                            <div class="mb-6 p-4 rounded-2xl bg-black/20 border border-white/10">
                                @if($isSet)
                                    <div class="flex items-center gap-3 mb-2">
                                        ...
                                        <span class="text-xs font-black uppercase tracking-wider {{ $isPast ? 'text-emerald-300' : 'text-blue-300' }}">
                                            {{ $isPast ? 'Sudah Dibuka' : 'Terjadwal (Menunggu)' }}
                                        </span>
                                    </div>
                                    <p class="text-white text-sm font-bold tracking-wide">
                                        {{ $announcementTime->translatedFormat('l, d F Y') }} <span class="text-blue-300 mx-1">•</span> {{ $announcementTime->format('H:i') }} WIB
                                    </p>
                                @else
                ...
                <div class="p-8 border-b border-slate-50 bg-slate-50/30 flex flex-col xl:flex-row xl:items-center justify-between gap-6">
                    ...
                    <div class="flex flex-wrap gap-2 items-center">
                        ...
                        <a href="{{ route('admin.ppdb.index') }}" 
                           class="{{ $btnBase }} {{ !request('status') ? $activeClass : $inactiveClass }}">
                           <i class="ph-bold ph-squares-four"></i> Semua
                        </a>
                        <a href="{{ route('admin.ppdb.index', ['status' => 'pending']) }}" 
                           class="{{ $btnBase }} {{ request('status') == 'pending' ? 'bg-yellow-500 text-white border-yellow-500' : $inactiveClass }}">
                           <i class="ph-bold ph-clock"></i> Pending
                        </a>
                        <a href="{{ route('admin.ppdb.index', ['status' => 'verified']) }}" 
                           class="{{ $btnBase }} {{ request('status') == 'verified' ? 'bg-blue-500 text-white border-blue-500' : $inactiveClass }}">
                           <i class="ph-bold ph-check-circle"></i> Verified
                        </a>
                        <a href="{{ route('admin.ppdb.index', ['status' => 'accepted']) }}" 
