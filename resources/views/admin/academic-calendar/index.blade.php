        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-1">
                <div class="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 sticky top-6">
                    ...
                    <form action="{{ route('admin.academic-calendar.store') }}" method="POST" class="space-y-4">
                        @csrf
                        <div>
            ...
            <div class="lg:col-span-2 space-y-4">
                @forelse($events as $event)
                    ...
                    @endphp
                    <div class="bg-white p-5 rounded-[2rem] shadow-sm border {{ $isPast ? 'border-slate-100 opacity-60' : 'border-slate-200' }} flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between group hover:border-blue-300 transition-colors">
                        <div class="flex items-start gap-4">
                            <div class="w-12 h-12 rounded-2xl flex items-center justify-center text-2xl shrink-0 {{ $badgeColor }} border">
                                <i class="ph-duotone {{ $icon }}"></i>
                            </div>
                            <div>
                                <div class="flex items-center gap-2 mb-1">
                                    <span class="text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded border {{ $badgeColor }}">
                                        {{ $event->type }}
                                    </span>
                                    @if($isPast)
                                ...
                                <h4 class="font-black text-slate-800 text-lg leading-tight mb-1">{{ $event->title }}</h4>
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            {{-- TOMBOL HAPUS --}}
                            <form action="{{ route('admin.academic-calendar.destroy', $event->id) }}" method="POST" onsubmit="return confirm('Hapus agenda ini?');">
                                @csrf
                                @method('DELETE')
