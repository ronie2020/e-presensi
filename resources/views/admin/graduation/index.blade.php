<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-8">
            <div class="animate-enter relative rounded-[2rem] md:rounded-[3rem] bg-gradient-to-br from-[#25D0FF] via-[#5295FF] to-[#FFC9B9] p-8 sm:p-10 text-[#2A3B52] shadow-[0_10px_40px_-10px_rgba(37,208,255,0.4)] overflow-hidden border border-white/40 group">
                ...
                <div class="relative z-10 flex flex-col xl:flex-row justify-between items-start xl:items-center gap-8">
                    <div class="space-y-3">
                        <a href="{{ route('dashboard') }}" class="group bg-white/40 hover:bg-white/60 text-[#2A3B52] px-5 py-2.5 rounded-xl font-bold text-sm backdrop-blur-sm border border-white/50 transition-all flex items-center gap-2 shadow-sm w-fit mb-4 mx-auto xl:mx-0">
                ...
                <div class="bg-white rounded-[2rem] fluent-card overflow-hidden">
                    ...
                    <div class="px-6 py-5 border-b border-slate-100 bg-white flex flex-col sm:flex-row justify-between items-center gap-4">
                        <div class="flex items-center gap-2">
                            <span class="bg-[#F3F9FD] border border-[#D0E7F8] text-[#5295FF] px-4 py-1.5 rounded-lg text-xs font-bold shadow-sm">Total: {{ $students->total() }} Siswa</span>
                        </div>
                                ...
                                <tr class="hover:bg-slate-50/50 transition-colors group">
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <div class="font-bold text-[#2A3B52] group-hover:text-[#5295FF] transition-colors">{{ $student->name }}</div>
                                    ...
                                    <td class="px-6 py-4">
                                        <select name="students[{{ $student->id }}][status]" class="w-36 py-2 px-3 rounded-lg text-xs font-bold border-slate-200 focus:ring-[#5295FF] focus:border-[#5295FF] cursor-pointer shadow-sm
                                            {{ $student->graduation?->status == 'LULUS' ? 'bg-[#DFF6DD] text-[#107C10] border-[#B7DFB9]' : '' }}
                                            {{ $student->graduation?->status == 'TIDAK LULUS' ? 'bg-[#FDE7E9] text-[#D13438] border-[#F4C3C9]' : '' }}">
                                    ...
                                    <td class="px-6 py-4 text-center bg-white border-l border-slate-100">
                                        <div class="flex items-center justify-center gap-2">
                                            <button type="button" onclick="saveSingle('{{ $student->id }}')" class="w-8 h-8 flex items-center justify-center rounded-lg text-slate-400 hover:bg-[#F3F9FD] hover:text-[#5295FF] border border-transparent hover:border-[#D0E7F8] transition-all" title="Simpan Baris Ini">
                                                <i class="ph-bold ph-check text-lg"></i>
                                            </button>
                                            {{-- Tombol Cetak PDF --}}
                                            @if($student->graduation?->status == 'LULUS')
                                                <a href="{{ route('graduation.print', $student->id) }}" target="_blank" class="w-8 h-8 flex items-center justify-center rounded-lg text-[#107C10] hover:bg-[#DFF6DD] border border-transparent hover:border-[#B7DFB9] transition-all" title="Cetak SKL">
                                                    <i class="ph-bold ph-printer text-lg"></i>
                                                </a>
                                            @endif
                                        </div>
                                        <span id="msg_{{ $student->id }}" class="text-[10px] text-[#107C10] font-bold hidden animate-pulse mt-1">Tersimpan!</span>
                                    </td>
                                </tr>
                                @empty
                                <tr>
    ...
    <dialog id="modalSkl" class="rounded-[2rem] p-0 fluent-modal w-full max-w-md bg-transparent">
        <form action="{{ route('admin.graduation.bulk_skl') }}" method="POST" class="bg-white p-8 rounded-[2rem]">
            @csrf
    ...
    <dialog id="modalGlobalDate" class="rounded-[2rem] p-0 fluent-modal w-full max-w-md bg-transparent">
        <form action="{{ route('admin.graduation.set_date') }}" method="POST" class="bg-white p-8 rounded-[2rem]">
            @csrf
            <div class="flex items-center gap-3 mb-6 border-b border-slate-100 pb-4">
    ...
    <dialog id="modalImport" class="rounded-[2rem] p-0 fluent-modal w-full max-w-md bg-transparent">
        <form action="{{ route('admin.graduation.import') }}" method="POST" enctype="multipart/form-data" class="bg-white p-8 rounded-[2rem]">
            @csrf
    ...
    <dialog id="modalSettings" class="rounded-[2rem] p-0 fluent-modal w-full max-w-md bg-transparent">
        <form action="{{ route('admin.graduation.save_settings') }}" method="POST" class="bg-white p-8 rounded-[2rem]">
            @csrf
            <div class="flex items-center gap-3 mb-6 border-b border-slate-100 pb-4">
            ...
            <div class="space-y-5 mb-8">
                <div>
                    ...
                    <input type="text" name="letter_number" value="{{ $settings['letter_number'] ?? '' }}" required class="w-full py-3 px-4 rounded-xl border-slate-200 text-sm font-bold text-[#2A3B52] focus:border-[#5295FF] focus:ring-[#5295FF] shadow-sm" placeholder="Contoh: 421.3/ ... /SMP.03/2026">
                ...
                <div>
                    ...
                    <input type="text" name="principal_name" value="{{ $settings['principal_name'] ?? '' }}" required class="w-full py-3 px-4 rounded-xl border-slate-200 text-sm font-bold text-[#2A3B52] focus:border-[#5295FF] focus:ring-[#5295FF] shadow-sm" placeholder="Nama beserta gelar">
                </div>
                <div>
                    <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2 ml-1">NIP Kepala Sekolah</label>
                    <input type="text" name="principal_nip" value="{{ $settings['principal_nip'] ?? '' }}" required class="w-full py-3 px-4 rounded-xl border-slate-200 text-sm font-bold text-[#2A3B52] focus:border-[#5295FF] focus:ring-[#5295FF] shadow-sm" placeholder="NIP Kepala Sekolah">
                </div>
            </div>
