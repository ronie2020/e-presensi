        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div class="bg-white p-5 rounded-[2rem] border border-slate-100 shadow-sm">
                    ...
                    <p class="text-2xl font-black text-slate-800">{{ $results->count() }} <span class="text-sm text-slate-400 font-bold">Siswa</span></p>
                </div>
            </div>
                        ...
                        <tbody class="divide-y divide-slate-50">
                            @forelse($results as $index => $res)
                                <tr x-show="search === '' || '{{ strtolower($res->student_name) }}'.includes(search.toLowerCase())" 
                                    ...
                                    <td class="px-6 py-4">
                                        <p class="font-bold text-slate-800 text-base">{{ $res->student_name }}</p>
                                        <p class="text-xs text-slate-400 font-mono mt-0.5">{{ $res->student_nisn ?? 'NISN Tidak Ada' }}</p>
                                    </td>

                                    <td class="px-6 py-4 text-center">
                                        <div class="inline-flex items-center gap-2 bg-slate-100 rounded-lg p-1.5">
                                            {{-- FIX: Menggunakan nama kolom yang benar (correct_answers & wrong_answers) --}}
                                            <span class="px-2 py-0.5 bg-emerald-100 text-emerald-700 rounded text-xs font-bold" title="Benar">{{ $res->correct_answers ?? 0 }}</span>
                                            <span class="text-slate-300">/</span>
                                            <span class="px-2 py-0.5 bg-rose-100 text-rose-700 rounded text-xs font-bold" title="Salah">{{ $res->wrong_answers ?? 0 }}</span>
                                        </div>
                                    </td>

                                    <td class="px-6 py-4 text-center">
                                        <span class="text-xl font-black {{ $res->total_score >= $exam->passing_grade ? 'text-emerald-600' : 'text-rose-500' }}">
                                            {{ $res->total_score ?? 0 }}
                                        </span>
                                    </td>
