        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="relative rounded-[2rem] bg-gradient-to-r from-[#56bbf1] via-[#e5eff5] to-[#f4d1c0] p-8 text-[#2c3f61] shadow-xl shadow-[#56bbf1]/10 overflow-hidden border border-white/60 print:hidden">
                ...
                <div class="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    <div>
                        ...
                        <p class="text-[#2c3f61]/80 text-sm font-medium">Mapel: {{ $exam->subject_name }} • Kelas {{ $exam->class_level }}</p>
                    </div>
                    ...
                    <div class="flex flex-wrap gap-3">
                        ...
                        <a href="{{ route('cbt.analysis', $exam->id) }}" class="group px-5 py-3 bg-white text-[#0d52a1] font-bold rounded-2xl hover:bg-slate-50 transition flex items-center gap-2 shadow-lg shadow-[#0d52a1]/10 border border-slate-100">
                            <i class="ph-duotone ph-chart-pie-slice text-xl text-[#56bbf1]"></i>
                            <span class="hidden sm:inline">Analisis Soal</span>
                        </a>

                        {{-- Tombol Export Excel --}}
                        <a href="{{ route('cbt.export', ['id' => $exam->id, 'type' => 'excel']) }}" target="_blank" class="group px-5 py-3 bg-emerald-500 text-white font-bold rounded-2xl hover:bg-emerald-600 transition flex items-center gap-2 shadow-lg shadow-emerald-500/20">
                            <i class="ph-duotone ph-microsoft-excel-logo text-xl group-hover:scale-110 transition-transform"></i> 
                            <span class="hidden sm:inline">Excel</span>
                        </a>
                        
                        {{-- Tombol Export PDF --}}
                        <a href="{{ route('cbt.export', ['id' => $exam->id, 'type' => 'pdf']) }}" target="_blank" class="group px-5 py-3 bg-rose-500 text-white font-bold rounded-2xl hover:bg-rose-600 transition flex items-center gap-2 shadow-lg shadow-rose-500/20">
            ...
            @if(isset($exam->exam_type) && $exam->exam_type == 'google_form')
                <div class="bg-white rounded-[2.5rem] p-12 text-center border border-emerald-200 shadow-xl shadow-[#56bbf1]/10 relative overflow-hidden print:hidden mt-8">
                    ...
                    <a href="{{ $exam->google_form_url }}" target="_blank" class="inline-flex items-center justify-center px-8 py-4 bg-emerald-500 text-white font-bold rounded-xl hover:bg-emerald-600 transition shadow-lg shadow-emerald-500/30 gap-2 relative z-10 active:scale-95">
            ...
            @else
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 print:grid-cols-4 print:gap-2">
                    ...
                    <div class="bg-white p-5 rounded-[2.5rem] border border-slate-100 shadow-xl shadow-[#56bbf1]/5 print:border-black print:rounded-none print:shadow-none">
                        ...
                        <p class="text-3xl font-black text-[#2c3f61]">{{ number_format($stats['average'], 1) }}</p>
                    </div>
                    ...
                    <div class="bg-white p-5 rounded-[2.5rem] border border-slate-100 shadow-xl shadow-[#56bbf1]/5 print:border-black print:rounded-none print:shadow-none">
                        ...
                        <p class="text-3xl font-black text-[#2c3f61]">{{ $stats['max_score'] }}</p>
                    </div>
                    ...
                    <div class="bg-white p-5 rounded-[2.5rem] border border-slate-100 shadow-xl shadow-[#56bbf1]/5 print:border-black print:rounded-none print:shadow-none">
                        ...
                        <p class="text-3xl font-black text-[#2c3f61]">{{ $stats['min_score'] }}</p>
                    </div>
                    ...
                    <div class="bg-white p-5 rounded-[2.5rem] border border-slate-100 shadow-xl shadow-[#56bbf1]/5 print:border-black print:rounded-none print:shadow-none">
                        ...
                        <div class="flex items-end justify-between mt-1">
                            <p class="text-3xl font-black text-[#2c3f61]">{{ $results->count() }} <span class="text-sm text-slate-400 font-bold print:hidden">Siswa</span></p>
                                ...
                                @forelse($results as $index => $res)
                                    <tr x-show="search === '' || '{{ strtolower($res->student_name) }}'.includes(search.toLowerCase())" 
                                        ...
                                        <td class="px-6 py-4 text-center">
                                            ...
                                            @else
                                                <span class="font-black text-slate-400 print:text-black">{{ $index + 1 }}</span>
                                            @endif
                                        </td>

                                        <td class="px-6 py-4">
                                            <p class="font-black text-[#2c3f61] text-base print:text-black">{{ $res->student_name }}</p>
                                        ...
                                        <td class="px-6 py-4 text-center">
                                            <span class="text-xl font-black {{ $res->total_score >= $exam->passing_grade ? 'text-emerald-500' : 'text-rose-500' }} print:text-black">
                                                {{ $res->total_score ?? 0 }}
                                            </span>
                                        </td>
