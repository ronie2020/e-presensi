<x-app-layout>
    ...
    {{-- PENGEMBANGAN: Tambahkan x-data untuk Live Search & Filter --}}
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @forelse($exams as $exam)
                    <div x-show="(filter === 'all' || (filter === 'active' && {{ $exam->is_active ? 'true' : 'false' }}) || (filter === 'inactive' && {{ !$exam->is_active ? 'true' : 'false' }})) && ('{{ strtolower(addslashes($exam->title)) }}'.includes(search.toLowerCase()) || '{{ strtolower(addslashes($exam->subject_name)) }}'.includes(search.toLowerCase()))"
                        ...
                        <div class="mb-5 pr-20">
                            ...
                            <h4 class="font-black text-xl text-slate-800 leading-tight group-hover:text-blue-600 transition-colors line-clamp-2" title="{{ $exam->title }}">
                                {{ $exam->title }}
                            </h4>
                        </div>
                        ...
                        <div class="flex-1 space-y-4">
                            <div class="bg-slate-50 border border-slate-100 rounded-2xl p-4 flex items-center justify-between group/token cursor-pointer hover:bg-blue-50 hover:border-blue-100 transition" onclick="copyToken('{{ $exam->token }}')">
                                <div>
                        ...
                        <div class="mt-6 pt-4 border-t border-slate-50 grid grid-cols-2 gap-2">
                            <!-- Tombol Kelola -->
                            <a href="{{ route('cbt.questions.manage', $exam->id) }}" class="flex items-center justify-center p-2.5 bg-slate-50 text-slate-600 rounded-xl text-xs font-bold hover:bg-slate-800 hover:text-white transition-all group/btn">
                                <i class="ph-bold ph-list-numbers text-lg mr-2"></i> Soal
                            </a>
                            
                            <!-- Tombol Monitor -->
                            <a href="{{ route('cbt.monitoring', $exam->id) }}" class="flex items-center justify-center p-2.5 bg-emerald-50 text-emerald-600 rounded-xl text-xs font-bold hover:bg-emerald-600 hover:text-white transition-all group/btn border border-emerald-100">
                ...
                @empty
                    <div class="col-span-1 md:col-span-2 lg:col-span-3 text-center py-20 bg-white rounded-[2.5rem] border-2 border-dashed border-slate-200">
                        ...
                        <a href="{{ route('cbt.create') }}" class="inline-flex items-center gap-2 px-6 py-3 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 transition shadow-lg shadow-blue-500/30 text-sm">
                            <i class="ph-bold ph-plus"></i> Buat Jadwal Baru
                        </a>
                    </div>
                @endforelse

                {{-- State Kosong untuk Hasil Pencarian --}}
