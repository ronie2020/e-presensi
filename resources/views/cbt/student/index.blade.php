            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                @foreach($exams as $index => $examItem)
                    ...
                    <div x-show='(activeFilter === "all" || activeFilter === "{{ $filterCategory }}") && 
                         ...
                         class="animate-enter group bg-white rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-2xl hover:shadow-[#56bbf1]/10 hover:border-[#56bbf1]/30 transition-all duration-300 overflow-hidden flex flex-col h-full hover:-translate-y-1" style="animation-delay: {{ ($index + 1) * 50 }}ms">
                        ...
                        <div class="p-6 md:p-8 relative overflow-hidden">
                            ...
                            <h3 class="relative z-10 text-lg md:text-xl font-black text-[#2c3f61] leading-tight group-hover:text-[#0d52a1] transition-colors mb-3 line-clamp-2" title="{{ $examItem->title }}">
                        ...
                        <div class="p-4 bg-slate-50/50 border-t border-slate-100" x-data="{ isNavigating: false }">
                            ...
                            @elseif($status == 'ongoing')
                                <a href="{{ route('student.exam.run', $examItem->id) }}" @click="isNavigating = true" class="w-full py-4 rounded-xl bg-[#c86845] text-white font-bold text-sm shadow-lg shadow-[#c86845]/20 hover:bg-[#a65639] transition-all flex items-center justify-center gap-2 group/btn relative overflow-hidden">
                            ...
                            @else
                                {{-- Status: Open --}}
                                <a href="{{ route('student.exam.show', $examItem->id) }}" @click="isNavigating = true" class="w-full py-4 rounded-xl bg-[#0d52a1] text-white font-bold text-sm shadow-lg shadow-[#0d52a1]/20 hover:bg-[#0a4282] transition-all flex items-center justify-center gap-2 group/btn relative overflow-hidden">
                                    <template x-if="!isNavigating">
