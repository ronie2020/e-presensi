<x-app-layout>
            ...
            <div class="mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-5 rounded-[2rem] shadow-sm border border-slate-100">
                <div>
                    <div class="flex items-center gap-2 text-slate-400 text-sm font-bold mb-1">
                        <a href="{{ route('bank.index') }}" class="hover:text-elevate-primary transition flex items-center gap-1">
                            <i class="ph-bold ph-folders"></i> Dashboard Folder
                        </a>
                        <span>/</span>
                        <span class="text-elevate-primary">{{ $folder->name }}</span>
                    </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @forelse($banks as $bank)
                    <div class="bg-white border border-slate-100 rounded-[2.5rem] p-6 hover:shadow-xl hover:shadow-elevate-accent/10 hover:border-elevate-accent/50 transition-all duration-300 group relative flex flex-col h-full">
                        ...
                        <div class="mb-4 pr-4">
                            ...
                            <h4 class="font-black text-xl text-elevate-dark leading-tight group-hover:text-elevate-primary transition-colors line-clamp-2">{{ $bank->title }}</h4>
                            <p class="text-slate-500 text-sm font-bold mt-1">{{ $bank->subject_name }}</p>
                        </div>
                        ...
                        <div class="flex-1 space-y-4 mt-auto">
                            <div class="bg-white border border-slate-200 rounded-2xl p-4 flex items-center justify-between group/token hover:bg-elevate-soft/50 hover:border-elevate-accent/50 transition shadow-sm">
                                <div>
                                    ...
                                    <span class="font-mono font-black text-xl text-elevate-dark tracking-widest group-hover/token:text-elevate-primary">{{ $bank->questions_count ?? 0 }} Butir</span>
                                </div>
                        ...
                        <div class="mt-5 pt-4 border-t border-slate-100 grid grid-cols-2 gap-2">
                            ...
                            <a href="{{ route('bank.manage', $bank->id) }}" class="col-span-2 flex items-center justify-center p-2.5 bg-elevate-soft text-elevate-primary border border-elevate-accent/30 rounded-xl text-xs font-bold hover:bg-elevate-primary hover:text-white transition-all active:scale-95">
                                <i class="ph-bold ph-list-numbers text-lg mr-2"></i> Kelola Soal
                            </a>

                            <!-- Baris 2: Edit & Hapus -->
                            <button onclick="editMapel({{ $bank->id }}, '{{ addslashes($bank->title) }}', '{{ addslashes($bank->subject_name) }}', '{{ $bank->class_level }}', '{{ $bank->cbt_bank_folder_id }}')" class="col-span-1 flex items-center justify-center p-2.5 bg-white border border-slate-200 text-slate-600 rounded-xl text-xs font-bold hover:bg-slate-50 hover:text-elevate-primary hover:border-elevate-accent/50 transition-all">
                                <i class="ph-bold ph-pencil-simple text-lg mr-2"></i> Edit
                            </button>

                            <button onclick="confirmDelete('{{ $bank->id }}')" class="col-span-1 flex items-center justify-center p-2.5 bg-white border border-rose-200 text-rose-500 rounded-xl text-xs font-bold hover:bg-rose-50 hover:text-rose-600 transition-all active:scale-95">
                                <i class="ph-bold ph-trash text-lg mr-2"></i> Hapus
                            </button>

                            <form id="delete-form-{{ $bank->id }}" action="{{ route('bank.destroy', $bank->id) }}" method="POST" class="hidden">
                                @csrf @method('DELETE')
                            </form>
                        </div>
                    </div>
                @empty
                    ...
                    <div class="col-span-1 md:col-span-2 lg:col-span-3 text-center py-20 bg-white rounded-[2.5rem] border-2 border-dashed border-slate-200 mt-4">
                        ...
                        <p class="text-slate-500 max-w-xs mx-auto mb-8 text-sm">Silakan buat bank soal mata pelajaran baru yang akan dimasukkan ke dalam folder <b>{{ $folder->name }}</b>.</p>
                        <a href="{{ route('bank.create', ['folder_id' => $folder->id]) }}" class="inline-flex items-center gap-2 px-6 py-3.5 bg-elevate-dark text-white rounded-xl font-bold hover:bg-elevate-dark/90 transition shadow-lg shadow-elevate-dark/30 text-sm">
    ...
    {{-- Ambil semua folder untuk dropdown pindah folder --}}
    @php
        $allFolders = \App\Models\CbtBankFolder::orderBy('name', 'asc')->get();
    @endphp

    {{-- MODAL EDIT MAPEL SEDERHANA --}}
