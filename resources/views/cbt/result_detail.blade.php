                ...
                <div class="relative p-8 overflow-hidden">
                    ...
                    <div class="relative z-10 flex flex-col md:flex-row gap-8 items-center md:items-start justify-between">
                        ...
                        <div class="flex items-center gap-6">
                            {{-- Score Display --}}
                            <div class="relative w-32 h-32 flex items-center justify-center rounded-full border-8 {{ $examSession->total_score >= $exam->passing_grade ? 'border-emerald-100 bg-emerald-50' : 'border-rose-100 bg-rose-50' }} print:border-black print:bg-white transition-colors duration-500">
                                <div class="text-center">
                                    <span id="displayTotalScore" class="block text-4xl font-black {{ $examSession->total_score >= $exam->passing_grade ? 'text-emerald-600' : 'text-rose-600' }} print:text-black transition-colors duration-500">
            ...
            <div class="space-y-6">
                ...
                @forelse($answers as $index => $item)
                    ...
                    <div class="bg-white rounded-[2rem] border {{ $isCorrect ? 'border-emerald-100' : ($isSkipped ? 'border-slate-200' : ($qType == 'essay' ? 'border-indigo-100' : 'border-rose-100')) }} p-6 shadow-sm relative overflow-hidden print-break print:border-black print:rounded-none"
                        ...
                        {{-- Status Bar Kiri --}}
                        <div class="absolute left-0 top-0 bottom-0 w-1.5 {{ $isCorrect ? 'bg-emerald-400' : ($isSkipped ? 'bg-slate-300' : ($qType == 'essay' ? 'bg-indigo-400' : 'bg-rose-400')) }} print:border-r print:border-black"></div>

                        {{-- Header Soal --}}
                        <div class="flex justify-between items-start mb-4 pl-4">
                            <div class="flex items-center gap-3">
                                <span class="w-8 h-8 rounded-lg flex items-center justify-center font-black text-sm {{ $isCorrect ? 'bg-emerald-100 text-emerald-700' : ($isSkipped ? 'bg-slate-100 text-slate-500' : ($qType == 'essay' ? 'bg-indigo-100 text-indigo-700' : 'bg-rose-100 text-rose-700')) }} print:border print:border-black print:bg-white print:text-black">
                                    {{ $index + 1 }}
                                </span>
                            ...
                            @elseif($qType == 'essay')
                                <div class="space-y-4">
                                    ...
                                    <div class="p-5 rounded-2xl border bg-indigo-50/50 border-indigo-100">
                                        ...
                                        <p class="font-medium text-slate-800 whitespace-pre-wrap leading-relaxed text-sm">{{ $item->student_answer ?: '(Tidak dijawab)' }}</p>
                                    </div>
                                    ...
                                    <div class="space-y-2">
                                        @foreach($correctPairs as $left => $right)
                                            ...
                                            <div class="flex flex-col sm:flex-row gap-2 items-center bg-white p-2 rounded-lg border {{ $isMatch ? 'border-emerald-200' : 'border-rose-200' }}">
                                                <div class="flex-1 text-sm font-bold text-slate-700 text-center sm:text-left">{{ $left }}</div>
                                                <i class="ph-bold ph-arrow-right text-slate-300"></i>
                                                <div class="flex-1 text-sm text-center sm:text-right font-bold {{ $isMatch ? 'text-emerald-600' : 'text-rose-500 line-through' }}">
                                                    {{ $studentSelected }}
                                                </div>
                                                @if(!$isMatch)
                                                    <div class="text-xs bg-emerald-100 text-emerald-700 px-2 py-1 rounded">{{ $right }}</div>
                                                @else
