        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-4 md:gap-6">
                ...
                @foreach($menus as $menu)
                    <a href="{{ $menu['link'] }}" class="glass bg-white/60 p-6 rounded-2xl hover:bg-white hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group text-center md:text-left flex flex-col items-center md:items-start" data-aos="fade-up" data-aos-delay="{{ $loop->index * 50 }}">
                        <div class="w-12 h-12 rounded-xl bg-{{ $menu['color'] }}-50 text-{{ $menu['color'] }}-600 flex items-center justify-center text-2xl mb-4 group-hover:scale-110 group-hover:bg-{{ $menu['color'] }}-600 group-hover:text-white transition-all shadow-sm">
                            <i class="ph-duotone {{ $menu['icon'] }}"></i>
                        </div>
                        <h3 class="font-bold text-slate-800 group-hover:text-{{ $menu['color'] }}-600 transition-colors">{{ $menu['title'] }}</h3>
                        <p class="text-xs text-slate-500 mt-1 leading-relaxed hidden md:block">{{ $menu['desc'] }}</p>
                    </a>
                @endforeach
            </div>
        </div>
                ...
                <div class="absolute top-0 right-0 w-64 h-64 bg-white opacity-5 rounded-full -translate-y-1/2 translate-x-1/3 blur-3xl"></div>
                 ...
                 <div class="relative shrink-0" data-aos="zoom-in">
                    <div class="w-48 h-48 md:w-64 md:h-64 rounded-full border-4 border-slate-700 p-2 relative z-10 bg-slate-800">
                        <img src="{{ asset('images/kasek.png') }}" alt="Kepala Sekolah" class="w-full h-full object-cover rounded-full filter grayscale hover:grayscale-0 transition-all duration-500">
                    </div>
