                             ...
                             <li class="relative">
                                ...
                                <a href="{{ isset($item['route']) ? route($item['route']) : '#' }}" 
                                   ...
                                   :class="sidebarExpanded ? 'px-4 justify-start' : 'justify-center px-0 w-full'">
                                    ...
                                    <div class="absolute left-0 top-1.5 bottom-1.5 w-1.5 rounded-r-full transition-all duration-300 {{ $isActive ? 'bg-elevate-accent shadow-[0_0_12px_rgba(56,189,248,0.8)]' : 'bg-transparent' }}"></div>

                                    <!-- Icon -->
                                    <i class="ph-duotone {{ $item['icon'] ?? 'ph-circle' }} shrink-0 transition-all duration-300 relative z-10
                                              {{ $isActive ? 'text-elevate-accent drop-shadow-[0_0_10px_rgba(56,189,248,0.6)]' : 'text-slate-400 group-hover:text-elevate-accent group-hover:scale-110' }}"
                                       :class="sidebarExpanded ? 'text-[1.35rem] mr-0' : 'text-2xl mx-auto'"></i>
                                    
                                    <!-- Text -->
                                    <span x-show="sidebarExpanded" 
                                          class="text-sm font-bold tracking-wide whitespace-nowrap overflow-hidden relative z-10 transition-transform duration-300 {{ $isActive ? 'translate-x-0' : 'group-hover:translate-x-1' }}">
                                        {{ $item['name'] }}
                                    </span>
