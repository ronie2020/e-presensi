                                        ...
                                        <div class="flex items-center justify-between p-4 rounded-2xl border border-slate-100 bg-white hover:border-elevate-peach/40 hover:shadow-lg hover:shadow-elevate-peach/10 transition-all group relative overflow-hidden">
                                            <div class="flex items-center gap-4 relative z-10">
                                                <div class="text-center bg-elevate-peach-light/50 p-2.5 rounded-xl border border-elevate-peach/20 min-w-[60px] group-hover:bg-elevate-peach group-hover:border-elevate-peach transition-colors">
                                                    <span class="block text-[10px] text-elevate-peach-dark font-black uppercase group-hover:text-white">{{ $agenda->event_date->format('M') }}</span>
                                                    <span class="block text-2xl font-black text-elevate-dark leading-none mt-0.5 group-hover:text-white">{{ $agenda->event_date->format('d') }}</span>
                                                </div>
                                                <div class="min-w-0">
                                                    <p class="text-sm font-bold text-elevate-dark line-clamp-1 group-hover:text-elevate-primary transition-colors" title="{{ $agenda->title }}">{{ $agenda->title }}</p>
                ...
                <div class="lg:col-span-1">
                    <div class="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden flex flex-col lg:sticky lg:top-24 h-auto lg:h-[calc(100vh-6rem)]">
                        <div class="p-6 border-b border-slate-50 flex items-center justify-between bg-elevate-peach-light/30 z-10">
                            ...
                            <span class="bg-white text-[10px] font-black px-3 py-1.5 rounded-full text-elevate-primary border border-slate-200 uppercase tracking-wide shadow-sm">{{ $announcements->count() }} Post</span>
                        </div>
                                    ...
                                    <div class="p-6 hover:bg-slate-50/80 transition-all group relative">
                                        ...
                                        <div class="flex items-center justify-between mb-3">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <span class="block text-[11px] font-black text-elevate-dark uppercase tracking-wide">{{ $announce->author->name ?? 'Admin' }}</span>
                                                    <span class="block text-[10px] text-elevate-text/60 font-medium">{{ $announce->created_at->diffForHumans() }}</span>
                                                </div>
                                            </div>
                                        </div>
