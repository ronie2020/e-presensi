    ...
    <div class="py-6 sm:py-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-8">
                    ...
                    <div class="p-6 md:p-8 relative z-10">
                        <div class="flex items-center gap-4 mb-6">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-500">Tampilkan informasi di Landing Page</p>
                            </div>
                        </div>
                        <form action="{{ route('announcements.store') }}" method="POST" class="space-y-5">
                            ...
                            <div>
                                ...
                                <textarea name="content" rows="4" class="w-full rounded-xl border-gray-200 bg-gray-50 focus:bg-white focus:border-blue-500 focus:ring-blue-500 text-sm p-4" placeholder="Tulis detail pengumuman di sini..." required></textarea>
                            </div>
                            <div class="flex justify-end">
                                <button type="submit" class="py-2.5 px-5 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 transition-all shadow-md shadow-blue-200 text-sm flex items-center gap-2">
                ...
                <!-- CARD 2: AGENDA KEGIATAN (PURPLE) - FITUR BARU -->
                <div class="bg-white rounded-3xl shadow-sm border border-purple-100 overflow-hidden relative group hover:shadow-lg hover:shadow-purple-100/50 transition-all duration-300">
                    ...
                    <div class="p-6 md:p-8 relative z-10">
                        <div class="flex items-center gap-4 mb-6">
                            ...
                            <div>
                                <h3 class="text-lg font-black text-gray-800">Agenda Kegiatan</h3>
                                <p class="text-sm text-gray-500">Jadwalkan event mendatang di Landing Page</p>
                            </div>
                        </div>
                        ...
                        <form action="{{ route('agendas.store') }}" method="POST" class="grid grid-cols-1 md:grid-cols-12 gap-5">
                            ...
                            <div class="md:col-span-5">
                                <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1.5">Nama Kegiatan</label>
                                <input type="text" name="title" class="w-full rounded-xl border-gray-200 bg-gray-50 focus:bg-white focus:border-purple-500 focus:ring-purple-500 font-bold text-gray-800 py-3" placeholder="Contoh: Ujian Tengah Semester" required>
                            </div>
                            <div class="md:col-span-3">
                                <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1.5">Tanggal</label>
                                <input type="date" name="event_date" class="w-full rounded-xl border-gray-200 bg-gray-50 focus:bg-white focus:border-purple-500 focus:ring-purple-500 font-medium text-gray-800 py-3" required>
                            </div>
                            <div class="md:col-span-4">
                                <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1.5">Lokasi / Keterangan</label>
                                <div class="flex gap-2">
                                    <input type="text" name="location" class="w-full rounded-xl border-gray-200 bg-gray-50 focus:bg-white focus:border-purple-500 focus:ring-purple-500 text-sm py-3" placeholder="Contoh: Aula Sekolah">
                                    <button type="submit" class="p-3 bg-purple-600 text-white rounded-xl hover:bg-purple-700 transition shadow-md shadow-purple-200">
                        ...
                        <!-- LIST AGENDA AKTIF (MINI) -->
                        <div class="mt-6 pt-6 border-t border-purple-50">
                            <h4 class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">Agenda Aktif</h4>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                @forelse($agendas as $agenda)
                                    <div class="flex items-center justify-between p-3 rounded-xl border border-gray-100 bg-gray-50 hover:bg-white hover:border-purple-100 transition group">
                                        <div class="flex items-center gap-3">
                                            <div class="text-center bg-white p-1.5 rounded-lg border border-gray-100 shadow-sm min-w-[50px]">
                                                <span class="block text-[10px] text-gray-400 font-bold uppercase">{{ $agenda->event_date->format('M') }}</span>
                                                <span class="block text-lg font-bold text-purple-600 leading-none">{{ $agenda->event_date->format('d') }}</span>
                                            </div>
                                            <div>
                                                <p class="text-sm font-bold text-gray-700 line-clamp-1">{{ $agenda->title }}</p>
                                                <p class="text-[10px] text-gray-500">{{ $agenda->location ?? 'Sekolah' }}</p>
                                            </div>
                                        </div>
                                        <form action="{{ route('agendas.destroy', $agenda->id) }}" method="POST">
                                            @csrf @method('DELETE')
                                            <button type="submit" class="text-gray-300 hover:text-red-500 p-2 opacity-0 group-hover:opacity-100 transition" onclick="return confirm('Hapus agenda ini?')">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                            </button>
                                        </form>
                                    </div>
                                @empty
                                    <p class="text-sm text-gray-400 italic">Belum ada agenda mendatang.</p>
                                @endforelse
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="p-6 md:p-8 relative z-10">
                        ...
                        <form action="{{ route('announcements.send') }}" method="POST" class="space-y-5">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                <div>
                                    ...
                                    <select name="target_type" id="target_type" class="w-full rounded-xl border-gray-200 bg-gray-50 focus:bg-white focus:border-emerald-500 focus:ring-emerald-500 text-sm py-3 font-medium transition-colors" onchange="toggleTarget(this.value)">
                                        ...
                                        <option value="student">Siswa Spesifik (Personal)</option>
                            ...
                            <div class="flex justify-end mt-4">
                                <button type="submit" onclick="return confirm('Yakin ingin mengirim pesan ini?')" class="py-2.5 px-5 bg-emerald-600 text-white font-bold rounded-xl hover:bg-emerald-700 transition-all shadow-md shadow-emerald-200 text-sm flex items-center gap-2">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"></path></svg>
                                    Kirim Pesan
                                </button>
                            </div>
