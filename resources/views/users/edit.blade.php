            ...
            <div class="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden relative">
                ...
                <div class="bg-gradient-to-r from-blue-900 to-blue-800 p-8 text-white relative overflow-hidden">
                    ...
                    <div class="relative z-10 flex flex-col md:flex-row items-center gap-6">
                        <div class="shrink-0">
                            @if($user->photo_path)
                                <img src="{{ asset('storage/' . $user->photo_path) }}" class="w-20 h-20 rounded-2xl object-cover border-4 border-white/20 shadow-lg">
                            @else
                ...
                <div class="p-8">
                    <form action="{{ route('users.update', $user->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8" x-data="{ role: '{{ $user->role }}' }">
                        ...
                        <div x-show="role !== 'Admin'" x-transition>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <textarea name="bio" rows="2" class="w-full rounded-2xl border-slate-200 bg-slate-50 focus:bg-white focus:border-blue-600 focus:ring-blue-600 font-medium text-slate-700 py-3 px-4 transition-colors placeholder:font-normal" placeholder="Tulis motto atau deskripsi singkat...">{{ old('bio', $user->bio) }}</textarea>
                                </div>
                                <div class="md:col-span-2">
                        ...
                        <div class="flex justify-end gap-3 pt-6 border-t border-slate-100">
                            <a href="{{ route('users.index') }}" class="px-6 py-3.5 rounded-2xl bg-slate-100 text-slate-600 font-bold hover:bg-slate-200 transition-colors text-sm">Batal</a>
