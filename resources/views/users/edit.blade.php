<x-app-layout>
    <div class="py-8 sm:py-10 font-sans text-slate-800">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {{-- Tombol Kembali --}}
            <a href="{{ route('users.index') }}" class="inline-flex items-center gap-2 text-sm font-bold text-slate-500 hover:text-blue-600 mb-6 transition-colors group">
                ...
                <div class="bg-gradient-to-r from-blue-900 to-blue-800 p-8 text-white relative overflow-hidden">
                    ...
                    <div class="relative z-10 flex flex-col md:flex-row items-center gap-6">
                        ...
                        <div class="text-center md:text-left">
                            ...
                            {{-- Tampilkan Role sebagai Badge (Support Multi Role) --}}
                            @php
                                $roles = is_string($user->role) ? json_decode($user->role) : $user->role;
                                if (!is_array($roles)) $roles = [$user->role]; // Fallback jika single role
                            @endphp
                    ...
                    <form action="{{ route('users.update', $user->id) }}" method="POST" enctype="multipart/form-data" class="space-y-10">
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                {{-- AREA MULTI-SELECT ROLE (CHECKBOX) --}}
                                <div class="md:col-span-2">
                                    ...
                                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                        @php
                                            ...
                                            // Ambil role user saat ini (support string lama atau json array baru)
                                            $currentRoles = is_string($user->role) ? json_decode($user->role, true) : $user->role;
                                            if (!is_array($currentRoles)) {
                                                // Coba explode jika formatnya "RoleA,RoleB" (csv) atau string biasa
                                                $currentRoles = is_string($user->role) ? explode(',', $user->role) : [$user->role];
                                            }
                                        @endphp
                                        ...
                                        @foreach($availableRoles as $option)
                                            <label class="relative flex items-start gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all hover:bg-blue-50
                                                {{ in_array($option, $currentRoles ?? []) ? 'border-blue-500 bg-blue-50/50' : 'border-slate-100 bg-slate-50' }}">
                                                <input type="checkbox" name="role[]" value="{{ $option }}" 
                                                       ...
                                                       {{ in_array($option, $currentRoles ?? []) ? 'checked' : '' }}>
                                                <div>
                                                    <span class="block text-sm font-bold {{ in_array($option, $currentRoles ?? []) ? 'text-blue-700' : 'text-slate-600' }}">
                                                        {{ $option }}
                                                    </span>
                                                </div>
                                            </label>
                        ...
                        <div x-data="{ isTeacher: {{ in_array('Guru', $currentRoles ?? []) || in_array('Wali Kelas', $currentRoles ?? []) || in_array('Guru Mata Pelajaran', $currentRoles ?? []) ? 'true' : 'false' }} }">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <input type="text" name="nip" value="{{ old('nip', $user->nip) }}" class="w-full rounded-2xl border-slate-200 bg-slate-50 font-bold text-slate-700 py-3 px-4 focus:ring-emerald-500 focus:border-emerald-500">
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 ml-1">Pangkat/Golongan</label>
                                    <input type="text" name="pangkat" value="{{ old('pangkat', $user->pangkat) }}" class="w-full rounded-2xl border-slate-200 bg-slate-50 font-bold text-slate-700 py-3 px-4 focus:ring-emerald-500 focus:border-emerald-500">
                                </div>
                                <div class="md:col-span-2">
                                    <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 ml-1">Jabatan / Mapel</label>
                                    <input type="text" name="position" value="{{ old('position', $user->position) }}" class="w-full rounded-2xl border-slate-200 bg-slate-50 font-bold text-slate-700 py-3 px-4 focus:ring-emerald-500 focus:border-emerald-500">
                                </div>
                                <div class="md:col-span-2">
                                    <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 ml-1">Bio Singkat</label>
                                    <textarea name="bio" rows="2" class="w-full rounded-2xl border-slate-200 bg-slate-50 font-medium text-slate-700 py-3 px-4 focus:ring-emerald-500 focus:border-emerald-500">{{ old('bio', $user->bio) }}</textarea>
                                </div>
                                <div class="md:col-span-2">
                            ...
                            <div x-show="isTeacher">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        ...
                                        <input type="text" name="tempat_lahir" value="{{ old('tempat_lahir', $user->tempat_lahir) }}" class="w-full rounded-2xl border-slate-200 bg-slate-50 font-bold text-slate-700 py-3 px-4 focus:ring-amber-500 focus:border-amber-500">
                                    </div>
                                    <div>
                                        <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 ml-1">Tanggal Lahir</label>
                                        <input type="date" name="tanggal_lahir" value="{{ old('tanggal_lahir', $user->tanggal_lahir) }}" class="w-full rounded-2xl border-slate-200 bg-slate-50 font-bold text-slate-700 py-3 px-4 focus:ring-amber-500 focus:border-amber-500">
                                    </div>
                                    <div>
                                    ...
                                    <div>
                                        ...
                                        <input type="text" name="agama" value="{{ old('agama', $user->agama) }}" class="w-full rounded-2xl border-slate-200 bg-slate-50 font-bold text-slate-700 py-3 px-4 focus:ring-amber-500 focus:border-amber-500">
                                    </div>
                                    <div>
                                        <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 ml-1">Status Pernikahan</label>
                                        <input type="text" name="status_pernikahan" value="{{ old('status_pernikahan', $user->status_pernikahan) }}" placeholder="Belum Menikah / Menikah" class="w-full rounded-2xl border-slate-200 bg-slate-50 font-bold text-slate-700 py-3 px-4 focus:ring-amber-500 focus:border-amber-500">
                                    </div>
                                    <div class="md:col-span-2">
                                        <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 ml-1">Alamat Lengkap</label>
                                        <textarea name="alamat" rows="2" class="w-full rounded-2xl border-slate-200 bg-slate-50 font-medium text-slate-700 py-3 px-4 focus:ring-amber-500 focus:border-amber-500">{{ old('alamat', $user->alamat) }}</textarea>
                                    </div>
                                    <div class="md:col-span-2 bg-amber-50/50 p-4 rounded-2xl border border-amber-100">
                                        ...
                                        <input type="text" name="keahlian" value="{{ old('keahlian', $user->keahlian) }}" placeholder="Cth: Desain Grafis, Microsoft Office, Mengajar Matematika" class="w-full rounded-2xl border-slate-200 bg-white font-bold text-slate-700 py-3 px-4 focus:ring-amber-500 focus:border-amber-500 mb-4">
                                        
                                        <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 ml-1">Hobi (Pisahkan dengan koma)</label>
                                        <input type="text" name="hobi" value="{{ old('hobi', $user->hobi) }}" placeholder="Cth: Membaca Buku, Traveling, Menulis" class="w-full rounded-2xl border-slate-200 bg-white font-bold text-slate-700 py-3 px-4 focus:ring-amber-500 focus:border-amber-500">
                                    </div>
                                </div>
                            </div>
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50/50 p-6 rounded-[2rem] border border-slate-200">
                                <div>
                                    ...
                                    <input type="text" name="phone" value="{{ old('phone', $user->phone) }}" class="w-full rounded-2xl border-slate-200 bg-white font-bold text-slate-700 py-3 px-4">
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 ml-1">Instagram</label>
                                    <input type="text" name="instagram" value="{{ old('instagram', $user->instagram) }}" class="w-full rounded-2xl border-slate-200 bg-white font-bold text-slate-700 py-3 px-4">
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 ml-1">TikTok</label>
                                    <input type="text" name="tiktok" value="{{ old('tiktok', $user->tiktok) }}" class="w-full rounded-2xl border-slate-200 bg-white font-bold text-slate-700 py-3 px-4">
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 ml-1">Facebook</label>
                                    <input type="text" name="facebook" value="{{ old('facebook', $user->facebook) }}" class="w-full rounded-2xl border-slate-200 bg-white font-bold text-slate-700 py-3 px-4">
                                </div>
                            </div>
                        </div>
                        ...
                        <div x-data="{ showPassword: false }">
                         ...
                         <!-- 5. KELOLA PORTOFOLIO GURU (KHUSUS ADMIN) -->
                        @if(in_array('Guru', $currentRoles ?? []) || in_array('Wali Kelas', $currentRoles ?? []) || in_array('Kepala Sekolah', $currentRoles ?? []) || in_array('Guru Mata Pelajaran', $currentRoles ?? []))
                        <div class="mt-8 bg-gradient-to-br from-blue-50 to-indigo-50 p-6 rounded-[2rem] border border-blue-100 flex flex-col md:flex-row items-center justify-between gap-6 shadow-sm">
                            <div class="flex items-start gap-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-slate-500 mt-1 leading-relaxed">Kelola riwayat pelatihan, materi ajar, galeri prestasi, dan artikel milik <b class="text-blue-700">{{ $user->name }}</b>.</p>
                                </div>
                            </div>
                            <a href="{{ route('portfolio.index', ['user_id' => $user->id]) }}" class="shrink-0 w-full md:w-auto px-6 py-3.5 bg-blue-600 text-white font-bold rounded-2xl hover:bg-blue-700 shadow-lg shadow-blue-600/20 transition-all flex items-center justify-center gap-2 group">
