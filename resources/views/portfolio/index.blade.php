           ...
           {{-- Hero Section (Elevate Theme) --}}
            ...
            @if(session('success'))
                <script>
                    document.addEventListener('DOMContentLoaded', function() {
                        ...
                        }).fire({ icon: 'success', title: '{{ session('success') }}', iconColor: '#0d52a1' });
                    });
                </script>
            @endif
                                ...
                                <div class="text-left text-sm mt-2">
                                    ...
                                    <ul class="list-disc pl-5 space-y-1">
                                        @foreach($errors->all() as $error)
                                            <li class="text-rose-500 font-bold">{{ $error }}</li>
                                        @endforeach
                                    </ul>
                                </div>
                            `,
            ...
            <div class="bg-white rounded-[2rem] border border-slate-200 shadow-sm flex flex-col md:flex-row min-h-[600px] relative mt-8 overflow-hidden">
                ...
                <div class="p-6 md:p-8 flex-1 overflow-hidden bg-white">
                    ...
                    <form action="{{ route('portfolio.exp.store') }}" method="POST" enctype="multipart/form-data" x-data="{ isSubmitting: false }" @submit="isSubmitting = true" class="bg-elevate-surface p-6 rounded-2xl border border-slate-100 shadow-sm mb-8 grid grid-cols-1 md:grid-cols-4 gap-4">
                        @csrf
                        ...
                        <div>
                            ...
                            <input type="number" name="year" value="{{ old('year') }}" placeholder="2023" class="w-full rounded-2xl border-slate-200 bg-elevate-soft focus:bg-white focus:border-elevate-accent focus:ring-elevate-accent/30 font-bold text-elevate-dark py-3 px-4 transition-all" required>
                        </div>
                        <div class="md:col-span-3">
                            ...
                            <input type="text" name="title" value="{{ old('title') }}" placeholder="Cth: Diklat Guru Penggerak..." class="w-full rounded-2xl border-slate-200 bg-elevate-soft focus:bg-white focus:border-elevate-accent focus:ring-elevate-accent/30 font-bold text-elevate-dark py-3 px-4 transition-all" required>
                        </div>
                        <div class="md:col-span-2">
                            <label class="block text-xs font-bold text-elevate-primary uppercase tracking-wider mb-2 ml-1">Penyelenggara</label>
                            <input type="text" name="organizer" value="{{ old('organizer') }}" placeholder="Cth: Kemdikbud..." class="w-full rounded-2xl border-slate-200 bg-elevate-soft focus:bg-white focus:border-elevate-accent focus:ring-elevate-accent/30 font-bold text-elevate-dark py-3 px-4 transition-all">
                        </div>
                            ...
                            @forelse($experiences ?? [] as $exp)
                                <div class="flex flex-col sm:flex-row sm:items-center justify-between p-5 bg-white border border-slate-200 rounded-2xl hover:shadow-xl hover:shadow-elevate-accent/10 hover:border-elevate-accent/50 transition-all duration-300 group gap-3 sm:gap-0">
                                    <div class="flex items-start gap-4 pr-4">
                                        <div class="px-3 py-1.5 bg-elevate-soft text-elevate-primary border border-elevate-accent/20 rounded-lg text-xs font-black mt-1">{{ $exp->year }}</div>
                                        <div>
                                            <h4 class="font-bold text-elevate-dark text-lg group-hover:text-elevate-primary transition-colors">{{ $exp->title }}</h4>
                                            <p class="text-sm font-medium text-slate-500 mt-0.5">{{ $exp->organizer }}</p>
                                            
                                            {{-- TOMBOL LIHAT SERTIFIKAT --}}
                                            @if($exp->certificate_path)
                                                <a href="{{ asset('storage/' . $exp->certificate_path) }}" target="_blank" class="inline-flex items-center gap-1.5 mt-3 text-[10px] uppercase tracking-wider font-black text-elevate-primary bg-elevate-soft border border-elevate-accent/20 px-2.5 py-1.5 rounded-lg hover:bg-elevate-primary hover:text-white transition-colors">
                                    ...
                                    <div class="flex items-center gap-2 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity justify-end border-t border-slate-100 sm:border-0 pt-3 sm:pt-0">
                                        <button type="button" @click="openEditModal('exp', @js($exp), '{{ route('portfolio.exp.update', ['id' => $exp->id, 'user_id' => request('user_id')]) }}')" class="w-10 h-10 flex items-center justify-center text-elevate-peach bg-elevate-peach-light/30 border border-elevate-peach/30 sm:text-slate-400 sm:bg-white sm:border-slate-200 sm:hover:text-white sm:hover:bg-elevate-peach sm:hover:border-elevate-peach rounded-xl transition-all" title="Edit">
                    ...
                    <div x-show="activeTab === 'materi'" x-cloak x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0">
                        ...
                        <form action="{{ route('portfolio.mat.store') }}" method="POST" enctype="multipart/form-data" x-data="{ isSubmitting: false, fileName: '' }" @submit="isSubmitting = true" class="bg-elevate-surface p-6 rounded-2xl border border-slate-100 shadow-sm mb-8 grid grid-cols-1 md:grid-cols-2 gap-5">
                            @csrf
                            @if(request('user_id')) <input type="hidden" name="user_id" value="{{ request('user_id') }}"> @endif
                            <div class="md:col-span-2">
                                <label class="block text-xs font-bold text-elevate-primary uppercase tracking-wider mb-2 ml-1">Judul Materi</label>
                                <input type="text" name="title" value="{{ old('title') }}" class="w-full rounded-2xl border-slate-200 bg-elevate-soft focus:bg-white focus:border-elevate-accent focus:ring-elevate-accent/30 font-bold text-elevate-dark py-3 px-4 transition-all" required>
                            </div>
                            <div>
                                ...
                                <input type="text" name="type" value="{{ old('type') }}" class="w-full rounded-2xl border-slate-200 bg-elevate-soft focus:bg-white focus:border-elevate-accent focus:ring-elevate-accent/30 font-bold text-elevate-dark py-3 px-4 transition-all">
                            </div>
                            <div>
                                ...
                                <input type="url" name="file_url" value="{{ old('file_url') }}" placeholder="https://..." class="w-full rounded-2xl border-slate-200 bg-elevate-soft focus:bg-white focus:border-elevate-accent focus:ring-elevate-accent/30 font-bold text-elevate-dark py-3 px-4 transition-all">
                            </div>
                                ...
                                <div class="flex flex-col sm:flex-row items-start gap-4 p-5 bg-elevate-gradient-card border border-slate-200 rounded-2xl hover:shadow-xl hover:shadow-elevate-accent/10 hover:border-elevate-accent/50 transition-all relative group">
                                    <div class="flex gap-4 w-full">
                                        ...
                                        <div class="flex-1 sm:pr-14">
                                            <h4 class="font-bold text-base text-elevate-dark line-clamp-2 leading-tight group-hover:text-elevate-primary transition-colors">{{ $mat->title }}</h4>
                                            <p class="text-xs font-semibold text-slate-500 mt-1 mb-3">{{ $mat->type }}</p>
                                            @if($mat->file_url) 
                                                <a href="{{ $mat->file_url }}" target="_blank" class="inline-flex items-center gap-1.5 text-[10px] uppercase tracking-wider font-black text-elevate-primary bg-white border border-elevate-accent/20 shadow-sm px-3 py-1.5 rounded-lg hover:bg-elevate-primary hover:text-white transition-all"><i class="ph-bold ph-link"></i> Buka Link</a>
                                            @elseif($mat->file_path)
                                                <a href="{{ asset('storage/'.$mat->file_path) }}" target="_blank" class="inline-flex items-center gap-1.5 text-[10px] uppercase tracking-wider font-black text-elevate-primary bg-white border border-elevate-accent/20 shadow-sm px-3 py-1.5 rounded-lg hover:bg-elevate-primary hover:text-white transition-all"><i class="ph-bold ph-download-simple"></i> Download</a>
                                            @endif
                                        </div>
                                    </div>
                                    <div class="flex flex-row sm:flex-col gap-2 w-full sm:w-auto justify-end sm:absolute sm:top-5 sm:right-5 border-t sm:border-t-0 border-slate-100 pt-3 sm:pt-0 mt-2 sm:mt-0 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                                        <button type="button" @click="openEditModal('mat', @js($mat), '{{ route('portfolio.mat.update', ['id' => $mat->id, 'user_id' => request('user_id')]) }}')" class="flex-1 sm:flex-none h-10 sm:w-10 flex items-center justify-center text-elevate-peach bg-elevate-peach-light/30 border border-elevate-peach/30 sm:text-slate-400 sm:bg-white sm:border-slate-200 sm:hover:text-white sm:hover:bg-elevate-peach sm:hover:border-elevate-peach rounded-xl transition-all"><i class="ph-bold ph-pencil-simple text-lg"></i></button>
                    ...
                    <div x-show="activeTab === 'portofolio'" x-cloak x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0">
                        ...
                        <form action="{{ route('portfolio.port.store') }}" method="POST" enctype="multipart/form-data" 
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-5 items-end">
                                ...
                                <div>
                                    <label class="block text-xs font-bold text-elevate-primary uppercase tracking-wider mb-2 ml-1">Tahun</label>
                                    <input type="text" name="year" value="{{ old('year') }}" class="w-full rounded-2xl border-slate-200 bg-elevate-soft focus:bg-white focus:border-elevate-accent focus:ring-elevate-accent/30 font-bold text-elevate-dark py-3 px-4 transition-all">
                                </div>
                                ...
                                <div class="relative group rounded-2xl overflow-hidden border border-slate-200 shadow-sm hover:shadow-xl hover:shadow-elevate-accent/20 hover:border-elevate-accent/50 transition-all duration-300 bg-white">
                                    ...
                                    <div class="absolute inset-0 bg-gradient-to-t from-elevate-dark/90 via-elevate-dark/40 to-transparent opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-between p-5">
                                        <div class="flex justify-end gap-2">
                                            <button type="button" @click="openEditModal('port', @js($port), '{{ route('portfolio.port.update', ['id' => $port->id, 'user_id' => request('user_id')]) }}')" class="w-10 h-10 bg-white/20 backdrop-blur-md border border-white/30 text-white rounded-xl hover:bg-elevate-peach hover:border-elevate-peach transition-all flex items-center justify-center shadow-sm"><i class="ph-bold ph-pencil-simple text-lg"></i></button>
                                        ...
                                        <div>
                                            <span class="inline-block px-2.5 py-1 bg-elevate-primary text-white text-[10px] font-black rounded-lg mb-2 shadow-sm">{{ $port->year }}</span>
                    ...
                    <div x-show="activeTab === 'artikel'" x-cloak x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0">
                        ...
                        <form action="{{ route('portfolio.art.store') }}" method="POST" enctype="multipart/form-data" x-data="{ isSubmitting: false, imagePreview: null }" @submit="isSubmitting = true" class="bg-elevate-surface p-6 rounded-2xl border border-slate-100 shadow-sm mb-8 grid grid-cols-1 md:grid-cols-2 gap-5">
                            @csrf
                            @if(request('user_id')) <input type="hidden" name="user_id" value="{{ request('user_id') }}"> @endif
                            <div class="md:col-span-2">
                            ...
                            <div>
                                ...
                                <input type="text" name="category" value="{{ old('category') }}" placeholder="Pendidikan, Opini..." class="w-full rounded-2xl border-slate-200 bg-elevate-soft focus:bg-white focus:border-elevate-accent focus:ring-elevate-accent/30 font-bold text-elevate-dark py-3 px-4 transition-all">
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-elevate-primary uppercase tracking-wider mb-2 ml-1">Tanggal Publikasi</label>
                                <input type="date" name="published_at" value="{{ old('published_at') }}" class="w-full rounded-2xl border-slate-200 bg-elevate-soft focus:bg-white focus:border-elevate-accent focus:ring-elevate-accent/30 font-bold text-elevate-dark py-3 px-4 transition-all text-sm">
                            </div>
                            <div class="md:col-span-2">
                                <label class="block text-xs font-bold text-elevate-primary uppercase tracking-wider mb-2 ml-1">Ringkasan / Excerpt</label>
                                <textarea name="excerpt" rows="2" class="w-full rounded-2xl border-slate-200 bg-elevate-soft focus:bg-white focus:border-elevate-accent focus:ring-elevate-accent/30 font-medium text-elevate-dark py-3 px-4 transition-all">{{ old('excerpt') }}</textarea>
                            </div>
                            <div>
                                ...
                                <input type="url" name="url" value="{{ old('url') }}" placeholder="https://..." class="w-full rounded-2xl border-slate-200 bg-elevate-soft focus:bg-white focus:border-elevate-accent focus:ring-elevate-accent/30 font-bold text-elevate-dark py-3 px-4 transition-all">
                            </div>
                                ...
                                <div class="flex flex-col sm:flex-row items-stretch gap-5 p-5 bg-elevate-gradient-card border border-slate-200 rounded-2xl relative group hover:shadow-xl hover:shadow-elevate-accent/10 hover:border-elevate-accent/50 transition-all">
                                    <div class="flex gap-5 w-full">
                                        ...
                                        <div class="flex-1 py-1 sm:pr-16">
                                            <span class="inline-block px-2.5 py-1 bg-elevate-soft border border-elevate-accent/20 text-elevate-primary text-[10px] font-black rounded-lg uppercase tracking-wider mb-2 shadow-sm">{{ $art->category ?? 'Umum' }}</span>
                                            <h4 class="font-bold text-elevate-dark text-lg leading-tight group-hover:text-elevate-primary transition-colors">{{ $art->title }}</h4>
                                            <p class="text-sm font-medium text-slate-500 mt-2 line-clamp-2">{{ $art->excerpt }}</p>
                                            @if($art->url) 
                                                <a href="{{ $art->url }}" target="_blank" class="inline-flex items-center gap-1.5 text-[10px] uppercase tracking-wider font-black text-elevate-primary hover:text-elevate-dark mt-3 transition-colors"><i class="ph-bold ph-link"></i> Baca di Web Asli</a> 
                                            @endif
                                        </div>
                                    </div>
                                    <div class="flex flex-row sm:flex-col gap-2 justify-end sm:absolute sm:top-5 sm:right-5 border-t sm:border-t-0 border-slate-100 pt-3 sm:pt-0 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                                        <button type="button" @click="openEditModal('art', @js($art), '{{ route('portfolio.art.update', ['id' => $art->id, 'user_id' => request('user_id')]) }}')" class="flex-1 sm:flex-none h-10 sm:w-10 flex items-center justify-center text-elevate-peach bg-elevate-peach-light/30 border border-elevate-peach/30 sm:text-slate-400 sm:bg-white sm:border-slate-200 sm:hover:text-white sm:hover:bg-elevate-peach sm:hover:border-elevate-peach rounded-xl transition-all"><i class="ph-bold ph-pencil-simple text-lg"></i></button>
                    ...
                    <div x-show="activeTab === 'pendidikan'" x-cloak x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0">
                        ...
                        <form action="{{ route('portfolio.edu.store') }}" method="POST" x-data="{ isSubmitting: false }" @submit="isSubmitting = true" class="bg-elevate-surface p-6 rounded-2xl border border-slate-100 shadow-sm mb-8 grid grid-cols-1 md:grid-cols-4 gap-4">
                            @csrf
                            @if(request('user_id')) <input type="hidden" name="user_id" value="{{ request('user_id') }}"> @endif
                            <div class="md:col-span-2">
                                ...
                                <input type="text" name="institution" value="{{ old('institution') }}" placeholder="Cth: Universitas Pendidikan..." class="w-full rounded-2xl border-slate-200 bg-elevate-soft focus:bg-white focus:border-elevate-accent focus:ring-elevate-accent/30 font-bold text-elevate-dark py-3 px-4 transition-all" required>
                            </div>
                            <div class="md:col-span-2">
                                <label class="block text-xs font-bold text-elevate-primary uppercase tracking-wider mb-2 ml-1">Gelar / Jurusan</label>
                                <input type="text" name="degree" value="{{ old('degree') }}" placeholder="Cth: S1 Pendidikan Matematika" class="w-full rounded-2xl border-slate-200 bg-elevate-soft focus:bg-white focus:border-elevate-accent focus:ring-elevate-accent/30 font-bold text-elevate-dark py-3 px-4 transition-all">
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-elevate-primary uppercase tracking-wider mb-2 ml-1">Tahun Masuk</label>
                                <input type="number" name="start_year" value="{{ old('start_year') }}" placeholder="Cth: 2010" class="w-full rounded-2xl border-slate-200 bg-elevate-soft focus:bg-white focus:border-elevate-accent focus:ring-elevate-accent/30 font-bold text-elevate-dark py-3 px-4 transition-all">
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-elevate-primary uppercase tracking-wider mb-2 ml-1">Tahun Lulus</label>
                                <input type="number" name="end_year" value="{{ old('end_year') }}" placeholder="Cth: 2014" class="w-full rounded-2xl border-slate-200 bg-elevate-soft focus:bg-white focus:border-elevate-accent focus:ring-elevate-accent/30 font-bold text-elevate-dark py-3 px-4 transition-all">
                            </div>
                        ...
                        <div class="space-y-4">
                            @forelse($educations ?? [] as $edu)
                                <div class="flex flex-col sm:flex-row sm:items-center justify-between p-5 bg-white border border-slate-200 rounded-2xl hover:shadow-xl hover:shadow-elevate-accent/10 hover:border-elevate-accent/50 transition-all duration-300 group gap-3 sm:gap-0">
                                    <div class="flex items-start gap-4 pr-4">
                                        <div class="px-3 py-1.5 bg-elevate-soft text-elevate-primary border border-elevate-accent/20 rounded-lg text-xs font-black mt-1 shadow-sm">{{ $edu->start_year ?? '-' }} - {{ $edu->end_year ?? 'Skrg' }}</div>
                                        <div>
                                            <h4 class="font-bold text-lg text-elevate-dark group-hover:text-elevate-primary transition-colors">{{ $edu->institution }}</h4>
                                            <p class="text-sm font-semibold text-slate-500 mt-0.5">{{ $edu->degree }}</p>
                                        </div>
                                    </div>
                                    <div class="flex items-center gap-2 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity justify-end border-t border-slate-100 sm:border-0 pt-3 sm:pt-0">
                                        <button type="button" @click="openEditModal('edu', @js($edu), '{{ route('portfolio.edu.update', ['id' => $edu->id, 'user_id' => request('user_id')]) }}')" class="w-10 h-10 flex items-center justify-center text-elevate-peach bg-elevate-peach-light/30 border border-elevate-peach/30 sm:text-slate-400 sm:bg-white sm:border-slate-200 sm:hover:text-white sm:hover:bg-elevate-peach sm:hover:border-elevate-peach rounded-xl transition-all"><i class="ph-bold ph-pencil-simple text-lg"></i></button>
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="p-4 bg-elevate-soft/50 border border-slate-200 rounded-2xl">
                                    ...
                                    <template x-if="editData.certificate_path">
                                        <a :href="'{{ asset('storage') }}/' + editData.certificate_path" target="_blank" class="inline-flex items-center gap-1.5 mt-3 text-xs font-bold text-elevate-primary hover:text-elevate-dark transition-colors">
                                    ...
                                    <template x-if="editImagePreviews.length === 0 && editData.images_array && editData.images_array.length > 0">
                                        <div class="flex gap-3 overflow-x-auto custom-scrollbar pb-2 mt-2">
                                            <template x-for="img in editData.images_array">
                                                <img :src="'{{ asset('storage') }}/' + img" class="w-20 h-20 rounded-xl object-cover border border-slate-200 shrink-0 shadow-sm">
                                            </template>
                                        </div>
                                    </template>
                                </div>
