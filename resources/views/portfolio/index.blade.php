                ...
                <div class="p-6 md:p-8 flex-1 overflow-hidden">
                    ...
                    <form action="{{ route('portfolio.exp.store') }}" method="POST" enctype="multipart/form-data" x-data="{ isSubmitting: false }" @submit="isSubmitting = true" class="bg-slate-50/50 p-6 rounded-3xl border border-slate-100 mb-8 grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <div>
                            ...
                            <input type="number" name="year" value="{{ old('year') }}" placeholder="2023" class="w-full rounded-2xl border-slate-200 bg-white focus:border-cyan-500 focus:ring-cyan-500 font-bold text-slate-700" required>
                        </div>
                        <div class="md:col-span-3">
                            ...
                            <input type="text" name="title" value="{{ old('title') }}" placeholder="Cth: Diklat Guru Penggerak..." class="w-full rounded-2xl border-slate-200 bg-white focus:border-cyan-500 focus:ring-cyan-500 font-bold text-slate-700" required>
                        </div>
                        <div class="md:col-span-2">
                            <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 ml-1">Penyelenggara</label>
                            <input type="text" name="organizer" value="{{ old('organizer') }}" placeholder="Cth: Kemdikbud..." class="w-full rounded-2xl border-slate-200 bg-white focus:border-cyan-500 focus:ring-cyan-500 font-bold text-slate-700">
                        </div>
                            ...
                            @forelse($experiences ?? [] as $exp)
                                <div class="flex flex-col sm:flex-row sm:items-center justify-between p-4 border border-slate-200 rounded-2xl hover:bg-cyan-50/50 transition-colors group gap-3 sm:gap-0">
                                    <div class="flex items-start gap-4 pr-4">
                                        <div class="px-3 py-1 bg-cyan-100 text-cyan-700 rounded-lg text-xs font-black mt-1">{{ $exp->year }}</div>
                                        <div>
                                            ...
                                            @if($exp->certificate_path)
                                                <a href="{{ asset('storage/' . $exp->certificate_path) }}" target="_blank" class="inline-flex items-center gap-1 mt-2 text-[10px] uppercase tracking-wider font-black text-cyan-600 bg-cyan-50 px-2 py-1 rounded-lg hover:bg-cyan-100 transition-colors">
                    ...
                    <div x-show="activeTab === 'materi'" x-cloak x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0">
                        ...
                        <form action="{{ route('portfolio.mat.store') }}" method="POST" enctype="multipart/form-data" x-data="{ isSubmitting: false, fileName: '' }" @submit="isSubmitting = true" class="bg-slate-50/50 p-6 rounded-3xl border border-slate-100 mb-8 grid grid-cols-1 md:grid-cols-2 gap-5">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <input type="text" name="title" value="{{ old('title') }}" class="w-full rounded-2xl border-slate-200 bg-white focus:border-cyan-500 focus:ring-cyan-500 font-bold text-slate-700" required>
                            </div>
                            <div>
                                ...
                                <input type="text" name="type" value="{{ old('type') }}" class="w-full rounded-2xl border-slate-200 bg-white focus:border-cyan-500 focus:ring-cyan-500 font-bold text-slate-700">
                            </div>
                            <div>
                                ...
                                <input type="url" name="file_url" value="{{ old('file_url') }}" placeholder="https://..." class="w-full rounded-2xl border-slate-200 bg-white focus:border-cyan-500 focus:ring-cyan-500 font-bold text-slate-700">
                            </div>
                                ...
                                <div class="flex flex-col sm:flex-row items-start gap-4 p-5 border border-slate-200 rounded-3xl bg-white shadow-sm hover:shadow-md transition-shadow relative group">
                                    <div class="flex gap-4 w-full">
                                        ...
                                        <div class="flex-1 sm:pr-14">
                                            ...
                                            @if($mat->file_url) 
                                                <a href="{{ $mat->file_url }}" target="_blank" class="inline-flex items-center gap-1 text-[10px] uppercase tracking-wider font-black text-cyan-600 bg-cyan-50 px-2 py-1 rounded-lg hover:bg-cyan-100"><i class="ph-bold ph-link"></i> Buka Link</a>
                                            @elseif($mat->file_path)
                                                <a href="{{ asset('storage/'.$mat->file_path) }}" target="_blank" class="inline-flex items-center gap-1 text-[10px] uppercase tracking-wider font-black text-cyan-600 bg-cyan-50 px-2 py-1 rounded-lg hover:bg-cyan-100"><i class="ph-bold ph-download-simple"></i> Download</a>
                                            @endif
                                        </div>
                                    </div>
                    ...
                    <div x-show="activeTab === 'portofolio'" x-cloak x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0">
                        ...
                        <form action="{{ route('portfolio.port.store') }}" method="POST" enctype="multipart/form-data" 
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-5 items-end">
                                ...
                                <div>
                                    <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 ml-1">Tahun</label>
                                    <input type="text" name="year" value="{{ old('year') }}" class="w-full rounded-2xl border-slate-200 bg-white focus:border-cyan-500 focus:ring-cyan-500 font-bold text-slate-700">
                                </div>
                            ...
                            @forelse($portfolios ?? [] as $port)
                                ...
                                <div class="relative group rounded-3xl overflow-hidden border border-slate-200 shadow-sm">
                                    ...
                                    <div class="absolute inset-0 bg-gradient-to-t from-slate-900/90 via-slate-900/40 to-black/20 md:to-transparent opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity flex flex-col justify-between p-4">
                                        ...
                                        <div>
                                            <span class="inline-block px-2 py-1 bg-cyan-500 text-white text-[10px] font-black rounded-lg mb-1">{{ $port->year }}</span>
                    ...
                    <div x-show="activeTab === 'artikel'" x-cloak x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0">
                        ...
                        <form action="{{ route('portfolio.art.store') }}" method="POST" enctype="multipart/form-data" x-data="{ isSubmitting: false, imagePreview: null }" @submit="isSubmitting = true" class="bg-slate-50/50 p-6 rounded-3xl border border-slate-100 mb-8 grid grid-cols-1 md:grid-cols-2 gap-5">
                            ...
                            <div>
                                ...
                                <input type="text" name="category" value="{{ old('category') }}" placeholder="Pendidikan, Opini..." class="w-full rounded-2xl border-slate-200 bg-white focus:border-cyan-500 focus:ring-cyan-500 font-bold text-slate-700">
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 ml-1">Tanggal Publikasi</label>
                                <input type="date" name="published_at" value="{{ old('published_at') }}" class="w-full rounded-2xl border-slate-200 bg-white focus:border-cyan-500 focus:ring-cyan-500 font-bold text-slate-700 text-sm">
                            </div>
                            <div class="md:col-span-2">
                                <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 ml-1">Ringkasan / Excerpt</label>
                                <textarea name="excerpt" rows="2" class="w-full rounded-2xl border-slate-200 bg-white focus:border-cyan-500 focus:ring-cyan-500 font-medium text-slate-700">{{ old('excerpt') }}</textarea>
                            </div>
                            <div>
                                ...
                                <input type="url" name="url" value="{{ old('url') }}" placeholder="https://..." class="w-full rounded-2xl border-slate-200 bg-white focus:border-cyan-500 focus:ring-cyan-500 font-bold text-slate-700">
                            </div>
                                ...
                                <div class="flex flex-col sm:flex-row items-stretch gap-4 p-4 border border-slate-200 rounded-3xl bg-white relative group hover:shadow-md transition-all">
                                    <div class="flex gap-4">
                                        ...
                                        <div class="flex-1 py-1 sm:pr-16">
                                            <span class="inline-block px-2 py-1 bg-cyan-100 text-cyan-700 text-[10px] font-black rounded-lg uppercase tracking-wider mb-1">{{ $art->category ?? 'Umum' }}</span>
                                            ...
                                            @if($art->url) 
                                                <a href="{{ $art->url }}" target="_blank" class="inline-flex items-center gap-1 text-[10px] uppercase tracking-wider font-black text-cyan-600 hover:text-cyan-800 mt-2"><i class="ph-bold ph-link"></i> Baca di Web Asli</a> 
                                            @endif
                                        </div>
                                    </div>
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="p-3 bg-slate-50 border border-slate-200 rounded-xl">
                                    ...
                                    <template x-if="editData.certificate_path">
                                        <a :href="'{{ asset('storage') }}/' + editData.certificate_path" target="_blank" class="inline-flex items-center gap-1 mt-2 text-xs font-bold text-cyan-600 hover:text-cyan-800">
