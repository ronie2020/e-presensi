    ...
    <div class="py-8 sm:py-10 font-sans text-elevate-text bg-slate-50 min-h-screen">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-6 flex items-center gap-4">
                <a href="{{ route('achievements.index') }}" class="w-10 h-10 flex items-center justify-center rounded-xl bg-white border border-slate-200 text-slate-500 hover:text-elevate-primary hover:border-elevate-primary/30 hover:bg-elevate-accent/10 transition-all shadow-sm group">
                    ...
                    <form action="{{ route('achievements.update', $achievement->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6" 
                        ...
                        <div x-show="type !== 'Siswa'" x-transition style="display: none;">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" name="name_manual" value="{{ old('name_manual', $achievement->name_manual) }}" placeholder="Contoh: Tim Futsal Guru" class="w-full pl-11 rounded-2xl border-slate-200 bg-slate-50 focus:bg-white focus:border-elevate-primary focus:ring-elevate-primary font-bold text-elevate-dark py-3 transition-colors text-sm">
                            </div>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="text" name="title" value="{{ old('title', $achievement->title) }}" required placeholder="Juara 1 Lomba..." class="w-full pl-11 rounded-2xl border-slate-200 bg-slate-50 focus:bg-white focus:border-elevate-primary focus:ring-elevate-primary font-bold text-elevate-dark py-3 transition-colors text-sm">
                            </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="date" name="date" value="{{ old('date', \Carbon\Carbon::parse($achievement->date)->format('Y-m-d')) }}" class="w-full px-3 py-3 rounded-2xl border-slate-200 bg-slate-50 focus:bg-white focus:border-elevate-primary focus:ring-elevate-primary font-bold text-elevate-dark text-xs">
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="url" name="video_link" value="{{ old('video_link', $achievement->video_link) }}" placeholder="https://..." class="w-full pl-11 rounded-2xl border-slate-200 bg-slate-50 focus:bg-white focus:border-elevate-primary focus:ring-elevate-primary font-bold text-elevate-dark py-3 transition-colors text-xs">
                                </div>
                            </div>
                            <div>
                                ...
                                @if($achievement->certificate_path)
                                    <p class="text-[10px] text-elevate-primary mt-2 ml-1 font-bold"><a href="{{ asset('storage/'.$achievement->certificate_path) }}" target="_blank" class="hover:underline">Lihat Sertifikat Saat Ini</a></p>
                                @endif
                            </div>
                        </div>

                        <div class="pt-4 flex gap-3">
                            <a href="{{ route('achievements.index') }}" class="flex-1 py-3.5 px-4 bg-slate-100 text-slate-600 font-bold rounded-2xl hover:bg-slate-200 transition-all flex items-center justify-center gap-2 text-center">
                                Batal
                            </a>
