<x-app-layout>
    ...
    <div class="py-6 font-sans text-slate-800 relative min-h-screen bg-slate-50/50">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {{-- HEADER BARU: Dengan Jam Digital --}}
                ...
                <div class="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6">
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div class="text-center md:text-right bg-white/10 backdrop-blur-md px-5 py-2.5 rounded-2xl border border-white/10">
                            ...
                            <div class="font-bold text-base text-white">{{ Auth::user()->name ?? 'Administrator' }}</div>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="bg-white rounded-[2rem] shadow-xl border border-slate-100 overflow-hidden min-h-[600px] flex flex-col relative h-full">
                        <div class="p-6 border-b border-slate-100 bg-white/80 backdrop-blur-md flex justify-between items-center sticky top-0 z-20">
                            ...
                            <div id="activeCountBadge" class="flex flex-col items-end">
                                <span class="text-3xl font-black text-slate-800 leading-none">{{ $activePermits->count() }}</span>
                            ...
                            @if($activePermits->count() > 0)
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div class="permit-card group relative bg-white p-5 rounded-2xl border transition-all animate-enter flex flex-col justify-between
                                        {{ $permit->is_overdue ? 'border-rose-300 shadow-lg shadow-rose-100' : 'border-slate-200 hover:border-indigo-300 hover:shadow-md' }}">
                                        ...
                                        <div class="flex items-start gap-4 mb-4">
                                            <div class="w-12 h-12 rounded-2xl flex-shrink-0 flex items-center justify-center text-lg font-bold shadow-sm
                                                {{ $permit->is_overdue ? 'bg-rose-50 text-rose-600' : 'bg-indigo-50 text-indigo-600' }}">
                                                {{ substr($permit->student->name, 0, 1) }}
                                            </div>
                                            <div>
                                                <h4 class="font-bold text-slate-800 leading-snug line-clamp-2">{{ $permit->student->name }}</h4>
                                                <p class="text-xs text-slate-500 font-medium mt-1">
                                                    {{ $permit->student->schoolClass->name ?? 'Kelas -' }}
                                                </p>
                                                <p class="text-[10px] font-mono text-slate-400 mt-0.5">{{ $permit->student->student_id }}</p>
                                            </div>
                                        </div>
                                        ...
                                        <div class="space-y-3">
                                            <div class="bg-slate-50 rounded-xl p-3 border border-slate-100">
                                                <div class="flex justify-between items-center mb-1">
                                                    ...
                                                    <span class="text-[10px] font-bold text-slate-600 px-2 py-0.5 bg-white rounded border border-slate-200">{{ $permit->reason_category }}</span>
                                                </div>
                                                @if($permit->notes)
                                                <p class="text-xs text-slate-500 italic line-clamp-2 leading-relaxed">"{{ $permit->notes }}"</p>
                                                @else
                                            ...
                                            <div class="flex items-end justify-between pt-2 border-t border-slate-50">
                                                <div class="text-xs text-slate-400">
                                                    Keluar: <span class="font-mono font-bold text-slate-600">{{ $permit->time_out->format('H:i') }}</span>
                                                </div>
                                                <div class="live-timer text-right" data-start="{{ $permit->time_out }}">
                                                    <span class="text-2xl font-black font-mono leading-none {{ $permit->is_overdue ? 'text-rose-600' : 'text-slate-700' }}">
    ...
    {{-- MODAL (Sama seperti sebelumnya, hanya styling sedikit dirapikan) --}}
