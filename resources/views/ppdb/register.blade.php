...
<body class="bg-slate-950 text-slate-800 antialiased min-h-screen relative selection:bg-blue-500 selection:text-white">
    ...
    <nav class="sticky top-0 z-50 backdrop-blur-xl bg-slate-900/80 border-b border-white/10">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
            <a href="{{ url('/') }}" class="group flex items-center gap-3 text-slate-300 hover:text-white transition">
    ...
    <main class="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
            ...
            <div class="bg-slate-50 border-b border-slate-200 px-4 pt-4 flex gap-2 overflow-x-auto custom-scrollbar no-scrollbar">
                ...
                @foreach($tabs as $key => $item)
                    <button type="button" @click="tab = '{{ $key }}'" 
                        :class="{ 
                            'bg-white text-blue-700 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] border-slate-200 border-b-white': tab === '{{ $key }}', 
                            'text-slate-500 hover:text-blue-600 hover:bg-slate-100 border-transparent': tab !== '{{ $key }}' 
                        }" 
                        ...
                        <i class="ph-bold {{ $item['icon'] }} text-lg" :class="tab === '{{ $key }}' ? 'text-blue-600' : 'text-slate-400 group-hover:text-blue-500'"></i>
                        {{ $item['label'] }}
                        
                        {{-- Indikator Aktif --}}
                        <div x-show="tab === '{{ $key }}'" class="absolute top-0 left-0 w-full h-1 bg-blue-600 rounded-t-full"></div>
                    </button>
                @endforeach
            </div>

            <form id="ppdbForm" action="{{ route('ppdb.store') }}" method="POST" enctype="multipart/form-data" class="p-6 sm:p-10 min-h-[400px]">
                @csrf
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <label class="cursor-pointer group">
                            ...
                            <div class="p-5 rounded-2xl border-2 border-slate-100 bg-slate-50 peer-checked:border-blue-600 peer-checked:bg-blue-50/50 transition-all h-full hover:border-blue-300 flex flex-col items-center text-center relative overflow-hidden">
                                ...
                                <span class="font-bold text-slate-700 peer-checked:text-blue-800">{{ $opt['label'] }}</span>
                                <span class="text-xs text-slate-500 mt-1">{{ $opt['desc'] }}</span>
                ...
                <div x-show="tab === 'pribadi'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-x-4" x-transition:enter-end="opacity-100 translate-x-0" class="space-y-8" style="display: none;">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="nisn" value="{{ old('nisn') }}" class="form-input" placeholder="0012345678" required>
                            <i class="ph-bold ph-identification-card input-icon"></i>
                        </div>

                        <div class="form-group">
                            <label class="form-label">NIK (No. KK)</label>
                            <input type="number" name="nik" value="{{ old('nik') }}" class="form-input" placeholder="3207xxxxxxxxxxxx" required>
                            <i class="ph-bold ph-address-book input-icon"></i>
                        </div>

                        <div class="form-group">
                            <label class="form-label">Tempat Lahir</label>
                            <input type="text" name="birth_place" value="{{ old('birth_place') }}" class="form-input" placeholder="Kota/Kabupaten" required>
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="student_phone" value="{{ old('student_phone') }}" class="form-input" placeholder="08xxxxxxxxxx">
                            <i class="ph-bold ph-device-mobile input-icon"></i>
                        </div>

                        <div class="md:col-span-2 form-group">
                            <label class="form-label">Alamat Lengkap (Sesuai KK)</label>
                            <textarea name="address" rows="3" class="form-input resize-none h-auto pl-4 leading-relaxed" placeholder="Jalan/Dusun, RT/RW, Desa/Kelurahan, Kecamatan, Kode Pos" required>{{ old('address') }}</textarea>
                ...
                <div x-show="tab === 'sekolah'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-x-4" x-transition:enter-end="opacity-100 translate-x-0" class="space-y-8" style="display: none;">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="school_origin" value="{{ old('school_origin') }}" class="form-input" placeholder="SDN 1 Lakbok" required>
                            <i class="ph-bold ph-buildings input-icon"></i>
                        </div>
                        <div class="form-group">
                            <label class="form-label">NPSN Sekolah (Opsional)</label>
                            <input type="number" name="npsn_school_origin" value="{{ old('npsn_school_origin') }}" class="form-input" placeholder="Nomor Pokok Sekolah">
                            <i class="ph-bold ph-hash input-icon"></i>
                        </div>
                        <div class="md:col-span-2 form-group">
                            <label class="form-label">Rata-rata Nilai Rapor (Total)</label>
                            <div class="relative">
                                <input type="number" step="0.01" name="average_grade" value="{{ old('average_grade') }}" class="form-input pl-12 text-lg font-bold text-teal-600" placeholder="00.00" required>
                        ...
                        {{-- INPUT NILAI PER MAPEL (KHUSUS PRESTASI AKADEMIK) --}}
                        <div class="md:col-span-2" x-show="track === 'prestasi' && achievement_type === 'akademik'" x-transition>
                            <div class="bg-teal-50/50 p-6 rounded-2xl border border-teal-100 mt-2">
                                ...
                                <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                                    ...
                                    <div>
                                        <label class="text-xs font-bold text-slate-600 mb-1 block">{{ $mapel }}</label>
                                        <input type="number" step="0.01" name="grade_{{ Str::slug($mapel) }}" class="w-full rounded-lg border-teal-200 focus:border-teal-500 focus:ring-teal-500 text-sm font-bold text-center py-2" placeholder="00" :required="track === 'prestasi' && achievement_type === 'akademik'">
                                    </div>
                                    @endforeach
                                </div>
                ...
                <div x-show="tab === 'orangtua'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-x-4" x-transition:enter-end="opacity-100 translate-x-0" class="space-y-8" style="display: none;">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="parent_job" value="{{ old('parent_job') }}" class="form-input" placeholder="Wiraswasta/PNS/Buruh/dll">
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="form-group">
                            <label class="form-label flex justify-between items-center">
                                <span>{{ $file['label'] }} <span class="text-red-500">*</span></span>
                            </label>
                            ...
                            <div class="relative group">
                                ...
                                <div id="preview-{{ $file['name'] }}-default" class="file-box-default border-2 border-dashed border-slate-300 rounded-xl bg-slate-50 p-4 flex flex-col sm:flex-row items-center gap-4 transition-all group-hover:border-blue-400 group-hover:bg-blue-50/30 peer-focus:border-blue-500 peer-focus:ring-4 peer-focus:ring-blue-500/10">
                                    ...
                                    <div class="text-center sm:text-left flex-1">
                                        ...
                                        <p class="text-xs text-slate-400">{{ $file['desc'] }}</p>
                                    </div>
                                ...
                                <div id="preview-{{ $file['name'] }}-selected" class="hidden border-2 border-solid border-emerald-500 bg-emerald-50/30 rounded-xl p-4 flex items-center justify-between gap-4 transition-all">
                                    ...
                                    <button type="button" class="text-xs font-bold text-red-500 hover:text-red-700 underline shrink-0 z-30 relative" onclick="resetFile('{{ $file['name'] }}', event)">Ganti</button>
                                </div>
                            </div>
                        </div>
                        @endforeach

                        {{-- UPLOAD KHUSUS PRESTASI (SERTIFIKAT) --}}
        ...
        <div class="mt-8 text-center">
            <p class="text-xs text-slate-500 font-medium">&copy; {{ date('Y') }} Panitia PPDB SMPN 3 Lakbok. Hak Cipta Dilindungi.</p>
        </div>
    </main>
