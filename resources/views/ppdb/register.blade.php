...
@section('content')
{{-- STYLE KHUSUS (Light Theme & Clean Design) --}}
<style>
    /* Animations */
                ...
                <div class="flex items-center justify-between min-w-max px-6 gap-2">
                    ...
                    @foreach($tabs as $key => $item)
                        <button type="button" @click="tab = '{{ $key }}'" 
                            ...
                            :class="tab === '{{ $key }}' ? 'bg-white text-blue-600 shadow-sm ring-1 ring-slate-200' : 'text-slate-400 hover:bg-white hover:text-slate-600'">
                            
                            <div class="flex flex-col items-start text-left">
                                <span class="text-[10px] font-extrabold uppercase tracking-wider mb-0.5" 
                                      :class="tab === '{{ $key }}' ? 'text-blue-600' : 'text-slate-400'">Step {{ $item['step'] }}</span>
                            ...
                            <div x-show="tab === '{{ $key }}'" class="absolute bottom-0 left-6 right-6 h-[3px] bg-blue-600 rounded-t-full"></div>
                        </button>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                        ...
                        <label class="cursor-pointer group relative">
                            ...
                            <div class="selection-card h-full p-6 rounded-[2rem] border border-slate-200 bg-white {{ $opt['border'] }} {{ $opt['bg'] }} peer-checked:ring-4 {{ $opt['ring'] }} transition-all duration-300 flex flex-col items-center justify-center text-center relative overflow-hidden">
                                ...
                                <span class="font-extrabold text-lg text-slate-800 mb-1">{{ $opt['label'] }}</span>
                                ...
                                <div class="absolute top-4 right-4 opacity-0 peer-checked:opacity-100 transition-all duration-300 transform scale-50 peer-checked:scale-100">
                                    <div class="{{ str_replace('text-', 'bg-', $opt['color']) }} text-white rounded-full p-1 shadow-md">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="form-group col-span-1">
                            ...
                            <div class="relative group">
                                ...
                                {{-- Default State --}}
                                <div id="preview-{{ $file['name'] }}-default" class="h-28 border border-dashed border-slate-300 rounded-2xl bg-slate-50 hover:bg-cyan-50 hover:border-cyan-300 transition-all flex items-center justify-center gap-4 group-hover:shadow-sm">
                                ...
                                {{-- Selected State --}}
                                <div id="preview-{{ $file['name'] }}-selected" class="hidden h-28 border border-solid border-cyan-200 bg-cyan-50 rounded-2xl px-6 flex items-center justify-between">
