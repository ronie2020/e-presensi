...
<div class="min-h-screen w-full relative overflow-hidden bg-slate-50 font-sans pb-20 flex flex-col items-center justify-center py-10">
    
    {{-- Background --}}
    <div class="fixed inset-0 z-0 pointer-events-none overflow-hidden">
        <div class="absolute top-[-10%] left-[-5%] w-[600px] h-[600px] bg-purple-100/50 rounded-full blur-[100px] opacity-60 mix-blend-multiply"></div>
        <div class="absolute bottom-[-10%] right-[-10%] w-[500px] h-[500px] bg-blue-100/50 rounded-full blur-[100px] opacity-60 mix-blend-multiply"></div>
    ...
    <div class="relative z-10 w-full max-w-5xl px-4">
        ...
        <div class="text-center mb-10 animate-enter">
            <div class="inline-flex items-center gap-2 py-1.5 px-4 rounded-full bg-white border border-purple-100 mb-6 shadow-sm">
                <span class="w-2.5 h-2.5 rounded-full bg-purple-500 animate-pulse"></span>
                <span class="text-xs font-black text-purple-600 tracking-wider uppercase">Mode Guru / Kolektif</span>
            </div>
            <h1 class="text-3xl md:text-5xl font-black text-slate-900 tracking-tight mb-4">
                Pendaftaran <span class="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-purple-600">Massal</span>
            </h1>
        ...
        <div class="bg-white rounded-[2.5rem] overflow-hidden shadow-2xl shadow-slate-200/50 border border-slate-100 animate-enter" style="animation-delay: 100ms" x-data="{ step: 1 }">
            ...
            <div class="border-b border-slate-100 bg-slate-50/50 p-2">
                <div class="flex gap-2">
                    <button @click="step = 1" class="flex-1 py-3 px-4 rounded-xl flex items-center justify-center gap-3 transition-all"
                        :class="step === 1 ? 'bg-white text-blue-600 shadow-sm ring-1 ring-slate-200' : 'text-slate-400 hover:bg-white hover:text-slate-600'">
                        <span class="w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold" 
                              :class="step === 1 ? 'bg-blue-600 text-white' : 'bg-slate-200 text-slate-500'">1</span>
                    ...
                    <button @click="step = 2" class="flex-1 py-3 px-4 rounded-xl flex items-center justify-center gap-3 transition-all"
                        :class="step === 2 ? 'bg-white text-blue-600 shadow-sm ring-1 ring-slate-200' : 'text-slate-400 hover:bg-white hover:text-slate-600'">
                        <span class="w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold" 
                              :class="step === 2 ? 'bg-blue-600 text-white' : 'bg-slate-200 text-slate-500'">2</span>
                ...
                <div x-show="step === 1" class="animate-enter h-full flex flex-col justify-between">
                    <div class="max-w-3xl mx-auto w-full">
                        <div class="mb-8 text-center md:text-left">
                            <h3 class="text-2xl font-black text-slate-800 mb-2">Persiapan Data</h3>
                        ...
                        <div class="bg-slate-50 border border-slate-200 rounded-[2rem] p-6 flex flex-col md:flex-row items-center justify-between gap-6 hover:border-emerald-200 hover:shadow-lg hover:shadow-emerald-100/50 transition-all group">
                            <div class="flex items-center gap-5">
                                ...
                                <div class="text-center md:text-left">
                                    <h4 class="text-slate-800 font-black text-lg">Template_PPDB_2025.xlsx</h4>
                    ...
                    <div class="flex justify-end pt-8 mt-4 border-t border-slate-100">
                        <button type="button" @click="step = 2" class="px-8 py-4 bg-slate-900 text-white font-bold rounded-xl hover:bg-blue-600 transition-all flex items-center gap-2 shadow-lg shadow-slate-900/20 hover:shadow-blue-600/30">
                ...
                <div x-show="step === 2" class="animate-enter h-full flex flex-col justify-between" style="display: none;">
                    <div class="max-w-2xl mx-auto w-full">
                        <div class="text-center mb-8">
                            <h3 class="text-2xl font-black text-slate-800 mb-2">Import Data Siswa</h3>
                            ...
                            <div class="relative group">
                                <div class="file-drop-zone h-48 rounded-[2rem] flex flex-col items-center justify-center cursor-pointer relative overflow-hidden">
                                    ...
                                    <div class="text-center pointer-events-none transition-transform group-hover:scale-110 duration-300">
                                        <i class="ph-duotone ph-cloud-arrow-up text-5xl text-slate-400 mb-3 group-hover:text-blue-500 transition-colors"></i>
                                        <p class="text-slate-700 font-bold text-sm">Klik atau geser file ke sini</p>
                                        <p class="text-slate-400 text-xs mt-1 font-medium" id="file-name">Format .xlsx / .xls (Max 5MB)</p>
                                    </div>
                                </div>
                            </div>

                            <button type="submit" class="w-full py-4 bg-slate-900 hover:bg-blue-600 text-white font-bold rounded-xl shadow-lg shadow-slate-900/20 hover:shadow-blue-600/30 transform hover:-translate-y-1 transition-all flex items-center justify-center gap-3">
                    ...
                    <div class="flex justify-between pt-8 mt-4 border-t border-slate-100">
                        <button type="button" @click="step = 1" class="px-6 py-4 text-slate-400 font-bold hover:text-slate-700 transition-all flex items-center gap-2 rounded-xl hover:bg-slate-50">
                            <i class="ph-bold ph-arrow-left"></i> Kembali
                        </button>
                        <a href="{{ route('ppdb.create') }}" class="px-6 py-4 text-xs font-bold text-blue-600 hover:text-blue-700 transition-colors flex items-center gap-2 bg-blue-50 rounded-xl border border-blue-100 hover:border-blue-200">
        ...
        <div class="mt-8 text-center pt-6">
            <p class="text-xs text-slate-400 font-bold tracking-widest uppercase">&copy; {{ date('Y') }} Panitia PPDB SMPN 3 Lakbok</p>
        </div>
    </div>
</div>
@endsection