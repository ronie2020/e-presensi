       ...
       <div class="animate-enter relative rounded-[2rem] md:rounded-[3rem] bg-elevate-gradient-main p-8 md:p-12 text-elevate-dark shadow-xl shadow-elevate-accent/20 overflow-hidden group border border-white/40 no-print">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-5 animate-enter no-print" style="animation-delay: 100ms">
            <div class="bg-white fluent-card p-6 rounded-[1.5rem] flex items-center gap-4 group hover:border-elevate-primary">
                ...
                <div>
                    <div class="text-2xl font-black text-elevate-dark tracking-tight">{{ $stats['total_students'] }}</div>
             ...
             <div class="bg-white fluent-card p-6 rounded-[1.5rem] flex items-center gap-4 group hover:border-emerald-500">
                ...
                <div class="flex-1 w-full">
                    <div class="text-2xl font-black text-elevate-dark tracking-tight">{{ $stats['fasting_count'] }} <span class="text-xs text-emerald-600 font-bold">({{ $stats['percentage_fasting'] }}%)</span></div>
                    <div class="text-[9px] uppercase font-bold text-slate-400 tracking-widest mb-1.5">Berpuasa</div>
                    <div class="w-full bg-slate-100 rounded-full h-1.5 overflow-hidden">
                        <div class="bg-emerald-600 h-1.5 rounded-full transition-all duration-1000" style="width: {{ $stats['percentage_fasting'] }}%"></div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white fluent-card p-6 rounded-[1.5rem] flex items-center gap-4 group hover:border-amber-500">
                ...
                <div>
                    <div class="text-2xl font-black text-elevate-dark tracking-tight">{{ $stats['prayer_complete_count'] }}</div>
  ...
  @if($isFriday)
            <div class="bg-white fluent-card p-6 rounded-[1.5rem] flex items-center gap-4 group hover:border-elevate-primary relative overflow-hidden">
                ...
                <div class="relative z-10">
                    <div class="text-2xl font-black text-elevate-dark tracking-tight">{{ $stats['friday_log_count'] }}</div>
        ...
        <div class="animate-enter bg-white rounded-[2rem] fluent-card p-6 md:p-8 no-print" style="animation-delay: 150ms">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                ...
                <div class="shrink-0 flex items-center gap-3">
                    ...
                    <a href="{{ route('admin.ramadan.leaderboard') }}" class="px-5 py-2 rounded-xl bg-elevate-dark hover:bg-elevate-primary text-white text-xs font-bold uppercase tracking-wider transition-colors shadow-sm flex items-center gap-2">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                @foreach($topStudents as $index => $topStudent)
                    <div class="flex items-center gap-4 p-5 rounded-[1.5rem] bg-white border border-slate-100 hover:border-elevate-primary transition-all hover:-translate-y-1 hover:shadow-lg hover:shadow-elevate-primary/10 fluent-card">
                        ...
                        {{-- Nama & Kelas --}}
                        <div class="flex-1 min-w-0">
                            <h4 class="font-bold text-elevate-dark text-sm truncate" title="{{ $topStudent->name }}">
                                {{ $topStudent->name }}
                            </h4>
                    ...
                    <table class="w-full text-left print-table text-elevate-dark">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors group">
                                ...
                                <td class="px-6 py-4 text-center">
                                    @if($log)
                                        <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-bold border {{ $prayerCount == 5 ? 'bg-emerald-50 text-emerald-600 border-emerald-200' : 'bg-amber-50 text-amber-600 border-amber-200' }}">
                                            {{ $prayerCount }}/5
                                        </span>
                                    @else
                                        <span class="text-slate-300 font-bold">-</span>
                                    ...
                                    @if($log && is_array($log->prayers))
                                        <div class="flex justify-center gap-1.5">
                                            @foreach(['subuh','dzuhur','ashar','maghrib','isya'] as $p)
                                                <div title="{{ ucfirst($p) }}" class="w-2.5 h-2.5 rounded-full border border-white {{ ($log->prayers[$p] ?? false) ? 'bg-elevate-primary' : 'bg-slate-200' }}"></div>
                                            @endforeach
                                        </div>
                                    @else
                                        -
                                ...
                                <td class="px-6 py-4 text-center bg-elevate-soft/30">
                                    @if($log && $log->friday_khotib)
                                        <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-md bg-elevate-soft text-elevate-primary text-[10px] font-bold border border-elevate-accent/30 shadow-sm" title="{{ Str::limit($log->friday_summary, 50) }}">
                                ...
                                <td class="px-6 py-4">
                                    @if($log && $log->tadarus_surah)
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span class="text-xs font-bold text-elevate-dark">{{ Str::limit($log->tadarus_surah, 12) }} <span class="text-slate-400 font-normal ml-1">Ayat {{ $log->tadarus_ayah }}</span></span>
                                        </div>
                                    @else
                                ...
                                <td class="px-6 py-4 text-center bg-white border-l border-slate-100">
                                    @if($log)
                                        <button type="button" 
                                            ...
                                            {{ $log->teacher_verified_at ? 'bg-emerald-50 text-emerald-600 border-emerald-200 hover:bg-emerald-100' : 'bg-white border-slate-200 text-slate-500 hover:bg-elevate-primary hover:text-white hover:border-elevate-primary' }}">
                ...
                <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 px-2">
                    ...
                    <div class="flex items-center gap-2 bg-slate-50 p-1.5 rounded-xl border border-slate-200 shadow-sm">
                        <a href="{{ request()->fullUrlWithQuery(['status' => null, 'page' => 1]) }}" 
                           class="px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition-all {{ !request('status') ? 'bg-white text-elevate-primary shadow-sm border border-slate-200' : 'text-slate-500 hover:text-elevate-dark hover:bg-slate-100' }}">
                            <i class="ph-bold ph-list-dashes"></i> Semua
                        </a>
                        <a href="{{ request()->fullUrlWithQuery(['status' => 'pending', 'page' => 1]) }}" 
                           class="px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition-all flex items-center gap-2 {{ request('status') == 'pending' ? 'bg-amber-50 text-amber-600 shadow-sm border border-amber-200' : 'text-slate-500 hover:text-elevate-dark hover:bg-slate-100' }}">
                        ...
                        <div class="group bg-white rounded-[2rem] p-6 fluent-card hover:border-elevate-primary transition-all duration-300 relative overflow-hidden">
                            ...
                            <div class="relative z-10">
                                ...
                                <div class="flex items-center gap-4 mb-6">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <h4 class="font-bold text-elevate-dark text-sm truncate group-hover:text-elevate-primary transition-colors">{{ $log->student->name }}</h4>
                                ...
                                <div class="flex gap-2 mb-6">
                                    <div class="flex-1 py-3 px-1 rounded-xl bg-slate-50 border border-slate-100 text-center transition-colors group-hover:bg-white group-hover:shadow-sm">
                                        <i class="ph-fill ph-bowl-food text-xl {{ $log->is_fasting ? 'text-emerald-600' : 'text-rose-600' }} mb-1.5 block"></i>
                                        <span class="text-[9px] font-bold text-slate-500 uppercase tracking-wide">Puasa</span>
                                    </div>
                                    <div class="flex-1 py-3 px-1 rounded-xl bg-slate-50 border border-slate-100 text-center transition-colors group-hover:bg-white group-hover:shadow-sm">
                                        <div class="flex items-center justify-center gap-0.5 text-xl font-black {{ $prayerCount == 5 ? 'text-emerald-600' : 'text-amber-600' }} mb-1.5">
                                    ...
                                    <div class="flex-1 py-3 px-1 rounded-xl bg-slate-50 border border-slate-100 text-center transition-colors group-hover:bg-white group-hover:shadow-sm">
                                        <i class="ph-fill ph-book-open-text text-xl {{ $log->tadarus_surah ? 'text-elevate-primary' : 'text-slate-300' }} mb-1.5 block"></i>
                ...
                @else
                    ...
                    <div class="text-center py-24 bg-white fluent-card rounded-[2.5rem]">
                        ...
                        @if(request('status') || request('search'))
                            <a href="{{ route('admin.ramadan.reports') }}" class="inline-block mt-6 px-5 py-2.5 bg-slate-100 border border-slate-200 text-slate-600 rounded-lg text-xs font-bold uppercase tracking-wider hover:bg-slate-200 hover:text-elevate-dark transition-colors shadow-sm">Reset Filter</a>
                        @endif
                    </div>
                @endif
            </div>
                    ...
                    <form :action="formAction" method="POST" id="gradingForm" class="space-y-5 pt-6 border-t border-slate-200">
                        ...
                        <div class="flex flex-col md:flex-row gap-6">
                            <div class="w-full md:w-1/3">
                                ...
                                @error('teacher_score')
                                    <p class="text-rose-600 text-[10px] mt-1.5 font-bold">{{ $message }}</p>
                                @enderror
                            </div>
