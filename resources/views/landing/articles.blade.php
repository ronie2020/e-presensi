...
<section id="artikel" class="py-24 bg-slate-50 dark:bg-slate-950 relative overflow-hidden border-t border-slate-100 dark:border-slate-900 transition-colors duration-300">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                ...
                <article class="bg-white dark:bg-slate-900 rounded-[2.5rem] p-2 border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-xl hover:shadow-elevate-dark/5 dark:hover:shadow-elevate-accent/5 overflow-hidden group hover:-translate-y-2 hover:border-elevate-accent/30 dark:hover:border-elevate-accent/50 transition-all duration-300 flex flex-col" data-aos="fade-up" data-aos-delay="{{ $loop->iteration * 100 }}">
                    ...
                    <div class="relative h-56 sm:h-64 rounded-[2rem] bg-slate-200 dark:bg-slate-800 overflow-hidden shrink-0">
                        ...
                        <div class="absolute top-4 left-4 z-20">
                            <span class="px-3 py-1.5 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md text-elevate-primary dark:text-elevate-accent text-[9px] font-black uppercase tracking-widest rounded-xl shadow-sm border border-white/50 dark:border-slate-700/50 transition-colors">{{ $article->category ?? 'Pendidikan' }}</span>
                        </div>
                    </div>
                    ...
                    <div class="p-6 flex flex-col flex-1 relative z-10 bg-white dark:bg-slate-900 rounded-b-[2rem] transition-colors">
                        ...
                        <div class="flex items-center justify-between mb-4">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-[1rem] bg-slate-100 dark:bg-slate-800 overflow-hidden border border-slate-200 dark:border-slate-700 shrink-0 shadow-sm transition-colors">
                                    <img src="{{ optional($article->user)->photo_path ? asset('storage/' . $article->user->photo_path) : 'https://ui-avatars.com/api/?name='.urlencode(optional($article->user)->name ?? 'A').'&background=e5eff5&color=0d52a1' }}" alt="Penulis" loading="lazy" class="w-full h-full object-cover">
                                </div>
                                <span class="text-xs font-bold text-elevate-dark dark:text-slate-300 line-clamp-1 transition-colors">{{ optional($article->user)->name ?? 'Anonim' }}</span>
                            </div>
                        ...
                        <a href="{{ $article->url ?? '#' }}" target="{{ $article->url ? '_blank' : '_self' }}" class="block group-hover:text-elevate-primary dark:group-hover:text-elevate-accent transition-colors">
                            <h3 class="text-lg font-black text-elevate-dark dark:text-slate-100 mb-3 leading-snug line-clamp-2 transition-colors">{{ $article->title }}</h3>
                        </a>
                        
                        {{-- Excerpt --}}
                        <p class="text-sm text-slate-500 dark:text-slate-400 line-clamp-3 mb-6 flex-1 font-medium leading-relaxed transition-colors">{{ Str::limit(strip_tags($article->excerpt), 150) }}</p>
                        ...
                        <div class="mt-auto pt-4 border-t border-slate-50 dark:border-slate-800/50 flex items-center justify-between transition-colors">
                            ...
                            <a href="{{ $article->url ?? route('teachers.show', $article->user_id) }}" target="{{ $article->url ? '_blank' : '_self' }}" aria-label="Baca selengkapnya tentang {{ $article->title }}" class="inline-flex items-center justify-center w-10 h-10 rounded-xl bg-elevate-soft dark:bg-slate-800 text-elevate-primary dark:text-elevate-accent hover:bg-elevate-primary hover:text-white dark:hover:bg-elevate-primary dark:hover:text-white transition-all shadow-sm border border-elevate-accent/20 dark:border-slate-700 group/link">
                                @if($article->url)
        ...
        <div class="text-center mt-12" data-aos="fade-up">
            <a href="{{ route('articles.index') }}" class="inline-flex items-center justify-center px-8 py-3.5 text-xs font-bold text-elevate-primary dark:text-white bg-elevate-soft dark:bg-elevate-primary border border-elevate-accent/30 dark:border-transparent rounded-full hover:bg-white dark:hover:bg-elevate-dark hover:border-elevate-primary dark:hover:border-elevate-primary transition-all shadow-sm group">
