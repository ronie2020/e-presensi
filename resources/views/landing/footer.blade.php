            ...
            <div class="grid gap-6 md:grid-cols-3">
                @forelse ($announcements as $index => $item)
                    ...
                    <article class="bg-white dark:bg-slate-900/50 backdrop-blur-md rounded-2xl p-6 border border-slate-200 dark:border-slate-800 hover:border-cyan-400 dark:hover:border-cyan-500/50 shadow-sm hover:shadow-lg transition-all duration-300 hover:-translate-y-1 group h-full flex flex-col cursor-pointer" @click="openAnnouncementByIndex({{ $index }})" data-aos="fade-up" data-aos-delay="{{ $loop->index * 100 }}">
                        ...
                        <h3 class="text-lg font-bold text-slate-800 dark:text-white mb-3 line-clamp-2 group-hover:text-cyan-600 dark:group-hover:text-cyan-400 transition-colors">{{ $item->title }}</h3>
        ...
        <div class="bg-white dark:bg-slate-900/50 rounded-3xl p-8 mb-16 border border-slate-200 dark:border-slate-800 backdrop-blur-md shadow-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                @forelse($agendas as $agenda)
                    ...
                    <div class="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border-l-4 border-{{ $color }}-500 flex items-start gap-4 hover:bg-slate-100 dark:hover:bg-slate-800 transition cursor-default group h-full">
                        <div class="text-center bg-white dark:bg-slate-900 border border-slate-200 dark:border-none p-2 rounded-lg min-w-[60px] shadow-sm dark:shadow-lg transition-colors shrink-0">
                            ...
                            <span class="block text-xl font-bold text-slate-800 dark:text-white">{{ $agenda->event_date->format('d') }}</span>
                        </div>
                        <div class="flex-1 min-w-0 py-0.5">
                            ...
                            <p class="text-slate-500 dark:text-slate-400 text-xs flex items-center gap-1.5">
                                <i class="ph-fill ph-map-pin shrink-0 text-{{ $color }}-500 dark:text-{{ $color }}-400"></i> 
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-12 mb-16 border-t border-slate-200 dark:border-slate-800 pt-16">
            ...
            <div>
                ...
                <ul class="space-y-3 text-sm text-slate-500 dark:text-slate-400">
                    ...
                    <li><a href="{{ route('login') }}" class="hover:text-cyan-600 dark:hover:text-cyan-400 transition-colors flex items-center gap-2"><i class="ph-bold ph-caret-right text-xs"></i> Login Staff</a></li>
                </ul>
            </div>
            <div>
