        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 md:gap-6">
            @php
                ...
                $menus = [
                    ...
                    ['icon' => 'ph-chalkboard-simple', 'color' => 'teal', 'title' => 'E-Learning', 'desc' => 'LMS & Tugas Online', 'link' => route('student.login.learning')],
                    ['icon' => 'ph-monitor-play', 'color' => 'amber', 'title' => 'Ujian CBT', 'desc' => 'Portal Ujian Online', 'link' => route('student.login.cbt')],
                    ...
                    ['icon' => 'ph-chalkboard-teacher', 'color' => 'slate', 'title' => 'Login Guru', 'desc' => 'Dashboard Admin & Guru', 'link' => route('login')],
                    ['icon' => 'ph-presentation-chart', 'color' => 'cyan', 'title' => 'Jurnal Mengajar', 'desc' => 'Pembelajaran Guru', 'link' => route('teaching.index')],  
                    ['icon' => 'ph-broadcast', 'color' => 'sky', 'title' => 'Display Jadwal', 'desc' => 'Informasi Bel Real-time', 'link' => route('display.schedules.show')],
                ];
            @endphp
            ...
            @foreach($menus as $menu)
                ...
                <a href="{{ $menu['link'] }}" 
                   ...
                   data-aos-delay="{{ $loop->index * 50 }}">
                    ...
                    <div class="absolute inset-0 bg-gradient-to-br from-transparent to-{{ $menu['color'] }}-500/5 dark:to-{{ $menu['color'] }}-400/10 opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>

                    <!-- Icon Container -->
                    <div class="w-14 h-14 rounded-2xl bg-{{ $menu['color'] }}-50 dark:bg-{{ $menu['color'] }}-900/20 text-{{ $menu['color'] }}-600 dark:text-{{ $menu['color'] }}-400 flex items-center justify-center text-3xl mb-5 group-hover:scale-110 group-hover:-rotate-6 transition-all duration-500 shadow-sm border border-{{ $menu['color'] }}-100/50 dark:border-{{ $menu['color'] }}-800/50 relative z-10">
                    ...
                    <div class="mt-4 opacity-0 group-hover:opacity-100 group-hover:translate-x-1 transition-all duration-500 relative z-10 hidden md:block">
                        <i class="ph-bold ph-arrow-right text-{{ $menu['color'] }}-500"></i>
                    </div>
                </a>
            @endforeach
        </div>
