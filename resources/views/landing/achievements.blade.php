...
<div id="prestasi" class="py-24 bg-slate-50 dark:bg-slate-950 relative overflow-hidden border-t border-slate-100 dark:border-slate-900 transition-colors duration-300" x-data="{ activeFilter: 'Terbaru' }">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                ...
                <div class="group bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-xl hover:shadow-elevate-dark/5 dark:hover:shadow-elevate-accent/5 hover:-translate-y-2 transition-all duration-500 relative overflow-hidden h-full flex flex-col hover:border-elevate-accent/30 dark:hover:border-elevate-accent/50" 
                    ...
                    <div class="h-48 w-full bg-slate-100 dark:bg-slate-800 relative overflow-hidden group">
                        ...
                        <div class="absolute top-3 right-3">
                             <span class="px-3 py-1.5 rounded-full bg-white/90 dark:bg-slate-900/90 backdrop-blur border border-white/20 dark:border-slate-700/50 text-[9px] font-black uppercase text-elevate-primary dark:text-elevate-accent tracking-widest shadow-sm">{{ $prestasi->level ?? 'Sekolah' }}</span>
                        </div>
                    </div>
                    
                    <div class="p-6 flex-1 flex flex-col relative z-10">
                        <div class="text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-widest mb-2 flex items-center gap-1.5"><i class="ph-bold ph-calendar-blank text-elevate-accent"></i> {{ isset($prestasi->date) ? \Carbon\Carbon::parse($prestasi->date)->format('d M Y') : '-' }}</div>
                        <h4 class="text-lg font-black text-elevate-dark dark:text-slate-100 mb-2 leading-tight group-hover:text-elevate-primary dark:group-hover:text-elevate-accent transition-colors line-clamp-2">{{ $prestasi->title ?? 'Juara Lomba' }}</h4>
                        ...
                        <div class="mt-auto pt-4 border-t border-slate-50 dark:border-slate-800/50 flex items-center gap-3">
                            ...
                            <div>
                                <p class="text-xs font-black text-elevate-dark dark:text-slate-300 line-clamp-1">{{ $prestasi->achiever_name ?? 'Siswa' }}</p>
                                <p class="text-[10px] text-slate-400 dark:text-slate-500 uppercase font-bold tracking-widest">{{ $prestasi->type ?? 'Siswa' }}</p>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="mt-12 text-center" data-aos="fade-up">
             <a href="{{ route('public.achievements') }}" class="inline-flex items-center justify-center px-8 py-3.5 text-xs font-bold text-elevate-primary dark:text-white bg-elevate-soft dark:bg-elevate-primary border border-elevate-accent/30 dark:border-transparent rounded-full hover:bg-white dark:hover:bg-elevate-dark hover:border-elevate-primary dark:hover:border-elevate-primary transition-all shadow-sm group">
