...
<div id="jadwal-ujian" class="py-24 bg-slate-50 dark:bg-slate-900/50 relative overflow-hidden border-t border-slate-100 dark:border-slate-800 transition-colors duration-300">
    <!-- Ornamen Background -->
    <div class="absolute top-0 right-0 -mr-20 -mt-20 w-72 h-72 bg-elevate-accent/10 rounded-full blur-[80px] pointer-events-none"></div>
    <div class="absolute bottom-0 left-0 -ml-20 -mb-20 w-72 h-72 bg-elevate-primary/10 rounded-full blur-[80px] pointer-events-none"></div>

    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        
        <div class="text-center max-w-3xl mx-auto mb-12" data-aos="fade-up">
            <span class="text-elevate-primary dark:text-elevate-accent font-bold tracking-wider text-xs uppercase mb-3 inline-flex items-center gap-1.5 px-3 py-1 bg-elevate-accent/10 dark:bg-elevate-accent/20 rounded-full border border-elevate-accent/20">
                <i class="ph-fill ph-monitor-play text-sm"></i> Info Akademik
            </span>
            <h2 class="text-2xl md:text-3xl font-extrabold text-elevate-dark dark:text-white leading-tight mb-3">Jadwal Ujian Berbasis Komputer (CBT)</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @if(isset($publicExams) && $publicExams->isNotEmpty())
                @foreach($publicExams as $index => $exam)
                    ...
                    <!-- Card Ujian -->
                    <div class="bg-white dark:bg-slate-800 rounded-[1.25rem] p-5 shadow-sm hover:shadow-xl border border-slate-100 dark:border-slate-700 transition-all duration-300 hover:-translate-y-1 group flex flex-col h-full hover:border-elevate-accent/50" data-aos="fade-up" data-aos-delay="{{ $loop->index * 100 }}">
                        ...
                        <div class="flex justify-between items-center mb-4">
                            ...
                            <div class="w-8 h-8 rounded-lg bg-slate-50 dark:bg-slate-700 text-slate-400 flex items-center justify-center border border-slate-100 dark:border-slate-600 group-hover:bg-elevate-primary/10 dark:group-hover:bg-elevate-primary/30 group-hover:text-elevate-primary dark:group-hover:text-elevate-accent transition-colors">
                        ...
                        <div class="flex-1 mb-5">
                            ...
                            <div class="flex flex-wrap items-center gap-1.5 mb-2">
                                <div class="inline-flex items-center text-[9px] font-black uppercase tracking-wider text-elevate-primary dark:text-elevate-accent bg-elevate-primary/10 dark:bg-elevate-primary/30 px-2 py-1 rounded border border-elevate-primary/20 dark:border-elevate-primary/50">
                                    {{ $exam->subject_name ?? 'Mata Pelajaran' }}
                                </div>
                                <!-- BADGE KELAS -->
                                <div class="inline-flex items-center gap-1 text-[9px] font-black uppercase tracking-wider text-elevate-primary dark:text-elevate-accent bg-elevate-primary/10 dark:bg-elevate-primary/30 px-2 py-1 rounded border border-elevate-primary/20 dark:border-elevate-primary/50">
                                    <i class="ph-fill ph-users-three text-xs"></i> {{ $exam->class_level ?? 'Semua Kelas' }}
                                </div>
                            </div>

                            <h3 class="text-lg md:text-xl font-bold text-elevate-primary dark:text-elevate-accent mb-3 line-clamp-2 transition-colors">
                                {{ $exam->title }}
                            </h3>
                            ...
                            <div class="bg-slate-50/80 dark:bg-slate-700/50 rounded-xl p-3 border border-slate-100 dark:border-slate-700">
                                ...
                                <div class="flex items-center gap-2.5 text-xs mb-2">
                                    ...
                                    <div class="flex items-center gap-1.5">
                                        ...
                                        <span class="font-bold text-slate-700 dark:text-slate-300">: {{ $startTime->translatedFormat('d M Y, H:i') }}</span>
                                    </div>
                                </div>
                                <!-- Baris Akhir -->
                                <div class="flex items-center gap-2.5 text-xs">
                                    ...
                                    <div class="flex items-center gap-1.5">
                                        ...
                                        <span class="font-bold text-slate-700 dark:text-slate-300">: {{ $endTime ? $endTime->translatedFormat('d M Y, H:i') : 'Tidak dibatasi' }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <!-- CTA Button -->
                        <div class="mt-auto">
                            <a href="{{ route('student.login') }}" class="w-full inline-flex justify-center items-center gap-2 px-4 py-2.5 rounded-xl bg-elevate-dark dark:bg-elevate-primary text-white text-xs font-bold hover:bg-elevate-primary dark:hover:bg-elevate-accent dark:hover:text-elevate-dark transition-colors shadow-md hover:shadow-elevate-primary/25 group/btn">
