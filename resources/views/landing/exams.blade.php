        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @if(isset($publicExams) && $publicExams->isNotEmpty())
                @foreach($publicExams as $index => $exam)
                    ...
                    <!-- Card Ujian -->
                    <div class="bg-white dark:bg-slate-800 rounded-[1.25rem] p-5 shadow-sm hover:shadow-xl border border-slate-100 dark:border-slate-700 transition-all duration-300 hover:-translate-y-1 group flex flex-col h-full" data-aos="fade-up" data-aos-delay="{{ $loop->index * 100 }}">
                        ...
                        <!-- CTA Button -->
                        <div class="mt-auto">
                            <a href="{{ route('student.login') }}" class="w-full inline-flex justify-center items-center gap-2 px-4 py-2.5 rounded-xl bg-slate-900 dark:bg-cyan-600 text-white text-xs font-bold hover:bg-cyan-600 dark:hover:bg-cyan-500 transition-colors shadow-md hover:shadow-cyan-500/25 group/btn">
