...
<section id="katalog-lms" x-data="{ selectedClass: 'all' }" class="py-12 sm:py-18 relative overflow-hidden font-sans">
        ...
        @if(isset($featuredMaterials) && count($featuredMaterials) > 0)
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 sm:gap-6">
                @foreach($featuredMaterials as $mat)
                    ...
                    @endphp
                    <div x-show="selectedClass === 'all' || '{{ $classVal }}'.includes(selectedClass)" x-transition class="rounded-2xl bg-[#031d3d]/80 backdrop-blur-xl border border-white/15 shadow-lg hover:border-sky-400/50 hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between group relative overflow-hidden">
                        ...
                        <div class="relative h-40 w-full overflow-hidden shrink-0">
                            ...
                            @else
                                ...
                                <div class="w-full h-full bg-gradient-to-br {{ $gradientTheme['bg'] }} p-4 flex flex-col justify-between relative overflow-hidden group-hover:scale-105 transition-transform duration-500">
                                    <div class="absolute -right-5 -bottom-5 text-white/10 text-8xl pointer-events-none">
                            ...
                            <div class="absolute top-3 left-3 z-10 flex items-center gap-1.5 flex-wrap">
                                <span class="px-2.5 py-0.5 rounded-full bg-[#021124]/85 backdrop-blur-md text-sky-300 border border-white/20 text-[9px] font-black uppercase tracking-wider shadow-md">
                                    {{ $mat->subject->name ?? 'Mata Pelajaran' }}
                                </span>
                                @if($mat->schoolClass)
                                    <span class="px-2 py-0.5 rounded-full bg-black/60 backdrop-blur-md text-slate-300 text-[9px] font-bold border border-white/10">
                        ...
                        <div class="p-4 sm:p-5 flex-1 flex flex-col justify-between">
                            <div>
                                <!-- Title -->
                                <h3 class="text-sm sm:text-base font-bold text-white tracking-tight leading-snug mb-1.5 group-hover:text-sky-300 transition-colors line-clamp-2">
                                    {{ $mat->title }}
                                </h3>

                                <!-- Description Snippet -->
                                <p class="text-slate-300 text-[11px] sm:text-xs line-clamp-2 leading-relaxed mb-3 font-medium">
                                    {{ Str::limit(strip_tags($mat->content ?? $mat->resume ?? 'Modul pembelajaran digital interaktif disiapkan oleh guru pengampu.'), 90) }}
                                </p>
                            </div>

                            <div>
                                <!-- Teacher Info & Meta Stats -->
                                <div class="pt-2.5 border-t border-white/10 flex items-center justify-between gap-2.5 mb-3">
                                    <div class="flex items-center gap-2 min-w-0">
                                        <div class="w-7 h-7 rounded-lg bg-gradient-to-tr from-sky-500 to-blue-600 p-[1px] shrink-0">
                                            <div class="w-full h-full bg-[#021124] rounded-[7px] flex items-center justify-center font-black text-sky-400 text-[10px]">
                                    ...
                                    <div class="flex items-center gap-1.5 shrink-0 text-[10px] text-slate-400 font-bold">
