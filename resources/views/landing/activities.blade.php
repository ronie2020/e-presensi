                ...
                <div class="group bg-white dark:bg-slate-800 rounded-3xl overflow-hidden shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] hover:shadow-2xl hover:shadow-elevate-primary/10 dark:hover:shadow-elevate-accent/10 hover:-translate-y-2 transition-all duration-500 border border-slate-100 dark:border-slate-700 flex flex-col h-full" data-aos="fade-up" data-aos-delay="{{ $loop->index * 100 }}">
                    ...
                    <a href="{{ route('public.activities') }}" class="relative h-60 overflow-hidden block">
                        ...
                        <div class="absolute top-4 left-4 z-20">
                            <span class="bg-white/95 dark:bg-slate-900/90 backdrop-blur text-slate-800 dark:text-slate-200 text-xs font-black uppercase tracking-wider px-3 py-1.5 rounded-full shadow-sm">{{ $activity->created_at->format('d M Y') }}</span>
                        </div>
                    ...
                    <div class="p-6 flex-1 flex flex-col">
                        ...
                        @if($activity->video_url)
                            <div class="mt-4 pt-4 border-t border-slate-100 dark:border-slate-700">
                                <a href="{{ $activity->video_url }}" target="_blank" class="inline-flex items-center justify-between px-4 py-2 bg-rose-50 dark:bg-rose-500/10 rounded-xl text-sm font-bold text-rose-600 dark:text-rose-400 hover:bg-rose-600 hover:text-white dark:hover:bg-rose-500 dark:hover:text-white transition-colors w-full group/video">
