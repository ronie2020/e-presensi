...
<div id="kegiatan" class="py-24 bg-white relative overflow-hidden border-t border-slate-100">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div class="flex flex-col md:flex-row justify-between items-end mb-12 gap-6" data-aos="fade-up">
            ...
            <a href="{{ route('public.activities') }}" class="hidden md:inline-flex items-center text-sm font-bold text-cyan-600 hover:text-cyan-800 transition group/btn">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            @forelse($activities as $activity)
                ...
                <!-- Card -->
                <div class="group bg-white rounded-3xl overflow-hidden shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] hover:shadow-2xl hover:shadow-cyan-900/10 hover:-translate-y-2 transition-all duration-500 border border-slate-100 flex flex-col h-full" data-aos="fade-up" data-aos-delay="{{ $loop->index * 100 }}">
                    ...
                    <a href="{{ route('public.activities') }}" class="relative h-60 overflow-hidden block">
                        ...
                        <div class="absolute top-4 left-4 z-20">
                            <span class="bg-white/95 backdrop-blur text-slate-800 text-xs font-black uppercase tracking-wider px-3 py-1.5 rounded-full shadow-sm">{{ $activity->created_at->format('d M Y') }}</span>
                        </div>
                    ...
                    <div class="p-6 flex-1 flex flex-col">
                        <a href="{{ route('public.activities') }}">
                            <h4 class="text-xl font-black text-slate-800 mb-3 group-hover:text-cyan-600 transition-colors line-clamp-2 leading-snug">{{ $activity->title }}</h4>
                        </a>
                        <p class="text-sm text-slate-500 leading-relaxed line-clamp-3 mb-4 flex-1 font-medium">{{ $activity->description }}</p>

                        @if($activity->video_url)
                            <div class="mt-4 pt-4 border-t border-slate-100">
                                <a href="{{ $activity->video_url }}" target="_blank" class="inline-flex items-center justify-between px-4 py-2 bg-rose-50 rounded-xl text-sm font-bold text-rose-600 hover:bg-rose-600 hover:text-white transition-colors w-full group/video">
        ...
        <div class="mt-10 text-center md:hidden" data-aos="fade-up">
            <a href="{{ route('public.activities') }}" class="inline-flex items-center justify-center px-6 py-3 text-sm font-bold text-cyan-600 bg-cyan-50 border border-cyan-100 rounded-full hover:bg-cyan-100 hover:text-cyan-800 transition-all shadow-sm active:scale-95">
