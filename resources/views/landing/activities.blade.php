...
<div id="kegiatan" class="py-24 bg-white relative overflow-hidden border-t border-slate-100">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div class="flex flex-col md:flex-row justify-between items-end mb-12 gap-6" data-aos="fade-up">
            ...
            {{-- Tombol Lihat Semua (Hanya Desktop) --}}
            <a href="{{ route('public.activities') }}" class="hidden md:inline-flex items-center text-sm font-bold text-indigo-600 hover:text-indigo-800 transition group/btn">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            @forelse($activities as $activity)
                ...
                <!-- Card -->
                <div class="group bg-white rounded-3xl overflow-hidden shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] hover:shadow-2xl hover:shadow-indigo-900/10 hover:-translate-y-2 transition-all duration-500 border border-slate-100 flex flex-col h-full" data-aos="fade-up" data-aos-delay="{{ $loop->index * 100 }}">
        ...
        {{-- TOMBOL LIHAT SEMUA KHUSUS MOBILE (Akan muncul di bagian bawah pada layar HP) --}}
        <div class="mt-10 text-center md:hidden" data-aos="fade-up">
            <a href="{{ route('public.activities') }}" class="inline-flex items-center justify-center px-6 py-3 text-sm font-bold text-indigo-600 bg-indigo-50 border border-indigo-100 rounded-full hover:bg-indigo-100 hover:text-indigo-800 transition-all shadow-sm active:scale-95">
