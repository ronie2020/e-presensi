...
<div id="guru" class="py-24 bg-slate-50 dark:bg-slate-950 relative overflow-hidden border-t border-slate-100 dark:border-slate-900 transition-colors duration-300">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            @forelse($teachers as $teacher)
                ...
                <div class="group bg-white dark:bg-slate-900 rounded-[2.5rem] p-2 border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-xl hover:shadow-elevate-dark/5 dark:hover:shadow-elevate-accent/5 hover:-translate-y-2 transition-all duration-500 h-full flex flex-col hover:border-elevate-accent/30 dark:hover:border-elevate-accent/50 overflow-hidden" data-aos="fade-up" data-aos-delay="{{ $loop->index * 100 }}">
                    ...
                    <div class="p-5 text-center relative flex-1 flex flex-col items-center justify-center">
                        ...
                        <h3 class="text-lg font-black text-elevate-dark dark:text-white group-hover:text-elevate-primary dark:group-hover:text-elevate-accent transition-colors line-clamp-1 leading-tight">{{ $teacher->name }}</h3>
                        
                        @if(!empty($teacher->nip))
                            <p class="text-[10px] font-bold text-slate-400 dark:text-slate-500 mt-1 uppercase tracking-wider">NIP. {{ $teacher->nip }}</p>
                        @endif
                    </div>
                </div>
            @empty
        ...
        <div class="text-center mt-16" data-aos="fade-up">
            <a href="{{ route('teachers.index') }}" class="inline-flex items-center justify-center px-8 py-3.5 text-xs font-bold text-elevate-primary dark:text-white bg-elevate-soft dark:bg-elevate-primary border border-elevate-accent/30 dark:border-transparent rounded-full hover:bg-white dark:hover:bg-elevate-dark hover:border-elevate-primary transition-all shadow-sm group">
                Lihat Seluruh Staff 
