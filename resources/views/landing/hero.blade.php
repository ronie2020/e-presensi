...
<section id="home" class="relative pt-32 pb-40 md:pt-48 md:pb-40 lg:pt-56 lg:pb-48 overflow-hidden bg-gradient-to-br from-cyan-500 via-blue-600 to-blue-900 block w-full max-w-[100vw]">
    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10 w-full">
        <div class="grid lg:grid-cols-2 gap-10 lg:gap-8 items-center">
            ...
            <div class="max-w-2xl text-center lg:text-left mx-auto lg:mx-0 w-full min-w-0" 
                ...
                {{-- Badge Status --}}
                <div class="inline-flex flex-wrap items-center justify-center lg:justify-start gap-2 px-3 py-1.5 rounded-3xl bg-white/10 border border-white/20 text-cyan-100 text-[10px] sm:text-xs font-bold uppercase tracking-wider mb-6 backdrop-blur-md shadow-sm w-fit max-w-full mx-auto lg:mx-0">
                ...
                {{-- Sub-judul --}}
                <p class="text-xs sm:text-base md:text-lg text-blue-50 mb-6 sm:mb-8 leading-relaxed max-w-xl mx-auto lg:mx-0 font-medium opacity-90 break-words w-full">
                ...
                <div class="flex flex-col sm:flex-row gap-3 justify-center lg:justify-start mb-10 w-full mx-auto sm:px-0">
                    <a href="{{ route('ppdb.create') }}" class="group relative px-4 sm:px-6 py-3.5 rounded-full bg-white text-blue-700 font-bold text-xs sm:text-sm shadow-lg hover:bg-slate-50 hover:-translate-y-1 transition-all overflow-hidden w-full sm:w-auto flex justify-center items-center gap-2">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 w-full max-w-sm sm:max-w-md mx-auto lg:mx-0">
                    <div class="col-span-2 sm:col-span-1 bg-white/90 backdrop-blur-md border border-white/50 p-3 sm:p-5 rounded-2xl shadow-lg flex flex-row sm:flex-col items-center justify-between sm:justify-center min-w-0">
                        <div class="text-3xl md:text-4xl font-black text-emerald-500 mb-0.5">{{ $stats['hadir'] ?? 0 }}</div>
                        <div class="text-[10px] md:text-xs uppercase font-bold text-slate-500 tracking-wider text-right sm:text-center truncate w-full">Hadir Hari Ini</div>
                    </div>
                    
                    <div class="col-span-1 bg-white/90 backdrop-blur-md border border-white/50 p-3 sm:p-5 rounded-2xl shadow-lg flex flex-col items-center justify-center text-center min-w-0">
                        <div class="text-2xl md:text-3xl font-black text-amber-500 mb-1">{{ $stats['terlambat'] ?? 0 }}</div>
                        <div class="text-[9px] sm:text-[10px] md:text-[11px] uppercase font-bold text-slate-500 tracking-wider truncate w-full">Terlambat</div>
                    </div>
                    
                    <div class="col-span-1 bg-white/90 backdrop-blur-md border border-white/50 p-3 sm:p-5 rounded-2xl shadow-lg flex flex-col items-center justify-center text-center min-w-0">
                        <div class="text-2xl md:text-3xl font-black text-rose-500 mb-1">{{ $stats['tidak_hadir'] ?? 0 }}</div>
                        <div class="text-[9px] sm:text-[10px] md:text-[11px] uppercase font-bold text-slate-500 tracking-wider truncate w-full">Absen</div>
                    </div>
                </div>
            </div>
            ...
            <div class="relative w-full max-w-lg mx-auto lg:ml-auto mt-12 lg:mt-0 min-w-0"
                ...
                <div class="relative w-full">
                    ...
                    {{-- OVERLAPPING SHAPES --}}
                    <!-- Shape Atas (Original) -->
                    <div class="absolute -top-4 -left-2 sm:-top-6 sm:-left-6 w-24 h-32 sm:w-40 sm:h-48 bg-blue-900/80 backdrop-blur rounded-[2rem] shadow-2xl transform -rotate-6 z-0 pointer-events-none"></div>
                    <div class="absolute -top-6 -right-2 sm:-top-10 sm:-right-8 w-32 h-32 sm:w-56 sm:h-56 bg-orange-200 rounded-[2.5rem] shadow-xl transform rotate-6 z-0 pointer-events-none"></div>

                    <!-- Shape Bawah (Tambahan Baru - Efek Transparan & Buram) -->
                    <div class="absolute -bottom-4 -right-2 sm:-bottom-8 sm:-right-6 w-28 h-28 sm:w-48 sm:h-48 bg-cyan-300/40 backdrop-blur-md blur-[2px] border border-white/30 rounded-[2.5rem] shadow-lg transform -rotate-12 z-0 pointer-events-none"></div>
                    ...
                    <div class="relative z-10 w-full rounded-[2rem] overflow-hidden border-[4px] sm:border-[6px] border-white/80 dark:border-slate-700/80 shadow-2xl bg-white/95 dark:bg-slate-800/95 backdrop-blur-xl flex flex-col p-4 sm:p-6 lg:p-8 transform hover:scale-[1.02] transition duration-500 max-w-full">
                        ...
                        {{-- PERBAIKAN IDE USER: Widget Sistem Terintegrasi diubah menjadi Watermark Background --}}
