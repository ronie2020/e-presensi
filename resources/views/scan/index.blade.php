<x-app-layout>
    ...
    {{-- Memuat html5-qrcode dari CDN yang stabil --}}
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-5 lg:gap-8">
                            ...
                            <div class="text-center md:text-left">
                                ...
                                <div class="flex flex-wrap justify-center md:justify-start gap-2">
                                    @if(isset($scheduleConfig) && ($scheduleConfig['is_holiday'] ?? false))
                                        <span class="px-3 py-1 rounded-full bg-rose-500/20 text-rose-200 border border-rose-500/30 text-xs font-bold uppercase tracking-wider">Libur: {{ $scheduleConfig['description'] }}</span>
                                    @else
                ...
                <div class="lg:col-span-5 flex flex-col gap-5 order-2 lg:order-2">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        ...
                        <button id="btn-{{ $mode['id'] }}" data-type="{{ $mode['type'] }}" class="scan-type-btn bg-white p-3 rounded-2xl shadow-sm border border-slate-100 hover:shadow-md hover:border-{{ $mode['color'] }}-200 transition-all text-left group">
                            <div class="w-10 h-10 rounded-xl bg-{{ $mode['color'] }}-50 text-{{ $mode['color'] }}-600 flex items-center justify-center text-xl mb-2 group-hover:scale-110 transition-transform">
                                <i class="ph-bold ph-{{ $mode['icon'] }}"></i>
                            </div>
                            <h3 class="font-bold text-slate-700 text-xs md:text-sm">{{ $mode['label'] }}</h3>
                            <div class="flex items-center justify-between mt-1">
                                <p class="text-[10px] text-slate-400">{{ $mode['sub'] }}</p>
                                    ...
                                    <tbody id="scan-log" class="text-sm divide-y divide-slate-50">
                                        @if(isset($recentScans))
                                            @foreach($recentScans as $scan)
                                                {{-- Menggunakan blade component atau inline logic yang rapi --}}
                                                <tr class="log-entry group hover:bg-slate-50 transition-colors"
                                                    data-type-raw="{{ $scan['type_raw'] }}">
                                                    
                                                    <td class="px-6 py-4">
                                                        <div class="font-bold text-slate-800">{{ $scan['student_name'] }}</div>
                                                        <div class="text-[10px] text-slate-400 font-mono">{{ $scan['student_id'] }}</div>
                                                    </td>
                                                    
                                                    {{-- Kolom Harian --}}
                                                    <td class="col-harian px-4 py-4 text-center">
                                                        @if($scan['time_in']) <span class="font-mono font-bold text-slate-600 bg-slate-100 px-2 py-1 rounded text-xs">{{ $scan['time_in'] }}</span>
                                                        @else <span class="text-slate-300">-</span> @endif
                                                    </td>
                                                    <td class="col-harian px-4 py-4 text-center">
                                                        @if($scan['time_out']) <span class="font-mono font-bold text-slate-600 bg-slate-100 px-2 py-1 rounded text-xs">{{ $scan['time_out'] }}</span>
                                                        @else <span class="text-slate-300">-</span> @endif
                                                    </td>

                                                    {{-- Kolom Waktu Generic (Makan/Sholat) --}}
                                                    <td class="col-waktu hidden-col px-4 py-4 text-center">
                                                         <span class="font-mono font-bold text-slate-600 bg-slate-100 px-2 py-1 rounded text-xs">{{ $scan['time_in'] ?? now()->format('H:i') }}</span>
                                                    </td>
                                                    ...
                                                    <td class="px-6 py-4 text-right">
                                                        <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-wide 
                                                            {{ Str::contains($scan['status'], 'Terlambat') ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700' }}">
                                                            {{ $scan['status'] }}
                                                        </span>
                                                    </td>
                                                </tr>
