        ...
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            <div class="relative rounded-[2rem] bg-gradient-to-r from-elevate-accent via-elevate-peach-light to-elevate-peach p-8 md:p-10 mb-8 text-elevate-dark shadow-xl shadow-elevate-accent/20 overflow-hidden border border-white/60 group">
                ...
                <div class="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    ...
                    <a href="{{ route('lms.materials.index') }}" class="w-full md:w-auto flex items-center justify-center gap-2 px-6 py-3.5 bg-white/60 hover:bg-white rounded-xl text-sm font-bold backdrop-blur-md transition-colors text-elevate-dark border border-white/60 btn-cancel-confirm shadow-sm active:scale-95 shrink-0">
            ...
            <div class="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/40 border border-slate-100 overflow-hidden animate-enter" style="animation-delay: 100ms">
                <form action="{{ route('lms.materials.store') }}" method="POST" enctype="multipart/form-data" 
                      id="createForm"
                      x-data="{ 
                          attachments: [ {id: Date.now(), type: 'file', link: '', name: ''} ],
                          ...
                          fetchTopics() {
                              ...
                              // Mengambil data Bab berdasarkan mapel via API
            ...
            <div class="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/40 border border-slate-100 overflow-hidden">
                ...
                <form action="{{ route('lms.materials.store') }}" method="POST" enctype="multipart/form-data" id="createAssignmentForm" 
                    ...
                    <div class="p-6 md:p-10 space-y-8">
                        ...
                        <!-- BAGIAN 4: LAMPIRAN FILE/VIDEO (DYNAMIC) -->
                        <div class="bg-elevate-soft/30 rounded-[2rem] border border-slate-100 p-6 md:p-8">
                            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                                <div>
                                    <label class="block text-sm font-black text-elevate-dark flex items-center gap-2">
                                    ...
                                    <p class="text-[11px] font-bold text-slate-400 mt-1">Upload dokumen PDF/Word, atau tautkan link YouTube pembelajaran.</p>
                                </div>
                                <button type="button" @click="attachments.push({id: Date.now(), type: 'file', link: '', name: ''})" 
                                        class="w-full sm:w-auto text-xs bg-white border border-slate-200 text-elevate-primary px-5 py-3 sm:py-2.5 rounded-xl font-bold hover:bg-elevate-soft hover:border-elevate-accent transition-colors shadow-sm flex items-center justify-center gap-2 active:scale-95">
                            ...
                            <div class="space-y-4">
                                <template x-for="(att, index) in attachments" :key="att.id">
                                    <div class="flex flex-col md:flex-row items-start gap-4 p-5 bg-white rounded-2xl border border-slate-200 relative group animate-enter hover:border-elevate-accent/50 transition-colors shadow-sm">
                                        ...
                                        <button type="button" @click="attachments = attachments.filter(i => i.id !== att.id)" x-show="attachments.length > 1" class="absolute top-3 right-3 md:static md:mt-1 w-10 h-10 flex items-center justify-center rounded-xl bg-white text-[#D13438] hover:bg-[#FDE7E9] border border-[#F4C3C9] transition-colors shadow-sm" title="Hapus Baris">
                                            <i class="ph-bold ph-trash text-lg"></i>
                                        </button>
                                    </div>
                                </template>
            ...
            if(form) {
                form.addEventListener('submit', function(e) {
                    ...
                    Swal.fire({
                        ...
                        html: 'Mohon tunggu sebentar, file materi sedang diunggah.',
