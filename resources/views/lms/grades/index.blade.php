<x-app-layout>
                            ...
                            <table class="w-full text-sm text-left border-collapse min-w-[800px]">
                                <thead class="bg-elevate-soft/50 text-elevate-primary uppercase font-black text-[10px] tracking-wider border-b border-slate-100">
                                    <tr>
                                        ...
                                        @foreach($assignments ?? [] as $task)
                                            <th class="px-5 py-5 text-center min-w-[140px] group relative border-r border-slate-100 hover:bg-white transition-colors">
                                                <div class="flex flex-col items-center gap-1.5">
                                                    <span class="block truncate w-32 cursor-help font-bold text-elevate-dark" title="{{ $task->title }}">
                                                        {{ Str::limit($task->title, 15) }}
                                                    </span>
                                                    <div class="flex items-center gap-1.5">
                                                        <span class="text-[9px] text-slate-500 font-bold bg-white border border-slate-100 px-2 py-0.5 rounded shadow-sm">{{ $task->created_at->format('d/m') }}</span>
                                                        <span class="text-[9px] px-2 py-0.5 rounded font-black uppercase tracking-widest shadow-sm {{ $task->assignment_type == 'quiz' ? 'bg-purple-100 text-purple-700 border border-purple-200' : 'bg-elevate-primary/10 text-elevate-primary border border-elevate-primary/20' }}">
                                        ...
                                        <th class="px-5 py-5 text-center {{ ($selectedPeriod ?? 'semester') == 'semester' ? 'bg-elevate-primary text-white border-elevate-primary' : 'bg-slate-100 text-elevate-dark border-slate-200' }} min-w-[110px]">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // 4. Session Messages
            @if(session('success'))
                Swal.fire({ icon: 'success', title: 'Berhasil', text: "{{ session('success') }}", customClass: { popup: 'rounded-[2rem] font-sans border-0 shadow-2xl' } });
            @endif
            @if(session('error'))
                Swal.fire({ icon: 'error', title: 'Gagal', text: "{{ session('error') }}", customClass: { popup: 'rounded-[2rem] font-sans border-0 shadow-2xl' } });
            @endif
        });
    </script>
    @endpush
