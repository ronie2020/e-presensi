                            ...
                            @foreach($allStudents as $student)
                                ...
                                <tr class="group hover:bg-slate-50/80 transition-colors student-row" data-class="{{ $student->schoolClass->name ?? '-' }}">
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        @if($submission)
                                            <button type="button" onclick="document.getElementById('form-grade-{{$submission->id}}').submit()" class="w-9 h-9 rounded-xl bg-blue-600 text-white flex items-center justify-center shadow-md hover:bg-blue-700 transition" title="Simpan Individu">
