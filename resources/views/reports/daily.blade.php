                ...
                <div class="animate-enter bg-gradient-to-br from-[#25D0FF] via-[#5295FF] to-[#FFC9B9] rounded-xl p-6 lg:p-8 text-[#2A3B52] fluent-card relative overflow-hidden flex flex-col justify-between min-h-[180px] lg:min-h-[200px] border border-white/40 group">
                    ...
                    <div class="relative z-10 mt-6">
                        <div class="inline-flex items-center gap-2 bg-white/40 backdrop-blur-md border border-white/50 px-4 py-2 rounded-xl text-sm font-bold shadow-sm">
                            ...
                            <span class="text-[#2A3B52]">{{ $selectedDate_db->translatedFormat('d F Y') }}</span>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="relative z-10">
                        ...
                        <form action="{{ route('reports.daily') }}" method="GET" class="flex flex-col md:flex-row gap-3 w-full" @submit.prevent="submitFilter">
                            ...
                            <div class="flex gap-2 w-full md:w-auto">
                                ...
                                <a href="{{ route('reports.printDaily', request()->all()) }}" target="_blank" class="flex-1 md:flex-none bg-white border border-slate-200 text-[#2A3B52] hover:bg-slate-50 px-5 rounded-xl h-11 font-bold text-sm flex items-center justify-center gap-2 transition-colors active:scale-95">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 lg:gap-6 mb-8">
                <div class="animate-enter bg-white p-5 lg:p-6 rounded-xl fluent-card flex items-center justify-between group hover:-translate-y-1 transition-transform" style="animation-delay: 200ms">
                    <div class="min-w-0">
                        ...
                        <h3 class="text-3xl lg:text-4xl font-black text-[#2A3B52] truncate">{{ $hadirCount }}</h3>
                        @if($terlambatCount > 0)
                ...
                <div class="animate-enter bg-white p-5 lg:p-6 rounded-xl fluent-card flex items-center justify-between group hover:-translate-y-1 transition-transform" style="animation-delay: 300ms">
                     <div class="min-w-0">
                        ...
                        <h3 class="text-3xl lg:text-4xl font-black text-[#2A3B52] truncate">{{ $sakitCount + $izinCount + $alfaCount }}</h3>
                    </div>
                ...
                <div class="animate-enter bg-white p-5 lg:p-6 rounded-xl fluent-card flex items-center justify-between group hover:-translate-y-1 transition-transform" style="animation-delay: 400ms">
                     <div class="min-w-0">
                        ...
                        <h3 class="text-3xl lg:text-4xl font-black text-[#2A3B52] truncate">{{ $belumAbsenList->total() }}</h3>
                    </div>
            ...
            <div class="animate-enter bg-white rounded-xl fluent-card overflow-hidden min-h-[500px]" style="animation-delay: 500ms">
                ...
                <div class="flex flex-wrap md:flex-nowrap border-b border-slate-100 bg-slate-50/80 p-2 gap-2 sticky top-0 z-20 no-print backdrop-blur-sm">
                    ...
                    <button @click="activeTab = 'belum'" :class="activeTab === 'belum' ? 'bg-white text-[#2A3B52] fluent-card ring-1 ring-slate-200/50' : 'text-slate-500 hover:bg-white/60 hover:text-slate-700'" class="flex-1 md:flex-none py-2.5 px-4 md:px-6 rounded-xl text-xs md:text-sm font-bold whitespace-nowrap transition-all">
                        Belum <span class="hidden sm:inline">Absen</span> <span class="ml-1 px-1.5 py-0.5 bg-[#FDE7E9] text-[#D13438] rounded-md text-[10px]">{{ $belumAbsenList->total() }}</span>
                    </button>
                        ...
                        <div class="grid grid-cols-1 gap-0">
                            @forelse ($attendancesHadir as $index => $att)
                                <div class="relative p-4 md:p-5 border-b border-slate-50 hover:bg-slate-50/80 transition-colors group flex items-center justify-between">
                                    <div class="absolute left-0 top-0 bottom-0 w-1 {{ $att->status == 'Terlambat' ? 'bg-[#D83B01]' : 'bg-[#107C10]' }} hidden group-hover:block"></div>
                                    <div class="flex items-center gap-3 md:gap-4 overflow-hidden w-full">
                                        <div class="w-12 h-12 rounded-xl {{ $att->status == 'Terlambat' ? 'bg-[#FFEFD6] text-[#D83B01]' : 'bg-[#DFF6DD] text-[#107C10]' }} flex items-center justify-center font-bold text-xs shrink-0">
                                             {{ $attendancesHadir->firstItem() + $index }}
                                        </div>
                                        <div class="flex-1 min-w-0">
                                            <h4 class="font-bold text-slate-800 truncate group-hover:text-[#5295FF] transition-colors">{{ $att->student->name }}</h4>
                                            <div class="flex flex-wrap items-center gap-2 md:gap-3 mt-1 text-xs">
                                                <span class="font-medium text-slate-500 bg-slate-100 border border-slate-200 px-2 py-0.5 rounded">{{ $att->student->schoolClass->name ?? '-' }}</span>
                                                <span class="flex items-center gap-1 font-bold text-slate-600">
                                                    <i class="ph-bold ph-arrow-right-circle text-[#107C10]"></i> {{ $att->time_in ? \Carbon\Carbon::parse($att->time_in)->format('H:i') : '-' }}
                                                </span>
                                                <span class="flex items-center gap-1 font-bold text-slate-600">
                                                    <i class="ph-bold ph-arrow-left-circle text-[#5295FF]"></i> {{ $att->time_out ? \Carbon\Carbon::parse($att->time_out)->format('H:i') : '-' }}
                                                </span>
                    ...
                    <div x-show="activeTab === 'lain'" style="display: none;" class="w-full">
                         <div class="grid grid-cols-1 gap-0">
                            @forelse ($attendancesLain as $att)
                                <div class="relative p-4 md:p-5 border-b border-slate-50 hover:bg-slate-50 transition-colors group flex items-center justify-between">
                                    <div class="absolute left-0 top-0 bottom-0 w-1 {{ $att->status == 'Alfa' ? 'bg-[#D13438]' : ($att->status == 'Izin' ? 'bg-[#D83B01]' : 'bg-[#5295FF]') }} hidden group-hover:block"></div>
                                    <div class="flex items-center gap-4 overflow-hidden">
                                        <div class="w-10 h-10 rounded-xl {{ $att->status == 'Alfa' ? 'bg-[#FDE7E9] text-[#D13438] border-[#F4C3C9]' : ($att->status == 'Izin' ? 'bg-[#FFEFD6] text-[#D83B01] border-[#FFD8A8]' : 'bg-slate-100 text-[#5295FF] border-slate-200') }} border flex items-center justify-center font-bold text-xs shrink-0">
                                             {{ substr($att->status, 0, 1) }}
                                        </div>
                                        <div class="min-w-0">
                                            <h4 class="font-bold text-slate-800 truncate">{{ $att->student->name }}</h4>
                                            <div class="flex items-center gap-2 mt-1">
                                                <span class="px-2 py-0.5 rounded text-[10px] font-bold {{ $att->status == 'Alfa' ? 'bg-[#FDE7E9] text-[#D13438] border-[#F4C3C9]' : ($att->status == 'Izin' ? 'bg-[#FFEFD6] text-[#D83B01] border-[#FFD8A8]' : 'bg-slate-100 text-[#5295FF] border-slate-200') }} border uppercase">{{ $att->status }}</span>
                                                @if($att->notes)
                                                    <span class="text-xs text-slate-500 italic max-w-[100px] md:max-w-none truncate">"{{ $att->notes }}"</span>
                                                @endif
                                            </div>
                                        </div>
                                    </div>
