                ...
                <div class="relative z-10 flex flex-wrap gap-3 w-full xl:w-auto items-center justify-center xl:justify-end">
                    ...
                    <form action="{{ route('reports.class') }}" method="GET" class="flex flex-col sm:flex-row gap-2 bg-white/10 backdrop-blur-md p-2 rounded-2xl border border-white/20 w-full sm:w-auto shadow-lg">
                        <div class="flex items-center gap-2 px-3 py-2 bg-white rounded-xl border border-blue-100 shadow-sm w-full sm:w-auto">
                            ...
                            <input type="date" name="start_date" value="{{ $startDate }}" class="border-none p-0 text-sm font-bold text-slate-700 focus:ring-0 cursor-pointer w-full sm:w-32 bg-transparent">
                        </div>
                        ...
                        <div class="flex items-center gap-2 px-3 py-2 bg-white rounded-xl border border-blue-100 shadow-sm w-full sm:w-auto">
                            ...
                            <input type="date" name="end_date" value="{{ $endDate }}" class="border-none p-0 text-sm font-bold text-slate-700 focus:ring-0 cursor-pointer w-full sm:w-32 bg-transparent">
                        </div>
                    ...
                    {{-- Export Buttons --}}
                        <a href="{{ route('reports.class.excel', ['start_date' => $startDate, 'end_date' => $endDate]) }}" target="_blank" class="bg-white hover:bg-emerald-50 text-emerald-600 border border-white/20 px-4 py-3 rounded-2xl font-bold text-sm transition flex items-center justify-center gap-2 shadow-lg flex-1 sm:flex-none" title="Download Excel">
                                ...
                                <tr class="group hover:bg-slate-50 transition-colors">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <div class="font-bold text-slate-700">{{ data_get($data, 'name', 'Kelas ?') }}</div>
                                    ...
                                    <td class="px-6 py-4 align-middle">
                                        <div class="flex w-full h-3 bg-slate-100 rounded-full overflow-hidden shadow-inner">
                                            @if($pctHadir > 0)
                                                <div style="width: {{ $pctHadir }}%" class="bg-emerald-500 hover:bg-emerald-400 transition-all" title="Hadir: {{ $pctHadir }}%"></div>
                                            @endif
                                            @if($pctTelat > 0)
                                                <div style="width: {{ $pctTelat }}%" class="bg-amber-500 hover:bg-amber-400 transition-all" title="Telat: {{ $pctTelat }}%"></div>
                                            @endif
                                            @if($pctIzin > 0)
                                                <div style="width: {{ $pctIzin }}%" class="bg-blue-500 hover:bg-blue-400 transition-all" title="Izin/Sakit: {{ $pctIzin }}%"></div>
                                            @endif
                                            @if($pctAlpha > 0)
                                    ...
                                    <td class="px-2 py-4 text-center">
                                        <div class="flex flex-col">
                                            <span class="font-bold text-emerald-600">{{ $pctHadir }}%</span>
                                            <span class="text-[10px] text-slate-400">{{ $hadir }}</span>
                                        </div>
                                    </td>
                                    <td class="px-2 py-4 text-center">
                                        <div class="flex flex-col">
                                            <span class="font-bold text-amber-600">{{ $pctTelat }}%</span>
                                            <span class="text-[10px] text-slate-400">{{ $telat }}</span>
                                        </div>
                                    </td>
                                    <td class="px-2 py-4 text-center">
                                        <div class="flex flex-col">
                                            <span class="font-bold text-blue-600">{{ $pctIzin }}%</span>
                                            <span class="text-[10px] text-slate-400">{{ $izin }}</span>
                                        </div>
                                    </td>
                                    <td class="px-2 py-4 text-center">
                                        <div class="flex flex-col">
                                            <span class="font-bold text-rose-600">{{ $pctAlpha }}%</span>
                                            <span class="text-[10px] text-slate-400">{{ $alpha }}</span>
                                        </div>
                                    </td>
                                    <td class="px-2 py-4 text-center border-l border-slate-100 border-dashed">
                                        <div class="flex flex-col">
                                            <span class="font-bold text-slate-400">{{ $pctNA > 0 ? $pctNA.'%' : '-' }}</span>
