            ...
            <div class="relative rounded-xl bg-gradient-to-br from-[#25D0FF] via-[#5295FF] to-[#FFC9B9] p-6 md:p-8 flex flex-col xl:flex-row items-center justify-between gap-6 overflow-hidden border border-white/40 shadow-[0_10px_40px_-10px_rgba(37,208,255,0.4)] group">
               ...
               {{-- Filter & Actions --}}
                <div class="relative z-10 flex flex-wrap gap-3 w-full xl:w-auto items-center justify-center xl:justify-end">
                    <form action="{{ route('reports.class') }}" method="GET" class="flex flex-col sm:flex-row gap-2 bg-white/30 backdrop-blur-md p-2 rounded-xl border border-white/40 w-full sm:w-auto shadow-sm">
                        <div class="flex items-center gap-2 px-3 py-2 bg-white rounded-lg border border-[#D0E7F8] shadow-sm w-full sm:w-auto">
                            ...
                            <input type="date" name="start_date" value="{{ $startDate }}" class="border-none p-0 text-sm font-bold text-[#2A3B52] focus:ring-0 cursor-pointer w-full sm:w-32 bg-transparent">
                        </div>
                        ...
                        <div class="flex items-center gap-2 px-3 py-2 bg-white rounded-lg border border-[#D0E7F8] shadow-sm w-full sm:w-auto">
                            ...
                            <input type="date" name="end_date" value="{{ $endDate }}" class="border-none p-0 text-sm font-bold text-[#2A3B52] focus:ring-0 cursor-pointer w-full sm:w-32 bg-transparent">
                        </div>
                   ...
                   {{-- Export Buttons --}}
                    <a href="{{ route('reports.class.excel', ['start_date' => $startDate, 'end_date' => $endDate]) }}" target="_blank" class="bg-white hover:bg-[#DFF6DD] text-[#107C10] border border-white/40 px-4 py-3 rounded-xl font-bold text-sm transition flex items-center justify-center gap-2 shadow-sm flex-1 sm:flex-none" title="Download Excel">
                        <i class="ph-bold ph-microsoft-excel-logo text-lg"></i> 
                        <span>Excel</span>
                    </a>
                    <a href="{{ route('reports.class.print', request()->all()) }}" target="_blank" class="bg-white hover:bg-[#FDE7E9] text-[#D13438] border border-white/40 px-4 py-3 rounded-xl font-bold text-sm transition flex items-center justify-center gap-2 shadow-sm flex-1 sm:flex-none" title="Cetak PDF">
                                ...
                                <tr class="group hover:bg-slate-50 transition-colors">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                           ...
                                           <div>
                                                <div class="font-bold text-[#2A3B52]">{{ data_get($data, 'name', 'Kelas ?') }}</div>
                                                <div class="text-[10px] text-slate-400 font-bold uppercase tracking-wide">{{ data_get($data, 'total_students', 0) }} Siswa</div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 align-middle">
                                       <div class="flex w-full h-3 bg-slate-100 rounded-full overflow-hidden shadow-inner">
                                            ...
                                            @if($pctIzin > 0)<div style="width: {{ $pctIzin }}%" class="bg-[#5295FF] hover:opacity-80 transition-all" title="Izin/Sakit: {{ $pctIzin }}%"></div>@endif
                                    ...
                                    {{-- Persentase Detail --}}
                                    <td class="px-2 py-4 text-center"><div class="flex flex-col"><span class="font-bold text-[#107C10]">{{ $pctHadir }}%</span><span class="text-[10px] text-slate-400">{{ $hadir }}</span></div></td>
                                    <td class="px-2 py-4 text-center"><div class="flex flex-col"><span class="font-bold text-[#D83B01]">{{ $pctTelat }}%</span><span class="text-[10px] text-slate-400">{{ $telat }}</span></div></td>
                                    <td class="px-2 py-4 text-center"><div class="flex flex-col"><span class="font-bold text-[#5295FF]">{{ $pctIzin }}%</span><span class="text-[10px] text-slate-400">{{ $izin }}</span></div></td>
                                    <td class="px-2 py-4 text-center"><div class="flex flex-col"><span class="font-bold text-[#D13438]">{{ $pctAlpha }}%</span><span class="text-[10px] text-slate-400">{{ $alpha }}</span></div></td>
                                    <td class="px-2 py-4 text-center border-l border-slate-100 border-dashed"><div class="flex flex-col"><span class="font-bold text-slate-400">{{ $pctNA > 0 ? $pctNA.'%' : '-' }}</span><span class="text-[10px] text-slate-300">N/A</span></div></td>
