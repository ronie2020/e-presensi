            ...
            @if($classId && $students->count() > 0)
                <div class="animate-enter bg-white rounded-[1.5rem] shadow-sm border border-slate-200 overflow-hidden" style="animation-delay: 200ms">
                   ...
                   <div class="p-4 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
                        <div>
                            <h3 class="font-bold text-slate-800 text-lg">{{ $classes->where('id', $classId)->first()->name ?? 'Kelas' }}</h3>
                            ...
                            <thead>
                                <tr class="bg-slate-100 border-b border-slate-200 text-slate-600">
                                    ...
                                    @foreach($dates as $date)
                                        <th class="p-1 font-bold text-[10px] text-center w-8 min-w-[32px] border-r border-slate-200 {{ $date->isSunday() ? 'bg-rose-50/50 text-rose-400' : '' }}">
                            ...
                            <tbody class="divide-y divide-slate-100">
                                @foreach($students as $index => $student)
                                    <tr class="hover:bg-slate-50 transition-colors group">
                                        ...
                                        {{-- Loop Absensi per Tanggal --}}
                                        @foreach($dates as $date)
                                            @php 
                                                $dateStr = $date->format('Y-m-d');
                                                $data = $student->attendance_map[$dateStr] ?? ['code' => '-', 'class' => 'text-slate-300'];
                                            @endphp
                                            <td class="p-1 text-center border-r border-slate-50 text-[10px] {{ $data['class'] }}">
                                                {{ $data['code'] }}
                                            </td>
                                        @endforeach

                                        {{-- Statistik --}}
                                        <td class="p-2 text-center font-bold text-emerald-600 bg-emerald-50/30 text-xs border-l border-slate-100">{{ $student->summary['H'] }}</td>
                                        <td class="p-2 text-center font-bold text-blue-600 bg-blue-50/30 text-xs">{{ $student->summary['S'] }}</td>
                                        <td class="p-2 text-center font-bold text-indigo-600 bg-indigo-50/30 text-xs">{{ $student->summary['I'] }}</td>
                                        <td class="p-2 text-center font-bold text-rose-600 bg-rose-50/30 text-xs">{{ $student->summary['A'] }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                    ...
                    @if(count($students) > 15)
                        <div class="p-4 bg-slate-50 border-t border-slate-200 text-center text-xs text-slate-500">
                            Menampilkan {{ count($students) }} Siswa
                        </div>
                    @endif
                </div>
            @elseif($classId)
