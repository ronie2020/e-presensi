            ...
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-6 no-print">
                <div class="animate-enter lg:col-span-1 bg-gray-900 bg-gradient-to-br from-indigo-900 via-blue-900 to-slate-900 rounded-[2rem] p-6 text-white shadow-xl shadow-blue-900/30 relative overflow-hidden flex flex-col justify-center min-h-[160px]">
                     ...
                     <div class="relative z-10">
                         {{-- Tombol Kembali mengarah ke Halaman Summary (indexClass) --}}
            ...
            @if($classId && $students->count() > 0)
                <div class="animate-enter bg-white rounded-[1.5rem] shadow-sm border border-slate-200 overflow-hidden" style="animation-delay: 200ms">
                   ...
                   <div class="p-4 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
                        <div>
                            {{-- Gunakan optional() untuk menghindari error jika class tidak ditemukan --}}
                            <h3 class="font-bold text-slate-800 text-lg">{{ optional($classes->where('id', $classId)->first())->name ?? 'Kelas' }}</h3>
                            ...
                            <tbody class="divide-y divide-slate-100">
                                @foreach($students as $index => $student)
                                    <tr class="hover:bg-slate-50 transition-colors group">
                                        ...
                                        {{-- Loop Absensi per Tanggal (2 Kolom M/P) --}}
                                        @foreach($dates as $date)
                                            @php 
                                                $dateStr = $date->format('Y-m-d');
                                                $data = data_get($student, 'attendance_map.' . $dateStr, ['in_code' => '-', 'in_class' => 'text-slate-300', 'out_code' => '-', 'out_class' => 'text-slate-300']);
                                            @endphp
                                            <td class="p-1 text-center border-r border-slate-50 text-[10px] {{ $data['in_class'] ?? 'text-slate-300' }}">
                                                {{ $data['in_code'] ?? '-' }}
                                            </td>
                                            <td class="p-1 text-center border-r border-slate-200 text-[10px] {{ $data['out_class'] ?? 'text-slate-300' }}">
                                                {{ $data['out_code'] ?? '-' }}
                                            </td>
                                        @endforeach
