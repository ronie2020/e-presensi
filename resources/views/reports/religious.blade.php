<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8 no-print">
                ...
                <div class="animate-enter lg:col-span-2 bg-white rounded-[2rem] p-6 lg:p-8 border border-slate-100 shadow-xl shadow-slate-200/40 relative" style="animation-delay: 100ms">
                    ...
                    <form action="{{ route('reports.religious') }}" method="GET" class="flex flex-col xl:flex-row gap-3 w-full" @submit.prevent="submitFilter">
                        ...
                        <div class="flex-1 min-w-[180px]">
                            <div x-show="reportType === 'daily'">
                                <input type="date" name="date" value="{{ request('date', $selectedDate_db->format('Y-m-d')) }}" class="w-full rounded-2xl border-slate-200 bg-elevate-soft font-bold h-14 text-sm px-4 focus:bg-white focus:ring-elevate-accent/30 focus:border-elevate-accent transition-colors">
                            </div>
                            <div x-show="reportType === 'weekly'" style="display: none;">
                                <input type="week" name="week" value="{{ request('week', date('Y-\WW')) }}" class="w-full rounded-2xl border-slate-200 bg-elevate-soft font-bold h-14 text-sm px-4 focus:bg-white focus:ring-elevate-accent/30 focus:border-elevate-accent transition-colors">
                            </div>
                            <div x-show="reportType === 'monthly'" style="display: none;">
                                <input type="month" name="month" value="{{ request('month', date('Y-m')) }}" class="w-full rounded-2xl border-slate-200 bg-elevate-soft font-bold h-14 text-sm px-4 focus:bg-white focus:ring-elevate-accent/30 focus:border-elevate-accent transition-colors">
                            </div>
                        </div>
                        ...
                        <div class="flex flex-wrap sm:flex-nowrap gap-2 w-full xl:w-auto">
                            <button type="submit" title="Terapkan Filter" class="flex-1 xl:flex-none bg-elevate-dark hover:bg-elevate-primary text-white px-5 rounded-2xl h-14 font-bold text-sm shadow-lg shadow-elevate-dark/20 flex items-center justify-center gap-2 active:scale-95 transition-all">
                                <i class="ph-bold ph-funnel text-lg"></i> <span class="xl:inline">Filter</span>
                            </button>
                            <a href="{{ route('reports.printReligious', request()->all()) }}" target="_blank" title="Cetak Laporan" class="flex-1 xl:flex-none bg-white border-2 border-slate-100 text-elevate-dark hover:bg-elevate-soft px-5 rounded-2xl h-14 font-bold text-sm flex items-center justify-center gap-2 active:scale-95 transition-colors shadow-sm">
                                <i class="ph-bold ph-printer text-xl"></i> <span class="xl:inline">Cetak</span>
                            </a>
                            <a href="{{ route('reports.exportReligious', request()->all()) }}" title="Unduh Excel (.xlsx)" class="flex-1 xl:flex-none bg-emerald-600 hover:bg-emerald-700 text-white px-5 rounded-2xl h-14 font-bold text-sm flex items-center justify-center gap-2 transition-colors active:scale-95 shadow-md shadow-emerald-600/20">
            ...
            <div class="mb-8 no-print">
                <div class="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
                    <div class="bg-white p-5 rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/30"><p class="text-[10px] md:text-xs font-bold text-elevate-primary uppercase mb-2">Total Siswa</p><h3 class="text-3xl md:text-4xl font-black text-elevate-dark">{{ $hadirCount + $izinUzurCount + $alfaCount + $belumAbsenCount }}</h3></div>
                    <div class="bg-white p-5 rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/30"><p class="text-[10px] md:text-xs font-bold text-[#107C10] uppercase mb-2">Total Hadir</p><h3 class="text-3xl md:text-4xl font-black text-elevate-dark">{{ $hadirCount }}</h3></div>
                    <div class="bg-white p-5 rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/30"><p class="text-[10px] md:text-xs font-bold text-[#D13438] uppercase mb-2">Belum Hadir</p><h3 class="text-3xl md:text-4xl font-black text-elevate-dark">{{ $belumAbsenCount + $alfaCount }}</h3></div>
                    <div class="bg-white p-5 rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/30"><p class="text-[10px] md:text-xs font-bold text-elevate-primary uppercase mb-2">Sakit / Izin</p><h3 class="text-3xl md:text-4xl font-black text-elevate-dark">{{ $izinUzurCount }}</h3></div>
                    <div class="bg-white p-5 rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/30 col-span-2 lg:col-span-1 flex items-center justify-between">
                         <div>
                            <p class="text-[10px] md:text-xs font-bold text-elevate-primary uppercase mb-1">Kehadiran</p>
                             @php $totalAll = $hadirCount + $izinUzurCount + $alfaCount + $belumAbsenCount; $percentage = $totalAll > 0 ? round(($hadirCount / $totalAll) * 100) : 0; @endphp
                            <h3 class="text-3xl md:text-4xl font-black text-elevate-dark">{{ $percentage }}%</h3>
                        </div>
                    </div>
                </div>
            ...
            <div x-show="viewMode === 'list'" class="animate-enter bg-white rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/40 overflow-hidden min-h-[500px]">
                <div class="flex flex-wrap md:flex-nowrap border-b border-slate-100 bg-elevate-soft/50 p-3 gap-3 sticky top-0 z-20 no-print backdrop-blur-md">
                    ...
                    <button @click="activeTab = 'belum'" :class="activeTab === 'belum' ? 'bg-white text-elevate-dark shadow-sm ring-1 ring-slate-200' : 'text-slate-500 hover:bg-white/60 hover:text-elevate-dark'" class="flex-1 md:flex-none py-3.5 px-6 rounded-2xl text-sm font-bold transition-all">Belum Absen <span class="bg-[#FDE7E9] text-[#D13438] px-2 py-1 ml-1 rounded-lg text-xs">{{ $belumAbsenList->total() }}</span></button>
                ...
                <div class="p-0">
                    <div x-show="activeTab === 'hadir'" class="w-full">
                        @forelse ($attendancesHadir as $index => $attendance)
                             <div class="p-5 border-b border-slate-50 flex items-center justify-between hover:bg-elevate-soft/30 transition-colors">
                                <div class="flex items-center gap-5 w-full">
                                    <div class="w-14 h-14 rounded-2xl bg-[#DFF6DD] text-[#107C10] flex items-center justify-center font-black text-sm">{{ $attendancesHadir->firstItem() + $index }}</div>
                                    <div class="flex-1">
                                        <button type="button" class="font-black text-lg text-elevate-dark hover:text-elevate-primary transition-colors truncate">{{ $attendance->student->name }}</button>
                                        <div class="flex items-center gap-2 mt-1.5">
                                            <span class="text-[10px] bg-[#DFF6DD] text-[#107C10] px-3 py-1 rounded-md font-black uppercase tracking-wider">{{ $attendance->status }}</span>
                                        </div>
                                    </div>
                                </div>
                                <button onclick="openEditModalReligious({{ $attendance->id }}, '{{ addslashes($attendance->student->name) }}', '{{ $attendance->status }}', `{{ addslashes($attendance->notes ?? '') }}`, '{{ $attendance->activity }}')" class="p-3 text-slate-400 hover:text-white hover:bg-elevate-peach-dark rounded-xl transition-all"><i class="ph-bold ph-pencil-simple text-xl"></i></button>
                            </div>
                        @empty
                         ...
                         @if($belumAbsenList->total() > 0)
                            <div class="p-6 bg-[#FDE7E9] border-b border-[#F4C3C9] flex justify-between items-center no-print">
                                ...
                                <form id="bulk-alpha-religious-form" action="{{ route('reports.bulkAlpha') }}" method="POST">
                                    @csrf <input type="hidden" name="date" value="{{ $selectedDate_db->format('Y-m-d') }}"><input type="hidden" name="type" value="Keagamaan"><input type="hidden" name="activity" value="{{ $selectedActivity }}">
                                    <button type="button" onclick="confirmBulkAlphaReligious('{{ $belumAbsenList->total() }}')" class="bg-[#D13438] text-white hover:bg-[#A4262C] transition-colors text-sm font-bold px-6 py-3.5 rounded-2xl shadow-lg shadow-[#D13438]/30"><i class="ph-bold ph-check-circle text-lg mr-2"></i> Tandai Semua Alfa</button>
                                </form>
                            </div>
                        @endif
                        @forelse ($belumAbsenList as $student)
                            <div class="p-5 border-b border-slate-50 flex items-center justify-between hover:bg-elevate-soft/30 transition-colors">
                                <div class="flex items-center gap-5"><div class="w-12 h-12 rounded-full bg-slate-100 text-slate-400 flex items-center justify-center font-black text-lg">!</div><h4 class="font-black text-lg text-elevate-dark">{{ $student->name }}</h4></div>
                                <button onclick="openManualModalForStudent({{ $student->id }}, '{{ addslashes($student->name) }}')" class="bg-white border-2 border-slate-100 text-elevate-dark px-6 py-3 rounded-xl text-xs font-bold hover:bg-elevate-soft transition-colors">Input Manual</button>
                            </div>
                        @empty
                    ...
                    <div x-show="activeTab === 'uzur'" style="display: none;" class="w-full">
                        @forelse ($attendancesUzur as $attendance)
                            <div class="p-5 border-b border-slate-50 flex items-center justify-between hover:bg-elevate-soft/30 transition-colors">
                                <div class="flex items-center gap-5 w-full">
                                    <div class="w-14 h-14 rounded-2xl {{ $attendance->status == 'Alfa' ? 'bg-[#FDE7E9] text-[#D13438]' : 'bg-elevate-soft text-elevate-primary' }} flex items-center justify-center font-black text-lg">{{ substr($attendance->status, 0, 1) }}</div>
                                    <h4 class="font-black text-lg text-elevate-dark">{{ $attendance->student->name }}</h4>
                                </div>
                                <button onclick="openEditModalReligious({{ $attendance->id }}, '{{ addslashes($attendance->student->name) }}', '{{ $attendance->status }}', `{{ addslashes($attendance->notes ?? '') }}`, '{{ $attendance->activity }}')" class="p-3 text-slate-400 hover:text-white hover:bg-elevate-peach-dark rounded-xl transition-all"><i class="ph-bold ph-pencil-simple text-xl"></i></button>
                            </div>
                        @empty
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="text-sm font-bold text-elevate-dark divide-y divide-slate-50">
                            ...
                            <tr class="hover:bg-elevate-soft/20 transition-colors">
                                <td class="p-5 font-black">{{ $rekap->className }}</td>
                                <td class="p-5 text-center text-lg">{{ $rekap->total_siswa }}</td>
                                <td class="p-5 bg-elevate-peach-light/10">
                                    <div class="w-full bg-slate-100 rounded-full h-3 mb-2 overflow-hidden"><div class="bg-elevate-peach-dark h-3 rounded-full" style="width: {{ $rekap->dhuha['percent'] }}%"></div></div>
                                    <div class="flex justify-between text-[10px] font-bold"><span class="text-[#D13438]">Alfa: {{ $rekap->dhuha['alfa'] }}</span><span>{{ $rekap->dhuha['percent'] }}%</span></div>
                                </td>
                                <td class="p-5 bg-elevate-soft/10">
                                    <div class="w-full bg-slate-100 rounded-full h-3 mb-2 overflow-hidden"><div class="bg-elevate-primary h-3 rounded-full" style="width: {{ $rekap->dhuhur['percent'] }}%"></div></div>
                                    <div class="flex justify-between text-[10px] font-bold"><span class="text-[#D13438]">Alfa: {{ $rekap->dhuhur['alfa'] }}</span><span>{{ $rekap->dhuhur['percent'] }}%</span></div>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
    ...
    {{-- PERBAIKAN: Modal dibungkus dengan <template x-teleport="body"> --}}
    <template x-teleport="body">
        <div id="manualInputModal" class="fixed inset-0 bg-elevate-dark/60 backdrop-blur-sm z-50 hidden flex items-center justify-center p-4 transition-opacity no-print">
            <div class="bg-white rounded-[2rem] w-full max-w-md shadow-2xl flex flex-col max-h-[90vh] border border-slate-100 animate-enter overflow-hidden">
                <div class="bg-elevate-peach-light/30 px-6 py-5 flex justify-between items-center border-b border-elevate-peach/30 shrink-0">
                    ...
                    <button type="button" onclick="closeManualModal()" class="text-slate-400 hover:text-elevate-dark bg-white hover:bg-elevate-soft p-2 rounded-full transition-colors"><i class="ph-bold ph-x text-lg"></i></button>
                </div>
                <form action="{{ route('reports.storeManual') }}" method="POST" class="p-6 md:p-8 space-y-5 overflow-y-auto custom-scrollbar" onsubmit="this.querySelector('button[type=submit]').disabled=true; this.querySelector('button[type=submit]').innerText='Menyimpan...';">
                    @csrf
                ...
                <div class="bg-elevate-dark px-8 py-5 flex justify-between items-center shrink-0">
                    <div>
                        ...
                        <p class="text-elevate-soft/80 font-medium text-xs mt-1">
                            {{ $selectedActivity }} • {{ $selectedDate_db->translatedFormat('d F Y') }}
                        </p>
                    </div>
    ...
    <template x-teleport="body">
        <div id="editReligiousModal" class="fixed inset-0 bg-elevate-dark/60 backdrop-blur-sm z-50 hidden flex items-center justify-center p-4 transition-opacity no-print">
            <div class="bg-white rounded-[2rem] w-full max-w-md shadow-2xl flex flex-col max-h-[90vh] border border-slate-100 animate-enter overflow-hidden">
                <div class="bg-elevate-soft px-6 py-5 flex justify-between items-center border-b border-slate-100 shrink-0">
                    ...
                    <button type="button" onclick="closeEditModalReligious()" class="text-slate-400 hover:text-elevate-dark bg-white hover:bg-slate-100 p-2 rounded-full transition-colors"><i class="ph-bold ph-x text-lg"></i></button>
                </div>
                <form id="editReligiousForm" method="POST" class="p-6 md:p-8 space-y-5 overflow-y-auto custom-scrollbar" onsubmit="this.querySelector('button[type=submit]').disabled=true; this.querySelector('button[type=submit]').innerText='Menyimpan...';">
                    @csrf @method('PUT')
        ...
        <div id="historyModal" class="fixed inset-0 bg-elevate-dark/60 backdrop-blur-sm z-50 hidden flex items-center justify-center p-4 transition-opacity no-print">
            <div class="bg-white rounded-[2rem] w-full max-w-lg shadow-2xl overflow-hidden border border-slate-100 animate-enter flex flex-col max-h-[80vh]">
                <div class="bg-white border-b border-slate-100 px-8 py-5 flex justify-between items-center shrink-0">
                    ...
                    <button type="button" onclick="document.getElementById('historyModal').classList.add('hidden')" class="w-10 h-10 rounded-full bg-elevate-soft hover:bg-slate-200 flex items-center justify-center text-elevate-dark transition-colors"><i class="ph-bold ph-x"></i></button>
                </div>
    ...
    <script>
        ...
        function openStudentHistory(studentId, studentName) {
            ...
            fetch(`{{ url('reports/religious/history') }}?student_id=${studentId}&activity={{ $selectedActivity }}`).then(r=>r.text()).then(h=>{contentDiv.innerHTML=h;}).catch(e=>{console.error(e);contentDiv.innerHTML=`<div class="p-8 text-center text-rose-500 font-bold text-sm">Gagal memuat data.</div>`;});
        }
        ...
        function closeEditModalReligious() { 
            document.getElementById('editReligiousModal').classList.add('hidden'); 
        }
        ...
        function checklistHandler() {
            return {
                ...
                async fetchStudents() {
                    ...
                    try {
                        const url = `{{ route('reports.getStudentsByClass') }}?class_id=${this.selectedClass}&date={{ $selectedDate_db->format('Y-m-d') }}&activity={{ $selectedActivity }}`;
