...
@section('content')
    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-6 pb-20 pt-24 relative z-10" 
        ...
        {{-- HEADER SECTION MICROSOFT ELEVATE --}}
        <div class="animate-enter relative rounded-[3rem] bg-elevate-gradient-main p-8 md:p-12 mb-10 text-elevate-dark shadow-xl shadow-elevate-accent/10 overflow-hidden border border-white/60">
            ...
            <div class="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
                <div>
                    <a href="{{ route('student.complaints.index') }}" class="inline-flex items-center gap-2 text-slate-500 hover:text-elevate-primary transition-colors mb-6 text-[10px] font-black uppercase tracking-[0.2em]">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-enter" style="animation-delay: 100ms">
            ...
            <div class="lg:col-span-8">
                <form action="{{ route('student.complaints.store') }}" method="POST" enctype="multipart/form-data" class="bg-white p-8 md:p-12 rounded-[3rem] shadow-sm border border-slate-100 relative overflow-hidden group hover:border-elevate-accent/30 transition-colors">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                            ...
                            <label class="cursor-pointer group/cat">
                                ...
                                <div class="border-2 border-slate-50 rounded-[1.8rem] p-6 flex flex-col items-center justify-center gap-3 hover:border-{{ $cat['color'] }}-200 hover:bg-{{ $cat['color'] }}-50 transition-all h-full relative overflow-hidden shadow-sm">
                                    <i class="ph-duotone {{ $cat['icon'] }} text-4xl text-{{ $cat['color'] }}-500 group-hover/cat:scale-110 transition-transform"></i>
                                    <span class="text-[10px] font-black text-slate-600 uppercase tracking-widest group-hover/cat:text-{{ $cat['color'] }}-700">{{ $cat['label'] }}</span>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                        <div class="space-y-3">
                            ...
                            <input type="date" name="incident_date" required max="{{ date('Y-m-d') }}" class="w-full bg-slate-50 border-transparent rounded-2xl focus:bg-white focus:ring-4 focus:ring-elevate-accent/20 focus:border-elevate-primary py-4 px-5 font-bold text-elevate-dark transition-all">
                        </div>
                        <div class="space-y-3">
            ...
            <div class="lg:col-span-4 space-y-6">
                ...
                <div class="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100 transition-all duration-500 relative overflow-hidden">
                    {{-- Tema gelap mengikuti elevate-dark jika anonim --}}
