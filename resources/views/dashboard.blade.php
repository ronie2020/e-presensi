<x-app-layout>
    ...
    <div x-data="{ 
        ...
        {{-- HERO SECTION --}}
        <div class="animate-enter relative rounded-[2.5rem] bg-gradient-to-r from-blue-900 via-slate-800 to-slate-900 p-8 md:p-10 mb-8 text-white shadow-2xl shadow-blue-900/20 overflow-hidden group border border-white/10">
            ...
            <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
                ...
                {{-- FILTER CONTROLS --}}
                <div class="flex flex-col gap-3 w-full md:w-auto min-w-[300px]">
                    ...
                    <div class="flex gap-2">
                        <div class="relative group/date flex-1">
                            ...
                            <input type="date" x-model="date" @change="updateFilter(period)" 
                                class="w-full bg-white/10 border border-white/10 text-white text-xs font-bold rounded-xl pl-10 pr-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-blue-400/50 focus:border-blue-400/50 cursor-pointer transition-all hover:bg-white/20 shadow-sm placeholder-blue-200">
                        </div>
                        
                        <button @click="printDashboard()" class="bg-white/10 text-white p-2.5 rounded-xl hover:bg-white/20 hover:scale-105 active:scale-95 transition-all shadow-sm shrink-0 border border-white/10" title="Cetak Dashboard">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4 md:gap-5">
            ...
            @php
                ...
                // LOGIKA WARNA & ICON (PRIORITAS JUDUL)
                if (str_contains($titleLower, 'hadir') && !str_contains($titleLower, 'belum')) {
                    $iconClass = 'ph-check-circle';
                    $colorKey = 'emerald';
                } elseif (str_contains($titleLower, 'telat') || str_contains($titleLower, 'lambat')) {
                    $iconClass = 'ph-clock'; // Force Clock
                    $colorKey = 'amber';
                    ...
                    $iconClass = (!empty($rawIcon) && !str_starts_with($rawIcon, 'M') && $rawIcon !== 'ph-hash') 
                                ? $rawIcon 
                                : 'ph-chart-bar';
                    $colorKey = 'blue';
                }

                // DEFINISI CLASS WARNA EKSPLISIT (Agar tidak kena PurgeCSS)
                $theme = match($colorKey) {
                    'emerald' => [
                        'bg' => 'bg-emerald-50', 'text' => 'text-emerald-600', 
                        'hover_bg' => 'group-hover:bg-emerald-600', 'hover_border' => 'hover:border-emerald-200'
                    ],
                    'amber' => [
                        'bg' => 'bg-amber-50', 'text' => 'text-amber-600', 
                        'hover_bg' => 'group-hover:bg-amber-600', 'hover_border' => 'hover:border-amber-200'
                    ],
                    'rose' => [
                        'bg' => 'bg-rose-50', 'text' => 'text-rose-600', 
                        'hover_bg' => 'group-hover:bg-rose-600', 'hover_border' => 'hover:border-rose-200'
                    ],
                    'purple' => [
                        'bg' => 'bg-purple-50', 'text' => 'text-purple-600', 
                        'hover_bg' => 'group-hover:bg-purple-600', 'hover_border' => 'hover:border-purple-200'
                    ],
                    'yellow' => [
                        'bg' => 'bg-yellow-50', 'text' => 'text-yellow-600', 
                        'hover_bg' => 'group-hover:bg-yellow-600', 'hover_border' => 'hover:border-yellow-200'
                    ],
                     'slate' => [
                        'bg' => 'bg-slate-100', 'text' => 'text-slate-600', 
                        'hover_bg' => 'group-hover:bg-slate-600', 'hover_border' => 'hover:border-slate-300'
                    ],
                    default => [
                        'bg' => 'bg-blue-50', 'text' => 'text-blue-600', 
                        'hover_bg' => 'group-hover:bg-blue-600', 'hover_border' => 'hover:border-blue-200'
                    ],
                };
            @endphp

            <a href="{{ url('attendance') }}?status={{ $card['filter_status'] ?? '' }}&period={{ request('period', 'today') }}" 
               class="animate-enter group bg-white rounded-2xl p-5 border border-slate-100 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.05)] hover:shadow-xl hover:shadow-slate-200 {{ $theme['hover_border'] }} transition-all duration-300 hover:-translate-y-1 relative overflow-hidden flex flex-col justify-between h-full"
               style="animation-delay: {{ ($index + 1) * 100 }}ms">
                
                {{-- WATERMARK ICON --}}
                <i class="ph-duotone {{ $iconClass }} absolute -right-4 -bottom-4 text-[5rem] opacity-[0.03] text-slate-900 group-hover:scale-110 group-hover:-rotate-12 transition-transform duration-500"></i>

                <div class="flex justify-between items-start mb-4 relative z-10">
                    <div class="w-12 h-12 rounded-xl flex items-center justify-center shadow-sm transition-all duration-300
                                {{ $theme['bg'] }} {{ $theme['text'] }} 
                                {{ $theme['hover_bg'] }} group-hover:text-white group-hover:scale-110">
        ...
        <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
            {{-- Grafik Batang --}}
            <div class="animate-enter xl:col-span-2 bg-white p-6 md:p-8 rounded-[2rem] shadow-sm border border-slate-100" style="animation-delay: 600ms">
                <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
                    <div>
                        <h3 class="text-lg font-bold text-slate-800 flex items-center gap-2">
                            <i class="ph-fill ph-chart-bar text-blue-600"></i> Analisis Tren Kehadiran
                        </h3>
                        <p class="text-xs text-slate-400 font-bold uppercase tracking-wide mt-1">Statistik Harian (Sekolah)</p>
                    </div>
                    <div class="flex gap-2">
                        <div class="px-3 py-1 rounded-lg bg-emerald-50 border border-emerald-100 flex items-center gap-2 text-[10px] font-bold text-emerald-700 uppercase">
                            <span class="w-2 h-2 rounded-full bg-emerald-500"></span> Hadir
                        </div>
                        <div class="px-3 py-1 rounded-lg bg-amber-50 border border-amber-100 flex items-center gap-2 text-[10px] font-bold text-amber-700 uppercase">
                            <span class="w-2 h-2 rounded-full bg-amber-500"></span> Telat
                        </div>
                        <div class="px-3 py-1 rounded-lg bg-rose-50 border border-rose-100 flex items-center gap-2 text-[10px] font-bold text-rose-700 uppercase">
                            <span class="w-2 h-2 rounded-full bg-rose-500"></span> Absen
                        </div>
                    </div>
                </div>
            ...
            {{-- Donut Chart --}}
            <div class="animate-enter bg-white p-6 md:p-8 rounded-[2rem] shadow-sm border border-slate-100 flex flex-col h-full" style="animation-delay: 700ms">
                ...
                <div class="grid grid-cols-2 gap-3 mt-auto">
                    <div class="bg-emerald-50/50 p-3 rounded-xl border border-emerald-100">
                        <span class="block text-[10px] font-bold text-emerald-600 uppercase mb-1">Hadir Tepat</span>
                        <span class="text-lg font-black text-slate-800">{{ $presentOnTimeCount ?? 0 }}</span>
                    </div>
                    <div class="bg-amber-50/50 p-3 rounded-xl border border-amber-100">
                        <span class="block text-[10px] font-bold text-amber-600 uppercase mb-1">Terlambat</span>
                        <span class="text-lg font-black text-slate-800">{{ $lateCount ?? 0 }}</span>
                    </div>
                    <div class="bg-rose-50/50 p-3 rounded-xl border border-rose-100">
                        <span class="block text-[10px] font-bold text-rose-600 uppercase mb-1">Alfa/Belum</span>
                        {{-- Menggabungkan Alpha tercatat + Belum Scan (yang dikirim controller sebagai absentCount di variabel terpisah atau kartu) --}}
                        {{-- Tapi di chart kita pakai $absentCount yang di controller baru berisi Alpha Recorded --}}
                        {{-- Untuk display di sini, kita gunakan variabel yang dikirim ke view. --}}
                         <span class="text-lg font-black text-slate-800">{{ $absentCount ?? 0 }}</span>
                    </div>
                     <div class="bg-blue-50/50 p-3 rounded-xl border border-blue-100">
                        <span class="block text-[10px] font-bold text-blue-600 uppercase mb-1">Izin/Sakit</span>
                        <span class="text-lg font-black text-slate-800">{{ $sickPermitCount ?? 0 }}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {{-- Activity Log (SMART FEED) --}}
            <div class="animate-enter bg-white p-6 md:p-8 rounded-[2rem] shadow-sm border border-slate-100 flex flex-col h-full" style="animation-delay: 800ms" x-data="{ tab: 'activity' }">
                <div class="flex items-center justify-between mb-6">
                    <div class="flex gap-6 border-b border-slate-100 w-full">
                        <button @click="tab = 'activity'" 
                            ...
                            class="text-sm font-bold pb-3 border-b-2 transition-all px-1">
                            Aktivitas Terbaru
                        </button>
                ...
                {{-- TAB AKTIVITAS TERBARU (DENGAN FIX WARNA & LOGIKA) --}}
                        ...
                        <div class="relative pl-6 border-l-2 border-slate-100 space-y-6 py-2 ml-2">
                            @foreach($recentActivities as $log)
                                @php
                                    ...
                                    // Definisikan Variabel Warna Default (Emerald - Hadir)
                                    $theme = [
                                        'bg_icon' => 'bg-emerald-50', 
                                        'border_icon' => 'border-emerald-100', 
                                        'text_icon' => 'text-emerald-600',
                                        'dot' => 'bg-emerald-400',
                                        'bg_badge' => 'bg-emerald-100',
                                        'text_badge' => 'text-emerald-700'
                                    ];
                                    $icon = 'ph-check-circle';

                                    // 1. KEAGAMAAN (Dhuha/Dhuhur)
                                    if ($type === 'Keagamaan') {
                                        $icon = 'ph-moon-stars';
                                        $statusText = $log->activity;
                                        $subText = 'Ibadah';
                                        $theme = [
                                            'bg_icon' => 'bg-purple-50', 'border_icon' => 'border-purple-100', 'text_icon' => 'text-purple-600',
                                            'dot' => 'bg-purple-400', 'bg_badge' => 'bg-purple-100', 'text_badge' => 'text-purple-700'
                                        ];
                                    } 
                                    // 2. EKSTRAKURIKULER
                                    elseif ($type === 'Ekstrakurikuler') {
                                        $icon = 'ph-trophy';
                                        $statusText = $log->activity;
                                        $subText = 'Ekstrakurikuler';
                                        $theme = [
                                            'bg_icon' => 'bg-orange-50', 'border_icon' => 'border-orange-100', 'text_icon' => 'text-orange-600',
                                            'dot' => 'bg-orange-400', 'bg_badge' => 'bg-orange-100', 'text_badge' => 'text-orange-700'
                                        ];
                                    }
                                    // 3. HARIAN / SEKOLAH
                                    else {
                                        if ($log->status == 'Terlambat') {
                                            ...
                                            $theme = [
                                                'bg_icon' => 'bg-amber-50', 'border_icon' => 'border-amber-100', 'text_icon' => 'text-amber-600',
                                                'dot' => 'bg-amber-400', 'bg_badge' => 'bg-amber-100', 'text_badge' => 'text-amber-700'
                                            ];
                                        } elseif ($type == 'Pulang') {
                                            ...
                                            $subText = 'Selesai KBM';
                                            $theme = [
                                                'bg_icon' => 'bg-blue-50', 'border_icon' => 'border-blue-100', 'text_icon' => 'text-blue-600',
                                                'dot' => 'bg-blue-400', 'bg_badge' => 'bg-blue-100', 'text_badge' => 'text-blue-700'
                                            ];
                                        } else {
                                            // Default Emerald (Hadir Tepat Waktu)
                            ...
                            <div class="relative group">
                                ...
                                <div class="flex items-start justify-between gap-3 p-3 rounded-2xl hover:bg-slate-50 transition-colors -mt-2 -ml-2">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p class="text-sm font-bold text-slate-800 line-clamp-1 group-hover:text-blue-700 transition-colors">
                                                {{ $log->student->name ?? 'Siswa' }}
                                            </p>
                                            <p class="text-[10px] text-slate-500 font-bold px-2 py-0.5 rounded-md inline-block mt-1 border border-slate-100 bg-white">
                                                {{ $subText }}
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div class="text-right shrink-0">
                                        ...
                                        <span class="text-[10px] font-bold px-2 py-1 rounded-lg {{ $theme['bg_badge'] }} {{ $theme['text_badge'] }}">
                                            {{ $statusText }}
                                        </span>
                                    </div>
                                </div>
                ...
                <div x-show="tab === 'late_recap'" style="display: none;" class="flex-1 overflow-y-auto max-h-[400px] custom-scrollbar">
                    @if(isset($topLateStudents) && count($topLateStudents) > 0)
                        <div class="space-y-3">
                            ...
                            <div class="flex items-center justify-between p-3 rounded-2xl border border-slate-50 hover:bg-red-50/30 transition-colors">
                                ...
                                <span class="text-xs font-black text-red-600 bg-red-50 px-3 py-1 rounded-xl border border-red-100">
                    ...
                    @else
                        <div class="text-center py-10 text-slate-400 text-xs font-bold">
                            Tidak ada siswa terlambat signifikan.
                        </div>
                    @endif
                </div>
            </div>
            ...
            {{-- Class Rank Table --}}
            <div class="animate-enter bg-white p-6 md:p-8 rounded-[2rem] shadow-sm border border-slate-100 h-full" style="animation-delay: 900ms">
                <div class="flex items-center justify-between mb-6">
                    <h3 class="text-lg font-bold text-slate-800 flex items-center gap-2">
                    ...
                    data: {
                        ...
                        datasets: [{ 
                            data: [
                                ...
                                {{ $lateCount ?? 0 }}, 
                                {{ $absentCount ?? 0 }}, // Gunakan variabel yang sama dengan di summary
                                {{ $sickPermitCount ?? 0 }}
                            ], 
