<x-app-layout>
    ...
    <div x-data="{ 
            ...
            updateFilter(newPeriod) {
                ...
                // Logika reload halaman saat filter berubah
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div class="bg-white p-1.5 rounded-xl shadow-sm border border-gray-200 flex flex-wrap gap-2">
                ...
                <div class="flex items-center">
                    <input x-show="period === 'today'" type="date" x-model="date" @change="updateFilter('today')" class="text-xs border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
            @php
                ...
                $absentCount = $absentCount ?? 0; // Belum Hadir / Tanpa Ket
                $earlyLeaveCount = $earlyLeaveCount ?? 0;
                
                // [PERBAIKAN 1]: Pisahkan Alpa dari Izin/Sakit agar data valid
                $sickPermitCount = ($sickCount ?? 0) + ($permitCount ?? 0); 
                $alphaOnlyCount = ($alphaCount ?? 0);
                ...
                $cards = [
                    ...
                    [
                        'title' => 'Sakit / Izin',
                        'value' => $sickPermitCount, // Hanya Sakit & Izin
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                <div class="flex items-center justify-between mb-6">
                    ...
                    <div class="flex gap-4 text-[10px] font-bold uppercase tracking-wide text-gray-500">
                        <div class="flex items-center gap-1"><span class="w-3 h-3 bg-emerald-500 rounded-sm"></span> Tepat</div>
                        <div class="flex items-center gap-1"><span class="w-3 h-3 bg-amber-500 rounded-sm"></span> Telat</div>
                        <div class="flex items-center gap-1"><span class="w-3 h-3 bg-red-500 rounded-sm"></span> Absen</div>
                    </div>
                </div>
            ...
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col">
                <h3 class="text-lg font-bold text-gray-800 mb-2 flex items-center gap-2">
                    <i class="ph-chart-pie-slice text-purple-500"></i> Komposisi Hari Ini
                </h3>
                ...
                <div class="mt-6 grid grid-cols-2 gap-3 text-xs text-gray-600">
                    ...
                    <div class="flex items-center gap-2"><span class="w-2 h-2 rounded-full bg-purple-500"></span> Izin/Sakit</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        const d_excused       = <?php echo json_encode($sickPermitCount ?? 0); ?>;
        const d_alpha         = <?php echo json_encode($alphaOnlyCount ?? 0); ?>;

        // --- 2. CHART BATANG (TREND) ---
        const ctxWeekly = document.getElementById('weeklyChart').getContext('2d');
        
        // [PERBAIKAN 2]: Jangan kurangi lagi dengan Late! Data Controller sudah bersih.
        // Jika rawPresentData dari controller sudah "Hadir Tepat Waktu", gunakan langsung.
        const onTimeData = rawPresentData; 
        ...
        new Chart(ctxWeekly, {
            ...
            options: {
                ...
                plugins: { legend: { display: false } }, // Legend sudah di custom HTML
                scales: { 
                    y: { 
                        beginAtZero: true, 
                        stacked: true, 
