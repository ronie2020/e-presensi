<x-app-layout>
    ...
    <div x-data="{ 
            ...
            <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
                ...
                <div class="flex flex-col gap-3 w-full md:w-auto md:min-w-[320px] filter-group no-print">
                    ...
                    <div class="bg-white/30 backdrop-blur-md p-1.5 rounded-xl flex border border-white/40 shadow-sm overflow-x-auto">
                        ...
                        <a href="{{ route('reports.printDaily', ['date' => request('date')]) }}" target="_blank" class="ml-2 bg-[#2A3B52] text-white p-2.5 rounded-lg hover:bg-[#182436] hover:scale-105 active:scale-95 transition-all shadow-sm border border-transparent shrink-0 flex items-center gap-2" title="Cetak Laporan Harian">
        ...
        <div class="animate-enter mb-8 no-print" style="animation-delay: 50ms">
                ...
                <div class="bg-[#FFEFD6] rounded-xl border border-[#FFD8A8] p-6 relative overflow-hidden fluent-card">
                    ...
                    <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative z-10">
                        <div>
                            ...
                            <p class="text-sm text-[#D83B01]/80">Terdapat <span class="font-bold">{{ $countOut }} siswa</span> yang belum kembali ke kelas saat ini.</p>
                        </div>
                        
                        <a href="{{ route('permit.index') }}" class="px-5 py-2.5 bg-white text-[#D83B01] text-sm font-bold rounded-lg shadow-sm border border-[#FFD8A8] hover:bg-[#FFE3B8] transition-colors flex items-center gap-2">
                    ...
                    <div class="mt-6 flex gap-3 overflow-x-auto pb-2 custom-scrollbar">
                        @foreach($studentsOut as $permit)
                            ...
                            @endphp
                            <div class="flex-shrink-0 w-64 bg-white p-3 rounded-xl border {{ $isOverdue ? 'border-[#F4C3C9] bg-[#FDE7E9]' : 'border-[#FFD8A8]' }} shadow-sm flex items-center gap-3">
                                <div class="w-10 h-10 rounded-lg {{ $isOverdue ? 'bg-[#FDE7E9] text-[#D13438]' : 'bg-[#FFEFD6] text-[#D83B01]' }} flex items-center justify-center font-bold text-sm">
                                    {{ substr($permit->student->name, 0, 1) }}
                                </div>
                                <div class="min-w-0">
                                    <p class="text-xs font-bold text-slate-700 truncate">{{ $permit->student->name }}</p>
                                    <p class="text-[10px] text-slate-500 truncate">{{ $permit->reason_category }} • <span class="{{ $isOverdue ? 'text-[#D13438] font-bold animate-pulse' : 'text-slate-500' }}">{{ $duration }} m</span></p>
                                </div>
                            </div>
                        @endforeach
                    </div>
            ...
            @else
                ...
                <div class="bg-[#DFF6DD] rounded-xl border border-[#B7DFB9] p-6 relative overflow-hidden flex items-center justify-between fluent-card">
                    ...
                    <a href="{{ route('permit.index') }}" class="px-5 py-2.5 bg-white text-[#107C10] text-sm font-bold rounded-lg shadow-sm border border-[#B7DFB9] hover:bg-[#D0F0C0] transition-colors hidden md:flex items-center gap-2">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4 mb-8 no-print animate-enter quick-actions" style="animation-delay: 100ms">
            <a href="{{ route('students.index') }}" @click.prevent="navigate('{{ route('students.index') }}')" class="group bg-white p-4 rounded-xl fluent-card flex flex-col items-center justify-center gap-2 hover:border-[#5295FF] cursor-pointer text-center">
             ...
             <a href="{{ route('teacher.habits.index') }}" @click.prevent="navigate('{{ route('teacher.habits.index') }}')" class="group bg-white p-4 rounded-xl fluent-card flex flex-col items-center justify-center gap-2 hover:border-[#107C10] cursor-pointer text-center">
            ...
            <a href="{{ route('cbt.index') }}" @click.prevent="navigate('{{ route('cbt.index') }}')" class="group bg-white p-4 rounded-xl fluent-card flex flex-col items-center justify-center gap-2 hover:border-[#2A3B52] cursor-pointer text-center">
            ...
            <a href="{{ route('lms.assignments.index') }}" @click.prevent="navigate('{{ route('lms.assignments.index') }}')" class="group bg-white p-4 rounded-xl fluent-card flex flex-col items-center justify-center gap-2 hover:border-[#D13438] cursor-pointer text-center">
            ...
            <a href="{{ route('lms.grades.index') }}" @click.prevent="navigate('{{ route('lms.grades.index') }}')" class="group bg-white p-4 rounded-xl fluent-card flex flex-col items-center justify-center gap-2 hover:border-[#107C10] cursor-pointer text-center">
                <div class="w-12 h-12 rounded-lg bg-[#DFF6DD] text-[#107C10] flex items-center justify-center group-hover:scale-110 transition-transform shadow-sm border border-[#B7DFB9]">
            ...
            <a href="{{ route('reports.class') }}" @click.prevent="navigate('{{ route('reports.class') }}')" class="group bg-white p-4 rounded-xl fluent-card flex flex-col items-center justify-center gap-2 hover:border-[#D83B01] cursor-pointer text-center">
            ...
            <a href="{{ route('admin.graduation.index') }}" @click.prevent="navigate('{{ route('admin.graduation.index') }}')" class="group bg-white p-4 rounded-xl fluent-card flex flex-col items-center justify-center gap-2 hover:border-[#D13438] cursor-pointer text-center">
                <div class="w-12 h-12 rounded-lg bg-[#FDE7E9] text-[#D13438] flex items-center justify-center group-hover:scale-110 transition-transform shadow-sm border border-[#F4C3C9]">
            ...
            <a href="{{ route('admin.ppdb.index') }}" @click.prevent="navigate('{{ route('admin.ppdb.index') }}')" class="group bg-white p-4 rounded-xl fluent-card flex flex-col items-center justify-center gap-2 hover:border-[#D83B01] cursor-pointer text-center">
                <div class="w-12 h-12 rounded-lg bg-[#FFEFD6] text-[#D83B01] flex items-center justify-center group-hover:scale-110 transition-transform shadow-sm border border-[#FFD8A8]">
            ...
            <a href="{{ route('letters.spt.index') }}" @click.prevent="navigate('{{ route('letters.spt.index') }}')" class="group bg-white p-4 rounded-xl fluent-card flex flex-col items-center justify-center gap-2 hover:border-[#5295FF] cursor-pointer text-center">
                <div class="w-12 h-12 rounded-lg bg-[#F3F9FD] text-[#5295FF] flex items-center justify-center group-hover:scale-110 transition-transform shadow-sm border border-[#D0E7F8]">
            ...
            <a href="{{ route('library.dashboard') }}" @click.prevent="navigate('{{ route('library.dashboard') }}')" class="group bg-white p-4 rounded-xl fluent-card flex flex-col items-center justify-center gap-2 hover:border-[#2A3B52] cursor-pointer text-center">
                <div class="w-12 h-12 rounded-lg bg-slate-100 text-[#2A3B52] flex items-center justify-center group-hover:scale-110 transition-transform shadow-sm border border-slate-200">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4 md:gap-5">
            ...
            <div onclick="showCardInfo('{{ $card['title'] }}', '{{ $card['value'] }}', '{{ $colorKey }}')" 
               class="cursor-pointer animate-enter group bg-white rounded-xl p-5 fluent-card {{ $theme['hover_border'] }} relative overflow-hidden flex flex-col justify-between h-full card-print"
               style="animation-delay: {{ ($index + 1) * 100 }}ms">
                ...
                <div class="flex justify-between items-start mb-4 relative z-10">
                    <div class="w-12 h-12 rounded-lg flex items-center justify-center shadow-sm border {{ $theme['border'] }} transition-all duration-300 {{ $theme['bg'] }} {{ $theme['text'] }} {{ $theme['hover_bg'] }} group-hover:text-white group-hover:scale-110">
                        <i class="ph-duotone {{ $iconClass }} text-2xl animate-wiggle"></i>
                    </div>
                    
                    @if(isset($card['percentage']))
                    <span class="text-[10px] font-bold px-2 py-1 rounded-md border {{ $card['percentage'] > 0 ? 'bg-[#DFF6DD] text-[#107C10] border-[#B7DFB9]' : 'bg-[#FDE7E9] text-[#D13438] border-[#F4C3C9]' }}">
                        {{ $card['percentage'] > 0 ? '+' : '' }}{{ $card['percentage'] }}%
                    </span>
                    @endif
                    
                    @if(isset($card['trend']) && $card['trend'] !== null)
                        <div class="text-[10px] font-bold px-2 py-1 rounded-md border flex items-center gap-1 {{ $card['trend'] >= 0 ? 'bg-[#DFF6DD] text-[#107C10] border-[#B7DFB9]' : 'bg-[#FDE7E9] text-[#D13438] border-[#F4C3C9]' }}">
                ...
                <div class="relative z-10 mt-auto">
                    ...
                    <h3 class="text-2xl md:text-3xl font-extrabold text-[#2A3B52] tracking-tight count-up" data-target="{{ $card['value'] }}">0</h3>
                    @if(isset($card['trend']) && $card['trend'] !== null)
            ...
            <div class="animate-enter bg-white p-5 md:p-8 rounded-xl fluent-card flex flex-col h-full card-print" style="animation-delay: 700ms">
                ...
                <div class="relative h-56 w-full flex items-center justify-center mb-6">
                    ...
                    <div class="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                        <span class="text-3xl md:text-4xl font-black text-[#2A3B52] count-up" data-target="{{ $totalStudents ?? 0 }}">0</span>
                        <span class="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Total Siswa</span>
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-3 mt-auto">
                    <div class="bg-[#F3F9FD] p-3 rounded-lg border border-[#D0E7F8]"><span class="block text-[10px] font-bold text-[#5295FF] uppercase mb-1">Hadir Tepat</span><span class="text-lg font-black text-[#2A3B52]">{{ $presentOnTimeCount ?? 0 }}</span></div>
                    <div class="bg-[#FFEFD6] p-3 rounded-lg border border-[#FFD8A8]"><span class="block text-[10px] font-bold text-[#D83B01] uppercase mb-1">Terlambat</span><span class="text-lg font-black text-[#2A3B52]">{{ $lateCount ?? 0 }}</span></div>
                    <div class="bg-[#FDE7E9] p-3 rounded-lg border border-[#F4C3C9]"><span class="block text-[10px] font-bold text-[#D13438] uppercase mb-1">Alfa</span><span class="text-lg font-black text-[#2A3B52]">{{ $absentCount ?? 0 }}</span></div>
                    <div class="bg-slate-50 p-3 rounded-lg border border-slate-200"><span class="block text-[10px] font-bold text-slate-500 uppercase mb-1">Belum Hadir</span><span class="text-lg font-black text-[#2A3B52]">{{ $notYetScannedCount ?? 0 }}</span></div>
                </div>
            </div>
        </div>
                            ...
                            <div class="relative group">
                                ...
                                <div class="flex items-start justify-between gap-3 p-3 rounded-xl hover:bg-slate-50 transition-colors -mt-2 -ml-2">
                                    <div class="flex items-center gap-3">
                                        <div class="w-10 h-10 rounded-lg {{ $theme['bg_icon'] }} border {{ $theme['border_icon'] }} flex items-center justify-center {{ $theme['text_icon'] }} font-bold text-xs shrink-0">
                                            <i class="ph-bold {{ $icon }} text-lg"></i>
                                        </div>
                                        <div>
                                            <p class="text-sm font-bold text-[#2A3B52] line-clamp-1 group-hover:text-[#5295FF] transition-colors">{{ $log->student->name ?? 'Siswa' }}</p>
                                    ...
                                    <div class="text-right shrink-0">
                                        ...
                                        <span class="text-[10px] font-bold px-2 py-1 rounded-md {{ $theme['bg_badge'] }} {{ $theme['text_badge'] }}">{{ $statusText }}</span>
                                    </div>
                                </div>
                            </div>
                            @endforeach
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="flex items-center justify-between p-3 rounded-xl border border-slate-50 hover:bg-[#FDE7E9] transition-colors">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <div class="text-xs font-bold text-[#2A3B52] line-clamp-1">{{ $student->student->name ?? 'Siswa' }}</div>
                                        <div class="text-[10px] text-slate-400">{{ $student->student->schoolClass->name ?? '-' }}</div>
                                    </div>
                                </div>
                                <span class="text-xs font-black text-[#D13438] bg-[#FDE7E9] px-3 py-1 rounded-md border border-[#F4C3C9]">{{ $student->total_late }}x</span>
                            </div>
                            @endforeach
                        </div>
                    @else
                            ...
                            <tbody class="divide-y divide-slate-50">
                                ...
                                <tr class="group hover:bg-slate-50 transition-colors">
                                    <td class="py-4 pl-1 w-10">
                                        @if($index == 0) <i class="ph-fill ph-medal text-[#D83B01] text-2xl drop-shadow-sm"></i>
                                    ...
                                    <td class="py-4">
                                        <div class="font-bold text-[#2A3B52] mb-1">{{ $rank->class_name }}</div>
                                        <div class="w-full bg-slate-100 rounded-full h-1.5 overflow-hidden max-w-[150px]">
                                            ...
                                            <div class="h-1.5 rounded-full {{ $index == 0 ? 'bg-[#D83B01]' : 'bg-[#5295FF]' }}" style="width: {{ $percent }}%"></div>
                                        </div>
                                    </td>
                                    <td class="py-4 text-right pr-2">
                                        <div class="font-black text-[#2A3B52]">{{ number_format($percent, 0) }}%</div>
                                ...
                                <tr class="group hover:bg-[#FDE7E9]/50 transition-colors">
                                    ...
                                    <td class="py-4">
                                        ...
                                        <div class="w-full bg-slate-100 rounded-full h-1.5 overflow-hidden max-w-[150px]">
                                            ...
                                            <div class="h-1.5 rounded-full bg-[#D13438]" style="width: {{ $percent }}%"></div>
                                        </div>
                                    </td>
                                    <td class="py-4 text-right pr-2">
                                        <div class="font-black text-[#D13438]">{{ $rank->absent_count }}</div>
